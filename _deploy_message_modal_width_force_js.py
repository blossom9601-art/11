...
FILES = [
    ...
    (
        'static/js/9.category/9-1.business/9-1-1.work_classification/1.work_classification_list.js',
        '/opt/blossom/web/static/js/9.category/9-1.business/9-1-1.work_classification/1.work_classification_list.js'
    ),
    (
...
_, stdout, _ = ssh.exec_command("grep -n 'work_operation_list.js?v=20260414c' /opt/blossom/web/app/templates/9.category/9-1.business/9-1-4.work_operation/1.work_operation_list.html")
print('JS version marker:')
print(stdout.read().decode().strip())

_, stdout, _ = ssh.exec_command("grep -n \"contentWrap.style.width = '620px'\" /opt/blossom/web/static/js/9.category/9-1.business/9-1-4.work_operation/1.work_operation_list.js")
print('Forced width marker:')
print(stdout.read().decode().strip())
