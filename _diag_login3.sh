#!/bin/bash
# Comprehensive login diagnostic: check admin user state + live login test
cd /opt/blossom/lumina/web
source /opt/blossom/lumina/venv/bin/activate

python3 << 'PYEOF'
import sys, os
os.environ.setdefault('FLASK_APP', 'run')
sys.path.insert(0, '.')
...
with app.app_context():
    ...
    # 6. Live login test with test_client (follow redirects off)
