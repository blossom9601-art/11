...
def init_hw_storage_type_table(app=None) -> None:
    ...
    try:
        with _get_connection(app) as conn:
            ...
            # Backfill hw_server_type entries so storage models are selectable by server_code.
            try:
                rows = conn.execute(
                    f"SELECT storage_code, model_name, manufacturer_code, storage_type, release_date, eosl_date, remark FROM {TABLE_NAME} WHERE is_deleted = 0"
                ).fetchall()
                for row in rows:
                    _sync_hw_server_type_from_storage_row(conn, dict(row), actor='system')
            except Exception:
