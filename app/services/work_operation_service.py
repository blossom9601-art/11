...
def _resolve_db_path(app=None) -> str:
    ...
    if not uri.startswith('sqlite'):
        fallback = app.config.get('WORK_OPERATION_SQLITE_PATH')
        if fallback:
            return os.path.abspath(fallback)
        return os.path.join(app.instance_path, 'work_operation.db')
    parsed = urlparse(uri)
    path = parsed.path or ''
...
def _generate_unique_code(conn: sqlite3.Connection, name: str) -> str:
    base = re.sub(r'[^A-Za-z0-9]+', '_', (name or 'OPERATION').upper()).strip('_') or 'OPERATION'
    base = base[:40]
    candidate = base
    suffix = 1
    while True:
        exists = conn.execute(
            f"SELECT 1 FROM {TABLE_NAME} WHERE operation_code = ?",
            (candidate,)
        ).fetchone()
        if not exists:
            return candidate
...
def _fetch_single(operation_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
            f"""
            SELECT id, operation_code, operation_name, description, hw_count, sw_count, remark,
        ...
        code = (item.get('operation_code') or '').strip()
        if not code:
            item['hw_count'] = 0
            item['sw_count'] = 0
            return item
        hw_counts = counts_by_code(conn, asset_table='hardware', code_column='work_operation_code')
        sw_counts = sw_counts_via_hardware(conn, code_column='work_operation_code')
        item['hw_count'] = hw_counts.get(code, 0)
        item['sw_count'] = sw_counts.get(code, 0)
        return item


def init_work_operation_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            conn.execute(
            ...
            conn.execute(
                f"CREATE UNIQUE INDEX IF NOT EXISTS idx_{TABLE_NAME}_code ON {TABLE_NAME}(operation_code)"
            )
            conn.commit()
...
def list_work_operations(app=None, search: Optional[str] = None, include_deleted: bool = False) -> List[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        ...
        if search:
            like = f"%{search}%"
            clauses.append("(operation_name LIKE ? OR operation_code LIKE ? OR description LIKE ? OR remark LIKE ?)")
            params.extend([like, like, like, like])
        query = (
            f"SELECT id, operation_code, operation_name, description, hw_count, sw_count, remark, created_at, created_by, updated_at, updated_by, is_deleted "
...
def create_work_operation(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    name = (data.get('operation_name') or data.get('wc_name') or '').strip()
    if not name:
        raise ValueError('operation_name is required')
    validate_business_work_display_name(name, field_label='업무 운영명')
    ...
    with _get_connection(app) as conn:
        code = (data.get('operation_code') or '').strip() or _generate_unique_code(conn, name)
        timestamp = _now()
        conn.execute(
            f"""
            INSERT INTO {TABLE_NAME}
                (operation_code, operation_name, description, hw_count, sw_count, remark, created_at, created_by, updated_at, updated_by, is_deleted)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            """,
            (
                code,
...
def update_work_operation(operation_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload: Dict[str, Any] = {}
    if 'operation_name' in data or 'wc_name' in data:
        name = (data.get('operation_name') or data.get('wc_name') or '').strip()
        if not name:
            raise ValueError('operation_name is required')
        validate_business_work_display_name(name, field_label='업무 운영명')
        payload['operation_name'] = name
    ...
    if not updates:
        return _fetch_single(operation_id, app)
    timestamp = _now()
    updates.extend(['updated_at = ?', 'updated_by = ?'])
    params.extend([timestamp, actor, operation_id])
    with _get_connection(app) as conn:
        cur = conn.execute(
...
def soft_delete_work_operations(ids: Sequence[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
