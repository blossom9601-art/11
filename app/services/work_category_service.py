...
TABLE_NAME = 'biz_work_category'


def _now() -> str:
    """Return UTC timestamp string suitable for SQLite."""
...
def _resolve_db_path(app=None) -> str:
    ...
    if not uri.startswith('sqlite'):
        fallback = app.config.get('WORK_CATEGORY_SQLITE_PATH')
        if fallback:
            return os.path.abspath(fallback)
        return os.path.join(app.instance_path, 'work_category.db')
    parsed = urlparse(uri)
    path = parsed.path or ''
...
def _copy_table_if_missing_or_empty(*, source_db: str, target_conn: sqlite3.Connection, table_name: str) -> None:
    """Best-effort: copy rows from source_db into target table.

    Intended to heal historical split-brain where some services wrote to project-root
    dev_blossom.db while SQLAlchemy used instance/dev_blossom.db.
    """
    ...
    try:
        ...
        existing = target_conn.execute(f"SELECT COUNT(1) FROM {table_name}").fetchone()
        ...
        try:
            ...
            # Avoid copying autoincrement PK.
...
def _generate_unique_code(conn: sqlite3.Connection, name: str) -> str:
    base = re.sub(r'[^A-Za-z0-9]+', '_', (name or 'CATEGORY').upper()).strip('_') or 'CATEGORY'
    base = base[:40]
    candidate = base
    suffix = 1
    while True:
        exists = conn.execute(
            f"SELECT 1 FROM {TABLE_NAME} WHERE category_code = ?",
            (candidate,)
        ).fetchone()
        if not exists:
            return candidate
...
def init_work_category_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            ...
            conn.execute(
                f"CREATE UNIQUE INDEX IF NOT EXISTS idx_{TABLE_NAME}_code ON {TABLE_NAME}(category_code)"
            )
            conn.commit()
...
def list_work_categories(app=None, search: Optional[str] = None, include_deleted: bool = False) -> List[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        ...
        if search:
            like = f"%{search}%"
            clauses.append("(category_name LIKE ? OR category_code LIKE ? OR description LIKE ? OR remark LIKE ?)")
            params.extend([like, like, like, like])
        query = (
            f"SELECT id, category_code, category_name, description, hw_count, sw_count, remark, created_at, created_by, updated_at, updated_by, is_deleted "
            f"FROM {TABLE_NAME} WHERE {' AND '.join(clauses)} ORDER BY id DESC"
        )
        rows = conn.execute(query, params).fetchall()
        hw_counts = counts_by_code(conn, asset_table='hardware', code_column='work_category_code')
        sw_counts = sw_counts_via_hardware(conn, code_column='work_category_code')
        out: List[Dict[str, Any]] = []
        for row in rows:
            item = _row_to_dict(row)
            code = (item.get('category_code') or '').strip()
            if code:
...
def create_work_category(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    ...
    with _get_connection(app) as conn:
        code = (data.get('category_code') or '').strip() or _generate_unique_code(conn, name)
        timestamp = _now()
        conn.execute(
            f"""
            INSERT INTO {TABLE_NAME}
                (category_code, category_name, description, hw_count, sw_count, remark, created_at, created_by, updated_at, updated_by, is_deleted)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            """,
            (
                code,
...
def update_work_category(category_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload: Dict[str, Any] = {}
    if 'category_name' in data or 'wc_name' in data:
        name = (data.get('category_name') or data.get('wc_name') or '').strip()
        if not name:
            raise ValueError('category_name is required')
    ...
    if not updates:
        row = _fetch_single(category_id, app)
        return row
    timestamp = _now()
    updates.extend(['updated_at = ?', 'updated_by = ?'])
    params.extend([timestamp, actor, category_id])
    with _get_connection(app) as conn:
        cur = conn.execute(
        ...
        if cur.rowcount == 0:
            return None
        conn.commit()
    return _fetch_single(category_id, app)


def _fetch_single(category_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
            f"SELECT id, category_code, category_name, description, hw_count, sw_count, remark, created_at, created_by, updated_at, updated_by, is_deleted "
...
def soft_delete_work_categories(ids: Sequence[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
