...
def _resolve_db_path(app=None) -> str:
    ...
    override = app.config.get('CMP_DISK_TYPE_SQLITE_PATH')
    if override:
        return os.path.abspath(override)
    if not uri.startswith('sqlite'):
        return os.path.join(app.instance_path, 'cmp_disk_type.db')
    parsed = urlparse(uri)
    path = parsed.path or ''
...
def _normalize_code(seed: str) -> str:
    base = (seed or 'DISK').upper()
    base = re.sub(r'[^A-Z0-9]+', '_', base).strip('_') or 'DISK'
    return base[:60]
...
def _generate_unique_code(conn: sqlite3.Connection, seed: str) -> str:
    ...
    while True:
        row = conn.execute(
            f"SELECT 1 FROM {TABLE_NAME} WHERE disk_code = ?",
            (candidate,),
        ).fetchone()
        if not row:
            return candidate
...
def _prepare_payload(data: Dict[str, Any], *, require_all: bool = False) -> Dict[str, Any]:
    ...
    mapping = {
        ...
        'disk_count': ['disk_count', 'qty', 'count'],
    ...
    if 'disk_count' in payload:
        payload['disk_count'] = _sanitize_int(payload['disk_count'])
    return payload
...
def init_cmp_disk_type_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            conn.execute(CREATE_TABLE_SQL)
            conn.execute(
                f"CREATE INDEX IF NOT EXISTS idx_{TABLE_NAME}_code ON {TABLE_NAME}(disk_code)"
            )
            conn.execute(
...
def list_cmp_disk_types(
    ...
    *,
    ...
    app = app or current_app
    with _get_connection(app) as conn:
        clauses = ['1=1' if include_deleted else 'd.is_deleted = 0']
        params: List[Any] = []
        if search:
            like = f"%{search}%"
            f = (field or '').strip().lower()
            if f in ('model', 'model_name'):
                clauses.append('d.model_name LIKE ?')
                params.append(like)
            elif f in ('spec', 'spec_summary'):
                clauses.append('d.spec_summary LIKE ?')
                params.append(like)
            elif f in ('vendor', 'manufacturer'):
                clauses.append('(d.manufacturer_code LIKE ? OR v.manufacturer_name LIKE ?)')
        ...
        query = (
            f"SELECT d.id, d.disk_code, d.model_name, d.spec_summary, d.manufacturer_code, d.part_number, "
            f"d.disk_count, d.remark, d.created_at, d.created_by, d.updated_at, d.updated_by, d.is_deleted, "
            f"v.manufacturer_name, "
            f"0 AS usage_count "
            f"FROM {TABLE_NAME} d "
            f"LEFT JOIN {MANUFACTURER_TABLE} v ON v.manufacturer_code = d.manufacturer_code AND v.is_deleted = 0 "
            f"WHERE {' AND '.join(clauses)} ORDER BY d.id DESC"
        )
        rows = conn.execute(query, params).fetchall()
        return [_row_to_dict(row) for row in rows]


def get_cmp_disk_type(record_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
            f"SELECT d.id, d.disk_code, d.model_name, d.spec_summary, d.manufacturer_code, d.part_number, "
            ...
            f"v.manufacturer_name "
            f"FROM {TABLE_NAME} d "
            f"LEFT JOIN {MANUFACTURER_TABLE} v ON v.manufacturer_code = d.manufacturer_code "
            f"WHERE d.id = ?",
            (record_id,),
        ).fetchone()
...
def create_cmp_disk_type(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    ...
    with _get_connection(app) as conn:
        ...
        disk_code = (payload.get('disk_code') or '').strip()
        if disk_code:
            _assert_unique_code(conn, disk_code)
        else:
            disk_code = _generate_unique_code(conn, model_name)
        conn.execute(
            f"""
            INSERT INTO {TABLE_NAME}
                (disk_code, model_name, spec_summary, manufacturer_code, part_number,
                 disk_count, remark, created_at, created_by, updated_at, updated_by, is_deleted)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            """,
            (
                disk_code[:60],
        ...
        conn.commit()
    return get_cmp_disk_type(new_id, app)


def update_cmp_disk_type(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=False)
    if not payload:
        return get_cmp_disk_type(record_id, app)
    with _get_connection(app) as conn:
        updates: List[str] = []
        params: List[Any] = []
        if 'disk_code' in payload:
            code = (payload['disk_code'] or '').strip()
            if code:
                _assert_unique_code(conn, code, record_id)
                updates.append('disk_code = ?')
                params.append(code[:60])
            else:
                payload.pop('disk_code', None)
...
def soft_delete_cmp_disk_types(ids: Iterable[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    safe_ids: List[int] = []
    for raw in ids:
