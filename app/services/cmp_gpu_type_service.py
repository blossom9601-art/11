...
def _resolve_db_path(app=None) -> str:
    ...
    override = app.config.get('CMP_GPU_TYPE_SQLITE_PATH')
    if override:
        return os.path.abspath(override)
    if not uri.startswith('sqlite'):
        return os.path.join(app.instance_path, 'cmp_gpu_type.db')
    parsed = urlparse(uri)
    path = parsed.path or ''
...
def _normalize_code(seed: str) -> str:
    base = (seed or 'GPU').upper()
    base = re.sub(r'[^A-Z0-9]+', '_', base).strip('_') or 'GPU'
    return base[:60]
...
def _generate_unique_code(conn: sqlite3.Connection, seed: str) -> str:
    ...
    while True:
        row = conn.execute(
            f"SELECT 1 FROM {TABLE_NAME} WHERE gpu_code = ?",
            (candidate,),
        ).fetchone()
        if not row:
            return candidate
...
def _prepare_payload(data: Dict[str, Any], *, require_all: bool = False) -> Dict[str, Any]:
    ...
    if 'gpu_count' in payload:
        payload['gpu_count'] = _sanitize_int(payload['gpu_count'])
    return payload
...
def init_cmp_gpu_type_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            conn.execute(CREATE_TABLE_SQL)
            conn.execute(
                f"CREATE INDEX IF NOT EXISTS idx_{TABLE_NAME}_code ON {TABLE_NAME}(gpu_code)"
            )
            conn.execute(
...
def list_cmp_gpu_types(
    ...
    *,
    ...
    app = app or current_app
    with _get_connection(app) as conn:
        clauses = ['1=1' if include_deleted else 'g.is_deleted = 0']
        params: List[Any] = []
        if search:
            like = f"%{search}%"
            f = (field or '').strip().lower()
            if f in ('model', 'model_name'):
                clauses.append('g.model_name LIKE ?')
                params.append(like)
            elif f in ('spec', 'spec_summary'):
                clauses.append('g.spec_summary LIKE ?')
                params.append(like)
            elif f in ('vendor', 'manufacturer'):
                clauses.append('(g.manufacturer_code LIKE ? OR m.manufacturer_name LIKE ?)')
        ...
        query = (
            f"SELECT g.id, g.gpu_code, g.model_name, g.spec_summary, g.manufacturer_code, g.part_number, "
            f"g.gpu_count, g.remark, g.created_at, g.created_by, g.updated_at, g.updated_by, g.is_deleted, "
            f"m.manufacturer_name, "
            f"0 AS usage_count "
            f"FROM {TABLE_NAME} g "
            f"LEFT JOIN {MANUFACTURER_TABLE} m ON m.manufacturer_code = g.manufacturer_code AND m.is_deleted = 0 "
            f"WHERE {' AND '.join(clauses)} ORDER BY g.id DESC"
        )
        rows = conn.execute(query, params).fetchall()
        return [_row_to_dict(row) for row in rows]


def get_cmp_gpu_type(record_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
            f"SELECT g.id, g.gpu_code, g.model_name, g.spec_summary, g.manufacturer_code, g.part_number, "
            ...
            f"m.manufacturer_name "
            f"FROM {TABLE_NAME} g "
            f"LEFT JOIN {MANUFACTURER_TABLE} m ON m.manufacturer_code = g.manufacturer_code "
            f"WHERE g.id = ?",
            (record_id,),
        ).fetchone()
...
def create_cmp_gpu_type(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    ...
    with _get_connection(app) as conn:
        ...
        gpu_code = (payload.get('gpu_code') or '').strip()
        if gpu_code:
            _assert_unique_code(conn, gpu_code)
        else:
            gpu_code = _generate_unique_code(conn, model_name)
        conn.execute(
            f"""
            INSERT INTO {TABLE_NAME}
                (gpu_code, model_name, spec_summary, manufacturer_code, part_number,
                 gpu_count, remark, created_at, created_by, updated_at, updated_by, is_deleted)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            """,
            (
                gpu_code[:60],
        ...
        conn.commit()
    return get_cmp_gpu_type(new_id, app)


def update_cmp_gpu_type(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=False)
    if not payload:
        return get_cmp_gpu_type(record_id, app)
    with _get_connection(app) as conn:
        updates: List[str] = []
        params: List[Any] = []
        if 'gpu_code' in payload:
            code = (payload['gpu_code'] or '').strip()
            if code:
                _assert_unique_code(conn, code, record_id)
                updates.append('gpu_code = ?')
                params.append(code[:60])
            else:
                payload.pop('gpu_code', None)
...
def soft_delete_cmp_gpu_types(ids: Iterable[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    safe_ids: List[int] = []
    for raw in ids:
