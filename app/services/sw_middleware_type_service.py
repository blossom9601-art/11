...
TABLE_NAME = 'sw_middleware_type'
MANUFACTURER_TABLE = 'biz_vendor_manufacturer'

CREATE_TABLE_SQL = f"""
CREATE TABLE IF NOT EXISTS {TABLE_NAME} (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    middleware_code TEXT NOT NULL UNIQUE,
    model_name TEXT NOT NULL,
    manufacturer_code TEXT NOT NULL,
    middleware_type TEXT NOT NULL,
    release_date TEXT,
    eosl_date TEXT,
    middleware_count INTEGER DEFAULT 0,
    remark TEXT,
...
def _resolve_db_path(app=None) -> str:
    ...
    if not uri.startswith('sqlite'):
        fallback = app.config.get('SW_MIDDLEWARE_TYPE_SQLITE_PATH')
        if fallback:
            return os.path.abspath(fallback)
        return os.path.join(app.instance_path, 'sw_middleware_type.db')
...
def _normalize_code(seed: str) -> str:
    base = (seed or 'MW').upper()
    base = re.sub(r'[^A-Z0-9]+', '_', base).strip('_') or 'MW'
    return base[:60]
...
def _generate_unique_code(conn: sqlite3.Connection, seed: str) -> str:
    ...
    while True:
        row = conn.execute(
            f"SELECT 1 FROM {TABLE_NAME} WHERE middleware_code = ?",
            (candidate,),
        ).fetchone()
        if not row:
            return candidate
        ...
        if counter > 9999:
            raise ValueError('고유 미들웨어 코드를 생성하지 못했습니다.')


def _assert_unique_code(conn: sqlite3.Connection, code: str, record_id: Optional[int] = None) -> None:
    row = conn.execute(
        f"SELECT id FROM {TABLE_NAME} WHERE middleware_code = ?",
        (code,),
    ).fetchone()
    if row and (record_id is None or row['id'] != record_id):
        raise ValueError('이미 사용 중인 미들웨어 코드입니다.')


def _row_to_dict(row: sqlite3.Row) -> Dict[str, Any]:
    if not row:
        return {}
    qty = row['middleware_count'] or 0
    remark = row['remark'] or ''
    model = row['model_name']
    mw_type = row['middleware_type']
    manufacturer_name = row['manufacturer_name'] if 'manufacturer_name' in row.keys() else None
    return {
        'id': row['id'],
        'public_id': make_public_id(TABLE_NAME, 'mw', row['id']),
        'middleware_code': row['middleware_code'],
        'model_name': model,
        'model': model,
...
def _prepare_payload(data: Dict[str, Any], *, require_all: bool = False) -> Dict[str, Any]:
    ...
    mapping = {
        'middleware_code': ['middleware_code', 'code', 'mw_code'],
        'model_name': ['model_name', 'model', 'middleware_name'],
        'manufacturer_code': ['manufacturer_code', 'vendor_code'],
        'manufacturer_name': ['manufacturer_name', 'vendor', 'manufacturer'],
        'middleware_type': ['middleware_type', 'mw_type', 'hw_type', 'type', 'category'],
        'release_date': ['release_date'],
        'eosl_date': ['eosl_date', 'eosl'],
        'middleware_count': ['middleware_count', 'qty', 'count'],
    ...
    for column, aliases in mapping.items():
        for alias in aliases:
            if alias in data and data.get(alias) not in (None, ''):
                ...
                break
    if require_all:
        missing = [key for key in ('model_name', 'middleware_type') if not payload.get(key)]
        if missing:
    ...
    if 'middleware_count' in payload:
        payload['middleware_count'] = _sanitize_int(payload['middleware_count'])
    return payload
...
def init_sw_middleware_type_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            conn.execute(CREATE_TABLE_SQL)
            conn.execute(
                f"CREATE INDEX IF NOT EXISTS idx_{TABLE_NAME}_code ON {TABLE_NAME}(middleware_code)"
            )
            conn.execute(
...
def list_sw_middleware_types(app=None, *, search: Optional[str] = None, include_deleted: bool = False) -> List[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        clauses = ['1=1' if include_deleted else 'm.is_deleted = 0']
        params: List[Any] = []
        if search:
            like = f"%{search}%"
            clauses.append('(' + ' OR '.join([
                'm.middleware_code LIKE ?',
                'm.model_name LIKE ?',
                'm.manufacturer_code LIKE ?',
                'm.middleware_type LIKE ?',
                'm.remark LIKE ?'
            ]) + ')')
            params.extend([like] * 5)
        query = (
            f"SELECT m.id, m.middleware_code, m.model_name, m.manufacturer_code, m.middleware_type, "
            f"m.release_date, m.eosl_date, m.middleware_count, m.remark, m.created_at, m.created_by, "
            f"m.updated_at, m.updated_by, m.is_deleted, v.manufacturer_name, "
            f"0 AS usage_count "
            f"FROM {TABLE_NAME} m "
            f"LEFT JOIN {MANUFACTURER_TABLE} v ON v.manufacturer_code = m.manufacturer_code "
            f"WHERE {' AND '.join(clauses)} ORDER BY m.id DESC"
        )
        rows = conn.execute(query, params).fetchall()
        return [_row_to_dict(row) for row in rows]


def get_sw_middleware_type(record_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
            f"SELECT m.id, m.middleware_code, m.model_name, m.manufacturer_code, m.middleware_type, "
...
def create_sw_middleware_type(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=True)
    model_name = payload['model_name'].strip()
    middleware_type = payload['middleware_type'].strip()
    timestamp = _now()
    with _get_connection(app) as conn:
        manufacturer_code = _resolve_manufacturer_code(conn, payload)
        middleware_code = (payload.get('middleware_code') or '').strip()
        if middleware_code:
            _assert_unique_code(conn, middleware_code)
        else:
            middleware_code = _generate_unique_code(conn, model_name)
        conn.execute(
            f"""
            INSERT INTO {TABLE_NAME}
                (middleware_code, model_name, manufacturer_code, middleware_type, release_date, eosl_date,
                 middleware_count, remark, created_at, created_by, updated_at, updated_by, is_deleted)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            """,
            (
                middleware_code[:60],
                ...
                payload.get('middleware_count', 0),
                payload.get('remark'),
                timestamp,
                actor,
                timestamp,
        ...
        conn.commit()
    return get_sw_middleware_type(new_id, app)


def update_sw_middleware_type(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=False)
    if not payload:
        return get_sw_middleware_type(record_id, app)
    with _get_connection(app) as conn:
        updates: List[str] = []
        params: List[Any] = []
        if 'middleware_code' in payload:
            code = (payload['middleware_code'] or '').strip()
            if code:
                _assert_unique_code(conn, code, record_id)
                updates.append('middleware_code = ?')
                params.append(code[:60])
            else:
                payload.pop('middleware_code', None)
        ...
        for column in (
            ...
            if column in payload:
                ...
                if column in ('model_name', 'middleware_type') and not value:
...
def soft_delete_sw_middleware_types(ids: Iterable[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    safe_ids: List[int] = []
    for raw in ids:
