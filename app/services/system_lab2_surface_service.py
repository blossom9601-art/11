...
TABLE_NAME = 'system_lab2_surface'
...
def _resolve_db_path(app=None) -> str:
    ...
    direct_path = app.config.get('SYSTEM_LAB2_SURFACE_SQLITE_PATH')
    if direct_path:
        return os.path.abspath(direct_path)
    uri = app.config.get('SQLALCHEMY_DATABASE_URI', 'sqlite:///dev_blossom.db')
    if not uri.startswith('sqlite'):  # fall back to instance-local sqlite file
        return os.path.join(app.instance_path, 'system_lab2_surface.db')
    parsed = urlparse(uri)
    path = parsed.path or ''
...
def init_system_lab2_surface_table(app=None) -> None:
    app = app or current_app
    schema_sql = f"""
...
def list_system_lab2_surfaces(*, center_code: Optional[str] = None, include_deleted: bool = False, search: Optional[str] = None, app=None) -> List[Dict[str, Any]]:
    app = app or current_app
    clauses = ['1=1']
    params: List[Any] = []
...
def get_system_lab2_surface(surface_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
...
def create_system_lab2_surface(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    ...
    with _get_connection(app) as conn:
        ...
        conn.commit()
    return get_system_lab2_surface(new_id, app)


def update_system_lab2_surface(surface_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    ...
    for pct_key in ('left_pct', 'top_pct', 'width_pct', 'height_pct'):
        if pct_key in data:
            ...
            if pct_key == 'left_pct':
                ...
                params.append(_coerce_int(round(pct_val * 1000)))
    if not assignments:
        return get_system_lab2_surface(surface_id, app)
    timestamp = _now()
...
def soft_delete_system_lab2_surfaces(ids: Sequence[int], actor: str, app=None) -> int:
    app = app or current_app
    if not ids:
        return 0
    actor = (actor or 'system').strip() or 'system'
