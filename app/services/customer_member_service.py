...
TABLE_NAME = 'biz_customer_member'
...
def _resolve_db_path(app=None) -> str:
    ...
    if not uri.startswith('sqlite'):
        fallback = app.config.get('CUSTOMER_MEMBER_SQLITE_PATH')
        if fallback:
            return os.path.abspath(fallback)
        return os.path.join(app.instance_path, 'customer_member.db')
...
def _get_connection(app=None) -> sqlite3.Connection:
    ...
    try:
        ...
        logger.warning('Could not enable foreign key enforcement for customer member table')
    return conn
...
def _normalize_code(seed: str) -> str:
    base = (seed or 'MEMBER').upper()
    base = re.sub(r'[^A-Z0-9]+', '_', base).strip('_') or 'MEMBER'
    return base[:60]
...
def _generate_unique_code(conn: sqlite3.Connection, seed: str) -> str:
    ...
    while True:
        ...
        if counter > 9999:
            raise ValueError('고유 회원사 코드 생성을 실패했습니다.')
...
def _assert_unique_code(conn: sqlite3.Connection, code: str, record_id: Optional[int] = None) -> None:
    ...
    if row and (record_id is None or row['id'] != record_id):
        raise ValueError('이미 사용 중인 회원사 코드입니다.')


def init_customer_member_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            ...
            _ensure_manager_count_column(conn)
...
def _ensure_manager_count_column(conn: sqlite3.Connection) -> None:
    try:
        rows = conn.execute(f"PRAGMA table_info({TABLE_NAME})").fetchall()
    except sqlite3.DatabaseError:
        logger.warning('PRAGMA table_info failed for %s', TABLE_NAME)
        return
    existing = {row['name'] for row in rows if isinstance(row, sqlite3.Row)}
    if 'manager_count' not in existing:
        try:
            conn.execute(
                f"ALTER TABLE {TABLE_NAME} ADD COLUMN manager_count INTEGER DEFAULT 0"
            )
        except sqlite3.DatabaseError:
            logger.exception('Failed to add manager_count column to %s', TABLE_NAME)
        else:
            logger.info('manager_count column added to %s', TABLE_NAME)
    ...
    for column, aliases in mapping.items():
        for alias in aliases:
            if alias in data and data.get(alias) not in (None, ''):
                ...
                break
    if require_all:
        missing = [key for key in ('customer_name',) if not payload.get(key)]
        if missing:
...
def list_customer_members(app=None, *, search: Optional[str] = None, include_deleted: bool = False) -> List[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        ...
        query = (
            f"SELECT id, customer_code, customer_name, phone, address, manager_count, line_count, remark, created_at, created_by, updated_at, updated_by, is_deleted "
...
def get_customer_member(record_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
            f"SELECT id, customer_code, customer_name, phone, address, manager_count, line_count, remark, created_at, created_by, updated_at, updated_by, is_deleted "
...
def create_customer_member(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    ...
    with _get_connection(app) as conn:
        ...
        conn.execute(
            ...
            INSERT INTO {TABLE_NAME}
                (customer_code, customer_name, phone, address, manager_count, line_count, remark,
        ...
        conn.commit()
    return get_customer_member(new_id, app)


def update_customer_member(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=False)
    if not payload:
        return get_customer_member(record_id, app)
    with _get_connection(app) as conn:
        ...
        updates: List[str] = []
        params: List[Any] = []
        for column in ('customer_name', 'customer_code', 'phone', 'address', 'manager_count', 'line_count', 'remark'):
...
def soft_delete_customer_members(ids: Iterable[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    safe_ids: List[int] = []
    for raw in ids:
