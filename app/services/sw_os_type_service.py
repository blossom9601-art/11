...
def _resolve_db_path(app=None) -> str:
    ...
    if not uri.startswith('sqlite'):
        fallback = app.config.get('SW_OS_TYPE_SQLITE_PATH')
        if fallback:
...
def _normalize_code(seed: str) -> str:
    base = (seed or 'OS').upper()
    base = re.sub(r'[^A-Z0-9]+', '_', base).strip('_') or 'OS'
    return base[:60]
...
def _generate_unique_code(conn: sqlite3.Connection, seed: str) -> str:
    ...
    while True:
        row = conn.execute(
            f"SELECT 1 FROM {TABLE_NAME} WHERE os_code = ?",
            (candidate,),
        ).fetchone()
        if not row:
            return candidate
...
def _row_to_dict(row: sqlite3.Row, manufacturer_name: str = '') -> Dict[str, Any]:
    ...
    return {
        ...
        'license_count': row['license_count'] or 0,
    ...
    for column, aliases in mapping.items():
        for alias in aliases:
            if alias in data and data.get(alias) not in (None, ''):
                ...
                break
    if require_all:
        missing = [key for key in ('model_name', 'os_type') if not payload.get(key)]
        if missing:
            raise ValueError('필수 필드가 누락되었습니다: ' + ', '.join(missing))
    if 'license_count' in payload:
        payload['license_count'] = _sanitize_int(payload['license_count'])
    return payload
...
def init_sw_os_type_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            conn.execute(CREATE_TABLE_SQL)
            conn.execute(
                f"CREATE INDEX IF NOT EXISTS idx_{TABLE_NAME}_code ON {TABLE_NAME}(os_code)"
            )
            conn.execute(
...
def list_sw_os_types(app=None, *, search: Optional[str] = None, include_deleted: bool = False) -> List[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        ...
        query = (
            f"SELECT id, os_code, model_name, manufacturer_code, os_type, release_date, eosl_date, "
            f"license_count, remark, created_at, created_by, updated_at, updated_by, is_deleted, "
            f"0 AS usage_count "
...
def create_sw_os_type(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    ...
    with _get_connection(app) as conn:
        ...
        os_code = (payload.get('os_code') or '').strip()
        if os_code:
            _assert_unique_code(conn, os_code)
        else:
            os_code = _generate_unique_code(conn, model_name)
        conn.execute(
            f"""
            INSERT INTO {TABLE_NAME}
                (os_code, model_name, manufacturer_code, os_type, release_date, eosl_date,
                 license_count, remark, created_at, created_by, updated_at, updated_by, is_deleted)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            """,
            (
                os_code[:60],
        ...
        conn.commit()
    return get_sw_os_type(new_id, app)


def update_sw_os_type(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=False)
    if not payload:
        return get_sw_os_type(record_id, app)
    with _get_connection(app) as conn:
        updates: List[str] = []
        params: List[Any] = []
        if 'os_code' in payload:
            code = (payload['os_code'] or '').strip()
            if code:
                _assert_unique_code(conn, code, record_id)
                updates.append('os_code = ?')
                params.append(code[:60])
            else:
                payload.pop('os_code', None)
        ...
        for column in (
            ...
            if column in payload:
                ...
                if column in ('model_name', 'os_type') and not value:
...
def soft_delete_sw_os_types(ids: Iterable[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    safe_ids: List[int] = []
    for raw in ids:
