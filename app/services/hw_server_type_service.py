...
def _resolve_db_path(app=None) -> str:
    ...
    if not uri.startswith('sqlite'):
        fallback = app.config.get('HW_SERVER_TYPE_SQLITE_PATH')
        if fallback:
            return os.path.abspath(fallback)
        return os.path.join(app.instance_path, 'hw_server_type.db')
...
def _normalize_code(seed: str) -> str:
    base = (seed or 'SERVER').upper()
    base = re.sub(r'[^A-Z0-9]+', '_', base).strip('_') or 'SERVER'
    return base[:60]
...
def _generate_unique_code(conn: sqlite3.Connection, seed: str) -> str:
    ...
    while True:
        row = conn.execute(
            f"SELECT 1 FROM {TABLE_NAME} WHERE server_code = ?",
            (candidate,),
        ).fetchone()
        if not row:
            return candidate
...
def _row_to_dict(row: sqlite3.Row) -> Dict[str, Any]:
    ...
    return {
        'id': row['id'],
        'public_id': make_public_id(TABLE_NAME, 'srv', row['id']),
...
def _prepare_payload(data: Dict[str, Any], *, require_all: bool = False) -> Dict[str, Any]:
    ...
    mapping = {
        ...
        'form_factor': ['form_factor', 'hw_type'],
        'release_date': ['release_date'],
        'eosl_date': ['eosl_date', 'eosl'],
        'server_count': ['server_count', 'qty'],
        'remark': ['remark', 'note'],
    }
    for column, aliases in mapping.items():
        for alias in aliases:
            if alias in data and data.get(alias) not in (None, ''):
                ...
                break
    if require_all:
        missing = [key for key in ('model_name', 'form_factor') if not payload.get(key)]
        if missing:
            raise ValueError('필수 필드가 누락되었습니다: ' + ', '.join(missing))
    if 'server_count' in payload:
        payload['server_count'] = _sanitize_int(payload['server_count'])
    return payload
...
def init_hw_server_type_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            conn.execute(CREATE_TABLE_SQL)
            conn.execute(
                f"CREATE INDEX IF NOT EXISTS idx_{TABLE_NAME}_code ON {TABLE_NAME}(server_code)"
            )
            conn.execute(
...
def list_hw_server_types(
    ...
    *,
    ...
    form_factors: Optional[Iterable[str]] = None,
) -> List[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        ...
        params: List[Any] = []
        if form_factors:
            ff = [str(v).strip() for v in form_factors if str(v).strip()]
            if ff:
                placeholders = ','.join(['?'] * len(ff))
                clauses.append(f"form_factor IN ({placeholders})")
                params.extend(ff)
        if search:
            like = f"%{search}%"
        ...
        query = (
            f"SELECT id, server_code, model_name, manufacturer_code, form_factor, release_date, eosl_date, "
            f"server_count, remark, created_at, created_by, updated_at, updated_by, is_deleted "
    ...
    try:
        counts = usage_counts_by_server_code(app, asset_category="SERVER", include_deleted=False)
    except Exception:
        logger.exception('Failed to compute hardware_asset usage counts for %s', TABLE_NAME)
        counts = {}

    for item in items:
        code = str(item.get('server_code') or '').strip()
        item['usage_count'] = int(counts.get(code, 0) or 0)
    return items


def get_hw_server_type(record_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
            f"SELECT id, server_code, model_name, manufacturer_code, form_factor, release_date, eosl_date, "
    ...
    try:
        ...
        logger.exception('Failed to compute hardware_asset usage count for %s id=%s', TABLE_NAME, record_id)
        item['usage_count'] = 0

    return item


def create_hw_server_type(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=True)
    model_name = payload['model_name'].strip()
    form_factor = payload['form_factor'].strip()
    timestamp = _now()
    with _get_connection(app) as conn:
        manufacturer_code = _resolve_manufacturer_code(conn, payload)
        server_code = (payload.get('server_code') or '').strip()
        if server_code:
            _assert_unique_code(conn, server_code)
        else:
            server_code = _generate_unique_code(conn, model_name)
        conn.execute(
            f"""
        ...
        conn.commit()
    return get_hw_server_type(new_id, app)


def update_hw_server_type(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=False)
    if not payload:
        return get_hw_server_type(record_id, app)
    with _get_connection(app) as conn:
        updates: List[str] = []
        params: List[Any] = []
        if 'server_code' in payload:
            code = (payload['server_code'] or '').strip()
            if code:
                _assert_unique_code(conn, code, record_id)
                updates.append('server_code = ?')
                params.append(code[:60])
            else:
                payload.pop('server_code', None)
        ...
        for column in (
            ...
            if column in payload:
                ...
                if column in ('model_name', 'form_factor') and not value:
...
def soft_delete_hw_server_types(ids: Iterable[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    safe_ids: List[int] = []
    for raw in ids:
