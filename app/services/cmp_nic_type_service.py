...
def _resolve_db_path(app=None) -> str:
    ...
    override = app.config.get('CMP_NIC_TYPE_SQLITE_PATH')
    if override:
        return os.path.abspath(override)
    if not uri.startswith('sqlite'):
        return os.path.join(app.instance_path, 'cmp_nic_type.db')
    parsed = urlparse(uri)
    path = parsed.path or ''
...
def _normalize_code(seed: str) -> str:
    base = (seed or 'NIC').upper()
    base = re.sub(r'[^A-Z0-9]+', '_', base).strip('_') or 'NIC'
    return base[:60]
...
def _generate_unique_code(conn: sqlite3.Connection, seed: str) -> str:
    ...
    while True:
        row = conn.execute(
            f"SELECT 1 FROM {TABLE_NAME} WHERE nic_code = ?",
            (candidate,),
        ).fetchone()
        if not row:
            return candidate
...
def _prepare_payload(data: Dict[str, Any], *, require_all: bool = False) -> Dict[str, Any]:
    ...
    if 'nic_count' in payload:
        payload['nic_count'] = _sanitize_int(payload['nic_count'])
    return payload
...
def init_cmp_nic_type_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            conn.execute(CREATE_TABLE_SQL)
            conn.execute(
                f"CREATE INDEX IF NOT EXISTS idx_{TABLE_NAME}_code ON {TABLE_NAME}(nic_code)"
            )
            conn.execute(
...
def list_cmp_nic_types(
    ...
    *,
    ...
    app = app or current_app
    with _get_connection(app) as conn:
        clauses = ['1=1' if include_deleted else 'n.is_deleted = 0']
        params: List[Any] = []
        if search:
            like = f"%{search}%"
            f = (field or '').strip().lower()
            if f in ('model', 'model_name'):
                clauses.append('n.model_name LIKE ?')
                params.append(like)
            elif f in ('spec', 'spec_summary'):
                clauses.append('n.spec_summary LIKE ?')
                params.append(like)
            elif f in ('vendor', 'manufacturer'):
                clauses.append('(n.manufacturer_code LIKE ? OR v.manufacturer_name LIKE ?)')
        ...
        query = (
            f"SELECT n.id, n.nic_code, n.model_name, n.spec_summary, n.manufacturer_code, n.part_number, "
            f"n.nic_count, n.remark, n.created_at, n.created_by, n.updated_at, n.updated_by, n.is_deleted, "
            f"v.manufacturer_name, "
            f"0 AS usage_count "
            f"FROM {TABLE_NAME} n "
            f"LEFT JOIN {MANUFACTURER_TABLE} v ON v.manufacturer_code = n.manufacturer_code AND v.is_deleted = 0 "
            f"WHERE {' AND '.join(clauses)} ORDER BY n.id DESC"
        )
        rows = conn.execute(query, params).fetchall()
        return [_row_to_dict(row) for row in rows]


def get_cmp_nic_type(record_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
            f"SELECT n.id, n.nic_code, n.model_name, n.spec_summary, n.manufacturer_code, n.part_number, "
            ...
            f"v.manufacturer_name "
            f"FROM {TABLE_NAME} n "
            f"LEFT JOIN {MANUFACTURER_TABLE} v ON v.manufacturer_code = n.manufacturer_code "
            f"WHERE n.id = ?",
            (record_id,),
        ).fetchone()
...
def create_cmp_nic_type(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    ...
    with _get_connection(app) as conn:
        ...
        nic_code = (payload.get('nic_code') or '').strip()
        if nic_code:
            _assert_unique_code(conn, nic_code)
        else:
            nic_code = _generate_unique_code(conn, model_name)
        conn.execute(
            f"""
            INSERT INTO {TABLE_NAME}
                (nic_code, model_name, spec_summary, manufacturer_code, part_number,
                 nic_count, remark, created_at, created_by, updated_at, updated_by, is_deleted)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            """,
            (
                nic_code[:60],
        ...
        conn.commit()
    return get_cmp_nic_type(new_id, app)


def update_cmp_nic_type(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=False)
    if not payload:
        return get_cmp_nic_type(record_id, app)
    with _get_connection(app) as conn:
        updates: List[str] = []
        params: List[Any] = []
        if 'nic_code' in payload:
            code = (payload['nic_code'] or '').strip()
            if code:
                _assert_unique_code(conn, code, record_id)
                updates.append('nic_code = ?')
                params.append(code[:60])
            else:
                payload.pop('nic_code', None)
...
def soft_delete_cmp_nic_types(ids: Iterable[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    safe_ids: List[int] = []
    for raw in ids:
