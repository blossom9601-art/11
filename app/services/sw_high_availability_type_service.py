...
def _resolve_db_path(app=None) -> str:
    ...
    override = app.config.get('SW_HA_TYPE_SQLITE_PATH')
    if override:
        return os.path.abspath(override)
    if not uri.startswith('sqlite'):
        return os.path.join(app.instance_path, 'sw_ha_type.db')
    parsed = urlparse(uri)
    path = parsed.path or ''
...
def _normalize_code(seed: str) -> str:
    base = (seed or 'HA').upper()
    base = re.sub(r'[^A-Z0-9]+', '_', base).strip('_') or 'HA'
    return base[:60]
...
def _generate_unique_code(conn: sqlite3.Connection, seed: str) -> str:
    ...
    while True:
        row = conn.execute(
            f"SELECT 1 FROM {TABLE_NAME} WHERE ha_code = ?",
            (candidate,),
        ).fetchone()
        if not row:
            return candidate
...
def _row_to_dict(row: sqlite3.Row) -> Dict[str, Any]:
    ...
    return {
        'id': row['id'],
        'public_id': make_public_id(TABLE_NAME, 'ha', row['id']),
        'ha_code': row['ha_code'],
        'ha_name': name,
        'model': name,
...
def _prepare_payload(data: Dict[str, Any], *, require_all: bool = False) -> Dict[str, Any]:
    ...
    for column, aliases in mapping.items():
        for alias in aliases:
            if alias in data and data.get(alias) not in (None, ''):
                ...
                break
    if require_all:
        missing = [key for key in ('ha_name', 'ha_mode') if not payload.get(key)]
        if missing:
    ...
    if 'ha_count' in payload:
        payload['ha_count'] = _sanitize_int(payload['ha_count'])
    return payload
...
def init_sw_ha_type_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            conn.execute(CREATE_TABLE_SQL)
            conn.execute(
                f"CREATE INDEX IF NOT EXISTS idx_{TABLE_NAME}_code ON {TABLE_NAME}(ha_code)"
            )
            conn.execute(
...
def list_sw_ha_types(app=None, *, search: Optional[str] = None, include_deleted: bool = False) -> List[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        clauses = ['1=1' if include_deleted else 'h.is_deleted = 0']
        params: List[Any] = []
        if search:
            like = f"%{search}%"
            clauses.append('(' + ' OR '.join([
                'h.ha_code LIKE ?',
                'h.ha_name LIKE ?',
                'h.manufacturer_code LIKE ?',
                'h.ha_mode LIKE ?',
                'h.remark LIKE ?'
            ]) + ')')
            params.extend([like] * 5)
        query = (
            f"SELECT h.id, h.ha_code, h.ha_name, h.manufacturer_code, h.ha_mode, "
            f"h.release_date, h.eosl_date, h.ha_count, h.remark, h.created_at, h.created_by, "
            f"h.updated_at, h.updated_by, h.is_deleted, m.manufacturer_name, "
            f"0 AS usage_count "
            f"FROM {TABLE_NAME} h "
            f"LEFT JOIN {MANUFACTURER_TABLE} m ON m.manufacturer_code = h.manufacturer_code "
            f"WHERE {' AND '.join(clauses)} ORDER BY h.id DESC"
        )
        rows = conn.execute(query, params).fetchall()
        return [_row_to_dict(row) for row in rows]


def get_sw_ha_type(record_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
            f"SELECT h.id, h.ha_code, h.ha_name, h.manufacturer_code, h.ha_mode, "
...
def create_sw_ha_type(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=True)
    ha_name = payload['ha_name'].strip()
    ha_mode = payload['ha_mode'].strip()
    timestamp = _now()
    with _get_connection(app) as conn:
        manufacturer_code = _resolve_manufacturer_code(conn, payload)
        ha_code = (payload.get('ha_code') or '').strip()
        if ha_code:
            _assert_unique_code(conn, ha_code)
        else:
            ha_code = _generate_unique_code(conn, ha_name)
        conn.execute(
            f"""
            INSERT INTO {TABLE_NAME}
                (ha_code, ha_name, manufacturer_code, ha_mode, release_date, eosl_date,
                 ha_count, remark, created_at, created_by, updated_at, updated_by, is_deleted)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            """,
            (
                ha_code[:60],
        ...
        conn.commit()
    return get_sw_ha_type(new_id, app)


def update_sw_ha_type(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=False)
    if not payload:
        return get_sw_ha_type(record_id, app)
    with _get_connection(app) as conn:
        updates: List[str] = []
        params: List[Any] = []
        if 'ha_code' in payload:
            code = (payload['ha_code'] or '').strip()
            if code:
                _assert_unique_code(conn, code, record_id)
                updates.append('ha_code = ?')
                params.append(code[:60])
            else:
                payload.pop('ha_code', None)
        ...
        for column in (
            ...
            if column in payload:
                ...
                if column in ('ha_name', 'ha_mode') and not value:
...
def soft_delete_sw_ha_types(ids: Iterable[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    safe_ids: List[int] = []
    for raw in ids:
