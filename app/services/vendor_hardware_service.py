...
TABLE_NAME = 'biz_vendor_hardware'
...
def _resolve_db_path(app=None) -> str:
    ...
    if not uri.startswith('sqlite'):
        fallback = app.config.get('VENDOR_HARDWARE_SQLITE_PATH')
        if fallback:
            return os.path.abspath(fallback)
        return os.path.join(app.instance_path, 'vendor_hardware.db')
...
def init_vendor_hardware_table(app=None) -> None:
    app = app or current_app
    try:
...
def _recalc_vendor_hardware_count(conn: sqlite3.Connection, *, vendor_kind: str, vendor_id: int) -> None:
    ...
    if vendor_kind == 'manufacturer':
        ...
        conn.execute(
            "UPDATE biz_vendor_manufacturer SET hw_count = ? WHERE id = ?",
            (total, vendor_id),
        )
    else:
        row = conn.execute(
        ...
        total = int(row['total'] or 0) if row else 0
        conn.execute(
            "UPDATE biz_vendor_maintenance SET hw_count = ? WHERE id = ?",
            (total, vendor_id),
        )


def create_vendor_hardware(payload: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    ...
    with _get_connection(app) as conn:
        ...
        _recalc_vendor_hardware_count(conn, vendor_kind=vendor_kind, vendor_id=vendor_id)
        row = conn.execute(f"SELECT * FROM {TABLE_NAME} WHERE id = ?", (new_id,)).fetchone()
        conn.commit()
        return _row_to_dict(row)


def update_vendor_hardware(item_id: int, payload: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    actor = (actor or 'system').strip() or 'system'
...
def delete_vendor_hardware(item_id: int, *, actor: str, app=None) -> bool:
    actor = (actor or 'system').strip() or 'system'
    item_id = _sanitize_int(item_id)
    if item_id <= 0:
        return False
    ...
    with _get_connection(app) as conn:
        ...
        _recalc_vendor_hardware_count(
            conn,
            vendor_kind=existing['vendor_kind'],
