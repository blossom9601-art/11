...
LAB_CONFIG = {
    'lab1': {
        'table': 'system_lab1_cctv',
        'center': '퓨처센터(5층)'
    },
    'lab2': {
        'table': 'system_lab2_cctv',
        'center': '퓨처센터(6층)'
    },
    'lab3': {
        'table': 'system_lab3_cctv',
        'center': '을지트윈타워(15층)'
    },
    'lab4': {
        'table': 'system_lab4_cctv',
        'center': '재해복구센터(4층)'
    },
}
...
def _build_insert_values(payload: Dict[str, Any], timestamp: str, actor: str) -> Tuple[Any, ...]:
    return (
        payload['cctv_code'],
        payload['name'],
        payload['status'],
...
def _resolve_db_path(app=None) -> str:
    ...
    direct = app.config.get('SYSTEM_LAB_CCTV_SQLITE_PATH')
    if direct:
...
def _ensure_table(key: str, app=None) -> None:
    ...
    schema_sql = f"""
        CREATE TABLE IF NOT EXISTS {table} (
            ...
            FOREIGN KEY (cctv_code) REFERENCES cctv(device_code),
...
def _row_to_dict(row: sqlite3.Row) -> Dict[str, Any]:
    ...
    return {
        'id': row['id'],
        'cctv_code': row['cctv_code'] or '',
        'name': row['name'] or '',
        'status': row['status'] or '',
        'cctv_type': row['cctv_type'] or '',
...
def _base_payload(data: Dict[str, Any], meta: Dict[str, str]) -> Dict[str, Any]:
    name = (data.get('name') or data.get('text') or data.get('slab_name') or '').strip()
    if not name:
        raise ValueError('CCTV 이름은 필수입니다.')
    box_identifier = (data.get('box_identifier') or '').strip()
    if not box_identifier:
        box_identifier = f"cctv_{uuid.uuid4().hex[:10]}"
    payload = {
        'cctv_code': (data.get('cctv_code') or data.get('device_code') or '').strip() or None,
        'name': name,
        'status': (data.get('status') or data.get('cctv_status') or '').strip() or None,
        'cctv_type': (data.get('cctv_type') or data.get('type') or '').strip() or None,
...
def _list_records(key: str, *, search: Optional[str] = None, include_deleted: bool = False, app=None) -> List[Dict[str, Any]]:
    ...
    if search:
        like = f"%{search.strip()}%"
        clauses.append('(name LIKE ? OR status LIKE ? OR cctv_type LIKE ? OR box_identifier LIKE ? OR remark LIKE ?)')
        params.extend([like, like, like, like, like])
    query = (
        f"SELECT id, cctv_code, name, status, cctv_type, center, position_x, position_y, width_pct, height_pct, width, height, remark, racks_json, box_identifier, created_at, created_by, updated_at, updated_by, is_deleted "
...
def _get_record(key: str, record_id: int, app=None) -> Optional[Dict[str, Any]]:
    ...
    with _get_connection(app) as conn:
        row = conn.execute(
            f"SELECT id, cctv_code, name, status, cctv_type, center, position_x, position_y, width_pct, height_pct, width, height, remark, racks_json, box_identifier, created_at, created_by, updated_at, updated_by, is_deleted FROM {table} WHERE id = ?",
            (record_id,),
        ).fetchone()
...
def _create_record(key: str, data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    ...
    timestamp = _now()
    insert_sql = _build_insert_sql(table)
    with _get_connection(app) as conn:
        cur = conn.execute(
            insert_sql,
            _build_insert_values(payload, timestamp, actor),
        )
...
def _update_record(key: str, record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    ...
    params: List[Any] = []
    mutable_fields = {
        'cctv_code': lambda v: (v or '').strip() or None,
        'name': lambda v: (v or '').strip(),
        'status': lambda v: (v or '').strip() or None,
        'cctv_type': lambda v: (v or '').strip() or None,
    ...
    for key_name, transformer in mutable_fields.items():
        if key_name in data:
            ...
            if key_name == 'name' and not value:
                raise ValueError('CCTV 이름은 비워둘 수 없습니다.')
        ...
        for payload in normalized:
            ...
            if row_id:
                conn.execute(
                    f"""
                    UPDATE {table}
                    SET cctv_code = ?, name = ?, status = ?, cctv_type = ?, center = ?, position_x = ?, position_y = ?,
...
def init_system_lab1_cctv_table(app=None) -> None:
    _ensure_table('lab1', app)


def init_system_lab2_cctv_table(app=None) -> None:
    _ensure_table('lab2', app)


def init_system_lab3_cctv_table(app=None) -> None:
    _ensure_table('lab3', app)


def init_system_lab4_cctv_table(app=None) -> None:
    _ensure_table('lab4', app)


def list_system_lab1_cctvs(*, search: Optional[str] = None, include_deleted: bool = False, app=None) -> List[Dict[str, Any]]:
    return _list_records('lab1', search=search, include_deleted=include_deleted, app=app)


def list_system_lab2_cctvs(*, search: Optional[str] = None, include_deleted: bool = False, app=None) -> List[Dict[str, Any]]:
    return _list_records('lab2', search=search, include_deleted=include_deleted, app=app)


def list_system_lab3_cctvs(*, search: Optional[str] = None, include_deleted: bool = False, app=None) -> List[Dict[str, Any]]:
    return _list_records('lab3', search=search, include_deleted=include_deleted, app=app)


def list_system_lab4_cctvs(*, search: Optional[str] = None, include_deleted: bool = False, app=None) -> List[Dict[str, Any]]:
    return _list_records('lab4', search=search, include_deleted=include_deleted, app=app)


def create_system_lab1_cctv(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    return _create_record('lab1', data, actor, app)


def create_system_lab2_cctv(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    return _create_record('lab2', data, actor, app)


def create_system_lab3_cctv(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    return _create_record('lab3', data, actor, app)


def create_system_lab4_cctv(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    return _create_record('lab4', data, actor, app)


def update_system_lab1_cctv(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    return _update_record('lab1', record_id, data, actor, app)


def update_system_lab2_cctv(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    return _update_record('lab2', record_id, data, actor, app)


def update_system_lab3_cctv(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    return _update_record('lab3', record_id, data, actor, app)


def update_system_lab4_cctv(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    return _update_record('lab4', record_id, data, actor, app)


def soft_delete_system_lab1_cctvs(ids: Sequence[Any], actor: str, app=None) -> int:
    return _soft_delete_records('lab1', ids, actor, app)


def soft_delete_system_lab2_cctvs(ids: Sequence[Any], actor: str, app=None) -> int:
    return _soft_delete_records('lab2', ids, actor, app)


def soft_delete_system_lab3_cctvs(ids: Sequence[Any], actor: str, app=None) -> int:
    return _soft_delete_records('lab3', ids, actor, app)


def soft_delete_system_lab4_cctvs(ids: Sequence[Any], actor: str, app=None) -> int:
    return _soft_delete_records('lab4', ids, actor, app)


def sync_system_lab1_cctvs(items: Sequence[Dict[str, Any]], actor: str, app=None) -> Dict[str, int]:
    return _sync_records('lab1', items, actor, app)


def sync_system_lab2_cctvs(items: Sequence[Dict[str, Any]], actor: str, app=None) -> Dict[str, int]:
    return _sync_records('lab2', items, actor, app)


def sync_system_lab3_cctvs(items: Sequence[Dict[str, Any]], actor: str, app=None) -> Dict[str, int]:
    return _sync_records('lab3', items, actor, app)


def sync_system_lab4_cctvs(items: Sequence[Dict[str, Any]], actor: str, app=None) -> Dict[str, int]:
