...
def _resolve_db_path(app=None) -> str:
    ...
    override = app.config.get('CMP_MEMORY_TYPE_SQLITE_PATH')
    if override:
        return os.path.abspath(override)
    if not uri.startswith('sqlite'):
        return os.path.join(app.instance_path, 'cmp_memory_type.db')
    parsed = urlparse(uri)
    path = parsed.path or ''
...
def _normalize_code(seed: str) -> str:
    base = (seed or 'MEM').upper()
    base = re.sub(r'[^A-Z0-9]+', '_', base).strip('_') or 'MEM'
    return base[:60]
...
def _generate_unique_code(conn: sqlite3.Connection, seed: str) -> str:
    ...
    while True:
        row = conn.execute(
            f"SELECT 1 FROM {TABLE_NAME} WHERE memory_code = ?",
            (candidate,),
        ).fetchone()
        if not row:
            return candidate
...
def _prepare_payload(data: Dict[str, Any], *, require_all: bool = False) -> Dict[str, Any]:
    ...
    mapping = {
        ...
        'module_count': ['module_count', 'qty', 'count'],
    ...
    if 'module_count' in payload:
        payload['module_count'] = _sanitize_int(payload['module_count'])
    return payload
...
def init_cmp_memory_type_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            conn.execute(CREATE_TABLE_SQL)
            conn.execute(
                f"CREATE INDEX IF NOT EXISTS idx_{TABLE_NAME}_code ON {TABLE_NAME}(memory_code)"
            )
            conn.execute(
    ...
    with _get_connection(app) as conn:
        ...
        if search:
            ...
            if f in ('model', 'model_name'):
                ...
                clauses.append('m.spec_summary LIKE ?')
                params.append(like)
            elif f in ('vendor', 'manufacturer'):
                clauses.append('(m.manufacturer_code LIKE ? OR v.manufacturer_name LIKE ?)')
        ...
        query = (
            f"SELECT m.id, m.memory_code, m.model_name, m.spec_summary, m.manufacturer_code, m.part_number, "
            f"m.module_count, m.remark, m.created_at, m.created_by, m.updated_at, m.updated_by, m.is_deleted, "
            f"v.manufacturer_name, "
            f"0 AS usage_count "
            f"FROM {TABLE_NAME} m "
            f"LEFT JOIN {MANUFACTURER_TABLE} v ON v.manufacturer_code = m.manufacturer_code AND v.is_deleted = 0 "
...
def get_cmp_memory_type(record_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
            f"SELECT m.id, m.memory_code, m.model_name, m.spec_summary, m.manufacturer_code, m.part_number, "
...
def create_cmp_memory_type(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    ...
    with _get_connection(app) as conn:
        ...
        memory_code = (payload.get('memory_code') or '').strip()
        if memory_code:
            _assert_unique_code(conn, memory_code)
        else:
            memory_code = _generate_unique_code(conn, model_name)
        conn.execute(
            f"""
            INSERT INTO {TABLE_NAME}
                (memory_code, model_name, spec_summary, manufacturer_code, part_number,
                 module_count, remark, created_at, created_by, updated_at, updated_by, is_deleted)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            """,
            (
                memory_code[:60],
        ...
        conn.commit()
    return get_cmp_memory_type(new_id, app)


def update_cmp_memory_type(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=False)
    if not payload:
        return get_cmp_memory_type(record_id, app)
    with _get_connection(app) as conn:
        updates: List[str] = []
        params: List[Any] = []
        if 'memory_code' in payload:
            code = (payload['memory_code'] or '').strip()
            if code:
                _assert_unique_code(conn, code, record_id)
                updates.append('memory_code = ?')
                params.append(code[:60])
            else:
                payload.pop('memory_code', None)
...
def soft_delete_cmp_memory_types(ids: Iterable[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    safe_ids: List[int] = []
    for raw in ids:
