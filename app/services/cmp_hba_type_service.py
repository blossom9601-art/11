...
def _resolve_db_path(app=None) -> str:
    ...
    override = app.config.get('CMP_HBA_TYPE_SQLITE_PATH')
    if override:
        return os.path.abspath(override)
    if not uri.startswith('sqlite'):
        return os.path.join(app.instance_path, 'cmp_hba_type.db')
    parsed = urlparse(uri)
    path = parsed.path or ''
...
def _normalize_code(seed: str) -> str:
    base = (seed or 'HBA').upper()
    base = re.sub(r'[^A-Z0-9]+', '_', base).strip('_') or 'HBA'
    return base[:60]
...
def _generate_unique_code(conn: sqlite3.Connection, seed: str) -> str:
    ...
    while True:
        row = conn.execute(
            f"SELECT 1 FROM {TABLE_NAME} WHERE hba_code = ?",
            (candidate,),
        ).fetchone()
        if not row:
            return candidate
...
def _prepare_payload(data: Dict[str, Any], *, require_all: bool = False) -> Dict[str, Any]:
    ...
    if 'hba_count' in payload:
        payload['hba_count'] = _sanitize_int(payload['hba_count'])
    return payload
...
def init_cmp_hba_type_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            conn.execute(CREATE_TABLE_SQL)
            conn.execute(
                f"CREATE INDEX IF NOT EXISTS idx_{TABLE_NAME}_code ON {TABLE_NAME}(hba_code)"
            )
            conn.execute(
    ...
    with _get_connection(app) as conn:
        ...
        if search:
            ...
            if f in ('model', 'model_name'):
                clauses.append('h.model_name LIKE ?')
                params.append(like)
            elif f in ('spec', 'spec_summary'):
                clauses.append('h.spec_summary LIKE ?')
                params.append(like)
            elif f in ('vendor', 'manufacturer'):
                clauses.append('(h.manufacturer_code LIKE ? OR v.manufacturer_name LIKE ?)')
        ...
        query = (
            f"SELECT h.id, h.hba_code, h.model_name, h.spec_summary, h.manufacturer_code, h.part_number, "
            f"h.hba_count, h.remark, h.created_at, h.created_by, h.updated_at, h.updated_by, h.is_deleted, "
            f"v.manufacturer_name, "
            f"0 AS usage_count "
            f"FROM {TABLE_NAME} h "
            f"LEFT JOIN {MANUFACTURER_TABLE} v ON v.manufacturer_code = h.manufacturer_code AND v.is_deleted = 0 "
...
def get_cmp_hba_type(record_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
            f"SELECT h.id, h.hba_code, h.model_name, h.spec_summary, h.manufacturer_code, h.part_number, "
            ...
            f"v.manufacturer_name "
            f"FROM {TABLE_NAME} h "
            f"LEFT JOIN {MANUFACTURER_TABLE} v ON v.manufacturer_code = h.manufacturer_code "
            f"WHERE h.id = ?",
            (record_id,),
        ).fetchone()
...
def create_cmp_hba_type(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    ...
    with _get_connection(app) as conn:
        ...
        hba_code = (payload.get('hba_code') or '').strip()
        if hba_code:
            _assert_unique_code(conn, hba_code)
        else:
            hba_code = _generate_unique_code(conn, model_name)
        conn.execute(
            f"""
            INSERT INTO {TABLE_NAME}
                (hba_code, model_name, spec_summary, manufacturer_code, part_number,
                 hba_count, remark, created_at, created_by, updated_at, updated_by, is_deleted)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            """,
            (
                hba_code[:60],
        ...
        conn.commit()
    return get_cmp_hba_type(new_id, app)


def update_cmp_hba_type(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=False)
    if not payload:
        return get_cmp_hba_type(record_id, app)
    with _get_connection(app) as conn:
        updates: List[str] = []
        params: List[Any] = []
        if 'hba_code' in payload:
            code = (payload['hba_code'] or '').strip()
            if code:
                _assert_unique_code(conn, code, record_id)
                updates.append('hba_code = ?')
                params.append(code[:60])
            else:
                payload.pop('hba_code', None)
...
def soft_delete_cmp_hba_types(ids: Iterable[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    safe_ids: List[int] = []
    for raw in ids:
