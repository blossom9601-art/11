...
TABLE_NAME = 'sw_db_type'
MANUFACTURER_TABLE = 'biz_vendor_manufacturer'

CREATE_TABLE_SQL = f"""
CREATE TABLE IF NOT EXISTS {TABLE_NAME} (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    db_code TEXT NOT NULL UNIQUE,
    db_name TEXT NOT NULL,
    manufacturer_code TEXT NOT NULL,
    db_family TEXT NOT NULL,
    release_date TEXT,
    eosl_date TEXT,
...
def _resolve_db_path(app=None) -> str:
    ...
    if not uri.startswith('sqlite'):
        fallback = app.config.get('SW_DB_TYPE_SQLITE_PATH')
        if fallback:
            return os.path.abspath(fallback)
        return os.path.join(app.instance_path, 'sw_db_type.db')
...
def _normalize_code(seed: str) -> str:
    base = (seed or 'DB').upper()
    base = re.sub(r'[^A-Z0-9]+', '_', base).strip('_') or 'DB'
    return base[:60]
...
def _generate_unique_code(conn: sqlite3.Connection, seed: str) -> str:
    ...
    while True:
        row = conn.execute(
            f"SELECT 1 FROM {TABLE_NAME} WHERE db_code = ?",
            (candidate,),
        ).fetchone()
        if not row:
            return candidate
...
def _row_to_dict(row: sqlite3.Row) -> Dict[str, Any]:
    ...
    return {
        'id': row['id'],
        'public_id': make_public_id(TABLE_NAME, 'db', row['id']),
        'db_code': row['db_code'],
        'db_name': row['db_name'],
        'manufacturer_code': row['manufacturer_code'],
        'db_family': row['db_family'],
    ...
    for column, aliases in mapping.items():
        for alias in aliases:
            if alias in data and data.get(alias) not in (None, ''):
                ...
                break
    if require_all:
        missing = [key for key in ('db_name', 'db_family') if not payload.get(key)]
        if missing:
            raise ValueError('필수 필드가 누락되었습니다: ' + ', '.join(missing))
    if 'db_count' in payload:
        payload['db_count'] = _sanitize_int(payload['db_count'])
    return payload
...
def init_sw_db_type_table(app=None) -> None:
    app = app or current_app
    try:
        with _get_connection(app) as conn:
            conn.execute(CREATE_TABLE_SQL)
            conn.execute(
                f"CREATE INDEX IF NOT EXISTS idx_{TABLE_NAME}_code ON {TABLE_NAME}(db_code)"
            )
            conn.execute(
...
def list_sw_db_types(app=None, *, search: Optional[str] = None, include_deleted: bool = False) -> List[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        ...
        query = (
            f"SELECT id, db_code, db_name, manufacturer_code, db_family, release_date, eosl_date, "
            f"db_count, remark, created_at, created_by, updated_at, updated_by, is_deleted, "
            f"0 AS usage_count "
...
def get_sw_db_type(record_id: int, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    with _get_connection(app) as conn:
        row = conn.execute(
            f"SELECT id, db_code, db_name, manufacturer_code, db_family, release_date, eosl_date, "
...
def create_sw_db_type(data: Dict[str, Any], actor: str, app=None) -> Dict[str, Any]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=True)
    db_name = payload['db_name'].strip()
    db_family = payload['db_family'].strip()
    timestamp = _now()
    with _get_connection(app) as conn:
        manufacturer_code = _resolve_manufacturer_code(conn, payload)
        db_code = (payload.get('db_code') or '').strip()
        if db_code:
            _assert_unique_code(conn, db_code)
        else:
            db_code = _generate_unique_code(conn, db_name)
        conn.execute(
            f"""
            INSERT INTO {TABLE_NAME}
                (db_code, db_name, manufacturer_code, db_family, release_date, eosl_date,
                 db_count, remark, created_at, created_by, updated_at, updated_by, is_deleted)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            """,
            (
                db_code[:60],
        ...
        conn.commit()
    return get_sw_db_type(new_id, app)


def update_sw_db_type(record_id: int, data: Dict[str, Any], actor: str, app=None) -> Optional[Dict[str, Any]]:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    payload = _prepare_payload(data, require_all=False)
    if not payload:
        return get_sw_db_type(record_id, app)
    with _get_connection(app) as conn:
        updates: List[str] = []
        params: List[Any] = []
        if 'db_code' in payload:
            code = (payload['db_code'] or '').strip()
            if code:
                _assert_unique_code(conn, code, record_id)
                updates.append('db_code = ?')
                params.append(code[:60])
            else:
                payload.pop('db_code', None)
        ...
        for column in (
            ...
            if column in payload:
                ...
                if column in ('db_name', 'db_family') and not value:
...
def soft_delete_sw_db_types(ids: Iterable[Any], actor: str, app=None) -> int:
    app = app or current_app
    actor = (actor or 'system').strip() or 'system'
    safe_ids: List[int] = []
    for raw in ids:
