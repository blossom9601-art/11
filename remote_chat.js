...
(function(){
  ...
  let lastListStructSig = null;

  function computeRoomsSignature(list){
    if (!Array.isArray(list) || !list.length) return '';
    return list.map(conv => {
      // Use second-precision timestamp to avoid sub-second jitter between
      // the conversations list API and the messages API causing spurious re-renders.
      const ts = Number.isFinite(Number(conv.lastInteracted)) ? Math.floor(Number(conv.lastInteracted) / 1000) : 0;
      ...
      return `${conv.id}|${roomId}|${conv.preview||''}|${ts}|${unread}|${fav}`;
    }).join(';;');
  }
  ...
  function isV2ChatApi(){
    return /\/api\/chat\/v2(?:\/|$)/.test(chatConfig.apiRoot || '') || /\/api\/chat\/v2\/channels(?:\/|$)/.test(getRoomsUrl());
  }

  function getLegacyChatApiRoot(){
    if (!chatConfig.apiRoot) return '/api/chat';
    return isV2ChatApi() ? chatConfig.apiRoot.replace(/\/v2(?:\/)?$/, '') : chatConfig.apiRoot;
  }

  function getServerConversationKey(conv){
    if (!conv) return null;
    return conv.roomId != null ? conv.roomId : (conv.conversationId != null ? conv.conversationId : null);
  }

  function findConversationByServerKey(serverKey){
    if (serverKey == null) return null;
    const needle = String(serverKey);
    return conversations.find(conv => String(getServerConversationKey(conv)) === needle) || null;
  }
  ...
  function buildConversationMessagesUrl(conv, params){
    if (!conv) return '';
    if (isV2ChatApi()) {
      const query = params || new URLSearchParams();
      query.set('conversationId', String(conv.conversationId));
      return `${chatConfig.apiRoot}/messages?${query.toString()}`;
    }
    return `${buildRoomUrl(conv.roomId)}/messages?${(params || new URLSearchParams()).toString()}`;
  }

  function buildChannelDetailUrl(channelId){
    return `${getRoomsUrl().replace(/\/+$/, '')}/${channelId}`;
  }
  ...
  function normalizeRoomFromApi(room){
    ...
    // v2 DIRECT conversation (DM)
    if (isV2ChatApi() && room.conversation_type === 'DIRECT') {
      const convId = room.id;
      const partner = room.partner || null;
      const partnerName = partner?.name || room.title || 'DM';
      const partnerAvatar = resolveAvatarSrc(partner?.avatar, partnerName);
      ...
      const isSelf = !!room.is_self_dm || (state.currentUserId != null && partner && Number(partner.id) === Number(state.currentUserId));
      const displayName = isSelf ? '나에게' : partnerName;
      ...
      return {
        ...
        roomType: isSelf ? 'SELF' : 'DIRECT',
        createdByUserId: room.created_by ?? null,
        name: displayName,
    ...
    if (room.conversation_id != null || room.channel || room.conversation_type === 'CHANNEL') {
      const channel = room.channel || room;
      const members = Array.isArray(room.members) ? room.members.map(normalizeMemberFromApi).filter(Boolean) : [];
      const lastStamp = room.last_message_at || room.updated_at || room.created_at || channel.created_at;
      const lastInteracted = (lastStamp ? parseTimestampMs(lastStamp) : null) ?? 0;
      return {
        id: `channel-${channel.id}`,
        channelId: channel.id,
        conversationId: channel.conversation_id || room.id,
        roomType: 'CHANNEL',
        createdByUserId: room.created_by ?? room.createdByUserId ?? channel.created_by ?? null,
        name: channel.name || room.title || `Channel #${channel.id}`,
        preview: room.last_message_preview || channel.description || channel.topic || '',
        time: formatListTime(lastStamp) || '',
        displayTime: formatListTime(lastStamp) || '',
        lastInteracted,
        unread: 0,
        ...
        email: '',
        dept: '',
        avatar: resolveAvatarSrc('', channel.name || room.title || `channel-${channel.id}`),
        groupAvatars: members.length ? members.slice(0, 3).map(m => resolveAvatarSrc(m.avatar, m?.name || channel.name || 'member')) : [],
        previewSignature: computePreviewSignature(room.last_message_preview || channel.description || channel.topic || '', lastInteracted),
      };
    }
  ...
  function mapMessageFromApi(message){
    ...
    const when = message.created_at || message.updated_at || message.createdAt || message.updatedAt;
    const createdAt = when ? parseTimestampMs(when) : null;
    const isSystem = !!(message.is_system || String(message.content_type || message.type || '').toUpperCase() === 'SYSTEM');
    const senderUserId = message.sender_user_id != null ? message.sender_user_id : message.senderId;
    const isMine = !isSystem && state.currentUserId != null && Number(senderUserId) === Number(state.currentUserId);
    const rawFiles = Array.isArray(message.files) ? message.files : (Array.isArray(message.attachments) ? message.attachments : []);
    const files = rawFiles.map(f => ({
          id: f.id,
          name: f.original_name || f.fileName || f.name || '파일',
          size: f.file_size || f.fileSize,
  ...
  function ensureLiveRefreshTimer(){
    ...
    const hasLiveRooms = state.isLive || conversations.some(conv => !!getServerConversationKey(conv));
    if (!hasLiveRooms) return;
    liveRefreshTimer = setInterval(()=>{
      hydrateFromApi({ silent: true });
      const activeConv = conversations.find(c => c.id === activeId && getServerConversationKey(c));
      if (activeConv) {
        requestMessageRefresh(getServerConversationKey(activeConv));
      }
    }, POLL_INTERVAL_MS);
  }
  ...
  async function hydrateDirectoryFromApi(){
    ...
    try {
      ...
      if (filter === 'colleagues' || filter === 'chat') {
  ...
  async function hydrateFromApi(options = {}){
    ...
    try {
      ...
      const endpoint = isV2ChatApi()
        ? getRoomsUrl()
        : `${getRoomsUrl()}?${[
            'include_members=1',
            'limit=100',
            state.currentUserId != null ? `user_id=${state.currentUserId}` : null,
          ].filter(Boolean).join('&')}`;
      ...
      const mappedRaw = rows.map(normalizeRoomFromApi).filter(Boolean);
      ...
      const mapped = filtered.map(conv => {
        ...
        // Preserve _detailLoaded so ensureV2ConversationDetail is not retried on every poll.
        const _detailLoaded = !!(previous?._detailLoaded);
        return { ...conv, unread, lastReadMessageId, previewSignature: newSignature, _detailLoaded };
      });
      ...
      if (!silent) {
        ...
        if (activeId) {
          setActive(activeId);
          if (getServerConversationKey(firstRoom)) ensureMessagesLoaded(getServerConversationKey(firstRoom));
        } else {
          setActive(null);
        }
      } else {
        ...
        if (previousActive && knownIds.has(previousActive)) {
          ...
          if (getServerConversationKey(activeRoom)) requestMessageRefresh(getServerConversationKey(activeRoom));
        } else {
  ...
  function ensureMessagesLoaded(roomId, options = {}){
    ...
    if (!roomId) return;
    const conv = findConversationByServerKey(roomId);
    if (!conv) return;
  ...
  async function fetchRoomMessages(roomId, options = {}){
    ...
    try {
      ...
      const conv = findConversationByServerKey(roomId);
      if (!conv) return;
      const params = isV2ChatApi()
        ? new URLSearchParams({ limit: String(MESSAGE_FETCH_LIMIT) })
        : new URLSearchParams({
            per_page: MESSAGE_FETCH_LIMIT,
            order: MESSAGE_FETCH_ORDER,
            include_files: '1',
          });
      if (!isV2ChatApi() && state.currentUserId != null) {
        params.set('viewer_user_id', String(state.currentUserId));
      }
      const resp = await fetch(buildConversationMessagesUrl(conv, params), { credentials: 'same-origin' });
      if (resp.status === 404) {
      ...
      const data = await resp.json();
      const rawItems = Array.isArray(data?.items) ? data.items : (Array.isArray(data?.rows) ? data.rows : []);
        ...
        if (conv.id === activeId) {
          ...
          if (isV2ChatApi()) {
            ...
            if (latest && latest.id != null) {
              fetch(`${chatConfig.apiRoot}/messages/${latest.id}/read`, {
          ...
          if (lastRenderedFilter !== 'colleagues' && lastRenderedFilter !== 'chat') {
  ...
  function renderConversationList(filter='all', q=''){
    ...
    var items = conversations
      .filter(function(c){
        if (filter === 'all') return String(c.roomType || '').toUpperCase() === 'CHANNEL';
        if (filter === 'chat') return String(c.roomType || '').toUpperCase() !== 'CHANNEL';
        return true;
      })
      .filter(function(c){
        if (!ql) return true;
    ...
    // Two-level signature strategy:
    // structSig  — item ids in order; when this changes do a full DOM rebuild.
    // contentSig — preview/time/unread/active; when only this changes do in-place updates.
    // This prevents avatar/icon images from being destroyed and recreated every second
    // (which was causing the channel item to visually flicker/disappear).
    var structSig = items.length ? items.map(function(c){ return c.id; }).join(',') : '__empty';
    var contentSig = items.map(function(c){
      return [c.id, c.preview||'', c.displayTime||c.time||'', c.unread||0, c.id===activeId?1:0].join('|');
    }).join(';;');
    var sig = structSig + '||' + contentSig;
    if (sig === lastListRenderSignature) return;

    var needsFullRebuild = (structSig !== lastListStructSig);
    lastListRenderSignature = sig;
    lastListStructSig = structSig;

    // --- In-place update (structure unchanged) ---
    if (!needsFullRebuild && listEl.children.length) {
      items.forEach(function(c) {
        var li = listEl.querySelector('[data-id="' + CSS.escape(String(c.id)) + '"]');
        if (!li) return;
        // active class
        if (c.id === activeId) li.classList.add('active'); else li.classList.remove('active');
        // preview text
        var previewEl = li.querySelector('.preview');
        if (c.preview) {
          if (previewEl) { previewEl.textContent = c.preview; }
        } else {
          if (previewEl) previewEl.remove();
        }
        // right: time + unread badge
        var rightEl = li.querySelector('.right');
        if (rightEl) {
          rightEl.innerHTML = '<span class="time">' + (c.displayTime||c.time||'') + '</span>' +
    ...
    if (!items.length) {
      ...
      items.forEach(c => {
        const isChannel = String(c.roomType || '').toUpperCase() === 'CHANNEL';
        const li = createEl('li', 'chat-item' + (isChannel ? ' channel' : '') + (c.id === activeId ? ' active' : ''));
        li.dataset.id = c.id;
        const contact = primaryContact(c);
        const selfConv = isSelfConversation(c);
        var left;
        if (isChannel) {
          // Use inline SVG to avoid HTTP round-trip flicker on every re-render.
          left = createEl('div', 'channel-icon', CHANNEL_ICON_SVG);
        } else if (Array.isArray(c.groupAvatars) && c.groupAvatars.length) {
          left = (()=>{
          ...
          left = avatarImg(selfConv ? resolveSelfAvatar() : (contact?.avatar || c.avatar), contact?.name || c.name);
        }
    ...
    if (!departments.length) {
      ...
      departments.forEach(deptLabel => {
        ...
        members.forEach(entry => {
          ...
          const selfBadge = entry.isSelf ? ' <span class="self-badge">나</span>' : '';
          const main = createEl('div', null,
            `<div class="name">${entry.name || ''}${selfBadge}</div>${entry.email ? `<div class="preview">${entry.email}</div>` : ''}`
          );
  ...
  function isGroupConversation(conv){
    ...
    if (typeof conv.roomType === 'string') {
      return ['GROUP', 'CHANNEL'].includes(conv.roomType.toUpperCase());
    }
    return Array.isArray(conv.groupAvatars) && conv.groupAvatars.length >= 2;
  }

  async function ensureV2ConversationDetail(conv){
    if (!isV2ChatApi() || !conv || !conv.channelId || conv._detailLoaded || conv._detailLoading) return;
    conv._detailLoading = true;
    try {
      const resp = await fetch(buildChannelDetailUrl(conv.conversationId || conv.channelId), { credentials: 'same-origin' });
      if (!resp.ok) throw new Error('channel detail fetch failed');
      const data = await resp.json();
      const item = data && data.item ? data.item : null;
      if (!item) return;
      const members = Array.isArray(item.members) ? item.members.map(normalizeMemberFromApi).filter(Boolean) : [];
      conv.members = members;
      conv.groupAvatars = members.length ? members.slice(0, 3).map(m => resolveAvatarSrc(m.avatar, m?.name || conv.name || 'member')) : [];
      conv.createdByUserId = item.created_by ?? item.owner_user_id ?? conv.createdByUserId;
      conv.preview = item.last_message_preview || conv.preview;
      conv.lastInteracted = parseTimestampMs(item.last_message_at || item.updated_at || item.created_at) || conv.lastInteracted;
      conv.previewSignature = computePreviewSignature(conv.preview || '', conv.lastInteracted || 0);
      conv._detailLoaded = true;
      if (activeId === conv.id) {
        updateProfilePanel(conv, true);
      ...
      // Mark as loaded to prevent repeated retries on permanent errors (e.g. 404).
      conv._detailLoaded = true;
    } finally {
      conv._detailLoading = false;
    }
  }
  ...
  function updateProfilePanel(conv, isGroup){
    ...
    if (profile.avatarEl) {
      const isChannel = conv && String(conv.roomType || '').toUpperCase() === 'CHANNEL';
      if (isChannel) {
        profile.avatarEl.innerHTML = '<div class="channel-icon">' + CHANNEL_ICON_SVG + '</div>';
      } else {
  ...
  async function createDirectRoomForEntry(entry){
    if (isV2ChatApi()) {
      return await createV2DmForEntry(entry);
    }
  ...
  async function createV2DmForEntry(entry){
    if (!entry || entry.userId == null) return null;
    const apiRoot = chatConfig.apiRoot || '/api/chat/v2';
    try {
      const resp = await fetch(`${apiRoot}/dm`, {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: entry.userId }),
      });
      if (!resp.ok) throw new Error('dm create failed');
      const body = await resp.json();
      const item = (body && (body.item || body)) || null;
      const normalized = normalizeRoomFromApi(item);
      if (!normalized) return null;
      const existingIdx = conversations.findIndex(c => c.id === normalized.id);
      if (existingIdx >= 0) {
        conversations[existingIdx] = Object.assign(conversations[existingIdx], normalized);
      } else {
        state.isLive = true;
        conversations.unshift(normalized);
        messageMap[normalized.id] = messageMap[normalized.id] || [];
      ...
      syncFavoriteFlag(normalized);
      rebuildDirectory();
      return conversations.find(c => c.id === normalized.id) || normalized;
    } catch (err) {
      console.warn('[chat] v2 DM create failed', err);
      return null;
    }
  }
  ...
  async function startConversationFromEntry(entry){
    ...
    if (isV2ChatApi()) {
      ...
      const dm = await createV2DmForEntry(entry);
      if (dm) {
        setActive(dm.id);
        renderList(getActiveFilter(), searchEl.value);
      } else {
        window.alert('DM 생성에 실패했습니다. 잠시 후 다시 시도해주세요.');
      }
      return;
    }
  ...
  async function inviteEntryToV2Channel(entry, conv){
    if (!entry || !conv || !conv.channelId) return false;
    const userId = entry.userId;
    if (userId == null) {
      window.alert('초대 대상의 사용자 정보를 확인할 수 없습니다.');
      return false;
    }
    try {
      const url = `${getRoomsUrl().replace(/\/+$/, '')}/${conv.channelId}/invite`;
      const resp = await fetch(url, {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ members: [{ userId: userId, role: 'member' }] }),
      });
      if (!resp.ok) throw new Error('invite failed');
      await ensureV2ConversationDetail(conv);
      updateProfilePanel(conv, true);
      return true;
    } catch (err) {
      console.warn('[chat] v2 channel invite failed', err);
      window.alert('채널 초대 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.');
      return false;
    }
  }
  ...
  async function addEntryToActiveConversation(entry){
    ...
    if (isV2ChatApi() && activeConv.channelId) {
      await inviteEntryToV2Channel(entry, activeConv);
      return;
    }
  ...
  function setActive(id){
    ...
    if (leaveBtn) {
      if (isV2ChatApi() && conv && conv.channelId) {
        leaveBtn.disabled = false;
        leaveBtn.setAttribute('aria-disabled', 'false');
        leaveBtn.title = '채널 나가기';
      } else {
      const isCreator = !!(
    ...
    if (addMemberBtn) {
      const canInvite = !!conv && (
        (isV2ChatApi() && conv.channelId)
        || (!isV2ChatApi() && !!conv.roomId)
        || !isV2ChatApi()
      );
      addMemberBtn.disabled = !canInvite;
      addMemberBtn.setAttribute('aria-disabled', canInvite ? 'false' : 'true');
      addMemberBtn.title = canInvite ? (isV2ChatApi() && conv.channelId ? '채널 멤버 초대' : '대화상대 추가') : '대화상대 추가';
    }
    if (thread.nameEl) {
    ...
    if (conv && state.isLive && getServerConversationKey(conv)) {
      ensureMessagesLoaded(getServerConversationKey(conv), { force: true });
      ensureV2ConversationDetail(conv);
    }
  ...
  function updateSearchPlaceholder(byFilter){
    ...
    else if (byFilter === 'chat') searchEl.setAttribute('placeholder', 'DM 또는 그룹 채팅 검색...');
    else searchEl.setAttribute('placeholder', '채널 검색...');
  }
  ...
  async function send(){
    ...
    if (requiresServer) {
      ...
      if (!isV2ChatApi() && !conv.roomId && chatConfig.roomsUrl) {
      ...
      if (!getServerConversationKey(conv)) {
        restoreDraft();
    ...
    if (getServerConversationKey(conv) && chatConfig.roomsUrl && state.currentUserId != null) {
      try {
        ...
        const payload = isV2ChatApi()
          ? {
              conversationId: conv.conversationId,
              content: text || '',
              type: (!text && attachments.length) ? 'file' : 'text',
              attachments: [],
            }
          : {
        ...
        let resp = await fetch(isV2ChatApi() ? `${chatConfig.apiRoot}/messages` : `${buildRoomUrl(conv.roomId)}/messages`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'same-origin',
          body: JSON.stringify(payload),
        ...
        if (!isV2ChatApi() && resp.status === 404) {
          try {
        ...
        const savedRaw = await resp.json();
        const saved = savedRaw && savedRaw.item ? savedRaw.item : savedRaw;
        ...
        if (attachments.length) {
          ...
          const uploadAndAttach = async (file, upRec)=>{
            const v2 = isV2ChatApi();
            const url = v2 ? `${chatConfig.apiRoot}/messages/${saved.id}/attachments` : `${chatConfig.apiRoot}/messages/${saved.id}/files`;
            const body = v2 ? {
              url: `/api/uploads/${upRec.id}/download`,
              fileName: upRec.name || file.name,
              fileSize: upRec.size,
              fileType: file.type || '',
            } : {
            ...
            const fileMetaResp = await fetch(url, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              credentials: 'same-origin',
            ...
            attachedFiles.push(fileMeta && fileMeta.item ? fileMeta.item : fileMeta);
          };
  ...
  async function ensureCurrentUserId(options = {}){
    ...
    try {
      const primaryUrl = `${chatConfig.apiRoot}/whoami`;
      let resp = await fetch(primaryUrl, { credentials: 'same-origin' });
      if (!resp.ok && isV2ChatApi()) {
        resp = await fetch(`${getLegacyChatApiRoot()}/whoami`, { credentials: 'same-origin' });
      }
      if (!resp.ok) return;
      const data = await resp.json();
  ...
  async function requestLeaveConversation(conv){
    if (!conv) return false;
    if (getServerConversationKey(conv) && chatConfig.roomsUrl) {
    ...
    try {
      if (isV2ChatApi() && conv.channelId) {
        const resp = await fetch(`${getRoomsUrl().replace(/\/+$/, '')}/${conv.channelId}/leave`, {
          method: 'POST',
          credentials: 'same-origin',
          headers: { 'Content-Type': 'application/json' },
        });
...
// ===== Robust New-Channel modal bootstrap (outside main IIFE) =====
// Runs even if the main IIFE crashes due to a runtime error elsewhere.
(function bootstrapNewChannelStandalone(){
  ...
  function getCfg(){
    ...
    return {
      apiRoot: (el && el.getAttribute('data-api-root')) || '/api/chat/v2',
  ...
  function loadDirectory(){
    ...
    return fetch(cfg.directoryUrl, { credentials: 'same-origin', headers: { 'X-Requested-With': 'XMLHttpRequest' } })
      ...
      .then(function(rows){
        ...
        console.log('[new-channel] directory loaded', directory.length);
      })
      .catch(function(err){
        directory = [];
        directoryLoaded = true;
        directoryError = (err && err.message) || '로드 실패';
        console.warn('[new-channel] directory load failed', err);
      });
  }
  function renderMembers(){
    ...
    if (!filtered.length) {
      ...
      if (directoryError) {
        li.className = 'error';
        li.textContent = '동료 목록을 불러오지 못했습니다 (' + directoryError + ')';
      } else if (!directoryLoaded) {
        li.className = 'empty';
        li.textContent = '동료 목록을 불러오는 중입니다...';
      } else {
        li.className = 'empty';
    ...
    if (matched.length > LIMIT) {
      ...
      more.textContent = '… ' + (matched.length - LIMIT) + '명 더 있음. 검색으로 좌혀보세요.';
  ...
  function hideDeptPopover(){
    var pop = $('new-channel-dept-popover');
    if (pop) { pop.hidden = true; }
  }
  function toggleDeptPopover(){
    var pop = $('new-channel-dept-popover');
    if (!pop) return;
    if (!pop.hidden) { pop.hidden = true; return; }
    if (!directoryLoaded) { loadDirectory().then(function(){ buildDeptPopover(); pop.hidden = false; }); return; }
    buildDeptPopover();
    pop.hidden = false;
  }
  function buildDeptPopover(){
    ...
    if (directoryError) {
      ...
      er.className = 'nc-dept-empty';
      er.textContent = '\ub3d9\ub8cc \ubaa9\ub85d \ub85c\ub4dc \uc2e4\ud328 (' + directoryError + ')';
      pop.appendChild(er); return;
    }
    var groups = {};
    ...
    if (!deptNames.length) {
      ...
      em.className = 'nc-dept-empty';
      em.textContent = '\ucd08\ub300\ud560 \ub3d9\ub8cc\uac00 \uc5c6\uc2b5\ub2c8\ub2e4.';
      pop.appendChild(em); return;
    }
    deptNames.forEach(function(dname){
      var sec = document.createElement('div');
      sec.className = 'nc-dept-section';
      var head = document.createElement('div');
      head.className = 'nc-dept-head';
      head.textContent = dname + ' (' + groups[dname].length + ')';
      sec.appendChild(head);
      var body = document.createElement('div');
      body.className = 'nc-dept-body';
      groups[dname].forEach(function(u){
        var uid = u.user_id != null ? u.user_id : u.id;
        var row = document.createElement('div');
        row.className = 'nc-dept-row' + (selectedIds.has(uid) ? ' selected' : '');
        row.textContent = u.name || u.display_name || ('#' + uid);
  ...
  // ===== Member Picker (separate modal for large directories) =====
  var mpSelectedDept = null; // null = ALL
  ...
  function openMemberPicker(){
    ...
    if (!modal) { console.warn('[member-picker] modal missing'); return; }
    ...
    // Direct bindings (defensive)
    var applyBtn = $('member-picker-apply');
    if (applyBtn && !applyBtn.__mp_bound) { applyBtn.__mp_bound = true; applyBtn.addEventListener('click', function(e){ e.preventDefault(); console.log('[member-picker] apply'); applyMemberPicker(); }); }
    var mpC1 = $('member-picker-cancel');
    if (mpC1 && !mpC1.__mp_bound) { mpC1.__mp_bound = true; mpC1.addEventListener('click', function(e){ e.preventDefault(); closeMemberPicker(); }); }
    var mpC2 = $('member-picker-cancel-2');
    if (mpC2 && !mpC2.__mp_bound) { mpC2.__mp_bound = true; mpC2.addEventListener('click', function(e){ e.preventDefault(); closeMemberPicker(); }); }
    var mpBd = $('member-picker-backdrop');
    if (mpBd && !mpBd.__mp_bound) { mpBd.__mp_bound = true; mpBd.addEventListener('click', function(e){ e.preventDefault(); closeMemberPicker(); }); }
    if (!directoryLoaded) {
  ...
  function openModal(){
    console.log('[new-channel] openModal() called');
    var modal = $('chat-new-channel-modal');
    if (!modal) { console.warn('[new-channel] modal element missing'); return; }
    selectedIds.clear();
    ...
    // background refresh of directory
    ...
    // BIND CONFIRM BUTTON DIRECTLY (in case delegation fails for any reason)
    var confirmBtn = $('new-channel-confirm');
    if (confirmBtn && !confirmBtn.__nc_bound) {
      confirmBtn.__nc_bound = true;
      confirmBtn.addEventListener('click', function(e){
        e.preventDefault(); e.stopPropagation();
        console.log('[new-channel] direct confirm click');
        submitForm();
      });
      console.log('[new-channel] direct click handler bound to confirm button');
    }
    var cancelBtn1 = $('new-channel-cancel');
    if (cancelBtn1 && !cancelBtn1.__nc_bound) { cancelBtn1.__nc_bound = true; cancelBtn1.addEventListener('click', function(e){ e.preventDefault(); closeModal(); }); }
    var cancelBtn2 = $('new-channel-cancel-2');
    if (cancelBtn2 && !cancelBtn2.__nc_bound) { cancelBtn2.__nc_bound = true; cancelBtn2.addEventListener('click', function(e){ e.preventDefault(); closeModal(); }); }
    var deptToggle = $('new-channel-dept-toggle');
    if (deptToggle && !deptToggle.__nc_bound) { deptToggle.__nc_bound = true; deptToggle.addEventListener('click', function(e){ e.preventDefault(); openMemberPicker(); }); }
  ...
  function submitForm(){
    console.log('[new-channel] submitForm() called');
    var nameEl = $('new-channel-name');
    var descEl = $('new-channel-desc');
    var modal = $('chat-new-channel-modal');
    var confirmBtn = $('new-channel-confirm');
    ...
    var payload = { name: name, type: visibility };
    if (description) payload.description = description;
    if (memberIds.length) payload.memberIds = memberIds;
    if (confirmBtn) confirmBtn.disabled = true;
    var cfg = getCfg();
    var url = cfg.apiRoot + '/channels';
    console.log('[new-channel] POST', url, payload);
    fetch(url, {
      method: 'POST',
      credentials: 'same-origin',
      ...
      console.log('[new-channel] response', res.status, res.data);
      if (!res.ok) {
      ...
      console.error('[new-channel] network error', err);
  ...
  ready(function(){
    console.log('[new-channel] standalone bootstrap ready');
    // Remove any leftover body-level modals from previous SPA loads (avoid duplicate IDs)
    Array.prototype.forEach.call(document.body.querySelectorAll(':scope > #chat-new-channel-modal'), function(el){ try { el.remove(); } catch(_){} });
    Array.prototype.forEach.call(document.body.querySelectorAll(':scope > #member-picker-modal'), function(el){ try { el.remove(); } catch(_){} });
    // Move modal to body level so blur on .main-content doesn't affect it
    var modal = document.getElementById('chat-new-channel-modal');
    if (modal && modal.parentNode !== document.body) {
      document.body.appendChild(modal);
      console.log('[new-channel] modal relocated to body');
    }
    var pickerModal = document.getElementById('member-picker-modal');
    if (pickerModal && pickerModal.parentNode !== document.body) {
      document.body.appendChild(pickerModal);
      console.log('[member-picker] modal relocated to body');
    }
    // Direct bind on the trigger button (most reliable)
    function bindTriggerButton(){
      var triggers = document.querySelectorAll('#btn-new-channel');
      console.log('[new-channel] found triggers:', triggers.length);
      Array.prototype.forEach.call(triggers, function(b){
        if (b.__nc_trigger_bound) return;
        b.__nc_trigger_bound = true;
        b.addEventListener('click', function(e){
          console.log('[new-channel] trigger button clicked (direct)');
          e.preventDefault(); e.stopPropagation();
          openModal();
        });
      });
    ...
    bindTriggerButton();
    // Re-bind whenever DOM changes (SPA may re-insert the button)
    try {
      var mo = new MutationObserver(function(){ bindTriggerButton(); });
      mo.observe(document.body, { childList: true, subtree: true });
    } catch(_){ }
    // ULTIMATE FALLBACK: window-level capturing click that catches the button
    // by closest('#btn-new-channel') no matter what intercepts in between.
    if (!window.__nc_window_click_bound) {
      window.__nc_window_click_bound = true;
      window.addEventListener('click', function(e){
        var t = e.target;
        if (!t || !t.closest) return;
        var hit = t.closest('#btn-new-channel, .chat-new-channel');
        if (hit) {
          console.log('[new-channel] window-capture click on btn-new-channel');
          e.preventDefault();
          e.stopPropagation();
          openModal();
          return;
        }
        var confirmHit = t.closest('#new-channel-confirm');
        if (confirmHit) {
          console.log('[new-channel] window-capture click on confirm');
          e.preventDefault();
          e.stopPropagation();
          submitForm();
          return;
        }
        var cancelHit = t.closest('#new-channel-cancel, #new-channel-cancel-2, #chat-new-channel-backdrop');
        if (cancelHit) {
          console.log('[new-channel] window-capture click on cancel/backdrop');
          e.preventDefault(); e.stopPropagation(); closeModal();
          return;
        }
        var deptHit = t.closest('#new-channel-dept-toggle');
        if (deptHit) {
          console.log('[member-picker] window-capture click on dept-toggle');
          e.preventDefault(); e.stopPropagation(); openMemberPicker();
          return;
        }
        var applyHit = t.closest('#member-picker-apply');
        if (applyHit) {
          console.log('[member-picker] window-capture click on apply');
          e.preventDefault(); e.stopPropagation(); applyMemberPicker();
          return;
        }
        var mpCancelHit = t.closest('#member-picker-cancel, #member-picker-cancel-2, #member-picker-backdrop');
        if (mpCancelHit) {
          console.log('[member-picker] window-capture click on cancel/backdrop');
          e.preventDefault(); e.stopPropagation(); closeMemberPicker();
          return;
        }
      }, true); // <-- capturing phase, runs before any other handler
      console.log('[new-channel] window-capture handler installed');
    }
    // Use document-level delegation so it works even if elements re-render
    document.addEventListener('click', function(e){
      var t = e.target;
      if (!t) return;
      var btn = (t.closest && t.closest('#btn-new-channel'));
      if (btn) { e.preventDefault(); openModal(); return; }
      var cancel = (t.closest && (t.closest('#new-channel-cancel') || t.closest('#new-channel-cancel-2')));
      if (cancel) { e.preventDefault(); closeModal(); return; }
      var bd = (t.closest && t.closest('#chat-new-channel-backdrop'));
      if (bd) { e.preventDefault(); closeModal(); return; }
      var confirm = (t.closest && t.closest('#new-channel-confirm'));
      if (confirm) { console.log('[new-channel] confirm clicked'); e.preventDefault(); submitForm(); return; }
    }, false);
    ...
    document.addEventListener('click', function(e){
      var t = e.target;
      if (!t) return;
      if (t.closest && t.closest('#new-channel-dept-toggle')) { e.preventDefault(); openMemberPicker(); return; }
      if (t.closest && (t.closest('#member-picker-cancel') || t.closest('#member-picker-cancel-2') || t.closest('#member-picker-backdrop'))) { e.preventDefault(); closeMemberPicker(); return; }
      if (t.closest && t.closest('#member-picker-apply')) { e.preventDefault(); applyMemberPicker(); return; }
      if (!(t.closest && (t.closest('#new-channel-member-list') || t.closest('#new-channel-member-search')))) {
        var le = $('new-channel-member-list'); if (le) { le.hidden = true; }
      }
    }, true);
    document.addEventListener('keydown', function(e){
      if (e.key === 'Enter' && e.target && e.target.id === 'new-channel-name') {
  ...
  (function initSettingsDrawer(){
    ...
    // 대화 비활성화 시 드로어 닫기
    const origSetActive = window._origSetActive; // fallback if needed
    // activeId 변경 감지: setActive 이후 hydrateFromApi 폴링 때 반영됨
    // 단순히 다른 대화 클릭 시 닫기
    document.querySelector('.chat-sidebar .room-list, #chat-list, .chat-list')?.addEventListener('click', function(e){
      const li = e.target.closest('[data-id]');
      if (li && !drawer.hidden) { closeDrawer(); }
    });

    // --- 드로어 내용 채우기 ---
    function populateDrawer(conv){
      ...
      if (descSection) {
        if (isChannel) {
          descSection.classList.remove('csd-hidden');
          const descInput = document.getElementById('csd-desc-input');
          if (descInput) descInput.value = conv.description || '';
        } else {
          descSection.classList.add('csd-hidden');
        }
      }

      // 멤버 초대 버튼 (DM 1:1은 숨기기)
      const inviteSection = document.getElementById('csd-section-invite');
      if (inviteSection) {
        const isDirect = String(conv.roomType || '').toUpperCase() === 'DIRECT';
        inviteSection.style.display = isDirect ? 'none' : '';
      }
      ...
      if (csdLeave && leaveBtn) {
        csdLeave.disabled = leaveBtn.disabled;
        csdLeave.style.opacity = leaveBtn.disabled ? '0.4' : '';
      }
      // 삭제 버튼
      const csdDel = document.getElementById('csd-btn-delete');
      if (csdDel && delBtn) {
        csdDel.disabled = delBtn.disabled;
        csdDel.style.opacity = delBtn.disabled ? '0.4' : '';
        csdDel.title = delBtn.title || '';
      }
    }

    // --- 이름 저장 ---
    const nameSaveBtn = document.getElementById('csd-name-save');
    if (nameSaveBtn) {
      nameSaveBtn.addEventListener('click', async function(){
        ...
        if (!newName) { nameInput && nameInput.focus(); return; }
        const isChannel = String(conv.roomType || '').toUpperCase() === 'CHANNEL';
        try {
          let url, body;
          if (isChannel && conv.channelId) {
            url = `${chatConfig.apiRoot}/channels/${conv.channelId}`;
          } else if (conv.conversationId) {
            url = `${chatConfig.apiRoot}/conversations/${conv.conversationId}`;
          } else {
            return;
          }
          const resp = await fetch(url, {
          ...
          if (resp.ok) {
            conv.name = newName;
            if (thread.nameEl) thread.nameEl.textContent = newName;
            if (profile.nameEl) profile.nameEl.textContent = newName;
            lastListRenderSignature = null; lastListStructSig = null;
            renderList(getActiveFilter(), searchEl ? searchEl.value : '');
          }
        } catch(_){}
      });
    }
    ...
    // --- 설명 저장 (채널만) ---
    const descSaveBtn = document.getElementById('csd-desc-save');
    if (descSaveBtn) {
      descSaveBtn.addEventListener('click', async function(){
        const conv = conversations.find(c => c.id === activeId);
        if (!conv || !conv.channelId) return;
        const descInput = document.getElementById('csd-desc-input');
        const newDesc = descInput ? descInput.value : '';
        try {
          const resp = await fetch(`${chatConfig.apiRoot}/channels/${conv.channelId}`, {
            method: 'PATCH',
            credentials: 'same-origin',
            headers: { 'Content-Type': 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
            body: JSON.stringify({ description: newDesc }),
          });
          if (resp.ok) { conv.description = newDesc; }
        } catch(_){}
      });
    }
    ...
    if (muteSwitch) {
      ...
      muteSwitch.addEventListener('click', toggle);
      muteSwitch.addEventListener('keydown', e => { if (e.key === ' ' || e.key === 'Enter') { e.preventDefault(); toggle(); } });
    }

    // --- 멤버 초대 (기존 addMemberBtn 로직 재사용) ---
    const csdInviteBtn = document.getElementById('csd-btn-invite');
    if (csdInviteBtn && addMemberBtn) {
      csdInviteBtn.addEventListener('click', function(){
        closeDrawer();
        addMemberBtn.click();
      });
    }
    ...
    // --- 나가기 ---
    const csdLeaveBtn = document.getElementById('csd-btn-leave');
    if (csdLeaveBtn && leaveBtn) {
      csdLeaveBtn.addEventListener('click', function(){
        closeDrawer();
        leaveBtn.click();
      });
    }
    ...
    // --- 삭제 ---
    const csdDeleteBtn = document.getElementById('csd-btn-delete');
    if (csdDeleteBtn && delBtn) {
      csdDeleteBtn.addEventListener('click', function(){
        closeDrawer();
        delBtn.click();
      });
    }

    // 뮤트된 대화는 unread 증가 안 함 (hydrateFromApi에서 참조)
