#!/bin/bash
# Deploy and start Lumina agent on the manager server (self-monitoring)
# Target: 192.168.56.105 (lumina-server)
set -e
...
echo "=== Lumina Agent Installation (lumina-server / self-monitoring) ==="
...
# 3) Agent files are already staged in /tmp/agent_deploy/
...
# 4) Create HTTPS-only config (pointing to self)
