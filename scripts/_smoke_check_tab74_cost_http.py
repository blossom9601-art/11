...
def main() -> int:
    ...
    session_cookie = (os.environ.get("BLOSSOM_SESSION") or os.environ.get("BLOSSOM_SESSION_COOKIE") or "").strip()

    s = requests.Session()

    if session_cookie:
        host = urlparse(base).hostname or "127.0.0.1"
        s.cookies.set("session", session_cookie, domain=host, path="/")
        print("Using BLOSSOM_SESSION cookie for auth (host=", host, ")")
    else:
        ...
        if not s.cookies.get_dict().get("session"):
            raise RuntimeError(
                "Login did not yield a session cookie. "
                "Set BLOSSOM_EMP_NO/BLOSSOM_PASSWORD correctly, or set BLOSSOM_SESSION to an existing browser session cookie value."
            )
    ...
    payload_owned: Dict[str, Any] = {
        "project_name": f"SMOKE Tab74 Cost (EVA) {suffix}",
        "project_type": "SW",
        "status": "ACTIVE",
    }
    ...
    key = f"WBS-1 | Activity-A | Task-A | Owner-A | {suffix}"
    eva_map: Dict[str, Any] = {key: {"pv": 1000, "ev": 400, "ac": 250}}

    res_post = s.post(
        f"{base}/api/prj/projects/{project_id}/tabs/cost",
        json={"payload": {"eva_map": eva_map}},
        timeout=10,
    )
    print("POST /api/prj/projects/{id}/tabs/cost", res_post.status_code)
    _require(res_post.status_code, expected=201, body=res_post.text)

    res_list = s.get(f"{base}/api/prj/projects/{project_id}/tabs/cost", timeout=10)
    print("GET /api/prj/projects/{id}/tabs/cost", res_list.status_code)
    ...
    got_map = ((items[0] or {}).get("payload") or {}).get("eva_map")
    got_pv = ((got_map or {}).get(key) or {}).get("pv") if isinstance(got_map, dict) else None
    if got_pv != 1000:
        raise RuntimeError(f"Unexpected eva_map[{key!r}].pv: {got_pv!r}")

    print("OK: cost(tab74 EVA) payload.eva_map roundtrip verified")
