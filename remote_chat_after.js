...
(function(){
  ...
  function updateProfilePanel(conv, isGroup){
    ...
    if (profile.nameEl) {
      profile.nameEl.textContent = conv ? (isGroup ? displayNameForConversation(conv) : (contact?.name || displayNameForConversation(conv) || '')) : '';
    }
    if (profile.emailEl) {
  ...
  (function initSettingsDrawer(){
    ...
    function openDrawer(){
      ...
      if (titleEl) titleEl.textContent = '채팅 설정';
      if (nameInput) nameInput.value = threadNameEl ? (threadNameEl.textContent || '').trim() : '';
  ...
  // Fallback: if primary binding fails due stale script/runtime order,
  // delegated handler still opens/closes settings drawer.
  (function initSettingsDrawerFallback(){
    function getDrawer(){
      return document.getElementById('chat-settings-drawer');
    }
    function getSettingsBtn(){
      return document.getElementById('btn-chat-settings');
    }
    function openDrawer(){
    ...
    document.addEventListener('click', function(evt){
      ...
      const openBtn = evt.target.closest('#btn-chat-settings');
      if (openBtn) {
        evt.preventDefault();
        const drawer = getDrawer();
        if (!drawer) return;
      ...
      if (evt.target.closest('#btn-chat-settings-close')) {
        evt.preventDefault();
        closeDrawer();
        return;
      }
      if (evt.target.closest('#csd-btn-invite')) {
        evt.preventDefault();
        closeDrawer();
        if (addMemberBtn && !addMemberBtn.disabled) addMemberBtn.click();
        return;
      }
      if (evt.target.closest('#csd-btn-leave')) {
        evt.preventDefault();
        closeDrawer();
        if (leaveBtn && !leaveBtn.disabled) leaveBtn.click();
        return;
      }
      if (evt.target.closest('#csd-btn-delete')) {
        evt.preventDefault();
        closeDrawer();
        if (delBtn && !delBtn.disabled) delBtn.click();
      }
