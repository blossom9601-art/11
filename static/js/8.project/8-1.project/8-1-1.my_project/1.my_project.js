...
(function(){
  ...
  function loadPersistence(){
    try{
      const raw=localStorage.getItem('kanban_my_order');
      if(raw){
  ...
  function savePersistence(){ try{ localStorage.setItem('kanban_my_order', JSON.stringify(state.order)); }catch(_e){} }

  async function loadFromServer(){
    const url = '/api/prj/projects?scope=owned&limit=2000';
  ...
  function highlightHtml(text, tokens){
    ...
    // sequentially wrap tokens; simple approach is fine for small token sets
    let out = src;
  ...
  function cardHTML(r, tokens){
    ...
    return `
      <article class="kanban-card" draggable="true" data-id="${r.id}" aria-roledescription="card">
        <div class="card-top">
  ...
  function bindCardDnD(){
    ...
    document.querySelectorAll('.kanban-list').forEach(list=>{
      ...
      list.addEventListener('drop', e=>{
        ...
        }).catch(err => console.error('[my_project] status update failed', err));
      });
    });
  }
  ...
  function bindSearch(){
    ...
    // Global '/' focus shortcut (skip if typing in an input/textarea already)
    ...
    document.getElementById('prj-clear-confirm')?.addEventListener('click', async ()=>{
      ...
      try{
        await fetch('/api/prj/projects/batch-clear', {
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body: JSON.stringify({ scope:'owned', ids }),
        });
      }catch(_e){}
  ...
  async function addProjectFromForm(){
    ...
    try{
      ...
      console.error('[my_project] addProjectFromForm error', err);
