...
(function(){
    ...
    // render minimal grouped tables and exit (page-specific renderer only).
    if (document.getElementById('status-groups')) {
        (function(){
            const CURRENT_USER = window.CURRENT_USER || '홍길동';
            const MODE = (window.TASK_PAGE_MODE || 'participating').toLowerCase();
            const CONTAINER_ID = 'status-groups';
            const STATUS_ORDER = ['검토','승인','수행','완료대기','완료'];
            // Persist selected rows across re-render within this page
            ...
            function ensureLottie(cb){
                ...
                const load = (src, onfail)=>{
                    ...
                    s.onload = () => cb && cb();
                    s.onerror = () => onfail ? onfail() : (cb && cb());
                    document.head.appendChild(s);
                };
                load('https://unpkg.com/lottie-web@5.12.2/build/player/lottie.min.js', ()=>{
                ...
                root.querySelectorAll('.status-card').forEach(section =>{
                    ...
                    if(chkAll){
                        chkAll.addEventListener('change', () =>{
                            ...
                            // Reset indeterminate on explicit toggle
                    ...
                    // Row click toggles selection (excluding action button/checkbox)
                    ...
                    // Initialize header checkbox state after render based on current selection
                    const rowChecks = section.querySelectorAll('tbody .chk-row, tbody .system-row-select');
            ...
            async function handleInlineAction(reportId, action, btnEl){
                ...
                const HIDDEN_STATUSES = new Set(['검토','완료대기','완료']);
                const visibleData = data.filter(r => !HIDDEN_STATUSES.has(r.status));
                ...
                grouped.forEach((items, status) =>{
                    // 참여작업에서는 검토/완료대기/완료 숨김
                    if(status==='검토' || status==='완료대기' || status==='완료') return;
            ...
            function initClearModal(){
                ...
                document.getElementById('task-clear-confirm')?.addEventListener('click', async ()=>{
                    ...
                    for(const id of ids){
                        try{
                            await fetch(`/api/wrk/reports/${id}`, { method:'DELETE', headers:{'Content-Type':'application/json'}, credentials:'same-origin' });
                        }catch(_e){}
                    }
                    render();
                });
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        // esc close
    document.addEventListener('keydown', e=>{ if(e.key==='Escape'){ [ADD_MODAL_ID,EDIT_MODAL_ID,COLUMN_MODAL_ID,DISPOSE_MODAL_ID,DELETE_MODAL_ID,BULK_MODAL_ID,UPLOAD_MODAL_ID,'system-download-modal','system-message-modal','system-duplicate-modal','task-clear-modal','task-action-confirm-modal','task-result-modal'].forEach(closeModal); _resolveConfirm(false); }});
    ...
    document.addEventListener('keydown', e=>{ if(e.key==='Escape'){ closeModal(STATS_MODAL_ID); }});
        ...
        document.getElementById('task-result-close')?.addEventListener('click', ()=> closeModal('task-result-modal'));
        document.getElementById('task-result-cancel')?.addEventListener('click', ()=> closeModal('task-result-modal'));
        // Auto-calc duration from start/end time (flatpickr formatted: YYYY-MM-DD HH:MM)
        function _parseResultDT(v){
            if(!v) return null;
            const m = String(v).trim().match(/^(\d{4})-(\d{2})-(\d{2})\s+(\d{1,2}):(\d{2})$/);
            if(!m) return null;
        ...
        // Init flatpickr on result modal datetime fields
        async function _initResultDatePickers(){
            try { await ensureFlatpickr(); } catch(_e){ return; }
            if(!window.flatpickr) return;
            function ensureTodayBtn(fp){
