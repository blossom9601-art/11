...
(function(){
    ...
    const words = raw ? raw.split(/\s+/) : ['온/습도계'];
    ...
    // Read config from HTML data-* attributes (universal template)
    const _thermoCfg = document.getElementById('thermo-lab-config');
    const OVERLAY_STORE_KEY = (_thermoCfg && _thermoCfg.dataset.overlayStoreKey) || 'thermo_lab1_overlay_boxes';
    const OVERLAY_FLOOR_KEY = (_thermoCfg && _thermoCfg.dataset.overlayFloorKey) || 'thermo-future-5f';
    const OVERLAY_SYNC_DELAY = 900;
    ...
    function scheduleOverlaySync(snapshot){
        if(!OVERLAY_FLOOR_KEY) return;
        const boxes = Array.isArray(snapshot) ? snapshot : getOverlayData();
        if(!Array.isArray(boxes)) return;
        if(overlaySyncTimer) clearTimeout(overlaySyncTimer);
        overlaySyncTimer = setTimeout(()=>{
            fetch(`/api/datacenter/layout/${encodeURIComponent(OVERLAY_FLOOR_KEY)}`, {
                method:'POST',
    ...
    function renderOverlayBoxes(items){
        ...
        (items || []).forEach(item=>{
            ...
            box.className = 'slab-box slab-thermo';
            ...
            if(typeof item.leftPct === 'number' && typeof item.topPct === 'number'){
                ...
                box.style.width = '48px'; box.dataset.w = '48';
                box.style.height = '48px'; box.dataset.h = '48';
                ensureBoxPercents(box);
                applyBoxPercents(box);
            }
            ...
            const labelText = (item.text || '').trim() || '온/습도계';
            try { attachSampleMetrics(box, labelText); } catch(_e) {}
            const img = document.createElement('img');
            img.src = '/static/image/svg/datacenter/free-icon-thermometer.svg';
            img.alt = labelText;
            img.className = 'slab-icon';
            img.draggable = false;
            try { renderTempAura(box); } catch(_e) {}
            box.appendChild(img);
            ...
            if(!box.id && typeof assignBoxId === 'function') assignBoxId(box);
            try { attachSampleMetrics(box, labelText); renderTempAura(box); } catch(_e) {}
    ...
    function getOverlayData(){
        ...
        return boxes.map(b=>{
            ...
            return {
                id: b.id || '',
                leftPct: parseFloat(b.dataset.leftPct||'0'),
                topPct: parseFloat(b.dataset.topPct||'0'),
                widthPct: parseFloat(b.dataset.widthPct||'0'),
                heightPct: parseFloat(b.dataset.heightPct||'0'),
    ...
    function loadOverlayFromStorage(){
        ...
        fetch(`/api/datacenter/layout/${encodeURIComponent(OVERLAY_FLOOR_KEY)}`)
            ...
            .then(json=>{
                const remoteBoxes = Array.isArray(json?.boxes) ? json.boxes : [];
                if(remoteBoxes.length){
                    renderOverlayBoxes(remoteBoxes);
                    try { localStorage.setItem(OVERLAY_STORE_KEY, JSON.stringify(remoteBoxes)); } catch(_e){}
                } else {
                    renderOverlayBoxes(localSnapshot);
            ...
            .catch(()=>{ renderOverlayBoxes(localSnapshot); });
    }
    ...
    // ----- Sample metrics (온/습도계 기록) & temperature aura -----
    ...
    function attachSampleMetrics(box, labelText){
        if(!box) return;
        const key = (box.id && String(box.id)) || String(labelText||'thermo');
        const seed = hashStringInt(key);
        // Temperature (°C): avg 22~30, max a few degrees higher, current near avg
        const avgT = 22 + noise01(seed+1) * 8; // 22..30
        const maxT = avgT + 2.5 + noise01(seed+2) * 3.5; // +2.5..+6.0
        const curT = avgT + (noise01(seed+3) * 2 - 1); // avg ±1
        // Humidity (%): avg 40~60, max a bit higher
        const avgH = 40 + noise01(seed+4) * 20; // 40..60
        const maxH = Math.min(95, avgH + 8 + noise01(seed+5) * 12); // +8..+20
        box.dataset.tempAvg = String(round1(avgT));
        box.dataset.tempMax = String(round1(maxT));
        box.dataset.tempCurrent = String(round1(curT));
        box.dataset.humidAvg = String(round1(avgH));
        box.dataset.humidMax = String(round1(maxH));
    }
    function tempToColorHSLA(tempC){
        // Map 10°C -> 220 (blue), 25°C -> ~110 (green), 40°C -> 0 (red)
        const t = Math.max(10, Math.min(40, tempC));
        const ratio = (t - 10) / 30; // 0..1
        const hue = 220 - 220 * ratio; // 220..0
        const sat = 85; const light = 55;
        const aCenter = 0.35; const aEdge = 0;
        return {
            center: `hsla(${hue}, ${sat}%, ${light}%, ${aCenter})`,
            edge: `hsla(${hue}, ${sat}%, ${light}%, ${aEdge})`
        };
    }
    function renderTempAura(box){
        if(!box) return;
        const cur = parseFloat(box.dataset.tempCurrent || '0');
        const {center, edge} = tempToColorHSLA(isFinite(cur) ? cur : 24);
        let aura = box.querySelector('.temp-aura');
        if(!aura){
            aura = document.createElement('div');
            aura.className = 'temp-aura';
            // Insert before icon so icon stays on top
            box.prepend(aura);
        }
        // Size aura relative to icon size if known, else use default
        const base = 120; // px
        aura.style.width = base + 'px';
        aura.style.height = base + 'px';
        aura.style.background = `radial-gradient(circle, ${center} 0%, ${edge} 65%)`;
    }

    // View modal helpers
    function openViewModalForBox(box){
        // Use raw label to preserve spaces as typed
        const name = (box.getAttribute('data-label') || box.querySelector('.label')?.textContent || '온/습도계');
    ...
    const btnDup = document.getElementById('overlay-menu-duplicate');
        ...
        btnDup?.addEventListener('click', ()=>{
            ...
            clone.classList.add('slab-thermo');
            clone.classList.remove('is-active');
            // Remove any inherited aura and recalc metrics for the new id
            try { clone.querySelector('.temp-aura')?.remove(); } catch(_e) {}
            try { const lbl = clone.getAttribute('data-label') || clone.querySelector('.label')?.textContent || '온/습도계'; attachSampleMetrics(clone, lbl); renderTempAura(clone); } catch(_e) {}
    ...
    function setAddModalVisual(isEdit){
        ...
        if(isEdit){
            if(title) title.textContent = '온/습도계 수정';
            if(sub) sub.textContent = '온/습도계 정보를 수정합니다.';
            if(saveBtn) saveBtn.textContent = '저장';
        } else {
            if(title) title.textContent = '온/습도계 등록';
            if(sub) sub.textContent = '새로운 온/습도계를 등록합니다.';
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', ()=>{
            ...
            if(form.dataset.editTarget === '1' && targetId){
                ...
                if(box){
                    ...
                    if(labelEl) labelEl.textContent = name || '온/습도계';
                    box.setAttribute('data-label', name || '온/습도계');
                    const img = box.querySelector('img.slab-icon');
                    if(img) img.alt = name || '온/습도계';
                    setBoxRacks(box, racks);
                    setBoxRemark(box, remark);
                    ensureBoxPercents(box);
                    // Keep sample metrics deterministic by id; update aura if current temp mapping needs refresh
                    try { renderTempAura(box); } catch(_e) {}
                ...
                img.className = 'slab-icon';
                img.draggable = false;
                // Attach sample metrics and aura
                try { attachSampleMetrics(box, name || '온/습도계'); renderTempAura(box); } catch(_e) {}
                box.appendChild(img);
                // Visible label (caption)
                const label = document.createElement('span');
                label.className = 'label';
                label.textContent = name || '온/습도계';
                box.setAttribute('data-label', name || '온/습도계');
                box.appendChild(label);
                overlay.appendChild(box);
                assignBoxId(box);
                // Recompute metrics/aura with final id to lock values
                try { attachSampleMetrics(box, name || '온/습도계'); renderTempAura(box); } catch(_e) {}
                setBoxRacks(box, racks);
                setBoxRemark(box, remark);
                makeDraggableResizable(box);
        ...
        if(!layout || !img || !overlay){ showMessage('배치도를 찾을 수 없습니다.', '오류'); return; }
                ...
                boxes.forEach(b=>{
                    ...
                    if(text){
                        ...
                        const lines = words.length ? words : ['온/습도계'];
