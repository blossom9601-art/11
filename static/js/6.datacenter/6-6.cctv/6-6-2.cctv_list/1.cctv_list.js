...
(function(){
    ...
    function normalizeCctvRow(raw){
        if(!raw || typeof raw !== 'object') return null;
        return {
            id: raw.id,
            cctv_code: raw.cctv_code || raw.device_code || '',
            business_status: raw.biz_work_status || raw.business_status || '-',
    ...
    async function createCctv(payload){
    ...
    async function updateCctv(id, payload){
        if(id == null){
            throw new Error('수정할 대상이 지정되지 않았습니다.');
        }
        const url = `${ORG_CCTV_API}/${id}`;
    ...
    async function bulkDeleteCctvs(ids){
        const normalized = Array.isArray(ids) ? ids.map(v=> Number(v)).filter(v=> Number.isFinite(v)) : [];
        if(!normalized.length){
            return 0;
        }
        const response = await requestJSON(ORG_CCTV_BULK_DELETE_API, { method: 'POST', body: { ids: normalized } });
        const deleted = Number(response?.deleted);
        return Number.isFinite(deleted) ? deleted : 0;
    }

    async function bulkUpdateCctvs(ids, updates){
        const normalized = Array.isArray(ids) ? ids.map(v=> Number(v)).filter(v=> Number.isFinite(v)) : [];
        if(!normalized.length){
            return 0;
        }
        if(!updates || typeof updates !== 'object' || Array.isArray(updates)){
            throw new Error('변경할 값을 1개 이상 입력하세요.');
        }
        const response = await requestJSON(ORG_CCTV_BULK_UPDATE_API, { method: 'POST', body: { ids: normalized, updates } });
    ...
    const CCTV_SEARCH_BRIDGE_KEY = 'cctv_search_bridge';
    ...
    const SEARCH_SELECT_DEFINITIONS = {
        ...
        businessStatus: {
            ...
            normalize: (item)=>{
                const label = String(item?.wc_name || item?.status_name || '').trim();
    ...
    async function loadCctvData(options){
        const opts = options || {};
        if(!opts.silent){
        ...
        try {
            const payload = await requestJSON(ORG_CCTV_API);
            const items = Array.isArray(payload.items) ? payload.items : [];
            const normalized = items.map(normalizeCctvRow).filter(Boolean);
            ...
            console.error(error);
            state.isLoading = false;
            state.loadError = error.message || 'CCTV 목록을 불러오지 못했습니다.';
            state.data = [];
            state.filtered = [];
            render();
            if(!opts.silent){
    ...
    syncCctvSearchBridge(qRaw, flatTokens);
    }

    function syncCctvSearchBridge(rawQuery, tokens){
        ...
        try {
            if(trimmed){
                ...
                window.localStorage.setItem(CCTV_SEARCH_BRIDGE_KEY, JSON.stringify(payload));
            } else {
                window.localStorage.removeItem(CCTV_SEARCH_BRIDGE_KEY);
            }
        } catch(err){
            console.warn('[cctv-bridge] 저장 실패', err);
        }
    }
        ...
        slice.forEach((row)=>{
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    // 업무 상태: 활성/비활성 중심 색점 표시 (레거시 라벨 호환)
                    if(col === 'business_status'){
                        const v = String(displayVal).trim();
                        cellValue = dcAssetBusinessStatusTableCell(v === '-' ? '-' : v, highlight, col);
                    }
    ...
    function fillEditForm(row){
        ...
        const REQUIRED_FIELDS = new Set(['business_status', 'vendor', 'model', 'place']);
        groups.forEach(g=>{
    ...
    function generateFieldInput(col,value=''){
        ...
        const required = (col==='business_status' || col==='vendor' || col==='model' || col==='place');
    ...
    document.getElementById(COLUMN_APPLY_ID)?.addEventListener('click', handleColumnFormApply);
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            button.disabled = true;
            try {
                const updated = await updateCctv(recordId, payload);
                if(updated){
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            // UI 즉시 반응을 위해 모달은 먼저 닫고, 삭제 진행
            closeModal(DELETE_MODAL_ID);
            state.isLoading = true;
            render();

            try {
                const deleted = await bulkDeleteCctvs(ids);
                state.selected.clear();
                await loadCctvData({ silent: true });
                if(deleted > 0){ showMessage(`${deleted}개 항목이 삭제되었습니다.`, '완료'); }
                else { showMessage('삭제할 항목이 없거나 이미 삭제된 상태입니다.', '안내'); }
            } catch(err){
                console.error(err);
                state.isLoading = false;
                render();
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', async ()=>{
            ...
            const ids = [...state.selected];
            if(!ids.length){ showMessage('변경할 항목을 선택하세요.', '안내'); return; }

            const updates = {};
            entries.forEach(({field, value})=>{ updates[field] = value; });

            // UI 즉시 반응을 위해 모달은 먼저 닫고, 저장 진행
            closeModal(BULK_MODAL_ID);
            state.isLoading = true;
            render();

            try {
                const updated = await bulkUpdateCctvs(ids, updates);
                state.selected.clear();
                await loadCctvData({ silent: true });
                if(updated > 0){ showMessage(`${updated}개 항목이 일괄 변경되었습니다.`, '완료'); }
                else { showMessage('변경할 항목이 없거나 이미 삭제된 상태입니다.', '안내'); }
            } catch(err){
                console.error(err);
                state.isLoading = false;
                render();
                showMessage(err?.message || '일괄변경 중 오류가 발생했습니다.', '오류');
            }
        });
        // esc close
    ...
    function buildBulkForm(){
        ...
        function inputFor(col){
            ...
            if(sourceKey){
                const parentAttr = DEPENDENT_FIELD_MAP[col] ? ` data-parent-field="${DEPENDENT_FIELD_MAP[col]}"` : '';
                return `<input name="${col}" class="form-input search-select" placeholder="검색 선택" data-search-source="${sourceKey}"${parentAttr} data-bulk-field="${col}">`;
            }
        ...
        form.innerHTML = GROUPS.map(g=>{
            ...
            // Enhance newly-rendered search-select inputs in bulk form
            initSearchSelects(form);
    }
