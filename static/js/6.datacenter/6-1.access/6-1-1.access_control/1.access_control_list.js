...
(function(){
    // Mark body for Access Control page to enable page-scoped CSS in center.css
    try { document.body.classList.add('page-access-control'); } catch(_e){}
    ...
    // Initialize Flatpickr datetime pickers for access entry/exit datetime fields
    ...
    const BASE_VISIBLE_COLUMNS = [
        'status','person_type','affiliation','name','id_number','entry_datetime','exit_datetime','entry_purpose','entry_area','laptop_use','usb_lock_use',
    ...
    const COLUMN_MODAL_GROUPS = [
        { group: '출입 등록', columns: ['status','person_type','affiliation','name','id_number','entry_datetime','exit_datetime','entry_purpose','entry_area','laptop_use','usb_lock_use','work_management_dept','work_assignee','access_management_dept','access_assignee'] },
    ...
    const COLUMN_META = {
        ...
        person_type:{label:'구분',group:'출입 등록'},
        name:{label:'이름',group:'출입 등록'},
        affiliation:{label:'소속/부서',group:'출입 등록'},
    ...
    const ACCESS_API_BASE = '/api/datacenter/access/entries';
    const ACCESS_RECORDS_API_BASE = '/api/datacenter/access/records';
    const ACCESS_FETCH_LIMIT = 500;
    const ACCESS_DISABLED_MESSAGE = '해당 기능은 서버 연동 준비 중입니다.';
    ...
    const FK_SOURCE_CONFIG = {
        ...
        VENDOR_ALL_MANAGERS: {
            endpoint: '/api/vendor-manufacturers/all-managers',
            valueKey: 'name',
            labelKey: 'name'
        }
    };
    ...
    function buildUserOption(item){
        ...
        // Preserve legacy value format so existing saved rows (and initial selections)
        // keep matching option values.
        const detailParts = [dept, emp].filter(Boolean);
        const legacyDetail = detailParts.length ? ` · ${detailParts.join(' · ')}` : '';
        const legacyValue = `${name || emp}${legacyDetail}`.trim();

        // Display/search label: "부서, 이름" (requested)
        const displayParts = [];
        if(dept) displayParts.push(dept);
        if(name) displayParts.push(name);
        else if(emp) displayParts.push(emp);
        const label = (displayParts.join(', ') || legacyValue).trim();

        return { value: legacyValue, label };
    }
    ...
    function buildAssigneeOption(item){
        ...
        const dept = String(item?.department || '').trim();
        if(!name) return null;
        // Display only the name, but keep value compatible with existing rows
        // that may already store "부서, 이름".
    ...
    function _getDeptValueForAssignee(field, scope){
        ...
        const el = scope?.querySelector?.(`[name="${controlling}"], [data-bulk-field="${controlling}"]`);
    ...
    async function loadFkSource(sourceKey){
        ...
        try {
            ...
            console.warn('[ACCESS_CONTROL] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    async function getFkOptions(field, scopeEl){
        ...
        // Assignees must be filtered by selected department.
        ...
        try {
            ...
            if(initial){
                ...
                if(select.value !== initial){
                    // For dept-scoped assignees, do not keep out-of-scope values.
                    if(field === 'work_assignee' || field === 'access_assignee'){
                        select.value = '';
                    } else {
            ...
            console.warn('[ACCESS_CONTROL] Failed to populate FK select:', field, err);
    ...
    function wireDeptAssigneeDependencies(scope){
        ...
        pairs.forEach(p => {
            const deptEl = root.querySelector(`[name="${p.dept}"], [data-bulk-field="${p.dept}"]`);
            const assigneeEl = root.querySelector(`[name="${p.assignee}"], [data-bulk-field="${p.assignee}"]`);
            ...
            deptEl.addEventListener('change', async ()=>{
                try {
                    ...
                    console.warn('[ACCESS_CONTROL] Failed to refresh assignee options', p, err);
                }
            });
        });
    }

    // ── 구분(person_type) 변경 시 소속/부서, 이름, 사번/번호 필드 동적 전환 ──
    let _vendorManagersCache = null;
    async function _loadVendorManagers(){
        if(_vendorManagersCache) return _vendorManagersCache;
        try {
            const resp = await fetch('/api/vendor-manufacturers/all-managers', { headers:{ 'Accept':'application/json' }});
            const payload = await resp.json();
            _vendorManagersCache = Array.isArray(payload.items) ? payload.items : [];
        } catch(_e){ _vendorManagersCache = []; }
        return _vendorManagersCache;
    }

    function _cleanupFieldWrapper(row, name){
        // Remove existing .fk-searchable-control wrapper left by setupSearchableSelect
        var wrapper = row.querySelector('.fk-searchable-control');
        if(wrapper){
            wrapper.remove();
        }
        // Also remove any bare element with that name (plain input/select)
        var bare = row.querySelector('[name="'+name+'"]');
        if(bare) bare.remove();
    }

    function _replaceFieldWithSelect(row, name, options, placeholder, onChange){
        if(!row) return null;
        _cleanupFieldWrapper(row, name);
        var sel = document.createElement('select');
        sel.name = name;
        sel.className = 'form-input search-select';
        sel.setAttribute('data-placeholder', placeholder);
        var html = '<option value="">'+escapeHTML(placeholder)+'</option>';
        options.forEach(function(o){ html += '<option value="'+escapeAttr(o.value)+'">'+escapeHTML(o.label)+'</option>'; });
        sel.innerHTML = html;
        row.appendChild(sel);
        sel.disabled = false;
    ...
    function _replaceFieldWithInput(row, name, placeholder, readOnly){
        if(!row) return null;
        _cleanupFieldWrapper(row, name);
        ...
        inp.disabled = false;
        if(name === 'name') inp.required = true;
        row.appendChild(inp);
        return inp;
    }

    function wirePersonTypeSwitch(form){
        if(!form) return;
        const ptSelect = form.querySelector('[name="person_type"]');
        if(!ptSelect) return;
        if(ptSelect.dataset.ptWired === '1') return;
        ptSelect.dataset.ptWired = '1';

        const affRow = form.querySelector('#add-affiliation-row') || (function(){ const r = form.querySelector('[name="affiliation"]'); return r ? r.closest('.form-row') : null; })();
        const nameRow = form.querySelector('#add-name-row') || (function(){ const r = form.querySelector('[name="name"]'); return r ? r.closest('.form-row') : null; })();
        const idRow = form.querySelector('#add-id-number-row') || (function(){ const r = form.querySelector('[name="id_number"]'); return r ? r.closest('.form-row') : null; })();

        async function switchFields(){
            const ptype = ptSelect.value;
            if(!ptype){
                // 구분 미선택: 모두 disabled
                [affRow, nameRow, idRow].forEach(row => {
                    if(!row) return;
                    _replaceFieldWithInput(row, row === affRow ? 'affiliation' : row === nameRow ? 'name' : 'id_number', '구분을 먼저 선택하세요', false);
            ...
            if(ptype === '내부직원'){
                // 소속/부서: org_department select
                const depts = await loadFkSource('ORG_DEPARTMENT');
                const deptOpts = [];
                const seen = new Set();
                depts.forEach(d => {
                    const name = String(d.dept_name || '').trim();
                    if(name && !seen.has(name)){ seen.add(name); deptOpts.push({ value: name, label: name }); }
                });
                deptOpts.sort((a,b) => a.label.localeCompare(b.label, 'ko'));

                const deptSel = _replaceFieldWithSelect(affRow, 'affiliation', deptOpts, '소속/부서 선택', async function(){
                    // 부서 선택 시 이름 필터링
                    const selectedDept = deptSel.value;
                    const users = await loadFkSource('USER_PROFILE');
                    const filtered = selectedDept ? users.filter(u => String(u.department||'').trim() === selectedDept) : users;
                    const nameOpts = [];
                    const nseen = new Set();
                    filtered.forEach(u => {
                        const n = String(u.name||'').trim();
                        const emp = String(u.emp_no||'').trim();
                        if(n && !nseen.has(n)){ nseen.add(n); nameOpts.push({ value: n, label: n, emp_no: emp, department: String(u.department||'') }); }
                    });
                    nameOpts.sort((a,b) => a.label.localeCompare(b.label, 'ko'));
                    const nameSel = _replaceFieldWithSelect(nameRow, 'name', nameOpts, '이름 선택', function(){
                        // 이름 선택 시 사번 자동입력
                        const chosen = nameOpts.find(o => o.value === nameSel.value);
                        const idEl = idRow?.querySelector('[name="id_number"]');
                        if(idEl && chosen) idEl.value = chosen.emp_no || '';
                    });
                    if(nameSel) nameSel.required = true;
                    // 사번/번호: readonly
                    _replaceFieldWithInput(idRow, 'id_number', '자동 입력', true);
                });

                // 초기: 이름/사번 placeholder
                const initNameSel = _replaceFieldWithSelect(nameRow, 'name', [], '소속/부서를 먼저 선택하세요');
                if(initNameSel){ initNameSel.disabled = true; initNameSel.required = true; }
                _replaceFieldWithInput(idRow, 'id_number', '자동 입력', true);

            } else if(ptype === '협력직원'){
                // 소속/부서: vendor manufacturer names
                const managers = await _loadVendorManagers();
                const vendorNames = new Set();
                managers.forEach(m => { const vn = String(m.vendor_name||'').trim(); if(vn) vendorNames.add(vn); });
                const vendorOpts = [...vendorNames].sort((a,b) => a.localeCompare(b,'ko')).map(v => ({ value: v, label: v }));

                const vendorSel = _replaceFieldWithSelect(affRow, 'affiliation', vendorOpts, '소속 선택 (벤더)', async function(){
                    const selectedVendor = vendorSel.value;
                    const filtered = selectedVendor ? managers.filter(m => String(m.vendor_name||'').trim() === selectedVendor) : managers;
                    const nameOpts = [];
                    const nseen = new Set();
                    filtered.forEach(m => {
                        const n = String(m.name||'').trim();
                        const ph = String(m.phone||'').trim();
                        if(n && !nseen.has(n)){ nseen.add(n); nameOpts.push({ value: n, label: n, phone: ph, org: String(m.org||'') }); }
                    });
                    nameOpts.sort((a,b) => a.label.localeCompare(b.label, 'ko'));
                    const nameSel = _replaceFieldWithSelect(nameRow, 'name', nameOpts, '이름 선택', function(){
                        const chosen = nameOpts.find(o => o.value === nameSel.value);
                        const idEl = idRow?.querySelector('[name="id_number"]');
                        if(idEl && chosen) idEl.value = chosen.phone || '';
                    });
                    if(nameSel) nameSel.required = true;
                    _replaceFieldWithInput(idRow, 'id_number', '자동 입력', true);
                });

                const initNameSel = _replaceFieldWithSelect(nameRow, 'name', [], '소속을 먼저 선택하세요');
                if(initNameSel){ initNameSel.disabled = true; initNameSel.required = true; }
                _replaceFieldWithInput(idRow, 'id_number', '자동 입력', true);

            } else if(ptype === '외부직원'){
                // 모두 text input
                _replaceFieldWithInput(affRow, 'affiliation', '소속/부서 입력', false);
                _replaceFieldWithInput(nameRow, 'name', '이름 입력', false);
                _replaceFieldWithInput(idRow, 'id_number', '사번/번호 입력', false);
            }
        }

        ptSelect.addEventListener('change', switchFields);
    }
    ...
    let isFetchingEntries = false;

    function ensureLoadingStyle(){
        if(document.getElementById('access-loading-style')) return;
        const style = document.createElement('style');
        style.id = 'access-loading-style';
        style.textContent = `
    ...
    function normalizeRow(row){
        ...
        normalized.status = String(row.status ?? '').trim() || '입실';
        normalized.person_type = row.person_type || '';
    ...
    async function fetchAccessEntries(params){
        const query = buildQuery({
            view: 'registers',
            page_size: ACCESS_FETCH_LIMIT,
            ...params
        });
        const url = query ? `${ACCESS_API_BASE}?${query}` : ACCESS_API_BASE;
        ...
        try { payload = await resp.json(); } catch(_e) { payload = {}; }
        if(!resp.ok || payload.success === false){
            throw new Error(payload.message || '출입 내역을 불러오지 못했습니다.');
        }
        return payload;
    }

    async function loadEntriesFromServer(options){
        const opts = options || {};
        ...
        try {
            const payload = await fetchAccessEntries({ include_deleted: opts.includeDeleted ? '1' : undefined });
    ...
    function normalizeEntryPayload(raw, overrides){
        const merged = { ...(raw || {}), ...(overrides || {}) };
        if(Object.prototype.hasOwnProperty.call(merged, 'goods_qty')){
            const qty = coerceQty(merged.goods_qty);
            merged.goods_qty = qty === '' ? '' : qty;
        }
        return merged;
    }

    async function requestAccessApi(url, options){
        const opts = options || {};
        const fetchOptions = {
    ...
    async function createAccessEntry(payload){
        return requestAccessApi(ACCESS_API_BASE, {
            method: 'POST',
            body: payload
        });
    }

    async function updateAccessEntry(entryId, payload){
        return requestAccessApi(`${ACCESS_API_BASE}/${entryId}`, {
            method: 'PUT',
            body: payload
        });
    }

    async function deleteAccessEntries(ids){
        if(!Array.isArray(ids) || ids.length === 0) return { success: true, deleted: 0 };
        if(ids.length === 1){
            return requestAccessApi(`${ACCESS_API_BASE}/${ids[0]}`, { method: 'DELETE' });
        }
        return requestAccessApi(`${ACCESS_API_BASE}/bulk-delete`, {
            method: 'POST',
            body: { ids }
        });
    }
        ...
        slice.forEach((row)=>{
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    // Assignees should display as name-only in the table.
                    ...
                    // 대기 상태에서는 출입일시/퇴실일시는 항상 '-'로 표시
                    if((col==='entry_datetime' || col==='exit_datetime') && String(row.status)==='대기'){
                        displayVal = '-';
                    }
                    // 입실 상태에서는 퇴실일시는 항상 '-'로 표시
                    if(col==='exit_datetime' && String(row.status)==='입실'){
                        displayVal = '-';
                    }
                    ...
                    if(col === 'status'){
                        ...
                        // 매핑: 입실 -> ws-run, 대기 -> ws-wait, 퇴실 -> ws-idle
                        let cls = 'ws-wait';
                        if(v === '입실') cls = 'ws-run';
                ...
                + (()=>{
                        const st = String(row.status||'');
                        const isWaiting = (st === '대기');
                        const actionTitle = isWaiting ? '입실' : '퇴실';
                        const intent = isWaiting ? 'enter' : 'exit';
                        const icon = isWaiting
                            ? '/static/image/svg/datacenter/free-icon-entrance.svg'
                            : '/static/image/svg/datacenter/free-icon-leave.svg';
                        const disabledAttr = (st === '퇴실') ? ' disabled' : '';
                        return `<button type="button" class="action-btn" data-action="toggle" data-intent="${intent}" data-id="${row.id}" title="${actionTitle}" aria-label="${actionTitle}"${disabledAttr}>
                                    <img src="${icon}" alt="${actionTitle}" class="action-icon">
                                </button>`;
                   })()
                + `</td>`;
    ...
    function loadColumnSelection(){
        try {
            ...
            if(filtered.length > 0){
                ...
                // This avoids the common “saved in modal but not visible in table” confusion.
    ...
    function fillEditForm(row){
        ...
        form.innerHTML='';
        const groups = [
            { title:'출입 관리', cols:['person_type','affiliation','name','id_number','entry_purpose','entry_area','laptop_use','usb_lock_use','work_management_dept','work_assignee','access_management_dept','access_assignee'] },
        ...
        hydrateFkFields(form).catch(err => console.warn('[ACCESS_CONTROL] FK hydrate failed for edit form', err));
        wireDeptAssigneeDependencies(form);
        wirePersonTypeSwitch(form);
        // 편집 시 기존 person_type 값에 맞게 필드 전환
        const ptSel = form.querySelector('[name="person_type"]');
        if(ptSel && ptSel.value){
            setTimeout(function(){
                ptSel.dispatchEvent(new Event('change'));
                // 기존 값 복원
                setTimeout(function(){
                    const aff = form.querySelector('[name="affiliation"]');
                    const nm = form.querySelector('[name="name"]');
                    const idn = form.querySelector('[name="id_number"]');
                    if(aff) aff.value = row.affiliation || '';
                    if(nm) nm.value = row.name || '';
                    if(idn) idn.value = row.id_number || '';
                    // 검색 셀렉트 텍스트 갱신
                    [aff, nm].forEach(function(el){
    ...
    function generateFieldInput(col,value=''){
        // access registration fields
        if(col==='person_type'){
            const v = String(value??'');
            return `<select name="person_type" class="form-input search-select" data-placeholder="선택">
                <option value="" ${v===''?'selected':''}>선택</option>
                <option value="내부직원" ${v==='내부직원'?'selected':''}>내부직원</option>
                <option value="협력직원" ${v==='협력직원'?'selected':''}>협력직원</option>
                <option value="외부직원" ${v==='외부직원'?'selected':''}>외부직원</option>
            </select>`;
        }
        if(col==='status'){
            const v = String(value??'');
            return `<select name="status" class="form-input search-select" data-placeholder="선택">
                <option value="" ${v===''?'selected':''}>선택</option>
                <option value="입실" ${v==='입실'?'selected':''}>입실</option>
    ...
    function addRow(data){
        ...
        // If status is '대기', clear times so they render as '-'
        if(String(data.status)==='대기'){
            data.entry_datetime = '';
            data.exit_datetime = '';
        }
    ...
    function exportCSV(onlySelected){
        ...
        const mm = String(d.getMonth()+1).padStart(2,'0');
        const dd = String(d.getDate()).padStart(2,'0');
    const filename = `access_registration_list_${yyyy}${mm}${dd}.csv`;
            ...
            if(btn){
                ...
                if(action==='edit'){
                    ...
                    if(editSaveEl){
                        ...
                        if(row.id){ editSaveEl.setAttribute('data-entry-id', row.id); }
                        else { editSaveEl.removeAttribute('data-entry-id'); }
                    }
                } else if(action==='toggle'){
                    const curr = String(row.status || '');
                    if(!row.id){
                        showMessage('출입 내역 식별자를 찾을 수 없습니다.', '오류');
                        return;
                    }
                    const now = nowYmdHm();
                    let patch = null;
                    let toast = '';
                    if(curr === '대기'){
                        patch = { status: '입실', entry_datetime: now, exit_datetime: '' };
                        toast = '입실 처리되었습니다.';
                    } else if(curr === '입실'){
                        patch = { status: '퇴실', exit_datetime: now };
                        toast = '퇴실 처리되었습니다.';
                    } else {
                        patch = { status: '입실', entry_datetime: now, exit_datetime: '' };
                        toast = '입실 처리되었습니다.';
                    ...
                    btn.disabled = true;
                    try{
                        await updateAccessEntry(row.id, patch);
                        updateRow(realIndex, patch);
                        if(patch.status === '퇴실'){
                            try{
                                state.exitGrace.set(row.id, Date.now() + 5000);
                                setTimeout(()=>{ applyFilter(); }, 5000);
                            }catch(_e){}
                        }
                        try{ btn.setAttribute('data-intent', patch.status === '입실' ? 'exit' : 'enter'); }catch(_e){}
                        showMessage(toast, '완료');
                    } catch(err){
                        console.error(err);
                        showMessage(err.message || '상태 변경 중 오류가 발생했습니다.', '오류');
                    } finally {
                        btn.disabled = false;
                    }
                }
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> { openModal(ADD_MODAL_ID); });
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            if(!payload.entry_datetime || String(payload.entry_datetime).trim()===''){
                payload.entry_datetime = nowYmdHm();
            }
            payload.status = '입실';
            const saveBtn = document.getElementById(ADD_SAVE_ID);
            setButtonLoading(saveBtn, true, '저장 중...');
            try {
                await createAccessEntry(payload);
                form.reset();
                closeModal(ADD_MODAL_ID);
                await loadEntriesFromServer({ showSpinner: false });
                showMessage('출입 등록이 저장되었습니다.', '완료');
            } catch(err){
                console.error(err);
                showMessage(err.message || '출입 등록 저장 중 오류가 발생했습니다.', '오류');
            } finally {
    ...
    document.getElementById(EDIT_CLOSE_ID)?.addEventListener('click', ()=> closeModal(EDIT_MODAL_ID));
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            const index = parseInt(saveBtn?.getAttribute('data-index')||'-1',10);
            const entryId = parseInt(saveBtn?.getAttribute('data-entry-id')||'-1',10);
            const payload = normalizeEntryPayload(collectForm(form));
            setButtonLoading(saveBtn, true, '저장 중...');
            try {
                if(entryId > 0){
                    await updateAccessEntry(entryId, payload);
                ...
                showMessage('출입 등록이 수정되었습니다.', '완료');
            } catch(err){
                console.error(err);
                showMessage(err.message || '출입 등록 수정 중 오류가 발생했습니다.', '오류');
            } finally {
                setButtonLoading(saveBtn, false);
            }
        });
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            setButtonLoading(confirmBtn, true, '삭제 중...');
            try {
                const result = await deleteAccessEntries(ids);
                state.selected.clear();
                ...
                console.error(err);
                showMessage(err.message || '출입 등록 삭제 중 오류가 발생했습니다.', '오류');
            } finally {
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 출입 등록에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
    ...
    function buildBulkForm(){
        ...
        function inputFor(col){
            if(col === 'status') return `<select class="form-input" data-bulk-field="status"><option value="">선택</option><option value="입실">입실</option><option value="대기">대기</option></select>`;
            if(col === 'entry_area') return fkSelectMarkup('entry_area', { name: false, placeholder: '출입구역 선택', extraAttrs: ['data-bulk-field="entry_area"'] });
            if(col === 'laptop_use' || col === 'usb_lock_use') return `<select class="form-input" data-bulk-field="${col}"><option value="">선택</option><option value="O">O</option><option value="X">X</option></select>`;
            if(col === 'in_out_type') return `<select class="form-input" data-bulk-field="in_out_type"><option value="">선택</option><option value="반입">반입</option><option value="반출">반출</option><option value="반입/반출">반입/반출</option></select>`;
            if(col === 'goods_type') return `<select class="form-input" data-bulk-field="goods_type"><option value="">선택</option><option value="교체">교체</option><option value="구매">구매</option><option value="임대">임대</option></select>`;
            if(col === 'goods_qty') return `<input type="number" min="0" step="1" class="form-input" data-bulk-field="goods_qty" placeholder="0">`;
            if(col === 'note') return `<textarea class="form-input textarea-large" rows="6" data-bulk-field="note" placeholder="설명"></textarea>`;
            if(col === 'work_management_dept') return fkSelectMarkup('work_management_dept', { name: false, placeholder: '작업 관리부서 선택', extraAttrs: ['data-bulk-field="work_management_dept"'] });
            if(col === 'work_assignee') return fkSelectMarkup('work_assignee', { name: false, placeholder: '작업 담당자 선택', extraAttrs: ['data-bulk-field="work_assignee"'] });
            if(col === 'access_management_dept') return fkSelectMarkup('access_management_dept', { name: false, placeholder: '출입 관리부서 선택', extraAttrs: ['data-bulk-field="access_management_dept"'] });
            if(col === 'access_assignee') return fkSelectMarkup('access_assignee', { name: false, placeholder: '출입 담당자 선택', extraAttrs: ['data-bulk-field="access_assignee"'] });
            return `<input class="form-input" data-bulk-field="${col}" placeholder="값 입력">`;
        }
        const GROUPS = [
            { title:'출입 등록', cols:['status','name','affiliation','id_number','entry_datetime','exit_datetime','entry_purpose','entry_area','laptop_use','usb_lock_use','work_management_dept','work_assignee','access_management_dept','access_assignee'] },
            { title:'물품 반출', cols:['in_out_type','goods_type','goods_item','goods_qty','note'] }
        ];
        ...
        hydrateFkFields(form).catch(err => console.warn('[ACCESS_CONTROL] FK hydrate failed for bulk form', err));
        wireDeptAssigneeDependencies(form);
    }
    ...
    resetColumnSelection();
        ...
        try {
            await hydrateFkFields(document.getElementById(ADD_FORM_ID));
        } catch(err){
            console.warn('[ACCESS_CONTROL] FK hydrate failed for add form', err);
        }
        try {
            wireDeptAssigneeDependencies(document.getElementById(ADD_FORM_ID));
        } catch(_e){}
        wirePersonTypeSwitch(document.getElementById(ADD_FORM_ID));
        render();
...
await initData();
    ...
    function boot(){
        init().catch(err=>{
            console.error('Access control init failed', err);
