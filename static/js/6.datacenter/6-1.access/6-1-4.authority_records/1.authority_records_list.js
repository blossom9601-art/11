/**
 * 데이터센터 > 출입 관리 > 권한 기록 페이지 스크립트
 * - 서버 API 연동 (CRUD)
 * - 컬럼 토글, 페이징, 검색, CSV 다운로드
 */

(function(){
    // Mark body for Authority Records page to enable page-scoped CSS in center.css
    try { document.body.classList.add('page-authority-records'); } catch(_e){}

    const API_BASE = '/api/datacenter/access/authority-records';
    ...
    // Upload template (Authority Records audit schema)
    const UPLOAD_HEADERS_KO = [
        '상태','변경일시','변경유형','변경자','담당자','변경내용','변경사유'
    ];
    const HEADER_KO_TO_KEY = {
        '상태':'status','변경일시':'change_datetime','변경유형':'change_type','변경자':'changed_by','담당자':'manager','변경내용':'change_details','변경사유':'change_reason'
    };
    const ENUM_SETS = { change_type: ['정보 수정','정보 삭제','신규 등록'], status: ['활성','만료'] };
    ...
    const BASE_VISIBLE_COLUMNS = [
        'status','change_datetime','change_type','changed_by','manager','change_details','change_reason'
    ];
    const COLUMN_ORDER = [
        'status','change_datetime','change_type','changed_by','manager','change_details','change_reason'
    ];
    ...
    const COLUMN_MODAL_GROUPS = [
        { group: '권한 기록', columns: ['status','change_datetime','change_type','changed_by','manager','change_details','change_reason'] }
    ];

    /** 컬럼 메타 (라벨 + 그룹) */
    const COLUMN_META = {
        status:{label:'상태',group:'권한 기록'},
        change_datetime:{label:'변경일시',group:'권한 기록'},
    change_type:{label:'변경유형',group:'권한 기록'},
    changed_by:{label:'변경자',group:'권한 기록'},
        manager:{label:'담당자',group:'권한 기록'},
        change_details:{label:'변경내용',group:'권한 기록'},
        change_reason:{label:'변경사유',group:'권한 기록'}
    };
    ...
    async function apiFetch(url, options){
        const res = await fetch(url, Object.assign({ headers:{ 'Accept':'application/json','Content-Type':'application/json' }, credentials:'same-origin' }, options));
        const json = await res.json().catch(()=>({}));
        if(!res.ok) throw new Error(json.error || json.message || ('HTTP '+res.status));
        return json;
    }

    let _loadingFromServer = false;
    async function loadFromServer(){
        if(_loadingFromServer) return;
        _loadingFromServer = true;
        try {
            const items = await apiFetch(API_BASE+'?_='+Date.now());
            const arr = Array.isArray(items) ? items : (items.items || items.rows || []);
            state.data = arr.map(r => Object.assign({}, r, { id: Number(r.id) }));
            state.nextId = state.data.reduce((mx,r)=> Math.max(mx, r.id||0), 0) + 1;
        } catch(e){
            console.error('[authority-records] load failed:', e);
            state.data = [];
            state.nextId = 1;
        } finally {
    ...
    // 변경내용(멀티라인) 불릿 + 2줄 클램프/토글
    function ensureChangeDetailsStyles(){
        // Inject minimal, page-scoped CSS once
        if(document.getElementById('chg-more-clamp-styles')) return;
        const style = document.createElement('style');
        style.id = 'chg-more-clamp-styles';
        style.textContent = `
        .page-authority-records .chg-wrap{display:flex;flex-direction:column;gap:6px}
        .page-authority-records .chg-bullets{line-height:1.4;display:flex;flex-direction:column;gap:4px}
        .page-authority-records .chg-bullets.is-collapsed{max-height:calc(1.4em*2 + 4px);overflow:hidden;position:relative}
        .page-authority-records .chg-bullets.is-collapsed::after{content:"";position:absolute;left:0;right:0;bottom:0;height:1.2em;background:linear-gradient(to bottom,rgba(255,255,255,0),rgba(255,255,255,1))}
        @media (prefers-color-scheme: dark){
            .page-authority-records .chg-bullets.is-collapsed::after{background:linear-gradient(to bottom,rgba(0,0,0,0),rgba(0,0,0,1))}
        }
        .page-authority-records .chg-more-toggle{align-self:flex-start;font-size:12px;color:#64748b;background:none;border:0;padding:0;cursor:pointer;text-decoration:underline}
        .page-authority-records .chg-more-toggle:hover{color:#334155}
        `;
        document.head.appendChild(style);
    }
    function renderChangeDetails(val){
        const s = String(val??'').trim();
        // 표에서는 빈 값이나 불릿 문자를 제거한 결과가 비어 있으면 아무것도 표시하지 않음
        if(!s) return '';
        ...
        const bullets = `<div class="chg-bullets${hasMore ? ' is-collapsed' : ''}" data-expanded="${hasMore ? 'false' : 'true'}">${lines.map(line=> `<div class="chg-line"><span class="chg-dot" aria-hidden="true"></span><span class="chg-text">${escapeHTML(line)}</span></div>`).join('')}</div>`;
        if(!hasMore){
            return `<div class="chg-wrap">${bullets}</div>`;
        }
        const toggle = `<button type="button" class="chg-more-toggle" aria-expanded="false" aria-label="더보기">more</button>`;
        return `<div class="chg-wrap">${bullets}${toggle}</div>`;
    }

    function render(highlightContext){
        const tbody = document.getElementById(TBODY_ID);
        if(!tbody) return;
    // styles for 변경내용 bullets are now in center.css
        tbody.innerHTML='';
        // 정렬 적용 (필터 결과에 대해)
        ...
        if(state.filtered.length === 0){
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '권한 기록 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 권한 기록을 추가하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
        ...
        slice.forEach((row)=>{
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    // 변경내용 렌더링을 이미지 스타일처럼 불릿으로 표시
                    if(col === 'change_details'){
                        cellValue = renderChangeDetails(rawVal);
                    }
    ...
    function fillEditForm(row, opts){
        const form = document.getElementById(EDIT_FORM_ID); if(!form) return;
        form.innerHTML='';
        const groups = [
            { title:'권한 기록', cols:['status','change_datetime','change_type','changed_by','manager','change_reason','change_details'] }
        ];
        groups.forEach(g=>{
        ...
        // 관리 > 수정: 변경사유만 입력 가능
        if(opts && opts.reasonOnly){
            form.querySelectorAll('input,select,textarea').forEach(el=>{
                if(el.name === 'change_reason') return; el.disabled = true; el.readOnly = true;
            });
        }
    }
    ...
    function generateFieldInput(col,value=''){
        ...
        if(col==='change_type'){
            const v = String(value??'');
            const opts = ['', '정보 수정','정보 삭제','신규 등록'];
            return `<select name="change_type" class="form-input">${opts.map(o=>`<option value="${o}" ${o===v?'selected':''}>${o===''?'선택':o}</option>`).join('')}</select>`;
        }
        if(col==='change_datetime'){
            const v = String(value??'');
            return `<input name="change_datetime" class="form-input" value="${v}" placeholder="YYYY-MM-DD HH:MM">`;
        }
        if(col==='change_details'){
            const v = String(value??'');
            return `<textarea name="change_details" class="form-input textarea-large" rows="4">${v}</textarea>`;
        }
    ...
    const rows = dataForCsv.map((r,i)=> [i+1, ...visibleCols.map(c=> c==='status' ? (computeEffectiveStatus(r) || '') : (r[c]??''))]);
        ...
        const mm = String(d.getMonth()+1).padStart(2,'0');
        const dd = String(d.getDate()).padStart(2,'0');
    const filename = `authority_records_list_${yyyy}${mm}${dd}.csv`;
    ...
    function bindEvents(){
        ...
        tbodyEl?.addEventListener('click', e=>{
            // more/less toggle for 변경내용
            const moreBtn = e.target.closest('.chg-more-toggle');
            if(moreBtn){
                const wrap = moreBtn.closest('.chg-wrap');
                const bullets = wrap?.querySelector('.chg-bullets');
                if(bullets){
                    const wasCollapsed = bullets.classList.contains('is-collapsed');
                    bullets.classList.toggle('is-collapsed');
                    const isExpanded = !bullets.classList.contains('is-collapsed');
                    moreBtn.textContent = isExpanded ? 'less' : 'more';
                    moreBtn.setAttribute('aria-expanded', String(isExpanded));
                }
                return; // stop row selection toggle
            }
            ...
            if(btn){
                ...
                if(action==='edit'){
                    // 관리 > 수정: 변경사유만 입력 가능
                    fillEditForm(row, { reasonOnly: true });
                    openModal(EDIT_MODAL_ID);
                    const editSaveEl = document.getElementById(EDIT_SAVE_ID);
                    if(editSaveEl){ editSaveEl.setAttribute('data-index', realIndex); editSaveEl.setAttribute('data-mode','reason-only'); }
                }
    ...
    document.getElementById(COLUMN_APPLY_ID)?.addEventListener('click', handleColumnFormApply);
        ...
        // add modal
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> { openModal(ADD_MODAL_ID); enforceAuthDateFieldsEnabledByVisibleCols(ADD_FORM_ID); attachAccessLevelToggle(ADD_FORM_ID); initDatePickersSafe(); });
    function initDatePickersSafe(){ try{ initDatePickers(ADD_FORM_ID); }catch(_e){} }
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            if(!data.change_datetime){
                ...
                const hh = String(d.getHours()).padStart(2,'0');
                const mi = String(d.getMinutes()).padStart(2,'0');
                data.change_datetime = `${yyyy}-${mm}-${dd} ${hh}:${mi}`;
            }
            if(data.status && !ENUM_SETS.status.includes(data.status)){
                showMessage('상태는 활성/만료 중에서 선택하세요.', '입력 오류'); return;
            }
            if(data.change_type && !ENUM_SETS.change_type.includes(data.change_type)){
                showMessage('변경유형은 정보 수정/정보 삭제/신규 등록 중에서 선택하세요.', '입력 오류'); return;
            }
            try {
                await apiFetch(API_BASE, { method:'POST', body:JSON.stringify(data) });
                form.reset(); closeModal(ADD_MODAL_ID);
                await loadFromServer();
            } catch(e){ showMessage('등록 실패: '+e.message, '오류'); }
        });
        // edit modal
    document.getElementById(EDIT_CLOSE_ID)?.addEventListener('click', ()=> closeModal(EDIT_MODAL_ID));
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            const mode = indexEl?.getAttribute('data-mode')||'';
            const row = state.data[index];
            if(!row){ showMessage('수정 대상을 찾을 수 없습니다.', '오류'); return; }
            const data = collectForm(form);
            const payload = mode === 'reason-only' ? { change_reason: data.change_reason || '' } : data;
            if(mode !== 'reason-only'){
                if(data.status && !ENUM_SETS.status.includes(data.status)){
                    showMessage('상태는 활성/만료 중에서 선택하세요.', '입력 오류'); return;
                }
            ...
            try {
                await apiFetch(API_BASE+'/'+row.id, { method:'PUT', body:JSON.stringify(payload) });
                closeModal(EDIT_MODAL_ID);
                await loadFromServer();
            } catch(e){ showMessage('수정 실패: '+e.message, '오류'); }
        });
        // csv
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById(UPLOAD_TEMPLATE_BTN_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                // Set reasonable column widths for new schema
        wsTemplate['!cols'] = UPLOAD_HEADERS_KO.map((h)=>{
            const wide = ['변경내용','변경사유'];
            const mid = ['담당자','변경유형','변경일시','변경자'];
                        if(wide.includes(h)) return { wch: 40 };
                ...
                const rules = [
                    ...
                    ['- 변경유형은 다음 중 하나여야 합니다: 정보 수정, 정보 삭제, 신규 등록'],
                    ['- 상태는 활성 또는 만료 중 하나여야 합니다.'],
                    ['- 변경일시 형식 예: 2025-01-12 15:01 (YYYY-MM-DD HH:MM)'],
                    ['- 변경내용은 여러 줄로 작성할 수 있습니다. 각 줄은 "- "로 시작해도 되고, 업로드 시 자동 정리됩니다.'],
                    [''],
                    ['컬럼 순서 (복사/참고용)'],
                ...
                XLSX.writeFile(wb, 'authority_records_upload_template.xlsx');
            ...
            reader.onload = ()=>{
                try{
                    ...
                    for(let r=1; r<rows.length; r++){
                        ...
                        // Validate enums
                        if(rec.status && !ENUM_SETS.status.includes(rec.status)){
                            errors.push(`Row ${r+1}: 상태는 활성/만료 중에서 입력하세요.`);
                        }
                        if(rec.change_type && !ENUM_SETS.change_type.includes(rec.change_type)){
                            errors.push(`Row ${r+1}: 변경유형은 정보 수정/정보 삭제/신규 등록 중에서 입력하세요.`);
                        }
                        imported.push(rec);
                    }
                    if(errors.length){
                    ...
                    for(const item of imported){
                        try { await apiFetch(API_BASE, { method:'POST', body:JSON.stringify(item) }); ok++; }
                        catch(_e){ console.error('[upload] row failed:', _e); }
                    }
                    await loadFromServer();
        ...
        const closeStats = ()=>{ closeModal(STATS_MODAL_ID); };
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 권한 기록을 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 권한 기록을 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                const res = await apiFetch(API_BASE+'/bulk-delete', { method:'POST', body:JSON.stringify({ ids }) });
                const removed = res.deleted || ids.length;
                state.selected.clear();
                closeModal(DELETE_MODAL_ID);
                await loadFromServer();
                if(removed > 0){ setTimeout(()=> showMessage(`${removed}개 항목이 삭제되었습니다.`, '완료'), 0); }
            } catch(e){ showMessage('삭제 실패: '+e.message, '오류'); }
        });
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(count===1){
                ...
                const row = state.data[realIndex];
                // 단일 선택 시에도 수정 모달은 변경사유만 입력 가능하도록 동일 정책 적용
                fillEditForm(row, { reasonOnly: true });
                openModal(EDIT_MODAL_ID);
                const editSaveEl = document.getElementById(EDIT_SAVE_ID);
                if(editSaveEl){ editSaveEl.setAttribute('data-index', realIndex); editSaveEl.setAttribute('data-mode','reason-only'); }
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 권한 기록에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', async ()=>{
            ...
            for(const rid of ids){
                try { await apiFetch(API_BASE+'/'+rid, { method:'PUT', body:JSON.stringify(payload) }); ok++; }
                catch(_e){ console.error('[bulk] update failed for', rid, _e); }
            }
            closeModal(BULK_MODAL_ID);
            await loadFromServer();
            setTimeout(()=> showMessage(`${ok}개 항목에 일괄 변경이 적용되었습니다.`, '완료'), 0);
        });
        // esc close
    ...
    // include stats modal in esc close (already handled above list)
    ...
    function buildBulkForm(){
        ...
        function inputFor(col){
            if(col === 'status') return `<select class="form-input" data-bulk-field="status"><option value="">선택</option><option value="활성">활성</option><option value="만료">만료</option></select>`;
            if(col === 'change_type') return `<select class="form-input" data-bulk-field="change_type"><option value="">선택</option><option value="정보 수정">정보 수정</option><option value="정보 삭제">정보 삭제</option><option value="신규 등록">신규 등록</option></select>`;
            if(col === 'change_datetime') return `<input class="form-input" data-bulk-field="change_datetime" placeholder="YYYY-MM-DD HH:MM">`;
            if(col === 'change_details') return `<textarea class="form-input" data-bulk-field="change_details" rows="3" placeholder="여러 줄 입력 가능"></textarea>`;
            if(col === 'change_reason') return `<input class="form-input" data-bulk-field="change_reason" placeholder="사유">`;
            if(col === 'manager') return `<input class="form-input" data-bulk-field="manager" placeholder="담당자">`;
            if(col === 'changed_by') return `<input class="form-input" data-bulk-field="changed_by" placeholder="변경자">`;
            return `<input class="form-input" data-bulk-field="${col}" placeholder="값 입력">`;
        }
        const GROUPS = [
            { title:'권한 기록', cols:['status','change_datetime','change_type','changed_by','manager','change_details','change_reason'] }
        ];
    ...
    // ----- Stats: 최근 12개월 월별 변경유형 라인/막대 차트 -----
    let statsChart = null;
    async function ensureChartJs(){
        if(window.Chart) return;
        await loadScript('/static/vendor/chartjs/chart.umd.min.4.4.3.js');
    }
    function getLast12MonthLabels(baseYear){
        const labels = [];
        const now = new Date();
        const targetYear = Number.isFinite(baseYear)? baseYear : now.getFullYear();
        const endMonth = (targetYear === now.getFullYear()) ? now.getMonth() : 11; // 0-index
        for(let i=11; i>=0; i--){
            const d = new Date(targetYear, endMonth - i, 1);
    ...
    function parseRowMonth(row){
        const s = String(row.change_datetime||'').trim();
        // expect 'YYYY-MM-DD HH:MM'
        const m = s.match(/^(\d{4})-(\d{2})-(\d{2})/);
        if(!m) return null;
        return `${m[1]}-${m[2]}`;
    }
    function buildMonthlyCounts(rows, baseYear){
        const labels = getLast12MonthLabels(baseYear);
        const types = ENUM_SETS.change_type; // ['정보 수정','정보 삭제','신규 등록']
        const dataMap = {};
        types.forEach(t=>{ dataMap[t] = Object.fromEntries(labels.map(l=>[l,0])); });
        rows.forEach(r=>{
            const ym = parseRowMonth(r); if(!ym) return;
            if(!labels.includes(ym)) return; // only last 12 months
            const t = r.change_type; if(!types.includes(t)) return;
            dataMap[t][ym] = (dataMap[t][ym]||0) + 1;
        });
        return { labels, types, dataMap };
    }
    function accent(){ return resolveAccentColor(); }
    function palette(){
        // Fixed, accessible palette: bars use semi-transparent fills, lines use solid strokes
        return {
            bar: ['rgba(99,102,241,0.55)','rgba(16,185,129,0.55)','rgba(245,158,11,0.55)'],
            line: ['#6366f1','#10b981','#f59e0b']
        };
    }
    async function buildStats(){
        await ensureChartJs();
        const ctx = document.getElementById('stats-change-type-timeline');
        const emptyNote = document.getElementById('stats-empty-note');
        const yearSel = document.getElementById('stats-year-select');
        if(!ctx){ return; }
        // Initialize year dropdown (last 5 years including this year)
        (function initYearSelect(){
            if(!yearSel || yearSel.dataset.ready==='1') return;
            const now = new Date();
            const years = Array.from({length:5}, (_,i)=> now.getFullYear()-i);
            yearSel.innerHTML = years.map(y=> `<option value="${y}">${y}년</option>`).join('');
            yearSel.value = String(now.getFullYear());
            yearSel.dataset.ready = '1';
            yearSel.addEventListener('change', ()=>{ buildStats(); });
        })();
        const base = state.filtered.length ? state.filtered : state.data;
        const selectedYear = parseInt(yearSel?.value||'',10);
        const { labels, types, dataMap } = buildMonthlyCounts(base, Number.isFinite(selectedYear)? selectedYear : undefined);
        const total = types.reduce((sum,t)=> sum + labels.reduce((s,l)=> s + (dataMap[t][l]||0), 0), 0);
        if(total === 0){ if(emptyNote) emptyNote.style.display = 'block'; if(statsChart){ statsChart.destroy(); statsChart=null; } return; }
        if(emptyNote) emptyNote.style.display = 'none';
        const colors = palette();
        const barSets = types.map((t, idx)=>({
            type: 'bar',
            label: t,
            stack: 'counts',
            data: labels.map(l=> dataMap[t][l]||0),
            backgroundColor: colors.bar[idx % colors.bar.length],
            borderColor: colors.line[idx % colors.line.length],
            borderWidth: 1,
            borderRadius: 4,
            maxBarThickness: 28
        }));
        const lineSets = types.map((t, idx)=>({
            type: 'line',
            label: t, // same label, but will be filtered from legend
            data: labels.map(l=> dataMap[t][l]||0),
            borderColor: colors.line[idx % colors.line.length],
            backgroundColor: colors.line[idx % colors.line.length],
            borderWidth: 2,
            pointRadius: 2,
            pointHoverRadius: 4,
            tension: 0.25,
        ...
        const datasets = [...barSets, ...lineSets];
        const config = {
            type: 'bar',
                    ...
                    legend: {
                        ...
                        labels: {
                            filter: (item, chart) => {
                                ...
                                return ds && ds.type === 'bar'; // show only bar entries
                            }
                        }
                    },
                    tooltip: {
                        ...
                        filter: (ctx) => {
                            ...
                            return ds && ds.type === 'bar'; // avoid duplicate (line) in tooltip
                        }
                    }
                },
                interaction: { mode: 'index', intersect: false },
                scales: {
                    x: { stacked: true, title: { display: true, text: '월' } },
                    y: { stacked: true, beginAtZero: true, title: { display: true, text: '건수' }, ticks: { precision:0 } }
                }
            }
        };
        // make canvas parent a fixed height for readability
        try{ ctx.parentElement.style.height = '320px'; }catch(_e){}
        if(statsChart){ statsChart.destroy(); statsChart = null; }
        statsChart = new window.Chart(ctx, config);
    }
    }
