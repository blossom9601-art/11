...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__VPN_DETAIL_URL) || '/p/hw_security_vpn_detail';

    function buildDetailUrl(assetId){
        // Deep-linking: include asset_id so open-in-new-tab works.
        // The server will immediately redirect to a clean URL and stash the id in session.
        if(assetId != null && !isNaN(parseInt(assetId, 10))){
            return `${DETAIL_URL}?asset_id=${encodeURIComponent(String(parseInt(assetId,10)))}`;
        }
        return DETAIL_URL;
    }
    ...
    const VISIBLE_COLS_KEY = 'security_vpn_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/security/vpn/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'SEC-VPN';
    const SERVER_MODEL_FORM_FACTOR_FILTER = 'VPN';
    ...
    const ENABLE_SECURITY_VPN_DUPLICATE = false;
    const ENABLE_SECURITY_VPN_BULK_EDIT = false;
    ...
    const FK_SOURCE_CONFIG = {
        ...
        // IMPORTANT: hardware assets store model as hw_server_type(server_code) (even for SECURITY).
        // Using hw-security-types(security_code) can trigger FK failures when the codes are not mirrored.
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            console.warn('[SECURITY_VPN] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[SECURITY_VPN] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[SECURITY_VPN] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[SECURITY_VPN] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[SECURITY_VPN] FK preload failed', err);
            });
        }
    ...
    function buildAssetPayload(formData, existingRow){
        const payload = {
            // NOTE: Server will override these, but keep correct defaults to avoid confusion
            // and to keep any client-side validation/diagnostics accurate.
    ...
    function render(highlightContext){
        ...
        slice.forEach((row)=>{
            ...
            const assetId = (row && (row.id != null ? row.id : row.asset_id));
            const assetIdNum = (assetId == null) ? null : parseInt(assetId, 10);
            const checked = (assetIdNum != null && !isNaN(assetIdNum) && state.selected.has(assetIdNum)) ? 'checked' : '';
            tr.setAttribute('data-id', (assetIdNum != null && !isNaN(assetIdNum)) ? String(assetIdNum) : '');
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${(assetIdNum != null && !isNaN(assetIdNum)) ? String(assetIdNum) : ''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'work_name' && displayVal !== '-'){
                        cellValue = `<a href="${buildDetailUrl(assetIdNum)}" class="work-name-link" data-id="${(assetIdNum != null && !isNaN(assetIdNum)) ? String(assetIdNum) : ''}">${cellValue}</a>`;
                    }
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    const row = state.data.find(r=> (r && ((r.id != null && parseInt(r.id,10) === rid) || (r.asset_id != null && parseInt(r.asset_id,10) === rid))));
                    if(row){
                        sessionStorage.setItem('SECURITY_VPN:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('SECURITY_VPN:selected:system_name', String(row.system_name||''));
                        // Persist selection for detail hydration (support multiple legacy prefixes)
                        try{ sessionStorage.setItem('vpn:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('vpn:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ sessionStorage.setItem('SECURITY_VPN:selected:row', JSON.stringify(row)); }catch(_e2){}
                        try{ localStorage.setItem('SECURITY_VPN:selected:row', JSON.stringify(row)); }catch(_e3){}
                        try{ sessionStorage.setItem('hw_security_vpn:selected:row', JSON.stringify(row)); }catch(_e4){}
                        try{ localStorage.setItem('hw_security_vpn:selected:row', JSON.stringify(row)); }catch(_e5){}
                        e.preventDefault();
                        const assetId = (row.id != null ? row.id : row.asset_id);
                        if(assetId != null){
                            try{ sessionStorage.setItem('vpn:selected:asset_id', String(assetId)); }catch(_e0){}
                            try{ localStorage.setItem('vpn:selected:asset_id', String(assetId)); }catch(_e1){}
                            try{ sessionStorage.setItem('SECURITY_VPN:selected:asset_id', String(assetId)); }catch(_e6){}
                            try{ localStorage.setItem('SECURITY_VPN:selected:asset_id', String(assetId)); }catch(_e7){}
            ...
            if(btn){
                ...
                const realIndex = state.data.findIndex(r=> (r && ((r.id != null && parseInt(r.id,10) === rid) || (r.asset_id != null && parseInt(r.asset_id,10) === rid))));
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_VPN_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_VPN_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
