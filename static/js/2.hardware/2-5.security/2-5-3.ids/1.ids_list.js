...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__IDS_DETAIL_URL) || '/p/hw_security_ids_detail';
    ...
    const VISIBLE_COLS_KEY = 'security_ids_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/security/ids/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'SEC-IDS';
    const SERVER_MODEL_FORM_FACTOR_FILTER = 'IDS';
    ...
    const ENABLE_SECURITY_IDS_DUPLICATE = false;
    const ENABLE_SECURITY_IDS_BULK_EDIT = false;
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            console.warn('[SECURITY_IDS] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[SECURITY_IDS] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[SECURITY_IDS] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[SECURITY_IDS] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[SECURITY_IDS] FK preload failed', err);
            });
        }
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'work_name' && displayVal !== '-'){
                        ...
                        const assetId = (row.id != null ? row.id : (row.asset_id != null ? row.asset_id : ''));
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    if(row){
                        sessionStorage.setItem('SECURITY_IDS:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('SECURITY_IDS:selected:system_name', String(row.system_name||''));
                        try{ sessionStorage.setItem('ids:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('ids:selected:row', JSON.stringify(row)); }catch(_e){}
                        e.preventDefault();
                        const assetId = (row.id != null ? row.id : row.asset_id);
                        if(assetId != null){
                            try{ sessionStorage.setItem('ids:selected:asset_id', String(assetId)); }catch(_e0){}
                            try{ localStorage.setItem('ids:selected:asset_id', String(assetId)); }catch(_e1){}
                        }
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_IDS_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_IDS_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
