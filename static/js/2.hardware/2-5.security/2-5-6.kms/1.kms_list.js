...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__KMS_DETAIL_URL) || '/p/hw_security_kms_detail';
    ...
    const VISIBLE_COLS_KEY = 'security_kms_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/security/kms/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'SEC-KMS';
    const SERVER_MODEL_FORM_FACTOR_FILTER = 'KMS';
    ...
    const ENABLE_SECURITY_KMS_DUPLICATE = false;
    const ENABLE_SECURITY_KMS_BULK_EDIT = false;
    ...
    const FK_SOURCE_CONFIG = {
        ...
        SERVER_MODEL: { endpoint: '/api/hw-security-types', valueKey: 'security_code', labelKey: 'model_name' },
    ...
    function normalizeSecurityType(value){
        ...
        if(compact === 'fw' || compact === 'firewall' || lowered.includes('방화벽')) return 'FW';
        if(compact === 'vpn' || lowered.includes('가상사설') || lowered.includes('가상 사설')) return 'VPN';
        if(compact === 'ids' || lowered.includes('침입탐지') || lowered.includes('침입 탐지')) return 'IDS';
        if(compact === 'ips' || lowered.includes('침입방지') || lowered.includes('침입 방지')) return 'IPS';
        if(compact === 'hsm') return 'HSM';
        if(compact === 'kms') return 'KMS';
        if(compact === 'wips') return 'WIPS';
        if(compact === 'etc' || lowered.includes('기타')) return 'ETC';
        return raw.toUpperCase();
    }

    function isSecurityModelForPage(item){
        const type = normalizeSecurityType(item?.security_type || item?.hw_type || item?.type || item?.form_factor);
        const expected = normalizeSecurityType(SERVER_MODEL_FORM_FACTOR_FILTER);
        return !!type && !!expected && type === expected;
    }
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            if(sourceKey === 'SERVER_MODEL' && SERVER_MODEL_FORM_FACTOR_FILTER){
                const expected = normalizeSecurityType(SERVER_MODEL_FORM_FACTOR_FILTER);
                if(expected){
                    items = rawItems.filter(item => isSecurityModelForPage(item));
                }
            }
            ...
            console.warn('[SECURITY_KMS] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    function getAllowedManufacturerCodesForPage(){
        ...
        models.forEach(model => {
            if(!isSecurityModelForPage(model)) return;
    ...
    function enhanceFormSearchableSelects(formId){
        ...
        // Prefer shared global enhancer (loaded via layouts/_header.html).
        // This is more robust for modal content and keeps UI consistent with other pages.
        if(window.BlossomSearchableSelect && typeof window.BlossomSearchableSelect.enhance === 'function'){
            window.BlossomSearchableSelect.enhance(form);
    ...
    function ensureSelectHasOption(select, value, label){
        ...
        if(!v) return;
        const exists = Array.from(select.options || []).some(opt => String(opt.value || '').trim() === v);
        if(exists) return;
    ...
    function getRemoteSearchConfigForSelect(select){
            ...
            return {
                ...
                map: (items) => {
                    ...
                    return rows
                        ...
                        .map(item => {
                            const value = String(item?.manufacturer_code || '').trim();
                            const label = String(item?.manufacturer_name || '').trim() || value;
                            return value ? { value, label, searchLabel: label.toLowerCase(), valueLower: value.toLowerCase() } : null;
                        })
                        .filter(Boolean);
                }
            };
        ...
        if(field === 'model'){
            return {
                endpoint: '/api/hw-security-types',
                map: (items, context) => {
                    const vendorCode = String(context?.vendorCode || '').trim();
                    const rows = Array.isArray(items) ? items : [];
                    return rows
                        .filter(item => {
                            if(!isSecurityModelForPage(item)) return false;
                            if(!vendorCode) return true;
                            const code = extractManufacturerCodeFromModelRecord(item);
                            return code && code === vendorCode;
                        })
                        .map(item => {
                            const value = String(item?.security_code || '').trim();
                            const label = String(item?.model_name || '').trim() || value;
                            return value ? { value, label, searchLabel: label.toLowerCase(), valueLower: value.toLowerCase() } : null;
                        })
                        .filter(Boolean);
                }
    ...
    function getRelatedVendorValueForModelSelect(select){
        try{
            const form = select?.form;
            if(!form) return '';
            const parentField = select.getAttribute('data-parent-field') || 'vendor';
            const vendorSelect = form.querySelector(`[name="${parentField}"]`) || form.querySelector('[name="vendor"]');
    ...
    function openSearchDropdown(select){
        ...
        const remoteConfig = getRemoteSearchConfigForSelect(select);
        const state = {
            select,
            panel,
            ...
            focusIndex: -1,
            remote: remoteConfig ? { config: remoteConfig, timer: null, seq: 0 } : null
        };
    ...
    function commitSearchPanelSelection(state, value){
        // If remote search provides a value not present in the select, inject it so
        // the selected label is preserved in the UI.
        if(state && state.filtered && state.filtered.length){
            const match = state.filtered.find(opt => opt && opt.value === value);
            if(match){
                ensureSelectHasOption(state.select, match.value, match.label);
            }
        }
    ...
    function filterSearchPanelOptions(state){
        const termRaw = state.input.value.trim();
        const term = termRaw.toLowerCase();

        // Remote search mode: vendor/model can be queried from master APIs.
        if(state.remote && termRaw){
            const remote = state.remote;
            if(remote.timer){
                clearTimeout(remote.timer);
            }
            const seq = ++remote.seq;
            state.empty.textContent = '검색 중...';
            state.empty.hidden = false;
            state.list.innerHTML = '';
            remote.timer = setTimeout(async ()=>{
                try{
                    const url = `${remote.config.endpoint}?q=${encodeURIComponent(termRaw)}`;
                    const vendorCode = getRelatedVendorValueForModelSelect(state.select);
                    const data = await fetchJSON(url, { method:'GET', headers:{'Accept':'application/json'} });
                    const rawItems = Array.isArray(data?.items) ? data.items : (Array.isArray(data) ? data : []);
                    const mapped = remote.config.map(rawItems, { vendorCode }) || [];
                    if(!activeSearchPanel || activeSearchPanel !== state || remote.seq !== seq){
                        return;
                    }
                    // Replace option list in panel only (do not destroy select options).
                    state.options = mapped;
                    state.filtered = mapped;
                    state.focusIndex = state.filtered.findIndex(opt => opt.value === state.select.value);
                    state.empty.textContent = '검색 결과가 없습니다.';
                    renderSearchPanelOptions(state);
                }catch(err){
                    if(!activeSearchPanel || activeSearchPanel !== state || remote.seq !== seq){
                        return;
                    }
                    console.warn('[SECURITY_KMS] remote search failed', err);
                    state.options = [];
                    state.filtered = [];
                    state.focusIndex = -1;
                    state.empty.textContent = '검색 결과가 없습니다.';
        ...
        if(!term){
            state.options = buildSearchPanelOptions(state.select, state.placeholder);
            state.filtered = state.options.slice();
        } else {
            // Make sure options are up-to-date with the select.
            state.options = buildSearchPanelOptions(state.select, state.placeholder);
    ...
    function syncSearchableSelectCompat(select){
        if(window.BlossomSearchableSelect && typeof window.BlossomSearchableSelect.syncAll === 'function'){
            const scope = (select && (select.closest('.modal-overlay-full') || select.closest('form'))) || document;
    ...
    function populateFkSelectOptions(selectEl, field){
        ...
        syncSearchableSelectCompat(selectEl);
    }
    ...
    function buildModelOptionsMarkupForVendor(vendorCode, selectedValue, placeholderLabel){
        ...
        const filtered = (Array.isArray(records) ? records : []).filter(item => {
            if(!isSecurityModelForPage(item)){
                return false;
            }
            if(!normalizedVendor){
                return true;
    ...
    async function refreshModelSelect(modelSelect, vendorCode){
        ...
        syncSearchableSelectCompat(modelSelect);
    }
            ...
            if(!modelSelect.dataset.vendorAutoSelectBound){
                modelSelect.addEventListener('change', async ()=>{
                    ...
                    if(manufacturerCode && String(vendorSelect.value || '').trim() !== manufacturerCode){
                        ...
                        syncSearchableSelectCompat(vendorSelect);
                    }
                });
    ...
    function refreshRackSelectForCenter(rackSelect, centerCode, options){
        ...
        syncSearchableSelectCompat(rackSelect);
    }
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[SECURITY_KMS] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[SECURITY_KMS] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelectCompat(selectEl);
            console.warn('[SECURITY_KMS] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[SECURITY_KMS] FK preload failed', err);
            });
        }
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'work_name' && displayVal !== '-'){
                        ...
                        const detailHref = (assetId != null && String(assetId).trim() !== '')
                            ? (DETAIL_URL + (String(DETAIL_URL).indexOf('?') > -1 ? '&' : '?') + 'id=' + encodeURIComponent(String(assetId)))
                            : DETAIL_URL;
                        cellValue = `<a href="${detailHref}" class="work-name-link" data-id="${row.id??''}">${cellValue}</a>`;
                    }
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    if(row){
                        sessionStorage.setItem('SECURITY_KMS:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('SECURITY_KMS:selected:system_name', String(row.system_name||''));
                        try{ sessionStorage.setItem('kms:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('kms:selected:row', JSON.stringify(row)); }catch(_e){}
                        e.preventDefault();
                        const assetId = (row.id != null ? row.id : row.asset_id);
                        if(assetId != null){
                            try{ sessionStorage.setItem('kms:selected:asset_id', String(assetId)); }catch(_e0){}
                            try{ localStorage.setItem('kms:selected:asset_id', String(assetId)); }catch(_e1){}
                        }
                        const detailHref = (assetId != null && String(assetId).trim() !== '')
                            ? (DETAIL_URL + (String(DETAIL_URL).indexOf('?') > -1 ? '&' : '?') + 'id=' + encodeURIComponent(String(assetId)))
                            : DETAIL_URL;
                        blsSpaNavigate(detailHref);
                        return;
                    }
                }catch(_e){}
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_KMS_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        document.getElementById(DISPOSE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                const res = await fetch('/api/hardware/assets/dispose', {
                    ...
                    body: JSON.stringify({ ids: ids, category: 'security' })
                });
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_KMS_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
