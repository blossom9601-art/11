...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__HSM_DETAIL_URL) || '/p/hw_security_hsm_detail';
    ...
    const VISIBLE_COLS_KEY = 'security_hsm_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/security/hsm/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'SEC-HSM';
    const SERVER_MODEL_FORM_FACTOR_FILTER = 'HSM';
    ...
    const ENABLE_SECURITY_HSM_DUPLICATE = false;
    const ENABLE_SECURITY_HSM_BULK_EDIT = false;
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            console.warn('[SECURITY_HSM] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[SECURITY_HSM] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[SECURITY_HSM] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[SECURITY_HSM] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[SECURITY_HSM] FK preload failed', err);
            });
        }
                ...
                + COLUMN_ORDER.map(col=>{
                    ...
                    // '업무 이름'(work_name) 컬럼: 값이 있을 때만 링크로 변환 ("-"는 링크로 만들지 않음)
                                        if(col === 'work_name' && displayVal !== '-'){
                                            // Security: do not leak asset id via URL.
                                            // Selection is persisted to storage in the click handler below.
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    if(row){
                        sessionStorage.setItem('SECURITY_HSM:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('SECURITY_HSM:selected:system_name', String(row.system_name||''));
                        try{ sessionStorage.setItem('hsm:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('hsm:selected:row', JSON.stringify(row)); }catch(_e){}
                        e.preventDefault();
                        const assetId = (row.id != null ? row.id : row.asset_id);
                        if(assetId != null){
                            try{ sessionStorage.setItem('hsm:selected:asset_id', String(assetId)); }catch(_e0){}
                            try{ localStorage.setItem('hsm:selected:asset_id', String(assetId)); }catch(_e1){}
                        }
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_HSM_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_HSM_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
