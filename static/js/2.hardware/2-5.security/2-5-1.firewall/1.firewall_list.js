...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__FIREWALL_DETAIL_URL) || '/p/hw_security_firewall_detail';
    ...
    const VISIBLE_COLS_KEY = 'security_firewall_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/security/firewall/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'SEC-FW';
    const SERVER_MODEL_FORM_FACTOR_FILTER = 'FW';
    ...
    const ENABLE_SECURITY_FIREWALL_DUPLICATE = false;
    const ENABLE_SECURITY_FIREWALL_BULK_EDIT = false;
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            console.warn('[SECURITY_FIREWALL] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[SECURITY_FIREWALL] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[SECURITY_FIREWALL] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[SECURITY_FIREWALL] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[SECURITY_FIREWALL] FK preload failed', err);
            });
        }
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'work_name' && displayVal !== '-'){
                        ...
                        const aid = (row.id ?? row.asset_id ?? '');
                        const href = DETAIL_URL;
                        cellValue = `<a href="${href}" class="work-name-link" data-id="${row.id??''}" data-asset-id="${escapeHTML(String(aid??''))}">${cellValue}</a>`;
                    }
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    if(row){
                        let assetId = null;
                        sessionStorage.setItem('SECURITY_FIREWALL:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('SECURITY_FIREWALL:selected:system_name', String(row.system_name||''));
                        try{ sessionStorage.setItem('firewall:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('firewall:selected:row', JSON.stringify(row)); }catch(_e){}
                        // 보안: URL에 work/system/asset_id를 넣지 않고, storage로 컨텍스트 전달
                        try{
                            assetId = (row.id != null ? row.id : row.asset_id);
                            if(assetId != null){
                                try{ sessionStorage.setItem('firewall:selected:asset_id', String(assetId)); }catch(_e0){}
                                try{ localStorage.setItem('firewall:selected:asset_id', String(assetId)); }catch(_e1){}
                            }
                        }catch(_e2){}

                        // If the user is opening in a new tab/window (Ctrl/Meta/middle click),
                        // keep the URL clean but let the browser handle navigation.
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_FIREWALL_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_FIREWALL_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
