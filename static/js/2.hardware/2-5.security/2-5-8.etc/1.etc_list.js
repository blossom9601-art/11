...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__ETC_DETAIL_URL) || '/p/hw_security_etc_detail';
    ...
    const VISIBLE_COLS_KEY = 'security_etc_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/security/etc/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'SEC-ETC';
    const SERVER_MODEL_FORM_FACTOR_FILTER = 'ETC';
    ...
    const ENABLE_SECURITY_ETC_DUPLICATE = false;
    const ENABLE_SECURITY_ETC_BULK_EDIT = false;
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            console.warn('[SECURITY_ETC] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[SECURITY_ETC] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[SECURITY_ETC] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[SECURITY_ETC] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[SECURITY_ETC] FK preload failed', err);
            });
        }
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    if(row){
                        sessionStorage.setItem('SECURITY_ETC:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('SECURITY_ETC:selected:system_name', String(row.system_name||''));
                        try{
                            const assetId = (row.id != null ? row.id : row.asset_id);
                            if(assetId != null){
                                try{ sessionStorage.setItem('security-etc:selected:asset_id', String(assetId)); }catch(_e0){}
                                try{ localStorage.setItem('security-etc:selected:asset_id', String(assetId)); }catch(_e1){}
                            }
                        }catch(_e2){}
                        try{ sessionStorage.setItem('security-etc:selected:row', JSON.stringify(row)); }catch(_e){}
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_ETC_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_ETC_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
