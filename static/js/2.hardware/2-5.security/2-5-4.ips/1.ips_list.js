...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__IPS_DETAIL_URL) || '/p/hw_security_ips_detail';

    function buildDetailUrl(assetId){
        if(assetId != null && String(assetId).trim() !== ''){
            return DETAIL_URL + '?asset_id=' + encodeURIComponent(String(assetId));
        }
        return DETAIL_URL;
    }
    ...
    const VISIBLE_COLS_KEY = 'security_ips_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/security/ips/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'SEC-IPS';
    const SERVER_MODEL_FORM_FACTOR_FILTER = 'IPS';
    ...
    const ENABLE_SECURITY_IPS_DUPLICATE = false;
    const ENABLE_SECURITY_IPS_BULK_EDIT = false;
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            console.warn('[SECURITY_IPS] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[SECURITY_IPS] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[SECURITY_IPS] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[SECURITY_IPS] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[SECURITY_IPS] FK preload failed', err);
            });
        }
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'work_name' && displayVal !== '-'){
                        // Link to detail page (include asset_id for robustness)
                        const aid = (row.id != null ? row.id : row.asset_id);
                        cellValue = `<a href="${buildDetailUrl(aid)}" class="work-name-link" data-id="${row.id??row.asset_id??''}">${cellValue}</a>`;
                    }
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    const row = state.data.find(r=> String(r.id ?? r.asset_id ?? '') === String(rid));
                    if(row){
                        sessionStorage.setItem('SECURITY_IPS:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('SECURITY_IPS:selected:system_name', String(row.system_name||''));
                        // detail.js expects selected row under a short prefix key (ips:selected:row)
                        try{ sessionStorage.setItem('ips:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('ips:selected:row', JSON.stringify(row)); }catch(_e){}
                        const assetId = (row.id != null ? row.id : row.asset_id);
                        if(assetId != null){
                            try{ sessionStorage.setItem('ips:selected:asset_id', String(assetId)); }catch(_e0){}
                ...
                // Even if we couldn't resolve the full row, still navigate with asset_id
                try{
                    const rid2 = workLink.getAttribute('data-id') || workLink.closest('tr')?.getAttribute('data-id');
                    if(rid2){
                        e.preventDefault();
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_IPS_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_SECURITY_IPS_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
