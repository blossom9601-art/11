...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__HW_ONPREM_DETAIL_URL) || '/p/hw_server_onpremise_detail';
    ...
    const DEFAULT_ASSET_PREFIX = 'SRV-ONP';
    ...
    const ENABLE_ONPREM_DUPLICATE = false;
    const ENABLE_ONPREM_BULK_EDIT = false;
    ...
    function getAllowedManufacturerCodesForPage(){
        ...
        // If there are no server models yet, do not over-filter vendor options.
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            console.warn('[onpremise] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    async function refreshModelSelect(modelSelect, vendorCode){
        ...
        // Keep a stable default placeholder so a temporary disabled placeholder
        // (e.g. "제조사를 먼저 선택") doesn't become the permanent placeholder.
        if(!modelSelect.dataset.basePlaceholder){
            modelSelect.dataset.basePlaceholder = String(spec.placeholder || '선택');
        }
        const placeholder = String(modelSelect.dataset.basePlaceholder || spec.placeholder || '선택');
        ...
        if(!normalizedVendor){
            ...
            modelSelect.setAttribute('data-placeholder', '제조사를 먼저 선택');
            modelSelect.innerHTML = `<option value="">${escapeHTML('제조사를 먼저 선택')}</option>`;
            modelSelect.value = '';
    ...
    function setupModelDependenciesForForm(formId){
        ...
        getModelDependencyPairs().forEach(({ modelField, vendorField }) => {
            ...
            if(!vendorSelect.dataset.modelDependencyBound){
                vendorSelect.dataset.prevVendorCode = String(vendorSelect.value || '').trim();
                vendorSelect.addEventListener('change', ()=>{
                    const prev = String(vendorSelect.dataset.prevVendorCode || '').trim();
                    const next = String(vendorSelect.value || '').trim();
                    // Requirement: clearing vendor must also clear model.
                    if(prev && !next){
                        modelSelect.value = '';
                        modelSelect.removeAttribute('data-initial-value');
                    }
                    vendorSelect.dataset.prevVendorCode = next;
                    refreshModelSelect(modelSelect, next);
                });
    ...
    function getLocationCascadePairs(){
        // Currently only supports ORG_CENTER -> ORG_RACK cascading.
        // Keep it local to this page for minimal impact.
        return [{ parentField: 'location_place', childField: 'location_pos' }];
    }

    function extractCenterCodeFromRackRecord(item){
        // API payloads are expected to have center_code; keep fallbacks to be resilient.
    ...
    function buildRackOptionsMarkupForCenter(centerCode, selectedValue, placeholderLabel){
        ...
        if(!normalizedCenter){
            // Center not selected: keep rack disabled.
    ...
    function refreshRackSelectForCenter(rackSelect, centerCode, options){
        ...
        // Initial value should not persist after we have rehydrated.
    ...
    function setupLocationCascadeForForm(formId){
        ...
        getLocationCascadePairs().forEach(({ parentField, childField }) => {
            ...
            // Initialize once to remove unrelated racks and set disabled state.
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[onpremise] Failed to load user profiles for', cacheKey, err);
    ...
    async function refreshOwnerSelect(selectEl, deptCode, parentSelect){
        ...
        if(!normalizedCode){
            ...
            selectEl.innerHTML = `<option value="">${escapeHTML('부서를 먼저 선택')}</option>`;
            selectEl.disabled = true;
            selectEl.classList.add('fk-disabled');
            selectEl.setAttribute('data-placeholder', '부서를 먼저 선택');
            selectEl.value = '';
            selectEl.removeAttribute('data-initial-value');
            syncSearchableSelect(selectEl);
            return;
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[onpremise] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[onpremise] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[onpremise] FK preload failed', err);
            });
        }
    ...
    function buildAssetPayload(formData, existingRow){
        ...
        const CLEARABLE_FIELDS = new Set(['vendor','model','location_place','location_pos','sys_dept','sys_owner','svc_dept','svc_owner']);
    ...
    async function loadAssetsFromServer(options){
        ...
        try{
            ...
            console.error(err);
            showMessage(err.message || '물리서버 자산 목록을 불러오지 못했습니다.', '조회 실패');
        }finally{
        ...
        slice.forEach((row)=>{
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    // '업무 이름'(work_name) 컬럼: 값이 있을 때만 링크로 변환 (클라우드 페이지 패턴과 동일)
    ...
    function generateFieldInput(col,value=''){
        ...
        if(selectConfig[col]){
            ...
            const optionsMarkup = cfg.options.map(opt=>{
                ...
                const selectedAttr = optionValue === String(value??'') ? ' selected' : '';
                return `<option value="${optionValue}"${selectedAttr}>${optionLabel}</option>`;
            }).join('');
    ...
    function bindEvents(){
        ...
        function navigateToDetailFromRow(row, e){
            if(!row) return false;
            try{
                sessionStorage.setItem('onpremise:selected:work_name', String(row.work_name||''));
                sessionStorage.setItem('onpremise:selected:system_name', String(row.system_name||''));
                try{
                    const assetId = (row.id != null ? row.id : (row.asset_id != null ? row.asset_id : row.assetId));
                    if(assetId != null && String(assetId).trim() !== ''){
                        sessionStorage.setItem('onpremise:selected:asset_id', String(assetId));
                        try{ localStorage.setItem('onpremise:selected:asset_id', String(assetId)); }catch(_e0){}
                        try{ localStorage.setItem('onpremise:last_selected_asset_id', String(assetId)); }catch(_e1){}
                    }
                }catch(_e2){}
                try{ sessionStorage.setItem('onpremise:selected:row', JSON.stringify(row)); }catch(_e){}
                try{ localStorage.setItem('onpremise:selected:row', JSON.stringify(row)); }catch(_e){}
            }catch(_e){}
            // Avoid leaking descriptive values via URL querystring.
            if(e && typeof e.preventDefault === 'function') e.preventDefault();
            try{
                var _aid = (row.id != null ? row.id : (row.asset_id != null ? row.asset_id : ''));
                var _qs = _aid ? ('?asset_id=' + encodeURIComponent(String(_aid))) : '';
                window.location.href = DETAIL_URL + _qs;
            }catch(_eNav){
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            // 체크박스 클릭은 선택 로직(change)으로만 처리
            if(e.target.classList.contains('system-row-select') || e.target.closest('.system-row-select')) return;
            if(e.target.closest('.system-actions')) return; // 관리 버튼 영역 제외

            // 그 외: 행 클릭 시 선택 토글 (상세 이동은 관리 버튼에서만)
    ...
    document.getElementById(COLUMN_APPLY_ID)?.addEventListener('click', handleColumnFormApply);
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                console.error(err);
                showMessage(err.message || '물리서버 자산 등록 중 오류가 발생했습니다.', '등록 실패');
            }finally{
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                if(virtVal && _virtTabMap[virtVal] && _virtTabMap[virtVal] !== '/p/hw_server_onpremise'){
                ...
                console.error(err);
                showMessage(err.message || '물리서버 자산 수정 중 오류가 발생했습니다.', '수정 실패');
            }finally{
                if(btn){ btn.disabled = false; }
            }
        });
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_ONPREM_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
