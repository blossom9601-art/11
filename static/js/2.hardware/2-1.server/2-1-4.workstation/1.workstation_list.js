...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__HW_WORKSTATION_DETAIL_URL) || '/p/hw_server_workstation_detail';
    ...
    const VISIBLE_COLS_KEY = 'workstation_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/workstation/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'SRV-WKS';
    ...
    const ENABLE_WORKSTATION_DUPLICATE = false;
    const ENABLE_WORKSTATION_BULK_EDIT = false;
    const SERVER_MODEL_FORM_FACTOR_FILTER = '워크스테이션';
    ...
    async function loadFkSource(sourceKey, options){
        ...
        try{
            ...
            console.warn('[workstation] FK source load failed:', sourceKey, err);
            if(!forceRefresh || !fkSourceCache.has(sourceKey)){
                fkSourceCache.set(sourceKey, []);
            }
            return fkSourceCache.get(sourceKey) || [];
    ...
    function buildModelOptionsMarkupForVendor(vendorCode, selectedValue, placeholderLabel){
        ...
        if(!normalizedVendor){
            let html = `<option value="">${escapeHTML('제조사를 먼저 선택')}</option>`;
            if(current){
    ...
    async function refreshModelSelect(modelSelect, vendorCode){
        ...
        const shouldDisable = !normalizedVendor;
        modelSelect.disabled = shouldDisable;
        modelSelect.classList.toggle('fk-disabled', shouldDisable);
        if(shouldDisable){
            modelSelect.value = '';
        }
    ...
    async function loadOrgRacksScopedToPlace(normalizedCenter, locationPlaceSelect){
        ...
        try{
            ...
            console.warn('[workstation] ORG_RACK(center_match) load failed:', err);
            fkSourceCache.set('ORG_RACK', []);
            return [];
        }
    }
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[workstation] Failed to load user profiles for', cacheKey, err);
    ...
    async function refreshOwnerSelect(selectEl, deptCode, parentSelect){
        ...
        // When code is empty, parentLabel is usually a placeholder (e.g. "부서 선택").
        // Treat that as empty so the owner select stays disabled.
        ...
        let currentValue = selectEl.value || initialValue;
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[workstation] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[workstation] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[workstation] FK preload failed', err);
            });
        }
    ...
    function buildAssetPayload(formData, existingRow){
        ...
        const CLEARABLE_FIELDS = new Set(['sys_dept','svc_dept','sys_owner','svc_owner','location_place','location_pos','vendor','model']);
        Object.entries(FIELD_TO_PAYLOAD_KEY).forEach(([field, payloadKey])=>{
            const raw = formData[field];
            if(raw == null) return;
            if(raw === ''){
                if(CLEARABLE_FIELDS.has(field)) payload[payloadKey] = null;
                return;
            }
            let value = raw;
        ...
        if(formData.sys_owner && String(formData.sys_owner).trim() !== ''){
            const val = formData.sys_owner_display;
            if(val != null && String(val).trim() !== '') payload.system_owner_display = String(val).trim();
        } else {
            payload.system_owner_display = null;
        }
        if(formData.svc_owner && String(formData.svc_owner).trim() !== ''){
            const val = formData.svc_owner_display;
            if(val != null && String(val).trim() !== '') payload.service_owner_display = String(val).trim();
        } else {
            payload.service_owner_display = null;
        }

        // Cascade clear (onpremise behavior): clearing dept must also clear the owner.
        const sysDeptRaw = (formData.sys_dept == null) ? null : String(formData.sys_dept).trim();
        if(sysDeptRaw === ''){
            payload.system_department = null;
            payload.system_owner = null;
            payload.system_owner_display = null;
        }
        const svcDeptRaw = (formData.svc_dept == null) ? null : String(formData.svc_dept).trim();
        if(svcDeptRaw === ''){
        ...
        const centerRaw = (formData.location_place == null) ? null : String(formData.location_place).trim();
        if(centerRaw === ''){
        ...
        const vendorRaw = (formData.vendor == null) ? null : String(formData.vendor).trim();
        if(vendorRaw === ''){
            payload.vendor = null;
            payload.model = null;
        }
    ...
    async function loadAssetsFromServer(options){
        ...
        try{
            ...
            console.error(err);
            showMessage(err.message || '가상서버 자산 목록을 불러오지 못했습니다.', '조회 실패');
        }finally{
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'work_name' && displayVal !== '-'){
                        // Link to workstation detail page when clicking the work name
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    if(row){
                        sessionStorage.setItem('workstation:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('workstation:selected:system_name', String(row.system_name||''));
                        try{ sessionStorage.setItem('workstation:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('workstation:selected:row', JSON.stringify(row)); }catch(_e){}
    ...
    document.getElementById(COLUMN_APPLY_ID)?.addEventListener('click', handleColumnFormApply);
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                console.error(err);
                showMessage(err.message || '가상서버 자산 등록 중 오류가 발생했습니다.', '등록 실패');
            }finally{
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                if(virtVal && _virtTabMap[virtVal] && _virtTabMap[virtVal] !== '/p/hw_server_workstation'){
                ...
                console.error(err);
                showMessage(err.message || '가상서버 자산 수정 중 오류가 발생했습니다.', '수정 실패');
            }finally{
                if(btn){ btn.disabled = false; }
            }
        });
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_WORKSTATION_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_WORKSTATION_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
