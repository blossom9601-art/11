...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__HW_FRAME_DETAIL_URL) || '/p/hw_server_frame_detail';
    ...
    const VISIBLE_COLS_KEY = 'frame_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/frame/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'SRV-FRM';
    ...
    const ENABLE_FRAME_DUPLICATE = false;
    const ENABLE_FRAME_BULK_EDIT = false;
    const SERVER_MODEL_FORM_FACTOR_FILTER = '프레임';
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            if(sourceKey === 'SERVER_MODEL' && SERVER_MODEL_FORM_FACTOR_FILTER){
                ...
                const normalize = (raw)=>{
                    const v = String(raw == null ? '' : raw).replace(/\s+/g,'').toUpperCase();
                    if(!v) return '';
                ...
                items = items.filter(it => normalize(it?.form_factor || it?.hw_type || '') === targetNorm);
            }
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[frame] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[frame] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[frame] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[frame] FK preload failed', err);
            });
        }
    ...
    async function loadAssetsFromServer(options){
        ...
        try{
            ...
            console.error(err);
            showMessage(err.message || '프레임 자산 목록을 불러오지 못했습니다.', '조회 실패');
        }finally{
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    if(row){
                        sessionStorage.setItem('frame:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('frame:selected:system_name', String(row.system_name||''));
                        try{ sessionStorage.setItem('frame:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('frame:selected:row', JSON.stringify(row)); }catch(_e){}
    ...
    document.getElementById(COLUMN_APPLY_ID)?.addEventListener('click', handleColumnFormApply);
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                console.error(err);
                showMessage(err.message || '프레임 자산 등록 중 오류가 발생했습니다.', '등록 실패');
            }finally{
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                if(virtVal && _virtTabMap[virtVal] && _virtTabMap[virtVal] !== '/p/hw_server_frame'){
                ...
                console.error(err);
                showMessage(err.message || '프레임 자산 수정 중 오류가 발생했습니다.', '수정 실패');
            }finally{
                if(btn){ btn.disabled = false; }
            }
        });
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_FRAME_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
