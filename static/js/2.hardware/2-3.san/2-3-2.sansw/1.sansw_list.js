...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__SAN_SWITCH_DETAIL_URL) || '/p/hw_san_switch_detail';
    ...
    const VISIBLE_COLS_KEY = 'san_switch_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/san/switch/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'SAN-SW';
    // Used to scope model/vendor choices to hw_san_type rows for this page.
    // Accepts tolerant matching (e.g. SWITCH <-> SW).
    const SERVER_MODEL_FORM_FACTOR_FILTER = 'SWITCH';
    ...
    const ENABLE_SAN_SWITCH_DUPLICATE = false;
    const ENABLE_SAN_SWITCH_BULK_EDIT = false;
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            console.warn('[san_switch] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[san_switch] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[san_switch] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[san_switch] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[san_switch] FK preload failed', err);
            });
        }
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'work_name' && displayVal !== '-'){
                        // Link to SAN Switch detail page when clicking the work name
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    if(row){
                        sessionStorage.setItem('san_switch:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('san_switch:selected:system_name', String(row.system_name||''));
                        try{
                        ...
                        try{ sessionStorage.setItem('sansw:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('sansw:selected:row', JSON.stringify(row)); }catch(_e){}
                        e.preventDefault();
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_SAN_SWITCH_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_SAN_SWITCH_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
