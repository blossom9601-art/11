...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__SAN_DIRECTOR_DETAIL_URL) || '/p/hw_san_director_detail';
    ...
    const VISIBLE_COLS_KEY = 'san_director_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/san/director/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'SAN-DIR';
    // Used to scope model/vendor choices to hw_san_type rows for this page.
    // Accepts tolerant matching (e.g. DIRECTOR <-> DIR).
    const SERVER_MODEL_FORM_FACTOR_FILTER = 'DIRECTOR';
    ...
    const ENABLE_SAN_DIRECTOR_DUPLICATE = false;
    const ENABLE_SAN_DIRECTOR_BULK_EDIT = false;
    ...
    const FK_SOURCE_CONFIG = {
        ...
        // For SAN pages, model list must be constrained to values recorded in hw_san_type.
        // Value is san_code (stable identifier), label is model_name.
    ...
    function matchesSanType(candidate, target){
        ...
        const normKo = (s)=> String(s ?? '').trim().replace(/\s+/g, '');

        const c = norm(candidateRaw);
        const t = norm(targetRaw);
        const cKo = normKo(candidateRaw);
        const tKo = normKo(targetRaw);

        const toCanon = (upperCompact, koCompact)=>{
            if(!upperCompact && !koCompact) return '';
            if(
                upperCompact.includes('DIRECTOR') ||
                upperCompact === 'DIR' ||
                upperCompact.includes('SANDIRECTOR') ||
                (koCompact && koCompact.includes('디렉터'))
            ){
                return 'DIRECTOR';
            }
            if(
                ...
                (koCompact && koCompact.includes('스위치'))
            ){
                return 'SWITCH';
            }
            return '';
        ...
        const cCanon = toCanon(c, cKo);
        const tCanon = toCanon(t, tKo);
        if(cCanon && tCanon) return cCanon === tCanon;

        // Fallback fuzzy matching
        if(c.includes(t) || t.includes(c)) return true;
        if(cKo && tKo && (cKo.includes(tKo) || tKo.includes(cKo))) return true;
        return false;
    }
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            let items = rawItems;
            // Scope SAN model list to this page's san_type.
            if(sourceKey === 'SERVER_MODEL' && SERVER_MODEL_FORM_FACTOR_FILTER){
                const target = String(SERVER_MODEL_FORM_FACTOR_FILTER).trim();
                if(target){
                    items = rawItems.filter(item => matchesSanType(item?.san_type || item?.sanType, target));
                }
            }
            ...
            console.warn('[san_director] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    function getFkOptions(field){
        ...
        const records = fkSourceCache.get(spec.source) || [];
        ...
        // Restrict vendor choices to manufacturers that exist in hw_san_type.
        // (Vendor label still comes from vendor-manufacturers for readability.)
        let allowedVendorCodes = null;
        if(field === 'vendor'){
            const hasSanModelCache = fkSourceCache.has('SERVER_MODEL');
            const modelRows = fkSourceCache.get('SERVER_MODEL') || [];
            const codes = new Set();
            (Array.isArray(modelRows) ? modelRows : []).forEach(row => {
                const code = String(row?.manufacturer_code || '').trim();
                if(code) codes.add(code);
            });
            // If hw_san_type is loaded (even if empty), enforce strict filtering.
            if(hasSanModelCache){
                allowedVendorCodes = codes;
            }
        }
        const options = [];
        const seen = new Set();
        records.forEach(item => {
            ...
            if(allowedVendorCodes !== null && !allowedVendorCodes.has(value)) return;
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[san_director] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[san_director] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[san_director] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[san_director] FK preload failed', err);
            });
        }
    ...
    function normalizeAssetRecord(item){
        if(!item) return null;
        const inferredId = (item.id != null ? item.id : (item.asset_id != null ? item.asset_id : item.assetId));
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'work_name' && displayVal !== '-'){
                        // Link to SAN Director detail page when clicking the work name
    ...
    function collectForm(form){
        ...
        form.querySelectorAll('input,select,textarea').forEach(el=>{
            ...
            // Pages often use '-' as a display placeholder; never send it back to the API.
            data[el.name] = (raw === '-') ? '' : raw;
        });
    ...
    function generateFieldInput(col,value=''){
        ...
        if(['slot','u_size'].includes(col)){
            ...
            const safe = (numeric === '-' || numeric === '' || !/^\d+$/.test(numeric)) ? '' : numeric;
            return `<input name="${col}" type="number" min="0" step="1" class="form-input" value="${safe}">`;
        }
    ...
    function enforceVirtualizationDash(form){
        ...
        const makeDash = (el)=>{
            ...
            if(el.tagName === 'SELECT'){
                ...
                // Avoid setting '-' to numeric inputs (it triggers browser parse warnings).
                if(String(el.getAttribute('type') || '').toLowerCase() === 'number'){
                    el.value = '';
                } else {
                    el.value = '-';
                }
            ...
            if(workLink){
                try{
                    ...
                    const row = state.data.find(r=>{
                        const n = parseInt((r && (r.id != null ? r.id : r.asset_id)), 10);
                        return !isNaN(n) && n === rid;
                    });
                    if(row){
                        sessionStorage.setItem('san_director:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('san_director:selected:system_name', String(row.system_name||''));
                        try{
                            ...
                            if(assetId != null){
                                ...
                                // Also store under module-scoped alias (future-proof, avoids collisions).
                                try{ sessionStorage.setItem('san_director:selected:asset_id', String(assetId)); }catch(_e0a){}
                                try{ localStorage.setItem('san_director:selected:asset_id', String(assetId)); }catch(_e1a){}
                            }
                        }catch(_e2){}
                        try{ sessionStorage.setItem('director:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('director:selected:row', JSON.stringify(row)); }catch(_e){}

                        // Also store under module-scoped alias.
                        try{ sessionStorage.setItem('san_director:selected:row', JSON.stringify(row)); }catch(_e3){}
                        try{ localStorage.setItem('san_director:selected:row', JSON.stringify(row)); }catch(_e4){}
                        e.preventDefault();
    ...
    document.getElementById(COLUMN_APPLY_ID)?.addEventListener('click', handleColumnFormApply);
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                console.error(err);
                showMessage(err.message || 'SAN 디렉터 자산 수정 중 오류가 발생했습니다.', '수정 실패');
            }finally{
                if(btn){ btn.disabled = false; }
            }
        });
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_SAN_DIRECTOR_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_SAN_DIRECTOR_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
