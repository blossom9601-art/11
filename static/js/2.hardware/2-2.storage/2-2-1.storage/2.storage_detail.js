...
(function(){
  ...
  work_status: text('.basic-info-card:nth-child(1) .info-row:nth-child(1) .status-pill .status-text'),
      ...
      function fieldInput(col, value){
        ...
        function cleanNumberValue(v){
          ...
          // Keep digits only for number inputs; prevents browser warnings.
          var d = s.replace(/\D/g,'');
          return d;
        }
        ...
        if(col==='security_score') return '<input name="security_score" class="form-input" type="number" readonly placeholder="자동 합계" value="'+cleanNumberValue(value)+'">';
        ...
        if(['slot','u_size'].indexOf(col)>-1) return '<input name="'+col+'" type="number" min="0" step="1" class="form-input" value="'+cleanNumberValue(value)+'">';
  ...
  function setText(sel, val){ var el=document.querySelector(sel); if(el) el.textContent = String(val||''); }
    ...
    function saveBasicInfoToDB(){
      ...
      // Resolve asset_id
      var assetId = null;
      function getStoredAssetIdAny(){
        try{
          var keys = [
            'storage_san:selected:asset_id',
            'san_storage:selected:asset_id',
            'san:selected:asset_id'
          ];
          for(var i=0;i<keys.length;i++){
            var v = null;
            try{ v = sessionStorage.getItem(keys[i]) || localStorage.getItem(keys[i]); }catch(_e0){ v = null; }
            var n = parseInt(v, 10);
      ...
      // Persist for subsequent saves / reloads.
      try{ sessionStorage.setItem('storage_san:selected:asset_id', String(assetId)); }catch(_eP1){}
      try{ localStorage.setItem('storage_san:selected:asset_id', String(assetId)); }catch(_eP2){}
      ...
      return fetch('/api/hardware/storage/assets/' + encodeURIComponent(String(assetId)), {
        ...
        .then(function(r){
          ...
          // Update the Basic Info cards from the form values immediately
          // (covers 담당자/점검/시스템 IP 등 전체 영역).
          try{ updatePageFromForm(); }catch(_eUI){}
          return r.data.item;
        });
    }
