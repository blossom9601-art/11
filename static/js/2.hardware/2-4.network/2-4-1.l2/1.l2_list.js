...
(function(){
    // Detail page route (provided via HTML attribute to avoid inline scripts)
    const DETAIL_URL = (function(){
        try {
            const main = document.querySelector('main.main-content');
            const url = main && main.getAttribute('data-detail-url');
            if(url) return url;
        } catch(_e) {}
        return (typeof window !== 'undefined' && window.__L2_DETAIL_URL) || '/p/hw_network_l2_detail';
    })();
    ...
    const VISIBLE_COLS_KEY = 'network_l2_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/network/l2/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'NET-L2';
    // L2/L3 page: models/vendors must include both L2 and L3 network types
    const SERVER_MODEL_FORM_FACTOR_FILTER = ['L2', 'L3'];
    ...
    const ENABLE_NETWORK_L2_DUPLICATE = false;
    const ENABLE_NETWORK_L2_BULK_EDIT = false;
    ...
    function normalizeNetworkType(value){
        ...
        if(compact.includes('l3')) return 'L3';
        if(compact.includes('l4')) return 'L4';
        if(compact.includes('l7')) return 'L7';
        if(compact === 'ap' || compact === 'wifi' || compact === 'wireless' || lowered.includes('무선')) return 'AP';
        if(compact === 'cir' || compact === 'circuit' || compact === 'dedicateline' || lowered.includes('회선') || lowered.includes('전용')) return 'CIR';
        return raw.toUpperCase();
    }

    function isNetworkModelForPage(item){
        const type = normalizeNetworkType(item?.network_type || item?.hw_type || item?.type);
        const filter = SERVER_MODEL_FORM_FACTOR_FILTER;
        if(Array.isArray(filter)){
            const allowed = new Set(filter.map(v => String(v || '').trim()).filter(Boolean));
            return !!type && allowed.has(String(type).trim());
        }
        return type && type === String(filter || '').trim();
    }
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            console.warn('[network_l2] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[network_l2] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[network_l2] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[network_l2] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[network_l2] FK preload failed', err);
            });
        }
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'work_name' && displayVal !== '-'){
                        // Link to L2/L3 detail page when clicking the work name
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    if(row){
                        sessionStorage.setItem('network_l2:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('network_l2:selected:system_name', String(row.system_name||''));
                        try{ localStorage.setItem('network_l2:selected:work_name', String(row.work_name||'')); }catch(_e0){}
                        try{ localStorage.setItem('network_l2:selected:system_name', String(row.system_name||'')); }catch(_e1){}
                        // detail.js expects selected row under a short prefix key (l2:selected:row)
                        try{ sessionStorage.setItem('l2:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('l2:selected:row', JSON.stringify(row)); }catch(_e){}
                        // 보안: URL에 work/system/asset_id를 넣지 않고, storage로 컨텍스트 전달
                        try{
                            const assetId = (row.id != null ? row.id : row.asset_id);
                            if(assetId != null){
                                try{ sessionStorage.setItem('l2:selected:asset_id', String(assetId)); }catch(_e0){}
                                try{ localStorage.setItem('l2:selected:asset_id', String(assetId)); }catch(_e1){}
                            }
                        }catch(_e2){}
                        e.preventDefault();
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_NETWORK_L2_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_NETWORK_L2_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
