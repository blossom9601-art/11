...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__DEDICATED_DETAIL_URL) || '/p/hw_network_dedicateline_detail';
    ...
    const VISIBLE_COLS_KEY = 'network_circuit_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/network/circuit/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'NET-CIR';
    // Network type filter for this page: dedicated line / circuit devices.
    const SERVER_MODEL_FORM_FACTOR_FILTER = '회선장비';
    ...
    const ENABLE_NETWORK_CIRCUIT_DUPLICATE = false;
    const ENABLE_NETWORK_CIRCUIT_BULK_EDIT = false;
    ...
    function normalizeNetworkType(value){
        ...
        // Wireless
        if(compact === 'ap' || compact === 'wifi' || compact === 'wireless' || lowered.includes('무선')) return '무선장비';
    ...
    function looksLikeMojibakeCircuitType(rawType){
        // Some environments have legacy rows where Korean text was stored with a broken encoding,
        // and the API returns mojibake strings (U+0238 + U+FFFD x5 instead of "회선장비").
        // This page is the only one that targets "회선장비", so keep this heuristic page-scoped.
        const s = String(rawType || '').trim();
        if(!s) return false;
        const replacementChar = String.fromCharCode(0xfffd);
        const mojibakeCircuitType = String.fromCharCode(0x0238) + replacementChar + replacementChar + replacementChar + replacementChar + replacementChar;
        if(s === mojibakeCircuitType) return true;
        // Heuristic: replacement chars and common mojibake glyphs seen in cp949/utf8 mismatches.
        const hasReplacement = s.indexOf(replacementChar) !== -1;
        const hasMojibakeGlyph = /[ȸ¼±ÀºÑñ]/.test(s);
        return hasReplacement && hasMojibakeGlyph;
    }
    ...
    function isNetworkModelForPage(item){
        ...
        // Back-compat: accept common circuit synonyms when targeting '회선장비'.
        if(target === '회선장비'){
            if(rawType && (rawType.includes('회선') || rawType.includes('전용'))) return true;
            if(looksLikeMojibakeCircuitType(rawType)) return true;

            // Extra fallback: sometimes type is missing/garbled but code/name still hints circuit usage.
            const code = String(item?.network_code || item?.networkCode || '').trim().toLowerCase();
            const name = String(item?.model_name || item?.modelName || '').trim().toLowerCase();
            if(code.includes('cir') || code.includes('circuit') || code.includes('dedicated') || code.includes('leased')) return true;
            if(name.includes('circuit') || name.includes('dedicated') || name.includes('leased') || name.includes('회선') || name.includes('전용')) return true;
        }
    ...
    async function loadFkSource(sourceKey, options){
        ...
        try{
            ...
            console.warn('[network_circuit] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    function getFkOptions(field){
        ...
        if(field === 'vendor'){
            // Align with AP(무선장비) page behavior:
            // vendor dropdown should be derived from the *page-filtered* models,
            // not from vendor-manufacturers (which can be incomplete/mismatched).
            // Optionally enrich labels from vendor-manufacturers if available.
            const modelRows = fkSourceCache.get('SERVER_MODEL') || [];
            const filteredModelCodes = new Set();
            (Array.isArray(modelRows) ? modelRows : []).forEach(model => {
                if(!isNetworkModelForPage(model)) return;
                const code = String(model?.manufacturer_code || '').trim();
                if(code){
                    filteredModelCodes.add(code);
                }
            });

            // If models are not loaded yet or filter yields nothing,
            // keep the vendor-manufacturers list as a safe fallback.
            // (Otherwise the dropdown can appear permanently empty.)
            if(filteredModelCodes.size === 0){
                // no-op: keep `records` as vendor-manufacturers
            } else {
            ...
            filteredModelCodes.forEach(code => {
    ...
    function renderFkSelect(field, value){
        ...
        // Page-scoped: prevent global ui/fk_select.js from overriding vendor/model options
        // on this dedicated line(circuit) page.
        if(field === 'vendor' || field === 'model'){
            attrs.push('data-fk-ignore="1"');
        }
        if(selectedValue){
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[network_circuit] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[network_circuit] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[network_circuit] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[network_circuit] FK preload failed', err);
            });
        }
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    if(row){
                        sessionStorage.setItem('network_circuit:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('network_circuit:selected:system_name', String(row.system_name||''));
                        // detail.js expects selected row under a short prefix key (dedicateline:selected:row)
                        try{ sessionStorage.setItem('dedicateline:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('dedicateline:selected:row', JSON.stringify(row)); }catch(_e){}
                        e.preventDefault();
                        const assetId = (row.id != null ? row.id : row.asset_id);
                        if(assetId != null){
                            try{ sessionStorage.setItem('dedicateline:selected:asset_id', String(assetId)); }catch(_e0){}
                            try{ localStorage.setItem('dedicateline:selected:asset_id', String(assetId)); }catch(_e1){}
                        }
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_NETWORK_CIRCUIT_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_NETWORK_CIRCUIT_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
