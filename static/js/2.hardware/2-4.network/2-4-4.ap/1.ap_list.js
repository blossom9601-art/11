...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__AP_DETAIL_URL) || '/p/hw_network_ap_detail';
    ...
    const VISIBLE_COLS_KEY = 'network_ap_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/network/ap/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'NET-AP';
    // Network type filter for this page: ONLY wireless devices.
    // (Category table uses '무선장비'/'회선장비' labels.)
    const SERVER_MODEL_FORM_FACTOR_FILTER = '무선장비';
    ...
    const ENABLE_NETWORK_AP_DUPLICATE = false;
    const ENABLE_NETWORK_AP_BULK_EDIT = false;
    ...
    function normalizeNetworkType(value){
        ...
        if(compact.includes('l2')) return 'L2';
        if(compact.includes('l4')) return 'L4';
        if(compact.includes('l7')) return 'L7';
        // Wireless
        // Accept common labels like 'AP장비', 'AP-장비', 'wireless-ap' etc.
        if(
            compact === 'ap' || compact.startsWith('ap') ||
            compact === 'wifi' || compact.includes('wifi') ||
            compact === 'wireless' || compact.includes('wireless') ||
            lowered.includes('무선')
        ) return '무선장비';
        // Dedicated line / circuit
        if(compact === 'cir' || compact === 'circuit' || compact === 'dedicateline' || lowered.includes('회선') || lowered.includes('전용')) return '회선장비';
        return raw.toUpperCase();
    }

    function isNetworkModelForPage(item){
        const rawType = String(
            item?.network_type ||
            item?.hw_network_type ||
            item?.networkType ||
            item?.hw_type ||
            item?.type ||
            item?.form_factor ||
        ...
        const normalized = normalizeNetworkType(rawType);
        const target = String(SERVER_MODEL_FORM_FACTOR_FILTER || '').trim();
        if(!target) return true;
        if(normalized === target) return true;

        // Back-compat: accept common wireless synonyms when targeting '무선장비'.
        if(target === '무선장비' && rawType.includes('무선')) return true;
    ...
    const fkSourceErrorCache = new Map();
    const fkSourceLoadedAt = new Map();
    ...
    async function loadFkSource(sourceKey){
        ...
        if(!config){
            ...
            fkSourceErrorCache.set(sourceKey, 'FK source config missing');
            fkSourceLoadedAt.set(sourceKey, Date.now());
            return [];
        }
        try{
            ...
            fkSourceErrorCache.delete(sourceKey);
            fkSourceLoadedAt.set(sourceKey, Date.now());
            ...
            console.warn('[network_ap] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            fkSourceErrorCache.set(sourceKey, String(err?.message || err || 'unknown error'));
            fkSourceLoadedAt.set(sourceKey, Date.now());
            try{
                window.__networkApFkDiag = window.__networkApFkDiag || {};
                window.__networkApFkDiag.lastError = {
                    sourceKey,
    ...
    // Note: FK diagnostics popup removed by request.
    ...
    function getAllowedManufacturerCodesForPage(){
        ...
        const models = fkSourceCache.get('SERVER_MODEL') || [];
        const allowed = new Set();
        models.forEach(model => {
            if(!isNetworkModelForPage(model)) return;
            const code = String(model?.manufacturer_code || '').trim();
            if(code){
                allowed.add(code);
            }
        });
    ...
    function getFkOptions(field){
        ...
        if(field === 'vendor'){
            // Vendor list for this page must come from hw_network_type (wireless-only), not from vendor-manufacturers.
            // We optionally enrich labels using vendor-manufacturers if available.
            const modelRows = fkSourceCache.get('SERVER_MODEL') || [];
            const vendorRows = fkSourceCache.get('VENDOR') || records || [];
            const vendorNameByCode = new Map();
            (Array.isArray(vendorRows) ? vendorRows : []).forEach(v => {
                const code = String(v?.manufacturer_code || '').trim();
                if(!code) return;
                const name = String(v?.manufacturer_name || v?.vendor || '').trim();
                if(name){
                    vendorNameByCode.set(code, name);
                }
            });
            const derived = new Map();
            (Array.isArray(modelRows) ? modelRows : []).forEach(model => {
                if(!isNetworkModelForPage(model)) return;
                const code = String(model?.manufacturer_code || '').trim();
                if(!code) return;
                const label = vendorNameByCode.get(code) || code;
                derived.set(code, label);
            });
            records = Array.from(derived.entries()).map(([manufacturer_code, manufacturer_name]) => ({
                manufacturer_code,
                manufacturer_name,
    ...
    function ensureSearchableDataset(select){
        ...
        const field = (select.dataset && select.dataset.fk)
            ? select.dataset.fk
            : (select.getAttribute('data-fk') || select.name || '');
    ...
    function buildSearchPanelOptions(select, placeholder){
        ...
        if(!nativeOptions.length){
            // Safety: never render an empty panel.
            options.push({
                value: '',
                label: placeholder || '선택',
                searchLabel: String(placeholder || '선택').toLowerCase(),
                valueLower: '',
                searchLabelCompact: String(placeholder || '선택').toLowerCase().replace(/\s+/g, ''),
                valueLowerCompact: ''
            });
            return options;
        }
        ...
        nativeOptions.forEach(opt => {
            ...
            options.push({
                value,
                label,
                searchLabel: labelLower,
                valueLower,
                searchLabelCompact: labelLower.replace(/\s+/g, ''),
                valueLowerCompact: valueLower.replace(/\s+/g, '')
            });
        });
        return options;
    }

    function maybeHydrateSelectBeforeOpen(select){
        if(!select) return;
        const optionCount = (select.options && typeof select.options.length === 'number') ? select.options.length : 0;
        // Only intervene when the select is effectively empty (prevents unnecessary work).
        if(optionCount > 1){
            return;
        }
        const field = select.getAttribute('data-fk') || select.dataset?.fk || select.name || '';
        if(!field){
            return;
        }
        const spec = FK_FIELD_SPECS[field] || null;
        if(!spec){
            return;
        }
        // Skip dynamic owner selects; they depend on department selection.
        if(spec.skipAutoOptions){
            return;
        }
        // Model select depends on vendor; refresh with current vendor value.
        if(spec.source === 'SERVER_MODEL'){
            const vendorField = spec.dependsOn || select.getAttribute('data-parent-field') || 'vendor';
            const vendorSelect = select.form?.querySelector?.(`[name="${vendorField}"]`) || null;
            refreshModelSelect(select, vendorSelect?.value || '');
            return;
        }
        populateFkSelectOptions(select, field);
    }
    ...
    function openSearchDropdown(select){
        ...
        // If FK data isn't ready yet, defer opening to avoid rendering an empty panel.
        try{
            const fkField = select.getAttribute('data-fk') || select.dataset?.fk || '';
            const spec = fkField ? (FK_FIELD_SPECS[fkField] || null) : null;
            const optionCount = (select.options && typeof select.options.length === 'number') ? select.options.length : 0;
            const needsFk = !!spec;
            const needsServerModel = (fkField === 'vendor' || (spec && spec.source === 'SERVER_MODEL'));
            const serverModelReady = fkSourceCache.has('SERVER_MODEL');
            if(needsFk && needsServerModel && optionCount <= 1 && !serverModelReady){
                const meta0 = searchableSelectMeta.get(select);
                if(meta0 && meta0.displayBtn && meta0.displayBtn.dataset && meta0.displayBtn.dataset.fkPendingOpen !== '1'){
                    meta0.displayBtn.dataset.fkPendingOpen = '1';
                    const prevText = meta0.displayBtn.textContent;
                    meta0.displayBtn.textContent = '불러오는 중...';
                    meta0.displayBtn.disabled = true;
                    const p = ensureFkDataReady();
                    if(p && typeof p.then === 'function'){
                        p.then(()=>{
                            meta0.displayBtn.disabled = false;
                            meta0.displayBtn.dataset.fkPendingOpen = '0';
                            syncSearchableSelect(select);
                            // Ensure options exist before opening.
                            try{ maybeHydrateSelectBeforeOpen(select); }catch(_e){}
                            openSearchDropdown(select);
                        }).catch(()=>{
                            meta0.displayBtn.disabled = false;
                            meta0.displayBtn.dataset.fkPendingOpen = '0';
                            meta0.displayBtn.textContent = prevText || meta0.displayBtn.textContent;
                            syncSearchableSelect(select);
                        });
                        return;
                    }
        ...
        // Self-heal: if options are missing (common with async FK hydration), rebuild just-in-time.
        try{ maybeHydrateSelectBeforeOpen(select); }catch(_e){}
    ...
    function filterSearchPanelOptions(state){
        ...
        if(!term){
            ...
            state.filtered = state.options.filter(opt => {
                if(opt.searchLabel.includes(term) || opt.valueLower.includes(term)) return true;
                if(!termCompact) return false;
                return (opt.searchLabelCompact || '').includes(termCompact) || (opt.valueLowerCompact || '').includes(termCompact);
            });
        }
    ...
    async function refreshModelSelect(modelSelect, vendorCode){
        ...
        if(normalizedVendor && currentValue){
            ...
            const stillValid = (Array.isArray(records) ? records : []).some(item => {
                if(!isNetworkModelForPage(item)) return false;
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[network_ap] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[network_ap] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[network_ap] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[network_ap] FK preload failed', err);
            });
        }
    ...
    async function loadAssetsFromServer(options){
        ...
        try{
            ...
            console.error(err);
            showMessage(err.message || '온프레미스 자산 목록을 불러오지 못했습니다.', '조회 실패');
        }finally{
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'work_name' && displayVal !== '-'){
                        // Link to AP detail page when clicking the work name
        ...
        tbodyEl?.addEventListener('click', async e=>{
            ...
            if(workLink){
                try{
                    ...
                    if(row){
                        sessionStorage.setItem('network_ap:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('network_ap:selected:system_name', String(row.system_name||''));
                        try{ sessionStorage.setItem('ap:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('ap:selected:row', JSON.stringify(row)); }catch(_e){}
                        e.preventDefault();
                        const assetId = (row.id != null ? row.id : row.asset_id);
                        if(assetId != null){
                            try{ sessionStorage.setItem('ap:selected:asset_id', String(assetId)); }catch(_e0){}
    ...
    document.getElementById(COLUMN_APPLY_ID)?.addEventListener('click', handleColumnFormApply);
        ...
        document.getElementById(ADD_BTN_ID)?.addEventListener('click', async ()=> {
            ...
            // Populate after FK preload so dropdowns are not empty on first open.
            refreshAddFormFkFields();
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                console.error(err);
                showMessage(err.message || '온프레미스 자산 등록 중 오류가 발생했습니다.', '등록 실패');
            }finally{
                if(btn){ btn.disabled = false; }
            }
        });
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                console.error(err);
                showMessage(err.message || '온프레미스 자산 수정 중 오류가 발생했습니다.', '수정 실패');
            }finally{
                if(btn){ btn.disabled = false; }
            }
        });
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_NETWORK_AP_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_NETWORK_AP_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
