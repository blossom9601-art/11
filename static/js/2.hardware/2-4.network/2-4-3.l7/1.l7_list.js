...
(function(){
    ...
    const DETAIL_URL = (typeof window !== 'undefined' && window.__L7_DETAIL_URL) || '/p/hw_network_l7_detail';
    ...
    const VISIBLE_COLS_KEY = 'network_l7_visible_cols';
    ...
    const API_ENDPOINT = '/api/hardware/network/l7/assets';
    const API_PAGE_SIZE = 1000;
    const DEFAULT_ASSET_PREFIX = 'NET-L7';
    const SERVER_MODEL_FORM_FACTOR_FILTER = 'L7';
    ...
    const ENABLE_NETWORK_L7_DUPLICATE = false;
    const ENABLE_NETWORK_L7_BULK_EDIT = false;
    ...
    function isNetworkModelForPage(item){
        ...
        return type && type === String(SERVER_MODEL_FORM_FACTOR_FILTER || '').trim();
    }
    ...
    async function loadFkSource(sourceKey){
        ...
        try{
            ...
            console.warn('[network_l7] FK source load failed:', sourceKey, err);
            fkSourceCache.set(sourceKey, []);
            return [];
        }
    }
    ...
    async function fetchUserProfilesByDepartment(deptName, deptCode){
        ...
        try{
            ...
            console.warn('[network_l7] Failed to load user profiles for', cacheKey, err);
        ...
        try{
            ...
            if(canFallback){
                try{
                    ...
                    console.warn('[network_l7] owner fallback failed', fallbackErr);
                }
            }
            ...
            selectEl.removeAttribute('data-owner-fallback-tried');
            syncSearchableSelect(selectEl);
            console.warn('[network_l7] 사용자 선택 갱신 실패', err);
        }
    }
    ...
    function ensureFkDataReady(){
        if(!fkDataPromise){
            fkDataPromise = preloadFkData().then(()=>{
                ...
                console.warn('[network_l7] FK preload failed', err);
            });
        }
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'work_name' && displayVal !== '-'){
                        // Link to L7 detail page when clicking the work name
                        const assetId = (row.id != null ? row.id : row.asset_id);
                        cellValue = `<a href="${buildDetailUrl(assetId)}" class="work-name-link" data-id="${row.id??''}">${cellValue}</a>`;
                    }
        ...
        tbodyEl?.addEventListener('click', e=>{
            ...
            if(workLink){
                try{
                    ...
                    if(row){
                        sessionStorage.setItem('network_l7:selected:work_name', String(row.work_name||''));
                        sessionStorage.setItem('network_l7:selected:system_name', String(row.system_name||''));
                        try{ sessionStorage.setItem('l7:selected:row', JSON.stringify(row)); }catch(_e){}
                        try{ localStorage.setItem('l7:selected:row', JSON.stringify(row)); }catch(_e){}
                        e.preventDefault();
                        const assetId = (row.id != null ? row.id : row.asset_id);
                        if(assetId != null){
                            try{ sessionStorage.setItem('l7:selected:asset_id', String(assetId)); }catch(_e0){}
                            try{ localStorage.setItem('l7:selected:asset_id', String(assetId)); }catch(_e1){}
                        }
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            if(!ENABLE_NETWORK_L7_DUPLICATE){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', ()=>{
            if(!ENABLE_NETWORK_L7_BULK_EDIT){ showMessage('일괄변경은 서버 연동 준비 중입니다.', '준비 중'); return; }
