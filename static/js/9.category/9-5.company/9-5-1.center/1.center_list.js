...
function centerListMain(){
    ...
    const UPLOAD_HEADERS_KO = [
        '센터명','위치','용도','내진설계','RACK(수량)','하드웨어(수량)','소프트웨어(수량)','회선(수량)','비고'
    ];
    ...
    // Data + State
    // Center page requirement: no column-selection UI; hide 'note' on the table.
    // 'note' remains available in add/edit modals only.
    const BASE_VISIBLE_COLUMNS = [
        'center_name','location','usage','seismic','rack_qty','hw_qty','sw_qty','line_qty'
    ];
    const COLUMN_ORDER = [
        'center_name','location','usage','seismic','rack_qty','hw_qty','sw_qty','line_qty','note'
    ];
    ...
    /** 컬럼 메타 (라벨 + 그룹) */
    const COLUMN_META = {
        center_name:{label:'센터명',group:'센터'},
        location:{label:'위치',group:'센터'},
        usage:{label:'용도',group:'센터'},
    seismic:{label:'내진설계',group:'센터'},
        rack_qty:{label:'RACK(수량)',group:'센터'},
        hw_qty:{label:'하드웨어(수량)',group:'센터'},
        sw_qty:{label:'소프트웨어(수량)',group:'센터'},
        line_qty:{label:'회선(수량)',group:'센터'},
        note:{label:'비고',group:'센터'}
    };
    ...
    const API_ENDPOINT = '/api/org-centers';
    let isListLoading = false;

    function setListLoading(active){
        isListLoading = !!active;
        ...
        const tableWrap = document.querySelector('.system-table-container');
        if(tableWrap){
    ...
    async function apiRequest(url, options){
        ...
        } catch(_e){ /* response without JSON */ }
    ...
    function normalizeRow(row){
        if(!row) return null;
        const seismicValue = normalizeFloat(row.seismic ?? row.seismic_rating);
        return {
            id: row.id,
            ...
            usage: row.usage || '',
            seismic: seismicValue,
            seismic_rating: seismicValue,
            rack_qty: normalizeNumber(row.rack_qty),
            hw_qty: normalizeNumber(row.hw_qty),
            sw_qty: normalizeNumber(row.sw_qty),
            line_qty: normalizeNumber(row.line_qty),
            note: row.note || '',
    ...
    async function refreshData(showSpinner=true){
        ...
        let loaded = false;
        try{
            const payload = await apiRequest(API_ENDPOINT, { method: 'GET' });
            const items = Array.isArray(payload?.items) ? payload.items.map(normalizeRow).filter(Boolean) : [];
            state.data = items;
            state.selected.clear();
            loaded = true;
        } catch(err){
            console.error('Failed to load centers', err);
            showMessage(err.message || '센터 목록을 불러오는 중 오류가 발생했습니다.', '오류');
        } finally {
            if(showSpinner) setListLoading(false);
        }
        // Render only after loading flag is cleared to avoid early-return in render().
        ...
        if(state.filtered.length === 0){
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '센터 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 센터를 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
    ...
    function loadColumnSelection(){
        // Column selection feature removed for Center page; always show all columns.
        state.visibleCols = new Set(BASE_VISIBLE_COLUMNS);
    ...
    function generateFieldInput(col,value=''){
        if(col==='seismic'){
            ...
            return `<input name="seismic" type="number" min="0" step="0.1" class="form-input" value="${val}" placeholder="예: 6.5">`;
        }
    ...
    async function addRow(data){
        ...
        const item = normalizeRow(payload?.item);
        if(item){
    ...
    async function updateRow(rowId, data){
        ...
        const payload = await apiRequest(`${API_ENDPOINT}/${rowId}`, {
            method: 'PUT',
            body: JSON.stringify(data || {}),
        });
        const item = normalizeRow(payload?.item);
        if(item){
            state.data = state.data.map(row => row.id === rowId ? item : row);
            applyFilter();
        }
        return item;
    }
    ...
    async function deleteRows(rowIds){
        ...
        const ids = rowIds.filter(id => typeof id === 'number' && !Number.isNaN(id));
        if(!ids.length) return 0;
        await apiRequest(`${API_ENDPOINT}/bulk-delete`, {
            method: 'POST',
    ...
    const filename = `center_list_${yyyy}${mm}${dd}.csv`;
    ...
    tbodyEl?.addEventListener('click', e=>{
            ...
            if(btn){
                ...
                if(action==='edit'){
                    ...
                    if(editSaveEl){ editSaveEl.setAttribute('data-row-id', String(row.id)); }
        ...
        // column selection feature removed for Center page
    // add modal
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> {
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            if(typeof data.seismic !== 'undefined'){
                const sv = parseFloat(data.seismic||'');
                data.seismic = Number.isFinite(sv) && sv >= 0 ? sv : '';
            }
            ['rack_qty','hw_qty','sw_qty','line_qty'].forEach(k=>{
                if(typeof data[k] !== 'undefined'){
                    const v = parseInt(data[k]||'0',10);
            ...
            try {
                ...
                showMessage('센터가 등록되었습니다.', '완료');
            } catch(err){
                console.error('Failed to create center', err);
                showMessage(err.message || '센터 등록 중 오류가 발생했습니다.', '오류');
            } finally {
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            const rowId = parseInt(saveBtn?.getAttribute('data-row-id')||'-1',10);
            if(!rowId || Number.isNaN(rowId)){
                showMessage('수정할 센터를 찾을 수 없습니다.', '오류');
                return;
            }
            const data = collectForm(form);
            if(typeof data.seismic !== 'undefined'){
            ...
            try{
                ...
                showMessage('센터가 수정되었습니다.', '완료');
            } catch(err){
                console.error('Failed to update center', err);
                showMessage(err.message || '센터 수정 중 오류가 발생했습니다.', '오류');
            } finally {
                if(saveBtn) saveBtn.disabled = false;
            }
        });
        ...
        // upload feature removed for Center page
        // stats open
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', ()=>{
            ...
            const clones = originals.map(o=>{
                ...
                copy.center_name = copy.center_name ? (copy.center_name + '_COPY') : copy.center_name;
                return copy;
            });
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 센터를 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            if(!ids.length){ closeModal(DELETE_MODAL_ID); showMessage('삭제할 행이 선택되지 않았습니다.', '안내'); return; }
            const confirmBtn = document.getElementById(DELETE_CONFIRM_ID);
            if(confirmBtn) confirmBtn.disabled = true;
            try {
                const deleted = await deleteRows(ids);
                closeModal(DELETE_MODAL_ID);
                showMessage(`${deleted}개 센터가 삭제처리되었습니다.`, '완료');
            } catch(err){
                console.error('Failed to delete centers', err);
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 센터를 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(count===1){
                ...
                const row = state.data.find(r=> r.id === onlyId);
                if(!row){ showMessage('선택된 행을 찾을 수 없습니다.', '오류'); return; }
                fillEditForm(row);
                openModal(EDIT_MODAL_ID);
                const editSaveEl = document.getElementById(EDIT_SAVE_ID);
                if(editSaveEl){ editSaveEl.setAttribute('data-row-id', String(row.id)); }
            ...
            state.data = state.data.map(row=>{
                ...
                entries.forEach(({field, value})=>{
                    if(field === 'seismic'){
                        ...
                        updated[field] = Number.isFinite(sv) && sv >= 0 ? sv : '';
                    } else if(field === 'rack_qty' || field === 'hw_qty' || field === 'sw_qty' || field === 'line_qty'){
                        const iv = parseInt(value||'0',10);
                        updated[field] = Number.isFinite(iv) && iv >= 0 ? iv : 0;
                    } else {
                        updated[field] = value;
                    }
                });
    ...
    function buildBulkForm(){
        ...
        function inputFor(col){
            if(col === 'seismic'){
                return `<input type="number" min="0" step="0.1" class="form-input" data-bulk-field="seismic" placeholder="예: 6.5">`;
            }
            if(col === 'rack_qty' || col === 'hw_qty' || col === 'sw_qty' || col === 'line_qty') return `<input type="number" min="0" step="1" class="form-input qty-dashed-lock" data-bulk-field="${col}" placeholder="0">`;
    ...
    const checkEl = document.getElementById('stats-check');
        ...
        // 센터 통계
        // Bin seismic strengths by nearest 0.5 for a compact histogram
        const seismicDist = rows.reduce((acc, r)=>{
            const v = r.seismic;
            if(v === '' || v == null) return acc;
            const num = parseFloat(v);
            if(!Number.isFinite(num)) return acc;
            const binned = Math.round(num * 2) / 2; // nearest 0.5
            const key = binned.toFixed(1);
            acc[key] = (acc[key]||0)+1;
            return acc;
        }, {});
        renderStatBlock('stats-software', '내진설계(강도)', seismicDist);
        renderStatBlock('stats-versions', '용도', countBy(rows, 'usage'));
        renderStatBlock('stats-check', '지역', countBy(rows, 'location'));
    }
    ...
    function init(){
        ...
        initData();
        bindEvents();
        // render() will be triggered after data loads via applyFilter();
        // skipping the initial render prevents an empty-state flash.
