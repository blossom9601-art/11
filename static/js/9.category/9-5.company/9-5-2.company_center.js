// 회사 > 센터: dataset, table render, pagination, CSV (scoped to main content)
...
(function() {
	...
	function toCSV(rows) {
	const headers = ['센터 이름','주소','하드웨어','소프트웨어'];
	const body = rows.map(r => [r.name||'', r.address||'', r.hw??0, r.sw??0]);
