...
(function(){
    ...
    const HW_API_BASE = '/api/hw-security-types';
    ...
    function normalizeSecurityRecord(item){
        if(!item) return null;
        const vendorCode = item.manufacturer_code || '';
        const vendorName = vendorNameByCode[vendorCode] || vendorCode || '';
        const rawQty = (item.usage_count != null) ? item.usage_count : item.device_count;
    ...
    async function refreshSecurityData(options){
        ...
        try {
            const [vendorAllRes, vendorRes, securityRes] = await Promise.all([
            ...
            const mapped = (securityRes?.items || []).map(normalizeSecurityRecord).filter(Boolean);
            state.data = mapped;
            state.page = 1;
            ...
            console.error(err);
            state.loading = false;
            showMessage(err.message || '보안장비 목록을 불러오지 못했습니다.', '오류');
        }
    }

    function buildSecurityPayload(formData, opts){
        const existingRow = opts?.existingRow;
        const payload = {
            model_name: formData.model || '',
            security_type: formData.hw_type || '',
        ...
        if(!existingRow){
            ...
            payload.device_count = Number.isFinite(q) && q >= 0 ? q : 0;
    ...
    async function createSecurityRecord(formValues, opts){
        const payload = buildSecurityPayload(formValues, opts);
        const res = await callApi(HW_API_BASE, { method: 'POST', body: payload });
        const record = normalizeSecurityRecord(res?.item);
        if(record){
    ...
    async function updateSecurityRecord(recordId, formValues, opts){
        if(!recordId){
    ...
    async function deleteSecurityRecords(ids){
        ...
        const res = await callApi(`${HW_API_BASE}/bulk-delete`, { method: 'POST', body: { ids: numericIds } });
        ...
        if(state.loading){
            ...
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '보안장비 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 보안장비를 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
        ...
        slice.forEach((row)=>{
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + TABLE_COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'model'){
                        try{
                            var base = String(window.__HW_SECURITY_DETAIL_URL || '').trim();
                            if(base){
    ...
    const filename = `security_list_${yyyy}${mm}${dd}.csv`;
    ...
    function bindEvents(){
        ...
        if(!window.__blossom_hw_security_slash_focus_wired){
            window.__blossom_hw_security_slash_focus_wired = true;
    ...
    tbodyEl?.addEventListener('click', e=>{
            ...
            if(link){
                try {
                    ...
                    sessionStorage.setItem('security_selected_row', JSON.stringify(payload));
                } catch(_err){}
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> { openModal(ADD_MODAL_ID); initDatePickers(ADD_FORM_ID); });
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                await createSecurityRecord(data);
                form.reset();
                closeModal(ADD_MODAL_ID);
                showMessage('보안장비가 등록되었습니다.', '완료');
            } catch(err){
                console.error(err);
                showMessage(err.message || '보안장비 등록 중 오류가 발생했습니다.', '오류');
            }
        });
        // edit modal
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            const data = collectForm(form);
            try {
                await updateSecurityRecord(existingRow.id, data, { existingRow });
                closeModal(EDIT_MODAL_ID);
                showMessage('보안장비가 수정되었습니다.', '완료');
            } catch(err){
                console.error(err);
                showMessage(err.message || '보안장비 수정 중 오류가 발생했습니다.', '오류');
            }
        });
        // csv
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        wsTemplate['!cols'] = UPLOAD_HEADERS_KO.map((h)=>{
            ...
            const mid = ['제조사','유형'];
                ...
                XLSX.writeFile(wb, 'security_upload_template.xlsx');
                ...
                try{
                    ...
                    (async ()=>{
                        ...
                        for(const item of imported){
                            try {
                                await createSecurityRecord(item);
                                success += 1;
                            } catch(err){
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                const removed = await deleteSecurityRecords(ids);
                state.selected.clear();
                closeModal(DELETE_MODAL_ID);
                if(removed > 0){ setTimeout(()=> showMessage(`${removed}개 항목이 삭제되었습니다.`, '완료'), 0); }
            } catch(err){
                console.error(err);
                showMessage(err.message || '보안장비 삭제 중 오류가 발생했습니다.', '오류');
            }
        });
            ...
            try{
                ...
                for(const id of ids){
                    try{
                        ...
                        const updated = normalizeSecurityRecord(res?.item);
        ...
        // esc close
        // NOTE: Guarded so SPA swaps don't accumulate duplicate listeners.
        if(!window.__blossom_hw_security_esc_close_wired){
            window.__blossom_hw_security_esc_close_wired = true;
...
document.addEventListener('keydown', e=>{ if(e.key==='Escape'){ closeModal(STATS_MODAL_ID); }});
    ...
    function init(){
        // Guard: initialize only on the security list page instance.
        const pane = document.querySelector('#system-list-pane[data-page-key="cat_hw_security"]');
        if(!pane) return;
        if(pane.dataset.blossomInitHwSecurityList === '1') return;
        pane.dataset.blossomInitHwSecurityList = '1';
