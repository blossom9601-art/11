...
(function(){
    ...
    const HW_API_BASE = '/api/hw-network-types';
    ...
    function normalizeNetworkRecord(item){
        if(!item) return null;
        ...
        return {
            ...
            server_code: item.network_code,
            model: item.server_model_name || item.model_name || '',
            vendor: vendorName || '',
            vendor_code: vendorCode || '',
            hw_type: item.network_type || '',
            release_date: item.release_date || '',
            eosl: item.eosl_date || '',
            qty: (item.usage_count != null) ? (Number.isFinite(item.usage_count) ? item.usage_count : (parseInt(item.usage_count, 10) || 0)) : (Number.isFinite(item.device_count) ? item.device_count : (parseInt(item.device_count, 10) || 0)),
            note: item.remark || ''
        };
    }
    function syncNextId(){
    ...
    async function refreshNetworkData(options){
        ...
        try {
            const [vendorRes, networkRes] = await Promise.all([
            ...
            const mapped = (networkRes?.items || []).map(normalizeNetworkRecord).filter(Boolean);
            state.data = mapped;
            state.page = 1;
            ...
            state.loading = false;
            render();
            showMessage(err.message || '네트워크 목록을 불러오지 못했습니다.', '오류');
        }
    }
    function buildNetworkPayload(formData, opts){
        ...
        if(formData.network_code){ payload.network_code = formData.network_code; }
        ...
        if(state.loading){
            ...
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '네트워크 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 네트워크를 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
    ...
    function fillEditForm(row){
        ...
        form.innerHTML='';
    const group = { title:'네트워크', cols:['model','vendor','release_date','eosl','note'] };
    ...
    const filename = `network_list_${yyyy}${mm}${dd}.csv`;
    ...
    function bindEvents(){
        ...
        if(!window.__blossom_hw_network_slash_focus_wired){
            window.__blossom_hw_network_slash_focus_wired = true;
    ...
    tbodyEl?.addEventListener('click', e=>{
            ...
            if(link){
                try {
                    ...
                    sessionStorage.setItem('network_selected_row', JSON.stringify(payload));
                } catch(_err){}
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> { openModal(ADD_MODAL_ID); initDatePickers(ADD_FORM_ID); });
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                newRow = normalizeNetworkRecord(res?.item);
            }catch(err){
                console.error(err);
                showMessage(err.message || '네트워크 등록 중 오류가 발생했습니다.', '오류');
                return;
            }
            if(newRow){
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            const formData = collectForm(form);
            try{
                const payload = buildNetworkPayload(formData, { existingRow });
                const res = await callApi(`${HW_API_BASE}/${recordId}`, { method: 'PUT', body: payload });
                const updatedRow = normalizeNetworkRecord(res?.item);
                if(updatedRow){
                ...
                showMessage('네트워크가 수정되었습니다.', '완료');
            }catch(err){
                console.error(err);
                showMessage(err.message || '네트워크 수정 중 오류가 발생했습니다.', '오류');
            }
        });
        // csv
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        wsTemplate['!cols'] = UPLOAD_HEADERS_KO.map((h)=>{
            ...
            const mid = ['제조사','유형'];
                ...
                XLSX.writeFile(wb, 'network_upload_template.xlsx');
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 네트워크를 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 네트워크를 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                console.error(err);
                showMessage(err.message || '네트워크 삭제 중 오류가 발생했습니다.', '오류');
            }
        });
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 네트워크에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
            ...
            try{
                ...
                for(const id of ids){
                    try{
                        ...
                        const updated = normalizeNetworkRecord(res?.item);
        ...
        // esc close
        // NOTE: Guarded so SPA swaps don't accumulate duplicate listeners.
        if(!window.__blossom_hw_network_esc_close_wired){
            window.__blossom_hw_network_esc_close_wired = true;
    ...
    const GROUP = { title:'네트워크', cols:['model','vendor','eosl','note'] };
    ...
    function init(){
        // Guard: initialize only on the network list page instance.
        const pane = document.querySelector('#system-list-pane[data-page-key="cat_hw_network"]');
        if(!pane) return;
        if(pane.dataset.blossomInitHwNetworkList === '1') return;
        pane.dataset.blossomInitHwNetworkList = '1';
