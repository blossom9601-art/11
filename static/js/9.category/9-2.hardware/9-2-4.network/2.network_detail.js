...
(function(){
    ...
    function __detailInit(){
      ...
      function syncHeaderFromQueryOrSession(){
        try{
          ...
          try{
            var raw = sessionStorage.getItem('network_selected_row');
            if(!raw) return;
      ...
      (function(){
        try{
          var assignedKey = 'network:licAssignedSum';
      ...
      (function hydrateFromListSelection(){
        ...
        try{
          return;
          var raw = sessionStorage.getItem('network:selectedRow');
          if(!raw) return;
          var sel = null;
      ...
      function buildEditFormFromPage(){
        ...
        // Build Network category edit form identical to the Network list page
        var LABELS = {
  ...
  section.innerHTML = '<div class="section-header"><h4>네트워크</h4></div>';
  ...
  function recompute(){ if(!idleEl) return; var t=toInt(totalEl&&totalEl.value); var a=toInt(assignedEl&&assignedEl.value); var newVal = (t-a); idleEl.value=String(newVal); try{ updateIdleDot(newVal); }catch(_){ } }
      ...
      (function(){
        ...
        var HW_API_BASE = '/api/hw-network-types';
        ...
        function doSave(){
          ...
          .then(function(res){
            if(!res.ok || res.data.success === false){ showMessage(res.data.message || '네트워크 수정 중 오류가 발생했습니다.', '오류'); return; }
            ...
            try{
              ...
              try{ var sk='network_selected_row'; var sr=JSON.parse(sessionStorage.getItem(sk)||'{}');
        ...
        if(isAllocationSchema){
          ...
          function hwComputeAssignedSum(){
            try{
              ...
              try{ localStorage.setItem('network:licAssignedSum', String(sum)); }catch(_){ }
