...
(function(){
    ...
    function __detailInit(){
      ...
      function syncHeaderFromQueryOrSession(){
        try{
          ...
          try{
            var raw = sessionStorage.getItem('san_selected_row');
            if(!raw) return;
  ...
  function recompute(){ if(!idleEl) return; var t=toInt(totalEl&&totalEl.value); var a=toInt(assignedEl&&assignedEl.value); var newVal = (t-a); idleEl.value=String(newVal); try{ updateIdleDot(newVal); }catch(_){ } }
      ...
      (function(){
        ...
        var HW_API_BASE = '/api/hw-san-types';
        ...
        function doSave(){
          ...
          .then(function(res){
            if(!res.ok || res.data.success === false){ showMessage(res.data.message || 'SAN 수정 중 오류가 발생했습니다.', '오류'); return; }
            ...
            try{
              ...
              try{ var sk='san_selected_row'; var sr=JSON.parse(sessionStorage.getItem(sk)||'{}');
