...
(function(){
    ...
    const HW_API_BASE = '/api/hw-san-types';
    ...
    function normalizeSanRecord(item){
        if(!item) return null;
        ...
        return {
            ...
            hw_type: item.san_type || '',
            release_date: item.release_date || '',
            eosl: item.eosl_date || '',
            qty: (item.usage_count != null) ? (Number.isFinite(item.usage_count) ? item.usage_count : (parseInt(item.usage_count, 10) || 0)) : (Number.isFinite(item.san_count) ? item.san_count : (parseInt(item.san_count, 10) || 0)),
            note: item.remark || ''
        };
    }
    function syncNextId(){
    ...
    async function refreshSanData(options){
        ...
        try {
            const [vendorRes, sanRes] = await Promise.all([
            ...
            const mapped = (sanRes?.items || []).map(normalizeSanRecord).filter(Boolean);
            state.data = mapped;
            state.page = 1;
            ...
            state.loading = false;
            render();
            showMessage(err.message || 'SAN 목록을 불러오지 못했습니다.', '오류');
        }
    }
    function buildSanPayload(formData, opts){
        ...
        if(!existingRow){
            ...
            payload.san_count = Number.isFinite(q) && q >= 0 ? q : 0;
        ...
        if(formData.san_code){ payload.san_code = formData.san_code; }
            ...
            base = state.data.filter(row =>
                ...
                groups.every(alts => {
                    ...
                    return searchCols.some(col => {
                        ...
                        const cellCompact = normalizeForSearch(v);
                        return alts.some(tok => {
                            if(!tok) return false;
                            if(cell.includes(tok)) return true;
                            const tokCompact = normalizeForSearch(tok);
                            return tokCompact ? cellCompact.includes(tokCompact) : false;
                        });
                    });
                })
            );
        ...
        if(state.loading){
            ...
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = 'SAN 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 SAN을 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
    ...
    function fillEditForm(row){
        ...
        form.innerHTML='';
    const group = { title:'SAN', cols:['model','vendor','release_date','eosl','note'] };
    ...
    const filename = `san_list_${yyyy}${mm}${dd}.csv`;
    ...
    function bindEvents(){
        ...
        if(!window.__blossom_hw_san_slash_focus_wired){
            window.__blossom_hw_san_slash_focus_wired = true;
    ...
    tbodyEl?.addEventListener('click', e=>{
            ...
            if(link){
                ...
                if(row){ try{ sessionStorage.setItem('san_selected_row', JSON.stringify(row)); }catch(_s){} }
            }
        });
    tbodyEl?.addEventListener('click', e=>{
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> { openModal(ADD_MODAL_ID); initDatePickers(ADD_FORM_ID); });
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            let newRow = null;
            try{
                const payload = buildSanPayload(formData);
                const res = await callApi(HW_API_BASE, { method: 'POST', body: payload });
                newRow = normalizeSanRecord(res?.item);
            }catch(err){
                console.error(err);
                showMessage(err.message || 'SAN 등록 중 오류가 발생했습니다.', '오류');
                return;
            }
            if(newRow){
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            const formData = collectForm(form);
            try{
                const payload = buildSanPayload(formData, { existingRow });
                const res = await callApi(`${HW_API_BASE}/${recordId}`, { method: 'PUT', body: payload });
                const updatedRow = normalizeSanRecord(res?.item);
                if(updatedRow){
                ...
                showMessage('SAN이 수정되었습니다.', '완료');
            }catch(err){
                console.error(err);
                showMessage(err.message || 'SAN 수정 중 오류가 발생했습니다.', '오류');
            }
        });
        // csv
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        wsTemplate['!cols'] = UPLOAD_HEADERS_KO.map((h)=>{
            ...
            const mid = ['제조사','유형'];
                ...
                XLSX.writeFile(wb, 'san_upload_template.xlsx');
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 SAN을 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 SAN을 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                console.error(err);
                showMessage(err.message || 'SAN 삭제 중 오류가 발생했습니다.', '오류');
            }
        });
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(count===1){
                ...
                const realIndex = state.data.findIndex(r=> Number(r.id) === Number(onlyId));
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 SAN에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
            ...
            try{
                ...
                for(const id of ids){
                    try{
                        ...
                        const updated = normalizeSanRecord(res?.item);
        ...
        // esc close
        // NOTE: Guarded so SPA swaps don't accumulate duplicate listeners.
        if(!window.__blossom_hw_san_esc_close_wired){
            window.__blossom_hw_san_esc_close_wired = true;
    ...
    const GROUP = { title:'SAN', cols:['model','vendor','eosl','note'] };
    ...
    function init(){
        // Guard: initialize only on the SAN list page instance.
        const pane = document.querySelector('#system-list-pane[data-page-key="cat_hw_san"]');
        if(!pane) return;
        if(pane.dataset.blossomInitHwSanList === '1') return;
        pane.dataset.blossomInitHwSanList = '1';
