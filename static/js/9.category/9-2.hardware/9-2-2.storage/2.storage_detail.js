...
(function(){
    ...
    function __detailInit(){
      ...
      function syncHeaderFromQueryOrSession(){
        try{
          ...
          try{
            var raw = sessionStorage.getItem('storage_selected_row');
            if(!raw) return;
      ...
      function buildEditFormFromPage(){
        ...
        // Build Storage category edit form identical to the Storage list page
        var LABELS = {
  ...
  section.innerHTML = '<div class="section-header"><h4>스토리지</h4></div>';
  ...
  function recompute(){ if(!idleEl) return; var t=toInt(totalEl&&totalEl.value); var a=toInt(assignedEl&&assignedEl.value); var newVal = (t-a); idleEl.value=String(newVal); try{ updateIdleDot(newVal); }catch(_){ } }
      ...
      (function(){
        ...
        var HW_API_BASE = '/api/hw-storage-types';
        ...
        function doSave(){
          ...
          .then(function(res){
            if(!res.ok || res.data.success === false){ showMessage(res.data.message || '스토리지 수정 중 오류가 발생했습니다.', '오류'); return; }
            ...
            try{
              ...
              try{ var sk='storage_selected_row'; var sr=JSON.parse(sessionStorage.getItem(sk)||'{}');
      ...
      var initHardwareAllocationTable = function(){
    ...
    var isInventorySchema = !!table.querySelector('[data-col="qty"]') && !table.querySelector('[data-col="space"]');
      ...
      // [Tabs moved to /static/js/_detail/tab*.js]
        ...
        // Maintenance table (tab09) - optional on this page
        var mtTable = document.getElementById('mt-spec-table');
        function mtFormatNumberLocale(n){
          try{ return Number(n||0).toLocaleString('ko-KR'); }catch(_e){ return String(n||0); }
        }
        ...
        if(addBtn && mtTable){
          addBtn.addEventListener('click', function(){
            ...
            try{
              if(window.mtState && typeof window.mtPages==='function' && typeof window.mtRenderPage==='function'){
                window.mtState.page = window.mtPages();
                window.mtRenderPage();
              }
            }catch(_){ }
                try{ if(window.__blsInitDatePickers) window.__blsInitDatePickers(tr); }catch(_){}
            try{ if(typeof wireMtFormatters==='function') wireMtFormatters(tr); }catch(_){ }
          });
        }
  
        // Delegate actions
        if(mtTable){ mtTable.addEventListener('click', function(ev){
          ...
          if(target.classList.contains('js-mt-del')){
            ...
            try{ if(typeof updateEmptyState==='function') updateEmptyState(); }catch(_){ }
            return;
          }
          ...
          if(
            ...
            function toInput(name, placeholder){
              ...
              if(name==='amount'){
                ...
                var av = ad ? (mtFormatNumberLocale(ad)+'원') : '';
          ...
          if(
            ...
            (function(){ var v = read('amount'); var d = String(v||'').replace(/\D/g,''); commit('amount', d ? (mtFormatNumberLocale(d)+'원') : ''); })();
