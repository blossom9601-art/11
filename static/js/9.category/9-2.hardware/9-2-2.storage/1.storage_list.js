 ...
 * NOTE: hw_storage_type SQLite API와 동기화됨
 */
...
(function(){
    ...
    const HW_API_BASE = '/api/hw-storage-types';
    ...
    function buildVendorCodeByName(items){
        const map = {};
        (items||[]).forEach(item=>{
            const name = normalizeVendorKey(item?.manufacturer_name || '');
            const code = normalizeVendorKey(item?.manufacturer_code || '');
            if(!name || !code) return;
    ...
    function buildVendorOptionsHtml(selectedValue, opts){
        ...
        html += list.map(name=> `<option value="${escapeHTML(name)}" ${name===selected?'selected':''}>${escapeHTML(name)}</option>`).join('');
        return html;
    }
    ...
    function buildVendorMap(items){
        ...
        (items||[]).forEach(item=>{
            const code = normalizeVendorKey(item?.manufacturer_code);
            if(!code) return;
            map[code] = normalizeVendorKey(item.manufacturer_name || code);
        });
        return map;
    }

    function normalizeStorageRecord(item){
        if(!item) return null;
        const vendorCode = normalizeVendorKey(item.manufacturer_code || '');
        const vendorName = vendorNameByCode[vendorCode] || vendorCode;
        return {
            id: item.id,
            storage_code: item.storage_code,
            server_code: item.storage_code,
            model: item.server_model_name || item.model_name || '',
            vendor: vendorName || '',
            vendor_code: vendorCode || '',
            hw_type: item.storage_type || '',
            release_date: item.release_date || '',
            eosl: item.eosl_date || '',
            qty: (item.usage_count != null) ? (Number.isFinite(item.usage_count) ? item.usage_count : (parseInt(item.usage_count, 10) || 0)) : (Number.isFinite(item.storage_count) ? item.storage_count : (parseInt(item.storage_count, 10) || 0)),
            note: item.remark || ''
        };
    }
    ...
    async function refreshStorageData(options){
        state.loading = true;
        // Prevent empty-state flash during load
        try{ const emptyEl = document.getElementById('system-empty'); if(emptyEl) emptyEl.hidden = true; }catch(_e){}
        const includeDeleted = options?.includeDeleted ? '1' : '0';
        try {
            const [vendorAllRes, vendorRes, storageRes] = await Promise.all([
                callApi(`${VENDOR_MANUFACTURER_API}?include_deleted=1`),
                callApi(`${VENDOR_MANUFACTURER_API}?include_deleted=${includeDeleted}`),
                callApi(`${HW_API_BASE}?include_deleted=${includeDeleted}`)
            ]);
            vendorNameByCode = buildVendorMap(vendorAllRes?.items || []);
            vendorCodeByName = buildVendorCodeByName(vendorAllRes?.items || []);
            vendorNameOptions = buildVendorNameOptions(vendorRes?.items || []);
            syncVendorSelectsInModals();
            const mapped = (storageRes?.items || []).map(normalizeStorageRecord).filter(Boolean);
            state.data = mapped;
            state.page = 1;
            ...
            state.nextId = 1;
            showMessage(err.message || '스토리지 목록을 불러오지 못했습니다.', '오류');
            render();
        }
    }

    function buildStoragePayload(formData, opts){
        const existingRow = opts?.existingRow;
        const payload = {
            model_name: formData.model || '',
            storage_type: formData.hw_type || '',
            release_date: formData.release_date || '',
            eosl_date: formData.eosl || '',
            remark: formData.note || ''
        };
        // Qty is removed from modals. Do not overwrite counts on update.
        if(!existingRow){
            const q = parseInt(formData.qty || '0', 10);
            payload.storage_count = Number.isFinite(q) && q >= 0 ? q : 0;
        } else if(Object.prototype.hasOwnProperty.call(formData, 'qty')){
            const raw = String(formData.qty ?? '').trim();
            if(raw !== ''){
                const q = parseInt(raw, 10);
                payload.storage_count = Number.isFinite(q) && q >= 0 ? q : 0;
            }
        }
        if(formData.storage_code){ payload.storage_code = formData.storage_code; }
        const vendorInput = normalizeVendorKey(formData.vendor || '');
        if(existingRow && vendorInput && vendorInput === normalizeVendorKey(existingRow.vendor || '') && existingRow.vendor_code){
            payload.manufacturer_code = normalizeVendorKey(existingRow.vendor_code);
        } else if(vendorInput){
            // Always prefer manufacturer_code when we can resolve it.
            // - If the select value is already a code, send it.
            // - If the select value is a name, map name -> code and send it.
            if(vendorNameByCode && Object.prototype.hasOwnProperty.call(vendorNameByCode, vendorInput)){
                payload.manufacturer_code = vendorInput;
            } else if(vendorCodeByName && Object.prototype.hasOwnProperty.call(vendorCodeByName, vendorInput)){
                payload.manufacturer_code = normalizeVendorKey(vendorCodeByName[vendorInput]);
            } else {
                // Fallback: send name (backend will try to resolve).
    ...
    function render(highlightContext){
        ...
        if(state.loading){
            ...
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '스토리지 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 스토리지를 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
        ...
        slice.forEach((row)=>{
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + TABLE_COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'model'){
                        var base = (window.__HW_STORAGE_DETAIL_URL || '');
                        try{
        ...
        if(countEl){
            if(state.loading){
                // Avoid counter flicker during initial load / tab navigation.
                updatePagination();
    ...
    const filename = `storage_list_${yyyy}${mm}${dd}.csv`;
    ...
    function bindEvents(){
        ...
        if(!window.__blossom_hw_storage_slash_focus_wired){
            window.__blossom_hw_storage_slash_focus_wired = true;
    ...
    tbodyEl?.addEventListener('click', e=>{
            ...
            if(link){
                ...
                if(row){ try{ sessionStorage.setItem('storage_selected_row', JSON.stringify(row)); }catch(_s){} }
            }
        });
    tbodyEl?.addEventListener('click', e=>{
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> { openModal(ADD_MODAL_ID); initDatePickers(ADD_FORM_ID); });
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            let newRow = null;
            try{
                const payload = buildStoragePayload(formData);
                const res = await callApi(HW_API_BASE, { method: 'POST', body: payload });
                newRow = normalizeStorageRecord(res?.item);
            }catch(err){
                console.error(err);
                showMessage(err.message || '스토리지 등록 중 오류가 발생했습니다.', '오류');
                return;
            }
            if(newRow){
                if(newRow.vendor_code){
                    newRow.vendor = vendorNameByCode[newRow.vendor_code] || formData.vendor || newRow.vendor;
                }
                upsertRow(newRow);
            }
            form.reset();
            closeModal(ADD_MODAL_ID);
            showMessage('스토리지가 등록되었습니다.', '완료');
        });
        // edit modal
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            const formData = collectForm(form);
            try{
                const payload = buildStoragePayload(formData, { existingRow });
                const res = await callApi(`${HW_API_BASE}/${recordId}`, { method: 'PUT', body: payload });
                const updatedRow = normalizeStorageRecord(res?.item);
                if(updatedRow){
                    if(updatedRow.vendor_code){
                        updatedRow.vendor = vendorNameByCode[updatedRow.vendor_code] || formData.vendor || updatedRow.vendor;
                    }
                    upsertRow(updatedRow, { append:false });
                }
                closeModal(EDIT_MODAL_ID);
                showMessage('스토리지가 수정되었습니다.', '완료');
            }catch(err){
                console.error(err);
                showMessage(err.message || '스토리지 수정 중 오류가 발생했습니다.', '오류');
            }
        });
        // csv
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        wsTemplate['!cols'] = UPLOAD_HEADERS_KO.map((h)=>{
            ...
            const mid = ['제조사','유형'];
                ...
                XLSX.writeFile(wb, 'storage_upload_template.xlsx');
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 스토리지를 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 스토리지를 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                console.error(err);
                showMessage(err.message || '스토리지 삭제 중 오류가 발생했습니다.', '오류');
            }
        });
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 스토리지에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
            openModal(BULK_MODAL_ID);
            // init date pickers for bulk form
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', async ()=>{
            ...
            // Send as UI-field aliases; backend maps them (partial update)
            const payload = {};
            ...
            try{
                ...
                for(const id of ids){
                    try{
                        ...
                        const updated = normalizeStorageRecord(res?.item);
        ...
        // esc close
        // NOTE: Guarded so SPA swaps don't accumulate duplicate listeners.
        if(!window.__blossom_hw_storage_esc_close_wired){
            window.__blossom_hw_storage_esc_close_wired = true;
...
document.addEventListener('keydown', e=>{ if(e.key==='Escape'){ closeModal(STATS_MODAL_ID); }});
    ...
    const GROUP = { title:'스토리지', cols:['model','vendor','release_date','eosl'] };
    ...
    function init(){
        // Guard: initialize only on the storage list page instance.
        const pane = document.querySelector('#system-list-pane[data-page-key="cat_hw_storage"]');
        if(!pane) return;
        if(pane.dataset.blossomInitHwStorageList === '1') return;
        pane.dataset.blossomInitHwStorageList = '1';
        ...
        loadSortPreference();
        state.loading = true;
        // Prevent empty-state flash before first data arrives
        try{ const emptyEl = document.getElementById('system-empty'); if(emptyEl) emptyEl.hidden = true; }catch(_e){}
        refreshStorageData();
        bindEvents();
        // Do not call render() yet; refreshStorageData() will call applyFilter() -> render() when data is ready.
