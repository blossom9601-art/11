      ...
      (function hydrateFromListSelection(){
        try{
          ...
          try{
            ...
            if(hasAny){
              sel = tmp;
              try{ sessionStorage.setItem('hba_selected_row', JSON.stringify(sel)); }catch(_s){}
            }
          }catch(_q){ }
          ...
          if(!sel){
            var raw = null;
            try{ raw = sessionStorage.getItem('hba_selected_row'); }catch(_e){ raw = null; }
            if(raw){
          ...
          // Apply to HBA basic info (only exists on 기본정보 탭)
