...
(function(){
    ...
    function __detailInit(){
      ...
      (function hydrateFromListSelection(){
        try{
          ...
          try{
            ...
            if(hasAny){
              sel = tmp;
              try{ sessionStorage.setItem('memory_selected_row', JSON.stringify(sel)); }catch(_s){}
            }
          }catch(_q){ }
          ...
          if(!sel){
            var raw = null;
            try{ raw = sessionStorage.getItem('memory_selected_row'); }catch(_e){ raw = null; }
            if(raw){
          ...
          // Apply to MEMORY basic info (only exists on 기본정보 탭)
          setText('os-model', sel.model);
          setText('os-spec', sel.spec);
      ...
      function buildEditFormFromPage(){
        ...
        var LABELS_COMP = { model:'모델명', spec:'용량', vendor:'제조사', part_no:'부품번호', qty:'수량', note:'비고' };
        ...
        ['model','spec','vendor','part_no','note'].forEach(function(c){
