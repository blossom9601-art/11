      ...
      (function hydrateFromListSelection(){
        try{
          ...
          try{
            ...
            if(hasAny){
              sel = tmp;
              try{ sessionStorage.setItem('nic_selected_row', JSON.stringify(sel)); }catch(_s){}
            }
          }catch(_q){ }
          ...
          if(!sel){
            var raw = null;
            try{ raw = sessionStorage.getItem('nic_selected_row'); }catch(_e){ raw = null; }
            if(raw){
          ...
          // Apply to NIC basic info (only exists on 기본정보 탭)
