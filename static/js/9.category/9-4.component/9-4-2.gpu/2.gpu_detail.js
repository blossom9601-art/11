      ...
      (function hydrateFromListSelection(){
        try{
          ...
          function setHeader(model, vendor){
            ...
            var t = (model == null) ? '' : String(model).trim();
            var s = (vendor == null) ? '' : String(vendor).trim();
          ...
          function fromQuery(){
            try{
              ...
              var fields = ['model','spec','vendor','part_no','note'];
              var out = {}; var has = false;
              fields.forEach(function(k){
                var v = p.get(k);
                if(v != null && String(v).trim() !== ''){ out[k] = String(v); has = true; }
              });
              return has ? out : null;
            }catch(_){ return null; }
          }
          ...
          var sel = fromQuery();
          if(sel){
            try{ sessionStorage.setItem('gpu_selected_row', JSON.stringify(sel)); }catch(_e){}
          }
          if(!sel){
            var raw = null;
            try{ raw = sessionStorage.getItem('gpu_selected_row'); }catch(_e){ raw = null; }
            if(raw){
          ...
          // Basic info tab fields (if present)
