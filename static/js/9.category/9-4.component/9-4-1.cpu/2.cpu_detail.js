...
(function(){
    ...
    function __detailInit(){
      ...
      function showNoDataImage(container, altText){
        try{
          ...
          container.innerHTML = '';
          // Use a span wrapper so it's valid even if container is a <p>
          ...
          // Preferred: Lottie JSON animation
          ...
          function renderLottie(){
            try{
              ...
              try{
                ...
                // Caption text under animation (supports two lines via \n)
          ...
          function loadLottieAndRender(){
            try{
              ...
              script.async = true;
              script.onload = function(){ if(!renderLottie()) renderImageFallback(); };
              script.onerror = function(){ renderImageFallback(); };
          ...
          function renderImageFallback(){
            try{
              ...
              var altMsg = altText || '데이터 없음';
              img.alt = altMsg.split('\n')[0];
              img.style.maxWidth = '240px';
              img.style.width = '100%';
              img.style.height = 'auto';
              var candidates = [
                // Preferred base: /blossom
                '/blossom/static/image/svg/free-animated-no-data/no-data.svg',
                '/blossom/static/image/svg/free-animated-no-data/animated.svg',
                '/blossom/static/image/svg/free-animated-no-data/animation.svg',
                '/blossom/static/image/svg/free-animated-no-data/index.svg',
                '/blossom/static/image/svg/free-animated-no-data.svg',
                '/blossom/static/image/svg/free-animated-no-data/no-data.gif',
                '/blossom/static/image/svg/free-animated-no-data.gif',
                // Fallback base: /static
                '/static/image/svg/free-animated-no-data/no-data.svg',
                '/static/image/svg/free-animated-no-data/animated.svg',
                '/static/image/svg/free-animated-no-data/animation.svg',
                '/static/image/svg/free-animated-no-data/index.svg',
                '/static/image/svg/free-animated-no-data.svg',
                '/static/image/svg/free-animated-no-data/no-data.gif',
                '/static/image/svg/free-animated-no-data.gif'
              ];
              var idx = 0;
              function setNext(){ if(idx >= candidates.length) return; img.src = candidates[idx++]; }
              img.onerror = function(){ setNext(); };
              setNext();
              wrap.appendChild(img);
              // Caption text under image (supports two lines via \n)
              var capWrap = document.createElement('span');
              capWrap.style.display = 'block';
              capWrap.style.marginTop = '8px';
              capWrap.style.textAlign = 'center';
          ...
          // Try Lottie first; if not available, load it, otherwise fallback to image.
          if(!renderLottie()){
            if(!window.lottie){ loadLottieAndRender(); }
            else { renderImageFallback(); }
          }
        }catch(_){ /* ignore */ }
      }
      ...
      function setOsQuantity(n){
        try{
          var el = document.getElementById('os-qty');
          if(!el) return;
          var v = (typeof n==='number' && isFinite(n) && n>=0) ? n : 0;
          // 천 단위 콤마 적용 (ko-KR 로케일)
      ...
      function syncHeaderFromOs(){
        try{
          ...
          if(titleEl && modelEl){ var t=(modelEl.textContent||'').trim(); if(t && t !== '-') titleEl.textContent=t; }
          if(subEl && vendorEl){ var s=(vendorEl.textContent||'').trim(); if(s && s !== '-') subEl.textContent=s; }
        }catch(_){ }
      }
      ...
      (function renderStatsFromApi(){
        try{
          ...
          fetch(apiUrl).then(function(r){ return r.json(); }).then(function(data){
            ...
            // Deduplicate by hardware_id for stats (one hardware = one count)
            var seenHw = {};
            var uniqueRows = [];
            rows.forEach(function(r){
              var hid = r.hardware_id;
              if(hid != null && seenHw[hid]) return;
              if(hid != null) seenHw[hid] = true;
              uniqueRows.push(r);
            });
            // Update qty (sum from all rows, not deduplicated)
            try{ var qtyEl = document.getElementById('os-qty'); if(qtyEl){ var totalQty = rows.reduce(function(s,r){ return s + (Number(r.qty) || 0); }, 0); qtyEl.textContent = totalQty.toLocaleString('ko-KR'); } }catch(_){}
            // ---- Status pie (qty-weighted) ----
            ...
            // ---- Operation pie (qty-weighted) ----
            var opMap = {};
            uniqueRows.forEach(function(r){ var q = Number(r.qty) || 1; var op = (r.work_operation||'').trim() || '-'; opMap[op] = (opMap[op]||0)+q; });
      ...
      (function hydrateFromListSelection(){
        try{
          ...
          function setHeader(model, vendor){
            try{
              var titleEl = document.getElementById('page-header-title') || document.querySelector('.page-header h1');
              var subEl = document.getElementById('page-header-subtitle') || document.querySelector('.page-header p');
              var t = (model||'').toString().trim();
              var s = (vendor||'').toString().trim();
              if(titleEl && t) titleEl.textContent = t;
              if(subEl && s) subEl.textContent = s;
            }catch(_){ }
          }

          // 1) Prefer query params (list page deep-link)
          var sel = null;
          try{
            var params = new URLSearchParams(window.location.search || '');
            var hasAny = false;
            var tmp = {};
            ['model','spec','vendor','part_no','note'].forEach(function(k){
              if(params.has(k)){
                tmp[k] = params.get(k);
                if((tmp[k]||'').toString().trim() !== '') hasAny = true;
              }
            });
            if(hasAny){
              sel = tmp;
              try{ sessionStorage.setItem('cpu_selected_row', JSON.stringify(sel)); }catch(_s){}
            }
          }catch(_q){ }

          // 2) Fallback to sessionStorage selection (when moving between tabs)
          if(!sel){
            var raw = null;
            try{ raw = sessionStorage.getItem('cpu_selected_row'); }catch(_e){ raw = null; }
            if(raw){
              try{ sel = JSON.parse(raw); }catch(_j){ sel = null; }
            }
          }
          if(!sel) return;

          // Apply to CPU basic info (only exists on 기본정보 탭)
          setText('os-model', sel.model);
          setText('os-spec', sel.spec ? sel.spec + ' Core' : '');
          setText('os-vendor', sel.vendor);
          setText('os-part-no', sel.part_no);
          setText('os-qty', sel.qty);
          setText('os-note', sel.note);

          // TPMC 참조 정보 (천 단위 콤마)
          var _rt = sel.reference_tpmc;
          setText('tpmc-ref-value', (_rt != null && _rt !== '') ? Number(_rt).toLocaleString('ko-KR') : '');

          // Apply to header (all tabs)
          setHeader(sel.model, sel.vendor);
        }catch(_){ /* ignore */ }
      })();
      ...
      function buildEditFormFromPage(){
        ...
        // Build CPU component edit form identical to the CPU list page edit modal
        var LABELS_COMP = { model:'모델명', spec:'용량', vendor:'제조사', part_no:'부품번호', qty:'수량', reference_tpmc:'기준 TPMC', note:'비고' };
        function generateComponentField(col, value){
          if(col==='part_no'){
          ...
          if(col==='reference_tpmc'){
            var _fmtV = (value != null && value !== '') ? Number(String(value).replace(/,/g,'')).toLocaleString('ko-KR') : '';
          ...
          if(col==='note'){
            return '<textarea name="note" class="form-input textarea-large" rows="3">'+(value||'')+'</textarea>';
          }
          if(col==='spec'){
            return '<input name="spec" type="number" min="0" step="1" class="form-input" value="'+(value||'').replace(/[^0-9]/g,'')+'" placeholder="용량 (Core)" data-fk-ignore="1">';
          }
          // default text input for model, vendor
        ...
        var current = {
          ...
          spec: getText('os-spec').replace(/[^0-9]/g,''),
          vendor: getText('os-vendor'),
          part_no: getText('os-part-no'),
          qty: getText('os-qty'),
          reference_tpmc: getText('tpmc-ref-value').replace(/,/g,''),
          note: getText('os-note')
        };
        form.innerHTML = '';
        var section=document.createElement('div'); section.className='form-section';
        section.innerHTML = '<div class="section-header"><h4>컴포넌트</h4></div>';
        var grid=document.createElement('div'); grid.className='form-grid';
        ['model','spec','vendor','part_no','reference_tpmc'].forEach(function(c){
          var wrap=document.createElement('div');
          wrap.className = 'form-row';
          wrap.innerHTML = '<label>'+LABELS_COMP[c]+'</label>'+generateComponentField(c, current[c]||'');
        ...
        noteRow.innerHTML = '<label>'+LABELS_COMP.note+'</label>'+generateComponentField('note', current.note||'');
        section.appendChild(noteRow);
        form.appendChild(section);
        // 실시간 콤마 포맷팅
        form.querySelectorAll('.js-comma-input').forEach(function(inp){
      ...
      function updatePageFromForm(){
        ...
        // If the form is the OS-style form (has hw_type/release/eosl), update all OS fields
        var hasHwFields = !!(form.querySelector('[name="hw_type"]') || form.querySelector('[name="release_date"]') || form.querySelector('[name="eosl"]'));
        // If the form is the CPU component form (spec/part_no present, no hw fields), update only shared fields
        var isCpuComponentForm = !!(form.querySelector('[name="spec"]') || form.querySelector('[name="part_no"]'));
        if(hasHwFields || (!isCpuComponentForm && form.querySelector('[name="model"]'))){
        ...
        if(isCpuComponentForm){
          function val2(name){ var el=form.querySelector('[name="'+name+'"]'); return el ? el.value : ''; }
          function setText2(id, v){ var el=document.getElementById(id); if(el){ el.textContent = String(v||''); } }
          // Update only fields that exist on the page for CPU detail
          setText2('os-model', val2('model'));
          var _specRaw = val2('spec');
          setText2('os-spec', _specRaw ? _specRaw + ' Core' : '');
          setText2('os-vendor', val2('vendor'));
          setText2('os-part-no', val2('part_no'));
          setText2('os-qty', val2('qty'));
          setText2('os-note', val2('note'));
          // TPMC 참조 필드 업데이트 (천 단위 콤마)
          var refTpmc = val2('reference_tpmc').replace(/,/g,'');
          setText2('tpmc-ref-value', (refTpmc != null && refTpmc !== '') ? Number(refTpmc).toLocaleString('ko-KR') : '');
          // Persist to API
          try{
            var rawSel = null;
            try{ rawSel = sessionStorage.getItem('cpu_selected_row'); }catch(_e0){ }
            var selObj = rawSel ? JSON.parse(rawSel) : null;
            if(selObj && selObj.id){
              var payload = {
                model_name: val2('model'),
                spec_summary: val2('spec'),
                manufacturer_name: val2('vendor'),
                part_number: val2('part_no'),
                remark: val2('note')
              };
              var _tpmcClean = String(refTpmc).replace(/,/g,'').trim();
              if(_tpmcClean !== ''){ var _tpmcNum = parseFloat(_tpmcClean); payload.reference_tpmc = Number.isFinite(_tpmcNum) ? _tpmcNum : null; } else { payload.reference_tpmc = null; }
              fetch('/api/cmp-cpu-types/' + selObj.id, {
                method: 'PUT',
                ...
                if(d && d.success && d.item){
                  try{ sessionStorage.setItem('cpu_selected_row', JSON.stringify(d.item)); }catch(_s){}
                }
              }).catch(function(_e){ });
            }
          }catch(_ep){ }
          // Keep type/release/eosl as-is (do not blank)
          try{
            var headerTitle2 = document.getElementById('page-header-title');
            if(headerTitle2){ var t2 = (val2('model')||'').trim(); if(t2) headerTitle2.textContent = t2; }
            var headerSub2 = document.getElementById('page-header-subtitle');
            if(headerSub2){ var s2 = (val2('vendor')||'').trim(); if(s2) headerSub2.textContent = s2; }
          }catch(_){ }
          return;
        }
  ...
  function recompute(){ if(!idleEl) return; var t=toInt(totalEl&&totalEl.value); var a=toInt(assignedEl&&assignedEl.value); var newVal = (t-a); idleEl.value=String(newVal); try{ updateIdleDot(newVal); }catch(_){ } }
      ...
      (function(){
        ...
        if(saveBtn){ saveBtn.addEventListener('click', function(){ updatePageFromForm(); closeModalLocal(EDIT_MODAL_ID); }); }
      })();
        ...
        if(isAllocationSchema){
          ...
          table.addEventListener('click', function(ev){
            ...
            if(target.classList.contains('js-hw-toggle') && target.getAttribute('data-action') === 'save'){
              ...
              commit('work_group', read('work_group'));
