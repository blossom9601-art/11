...
(function(){
    ...
    const COLUMN_MODAL_GROUPS = [
        { group: '데이터베이스', columns: ['model','vendor','hw_type','release_date','eosl','qty','note'] }
    ];

    /** 컬럼 메타 (라벨 + 그룹) */
    const COLUMN_META = {
        model:{label:'모델명',group:'데이터베이스'},
        vendor:{label:'제조사',group:'데이터베이스'},
        hw_type:{label:'유형',group:'데이터베이스'},
        release_date:{label:'릴리즈 일자',group:'데이터베이스'},
        eosl:{label:'EOSL 일자',group:'데이터베이스'},
        qty:{label:'수량',group:'데이터베이스'},
        note:{label:'비고',group:'데이터베이스'}
    };
    ...
    const DB_API_BASE = '/api/sw-db-types';
    ...
    function normalizeDbRecord(item){
        if(!item) return null;
    ...
    async function refreshDbData(options){
        ...
        try {
            ...
            state.isLoading = false;
            applyFilter();
            showMessage(err.message || '데이터베이스 목록을 불러오지 못했습니다.', '오류');
        }
    }

    function buildDbPayload(formData, opts){
        const payload = {
            db_name: formData.model || '',
            db_family: formData.hw_type || '',
        ...
        if(formData.db_code){
            payload.db_code = formData.db_code;
        }
    ...
    async function createDbRecord(formValues, opts){
        const payload = buildDbPayload(formValues, opts);
        const res = await callApi(DB_API_BASE, { method: 'POST', body: payload });
        const record = normalizeDbRecord(res?.item);
        if(record){
    ...
    async function updateDbRecord(recordId, formValues, opts){
        if(!recordId){
            throw new Error('수정할 대상을 찾을 수 없습니다.');
        }
        const payload = buildDbPayload(formValues, opts);
        const res = await callApi(`${DB_API_BASE}/${recordId}`, { method: 'PUT', body: payload });
        const record = normalizeDbRecord(res?.item);
        if(record){
    ...
    async function deleteDbRecords(ids){
        ...
        const res = await callApi(`${DB_API_BASE}/bulk-delete`, { method: 'POST', body: { ids: numericIds } });
        ...
        if(state.filtered.length === 0){
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '데이터베이스 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 데이터베이스를 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
    ...
    function fillEditForm(row){
        ...
        form.innerHTML='';
    const group = { title:'데이터베이스', cols:['model','vendor','release_date','eosl','note'] };
    ...
    const filename = `db_list_${yyyy}${mm}${dd}.csv`;
    ...
    tbodyEl?.addEventListener('click', e=>{
            ...
            if(link){
                try {
                    ...
                    sessionStorage.setItem('db_selected_row', JSON.stringify(payload));
                } catch(_err){}
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> {
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                await createDbRecord(data);
                form.reset();
                closeModal(ADD_MODAL_ID);
                showMessage('데이터베이스가 등록되었습니다.', '완료');
            } catch(err){
                console.error(err);
                showMessage(err.message || '데이터베이스 등록 중 오류가 발생했습니다.', '오류');
            }
        });
        // edit modal
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            const data = collectForm(form);
            try {
                await updateDbRecord(existingRow.id, data, { existingRow });
                closeModal(EDIT_MODAL_ID);
                showMessage('데이터베이스가 수정되었습니다.', '완료');
            } catch(err){
                console.error(err);
                showMessage(err.message || '데이터베이스 수정 중 오류가 발생했습니다.', '오류');
            }
        });
        // csv
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        wsTemplate['!cols'] = UPLOAD_HEADERS_KO.map((h)=>{
            ...
            const mid = ['제조사','유형'];
                ...
                XLSX.writeFile(wb, 'database_upload_template.xlsx');
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 데이터베이스를 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 데이터베이스를 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                const removed = await deleteDbRecords(ids);
                state.selected.clear();
                closeModal(DELETE_MODAL_ID);
                if(removed > 0){ setTimeout(()=> showMessage(`${removed}개 항목이 삭제되었습니다.`, '완료'), 0); }
            } catch(err){
                console.error(err);
                showMessage(err.message || '데이터베이스 삭제 중 오류가 발생했습니다.', '오류');
            }
        });
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 데이터베이스에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
    ...
    const GROUP = { title:'데이터베이스', cols:['model','vendor','eosl','note'] };
