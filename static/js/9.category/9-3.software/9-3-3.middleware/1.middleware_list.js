...
(function(){
    ...
    const ENUM_SETS = { };
    const API_BASE_URL = '/api/sw-middleware-types';
    ...
    const COLUMN_MODAL_GROUPS = [
        { group: '미들웨어', columns: ['model','vendor','hw_type','release_date','eosl','qty','note'] }
    ];

    /** 컬럼 메타 (라벨 + 그룹) */
    const COLUMN_META = {
        model:{label:'모델명',group:'미들웨어'},
        vendor:{label:'제조사',group:'미들웨어'},
        hw_type:{label:'유형',group:'미들웨어'},
        release_date:{label:'릴리즈 일자',group:'미들웨어'},
        eosl:{label:'EOSL 일자',group:'미들웨어'},
        qty:{label:'수량',group:'미들웨어'},
        note:{label:'비고',group:'미들웨어'}
    };
    ...
    function normalizeMiddlewareItem(item){
        if(!item) return null;
        const normalizedId = Number(item.id);
        return {
            id: Number.isFinite(normalizedId) ? normalizedId : item.id,
            middleware_code: item.middleware_code || '',
            ...
            hw_type: item.hw_type ?? item.middleware_type ?? '',
            release_date: item.release_date ?? '',
            eosl: item.eosl ?? item.eosl_date ?? '',
            qty: item.usage_count ?? 0,
    ...
    function normalizeList(items){
        ...
        return items.map(normalizeMiddlewareItem).filter(Boolean);
    }

    function upsertRow(record, options = {}){
        const normalized = normalizeMiddlewareItem(record);
    ...
    function buildApiPayload(data, fallback){
        ...
        if('model' in data) payload.model = trim(data.model ?? '');
        if('hw_type' in data) payload.hw_type = trim(data.hw_type ?? '');
        if('release_date' in data) payload.release_date = trim(data.release_date ?? '');
        if('eosl' in data) payload.eosl = trim(data.eosl ?? '');
        if('note' in data) payload.note = trim(data.note ?? '');
        if('middleware_code' in data) payload.middleware_code = trim(data.middleware_code ?? '');
        if('qty' in data){
            const parsedQty = parseInt(data.qty, 10);
            payload.qty = Number.isFinite(parsedQty) && parsedQty >= 0 ? parsedQty : 0;
        }
        if('vendor_code' in data || 'manufacturer_code' in data){
            const code = trim(data.vendor_code ?? data.manufacturer_code ?? '');
            if(code) payload.vendor_code = code;
        } else if(fallback?.manufacturer_code || fallback?.vendor_code){
            payload.vendor_code = trim(fallback.manufacturer_code ?? fallback.vendor_code ?? '');
        }
        if('vendor' in data || 'manufacturer_name' in data){
            payload.manufacturer_name = trim(data.vendor ?? data.manufacturer_name ?? '');
        } else if(fallback?.vendor || fallback?.manufacturer_name){
            payload.manufacturer_name = trim(fallback.vendor ?? fallback.manufacturer_name ?? '');
        }
        return payload;
    }
    ...
    async function requestJson(url, options = {}){
        ...
        try {
            ...
            console.error('Network error while requesting', url, err);
    ...
    async function initData(){
        ...
        try {
            ...
            console.error(err);
            showMessage(err.message || '미들웨어 데이터를 불러오지 못했습니다.', '오류');
        } finally {
        ...
        if(state.filtered.length === 0){
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '미들웨어 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 미들웨어를 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
    ...
    function fillEditForm(row){
        ...
        form.innerHTML='';
    const group = { title:'미들웨어', cols:['model','vendor','release_date','eosl','note'] };
    ...
    function generateFieldInput(col,value=''){
        ...
        if(col==='hw_type'){
            ...
            const opts = ['', 'WEB', 'WAS', 'API', 'APM', 'FRAMEWORK'];
    ...
    async function addRow(data, options = {}){
        const payload = buildApiPayload(data);
        if(!('qty' in payload)) payload.qty = 0;
    ...
    const filename = `middleware_list_${yyyy}${mm}${dd}.csv`;
    ...
    tbodyEl?.addEventListener('click', e=>{
            ...
            // detail link click: persist selected row context then allow navigation
            const detailLink = e.target.closest('a.work-name-link[data-row]');
            if(detailLink){
                try {
                    const raw = detailLink.getAttribute('data-row');
                    if(raw){ sessionStorage.setItem('middleware_selected_row', raw); }
                } catch(_e){}
            }
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> {
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            // normalize numeric field(s)
            if(typeof data.qty !== 'undefined'){
                const q = parseInt(data.qty||'0',10);
                data.qty = Number.isFinite(q) && q >= 0 ? q : 0;
            }
            try {
                await addRow(data);
                form.reset();
                ...
                console.error(err);
                showMessage(err.message || '미들웨어 등록 중 오류가 발생했습니다.', '오류');
            }
        });
        // edit modal
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                ...
                console.error(err);
                showMessage(err.message || '미들웨어 수정 중 오류가 발생했습니다.', '오류');
            }
        });
        // csv
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        wsTemplate['!cols'] = UPLOAD_HEADERS_KO.map((h)=>{
            ...
            const mid = ['제조사','유형'];
                ...
                XLSX.writeFile(wb, 'middleware_upload_template.xlsx');
            ...
            reader.onload = async ()=>{
                try{
                    ...
                    for(const item of imported){
                        try {
                            await addRow(item, { silent: true });
                            created++;
                        } catch(err){
                            console.error(err);
                            uploadError = err.message || '업로드 중 오류가 발생했습니다.';
                            break;
                        }
                    }
                    applyFilter();
                    if(uploadError){
                        showMessage(`업로드가 중단되었습니다: ${uploadError}`, '업로드 실패');
                    } else {
                        showMessage(`${created}개 행이 업로드되었습니다.`, '업로드 완료');
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', async ()=>{
            ...
            const clones = originals.map(o=>{
                const copy = {...o};
                delete copy.id; // new id assigned by 서버
                delete copy.middleware_code;
            ...
            for(const clone of clones){
                try {
                    await addRow(clone, { silent: true });
                    created++;
                } catch(err){
                    console.error(err);
                    errorMessage = err.message || '복제 중 오류가 발생했습니다.';
                    break;
                }
            }
            applyFilter();
            ...
            if(errorMessage){
                showMessage(errorMessage, '오류');
            } else {
                showMessage(created + '개 행이 복제되었습니다.', '완료');
            }
        });
        // dispose (불용처리)
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 미들웨어를 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 미들웨어를 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            const snapshotRows = state.data.filter(row => idSet.has(row.id));
            try {
                const payload = await requestJson(`${API_BASE_URL}/bulk-delete`, {
                    method: 'POST',
                    headers: JSON_HEADERS,
                ...
                const removed = payload.deleted ?? snapshotRows.length;
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 미들웨어에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', async ()=>{
            ...
            const ids = [...state.selected];
            if(!ids.length){ showMessage('변경할 행을 찾을 수 없습니다.', '오류'); return; }
            const patch = entries.reduce((acc, cur)=>{ acc[cur.field] = cur.value; return acc; }, {});
            let updatedCount = 0;
            let errorMessage = '';
            for(const id of ids){
                ...
                if(index === -1) continue;
                try {
                    await updateRow(index, patch, { silent: true });
                    updatedCount++;
                } catch(err){
                    console.error(err);
                    errorMessage = err.message || '일괄 변경 중 오류가 발생했습니다.';
                    break;
                }
            }
            applyFilter();
    ...
    const GROUP = { title:'미들웨어', cols:['model','vendor','eosl','note'] };
    ...
    renderStatBlock('stats-versions', '유형', countBy(rows, 'hw_type', ['WEB','WAS','API','APM','FRAMEWORK']), ['WEB','WAS','API','APM','FRAMEWORK'], { hideZero:true, zeroNote:true });
        // EOSL 연도 분포
