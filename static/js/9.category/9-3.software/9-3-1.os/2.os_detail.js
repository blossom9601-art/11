...
(function(){
    ...
    function __detailInit(){
      ...
      (function renderStatsFromApi(){
        try{
          ...
          fetch(apiUrl).then(function(r){ return r.json(); }).then(function(data){
            ...
            // ---- Status pie (dynamic) ----
            var statColorMap = {'가동':'#6366f1','유휴':'#0ea5e9','대기':'#94a3b8','장애':'#ef4444'};
            var statColors = ['#6366f1','#0ea5e9','#94a3b8','#ef4444','#f59e0b','#10b981','#a855f7','#22c55e','#06b6d4','#f97316'];
            var stMap = {};
            rows.forEach(function(r){ var s = (r.work_status||'').trim() || '미설정'; stMap[s] = (stMap[s]||0)+1; });
            var stEntries = Object.keys(stMap).map(function(k){ return {label:k, count:stMap[k]}; });
            stEntries.sort(function(a,b){ return b.count - a.count; });
            var sTotal = stEntries.reduce(function(s,e){ return s+e.count; }, 0);
            var statLegend = document.getElementById('stat-legend');
            if(sTotal > 0){
              ...
              if(statLegend) statLegend.style.display='';
              var stops2=[]; var deg2=0; var segments2=[];
              stEntries.forEach(function(e,i){
                var c = statColorMap[e.label] || statColors[i % statColors.length];
                var d = Math.round((e.count/sTotal)*360);
                if(i === stEntries.length-1) d = 360-deg2;
                stops2.push(c+' '+deg2+'deg '+(deg2+d)+'deg');
                segments2.push({label:e.label,count:e.count,color:c,start:deg2,end:deg2+d});
                deg2 += d;
              });
              statPie.style.background = 'conic-gradient('+stops2.join(', ')+')';
              if(statLegend){
                statLegend.innerHTML='';
                stEntries.forEach(function(e,i){
                  var c = statColorMap[e.label] || statColors[i % statColors.length];
                  var p = Math.round((e.count*100)/sTotal);
                  var li = document.createElement('li');
                  li.className='legend-item';
                  li.innerHTML='<span class="legend-dot" style="background:'+c+'"></span><span class="legend-host">'+e.label+'</span><span class="legend-size">'+e.count+' ('+p+'%)</span>';
                  statLegend.appendChild(li);
                });
              }
              try{ attachPieInteractions(statPie, segments2, sTotal); }catch(_){}
            } else {
              hideChart(statPie, statEmpty, statLegend, '할당 시스템 내역이 없습니다.');
            }
            // ---- Operation pie ----
            var opMap = {};
      ...
      // Render Version Donut from System tab data (software_detail_version)
      (function renderVersionDonut(){
        try{
          var donut = document.getElementById('ver-donut'); var legend = document.getElementById('ver-legend'); var empty = document.getElementById('ver-empty');
          if(!donut || !legend) return;
          function computeFromTable(table){
            var map={}; if(!table) return map;
            var trs = table.querySelectorAll('tbody tr');
            trs.forEach(function(tr){
              var td = tr.querySelector('[data-col="software_detail_version"]');
              if(!td) return; var v=(td.textContent||'').trim(); if(!v) v='-';
              map[v] = (map[v]||0)+1;
            });
            return map;
          }
          function getCountsSync(){
            // Preferred: read cached JSON map from localStorage
            try{
              var raw = localStorage.getItem('os:version:counts');
              if(raw){ var obj = JSON.parse(raw); if(obj && typeof obj==='object' && Object.keys(obj).length){ return obj; }
              }
            }catch(_){ }
            // Fallback: scan an embedded table if present on the page (rare)
            return computeFromTable(document.getElementById('hw-spec-table'));
          }
          function render(map){
            var entries = Object.keys(map).map(function(k){ return {version:k, count:map[k]}; });
            entries.sort(function(a,b){ if(b.count!==a.count) return b.count-a.count; return (a.version>b.version?1:(a.version<b.version?-1:0)); });
            var total = entries.reduce(function(s,e){ return s+e.count; }, 0);
            if(!total){ if(empty) empty.style.display=''; return; }
            if(empty) empty.style.display='none';
            // Up to 10 segments total.
            // - If entries.length <= 10: show all entries (no 기타)
            // - If entries.length > 10: show top 9 + '기타' (rest)
            var topLimit = (entries.length <= 10) ? 10 : 9;
            var top = entries.slice(0, topLimit);
            var others = entries.slice(topLimit);
            if(others.length){
              var rest = others.reduce(function(s,e){ return s+e.count; }, 0);
              if(rest>0) top.push({version:'기타', count:rest});
            }
            var colors = ['#6366f1','#0ea5e9','#10b981','#f59e0b','#ef4444','#a855f7','#22c55e','#06b6d4','#f97316','#94a3b8'];
            var acc = 0; var degs=[];
            top.forEach(function(e){ var d = Math.round((e.count*360)/total); acc += d; degs.push(acc); });
            if(degs.length){ degs[degs.length-1] = 360; }
            var root = donut.style;
            // Clear previous vars up to 10
            for(var i=1;i<=10;i++){ root.removeProperty('--deg'+i); root.removeProperty('--seg'+i); }
            // Set deg/seg for up to 10 slices
            top.forEach(function(e, idx){
              var d = degs[idx] || 360; root.setProperty('--deg'+(idx+1), d+'deg');
              root.setProperty('--seg'+(idx+1), colors[idx] || colors[colors.length-1]);
            });
            // Center: show total count only (no label)
            try{
              var center = donut.querySelector('.donut-center');
              if(!center){ center = document.createElement('div'); center.className = 'donut-center'; donut.appendChild(center); }
              var totalText = (total||0).toLocaleString();
              center.innerHTML = '<div class="donut-total">'+totalText+'</div>';
            }catch(_){ }
            // Legend
            legend.innerHTML = '';
            top.forEach(function(e, idx){
              var li = document.createElement('li'); li.className='legend-item';
              var dot = document.createElement('span'); dot.className='legend-dot'; dot.style.backgroundColor = colors[idx] || colors[colors.length-1];
              var host = document.createElement('span'); host.className='legend-host'; host.textContent = e.version;
              var size = document.createElement('span'); size.className='legend-size'; var p = Math.round((e.count*100)/total); size.textContent = e.count+' ('+p+'%)';
              li.appendChild(dot); li.appendChild(host); li.appendChild(size); legend.appendChild(li);
            });
            // Reflect total into OS quantity as well
            setOsQuantity(total);
            // Interactivity: hover/click to show tooltip for donut segments
            var segs = [];
            var start = 0;
            top.forEach(function(e, idx){ var pdeg = degs[idx] || 360; var end = pdeg; segs.push({ label: e.version, count: e.count, start: start, end: end, color: colors[idx] || colors[colors.length-1] }); start = end; });
            attachPieInteractions(donut, segs, total, true /*hasHole*/);
          }
          var counts = getCountsSync();
          if(counts && Object.keys(counts).length){ render(counts); return; }
          // Last-resort: fetch system tab HTML and parse
          try{
            var url = '/app/templates/9.category/9-3.software/9-3-1.os/tab41-system.html';
            fetch(url, { cache:'no-cache' }).then(function(res){ return res.text(); }).then(function(html){
              try{
                ...
                var table = doc.getElementById('hw-spec-table');
                var map = computeFromTable(table);
                try{ if(Object.keys(map).length){ localStorage.setItem('os:version:counts', JSON.stringify(map)); } }catch(_s){}
                render(map);
              }catch(_p){ if(empty) empty.style.display=''; }
            }).catch(function(_e){ if(empty) empty.style.display=''; });
          }catch(_f){ if(empty) empty.style.display=''; }
        }catch(_){ }
      })();
      ...
      function ensureCategoryCalendarStyle(){if(document.getElementById('cat-detail-flatpickr-style'))return;var style=document.createElement('style');style.id='cat-detail-flatpickr-style';style.textContent=['#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr{border:1px solid #d5dbe4;border-radius:8px;box-shadow:0 10px 22px rgba(15,23,42,.14);padding:0;width:318px;}','#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-months{padding:6px 8px 0;}','#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-current-month{padding-top:3px;font-size:28px;}','#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr span.flatpickr-weekday{font-size:13px;font-weight:700;color:#111827;}','#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-day{border-radius:0;height:38px;line-height:38px;max-width:44px;margin:0;border-color:transparent;color:#4b5563;}','#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-day.prevMonthDay,#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-day.nextMonthDay{color:#c6ccd7;}','#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-day:hover{background:#eef2ff;border-color:transparent;color:#374151;}','#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-day.today:not(.selected):not(.startRange):not(.endRange){border-top:2px solid #ef4444;}','#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-day.selected,#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-day.startRange,#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-day.endRange,#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-day.selected:hover{background:#5c67e8;border-color:#5c67e8;color:#fff;}','#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .fp-today-btn{position:absolute;right:10px;bottom:8px;border:0;border-radius:7px;background:#5c67e8;color:#fff;font-size:12px;font-weight:700;padding:5px 10px;line-height:1.1;cursor:pointer;}','#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .fp-today-btn:hover{background:#4f58d9;}','#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-prev-month,#system-edit-modal .flatpickr-calendar.cat-detail-flatpickr .flatpickr-next-month{color:#6b7280;fill:#6b7280;}'].join('');document.head.appendChild(style);}
      function applyCategoryCalendarSkin(fp){var cal=fp&&fp.calendarContainer;if(!cal)return;cal.classList.add('cat-detail-flatpickr');}
      function initDatePickers(formId){var form=document.getElementById(formId);if(!form)return;ensureCategoryCalendarStyle();_ensureFlatpickr().then(function(){var startEl=form.querySelector('[name="release_date"]');var endEl=form.querySelector('[name="eosl"]');function addToday(fp){var cal=fp&&fp.calendarContainer;if(!cal)return;applyCategoryCalendarSkin(fp);if(cal.querySelector('.fp-today-btn'))return;var b=document.createElement('button');b.type='button';b.className='fp-today-btn';b.textContent='\uc624\ub298';b.addEventListener('click',function(){fp.setDate(new Date(),true);});cal.appendChild(b);}var opts={locale:(window.flatpickr&&window.flatpickr.l10ns&&window.flatpickr.l10ns.ko)?window.flatpickr.l10ns.ko:'ko',dateFormat:'Y-m-d',allowInput:true,disableMobile:true,prevArrow:'<',nextArrow:'>',onReady:function(_,__,inst){addToday(inst);},onOpen:function(_,__,inst){addToday(inst);}};if(startEl&&!startEl._flatpickr)window.flatpickr(startEl,opts);if(endEl&&!endEl._flatpickr)window.flatpickr(endEl,opts);}).catch(function(){});}
        ...
        if(isAllocationSchema){
          ...
          function allocBuildScopeKey(){
            try{
              var m = (location.pathname || '').match(/\/p\/(\w+)/);
              var key = m ? m[1] : '';
              // Normalize to the owning detail key (shared across tabs)
