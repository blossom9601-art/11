 ...
 * NOTE: SQLite 백엔드와 REST API를 통해 데이터를 주고받습니다.
 */
...
(function(){
    ...
    const COLUMN_MODAL_GROUPS = [
        { group: '운영체제', columns: ['model','vendor','hw_type','release_date','eosl','qty'] }
    ];

    /** 컬럼 메타 (라벨 + 그룹) */
    const COLUMN_META = {
        model:{label:'모델명',group:'운영체제'},
        vendor:{label:'제조사',group:'운영체제'},
        hw_type:{label:'유형',group:'운영체제'},
        release_date:{label:'릴리즈 일자',group:'운영체제'},
        eosl:{label:'EOSL 일자',group:'운영체제'},
        qty:{label:'수량',group:'운영체제'},
        note:{label:'비고',group:'운영체제'}
    };
    ...
    let state = {
        ...
        selected: new Set(),
        nextId: 1, // client-only fallback id seed
        sortKey: null,
        sortDir: 'asc',
        loading: false,
    ...
    async function callApi(url, options){
        ...
        const hasJsonBody = options && options.body && typeof options.body === 'object' && !(options.body instanceof FormData);
        if(opts.method !== 'GET' && opts.method !== 'HEAD' && hasJsonBody){
        ...
        if(options && Object.prototype.hasOwnProperty.call(options, 'body')){
            opts.body = hasJsonBody ? JSON.stringify(options.body) : options.body;
        }
        let response;
        try {
    ...
    function buildVendorCodeMap(items){
        const map = {};
        (items||[]).forEach(item=>{
    ...
    function findVendorCodeByName(name){
        ...
        if(!target) return '';
        if(vendorCodeByName[target]) return vendorCodeByName[target];
        const folded = target.toLowerCase();
        const keys = Object.keys(vendorCodeByName);
        for(let i=0;i<keys.length;i++){
            if(keys[i].toLowerCase() === folded){
                return vendorCodeByName[keys[i]];
            }
        }
        return '';
    }
    ...
    function normalizeOsRecord(item){
        if(!item) return null;
        const vendorCode = item.manufacturer_code || '';
        // API 응답에 manufacturer_name이 있으면 우선 사용 (코드 직접 노출 방지)
        const vendorName = (item.manufacturer_name && item.manufacturer_name !== vendorCode)
            ? item.manufacturer_name
            : (vendorNameByCode[vendorCode] || item.manufacturer_name || vendorCode || '');
    ...
    async function refreshOsData(options){
        ...
        try {
            const [vendorRes, osRes] = await Promise.all([
                callApi(`${VENDOR_MANUFACTURER_API}?include_deleted=${includeDeleted}`),
                callApi(`${OS_API_BASE}?include_deleted=${includeDeleted}`)
            ]);
            vendorNameByCode = buildVendorMap(vendorRes?.items || []);
            vendorCodeByName = buildVendorCodeMap(vendorRes?.items || []);
            vendorNameOptions = buildVendorOptions(vendorRes?.items || []);
            const mapped = (osRes?.items || []).map(normalizeOsRecord).filter(Boolean);
            state.data = mapped;
            state.page = 1;
            ...
            console.error(err);
            state.loading = false;
            try { const emptyEl1 = document.getElementById('system-empty'); if(emptyEl1) emptyEl1.hidden = true; } catch(_e) {}
            showMessage(err.message || '운영체제 목록을 불러오지 못했습니다.', '오류');
        }
    }

    function buildOsPayload(formData, opts){
        const payload = {
            model_name: formData.model || '',
            os_type: formData.hw_type || '',
            release_date: formData.release_date || '',
            eosl_date: formData.eosl || '',
            license_count: (function(){
                const q = parseInt(formData.qty || '0', 10);
                return Number.isFinite(q) && q >= 0 ? q : 0;
            })(),
            remark: formData.note || ''
        };
        if(formData.os_code){
            payload.os_code = formData.os_code;
        }
        ...
        if(existingRow && vendorInput === (existingRow.vendor || '') && existingRow.vendor_code){
            ...
            const matchedCode = findVendorCodeByName(vendorInput);
            if(matchedCode){
                payload.manufacturer_code = matchedCode;
            } else {
    ...
    async function createOsRecord(formValues, opts){
        const payload = buildOsPayload(formValues, opts);
        const res = await callApi(OS_API_BASE, { method: 'POST', body: payload });
        const record = normalizeOsRecord(res?.item);
        if(record){
            if(record.vendor_code && formValues.vendor){
                vendorNameByCode[record.vendor_code] = formValues.vendor;
                record.vendor = formValues.vendor;
            }
            upsertRow(record);
        }
        return record;
    ...
    async function updateOsRecord(recordId, formValues, opts){
        if(!recordId){
            throw new Error('수정할 대상을 찾을 수 없습니다.');
        }
        const payload = buildOsPayload(formValues, opts);
        const res = await callApi(`${OS_API_BASE}/${recordId}`, { method: 'PUT', body: payload });
        const record = normalizeOsRecord(res?.item);
        if(record){
            const fallbackVendor = formValues.vendor || opts?.existingRow?.vendor;
            if(record.vendor_code && fallbackVendor){
                vendorNameByCode[record.vendor_code] = fallbackVendor;
                record.vendor = fallbackVendor;
            }
            upsertRow(record, { append: true });
        }
        return record;
    }

    async function deleteOsRecords(ids){
        ...
        const res = await callApi(`${OS_API_BASE}/bulk-delete`, { method: 'POST', body: { ids: numericIds } });
        removeRowsByIds(numericIds);
        const removed = typeof res?.deleted === 'number' ? res.deleted : numericIds.length;
        return removed;
    }
        ...
        if(state.loading){
            ...
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '운영체제 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 운영체제를 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
        ...
        slice.forEach((row)=>{
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'model'){
                        try {
                            var base = String(window.__SW_OS_DETAIL_URL || '').trim();
                            if(base){
                                var u = new URL(base, window.location.origin);
                                if(row.id != null && String(row.id).trim() !== ''){
                                    u.search = '';
                                    u.searchParams.set('os_id', String(row.id));
                                } else {
    ...
    function fillEditForm(row){
        ...
        form.innerHTML='';
    const group = { title:'운영체제', cols:['model','vendor','release_date','eosl'] };
    ...
    const filename = `os_list_${yyyy}${mm}${dd}.csv`;
    ...
    tbodyEl?.addEventListener('click', e=>{
            ...
            // 모델명 링크 클릭 시: 상세 URL로 이동하기 전 컨텍스트 저장
            const link = e.target.closest('a.work-name-link');
            if(link){
                try {
                    var dataAttr = link.getAttribute('data-row');
                    var payload = dataAttr ? JSON.parse(dataAttr) : null;
                    if(!payload){
                        const tr = link.closest('tr');
                        payload = {
                            model: tr?.querySelector('[data-col="model"]')?.textContent?.trim() || '',
                            vendor: tr?.querySelector('[data-col="vendor"]')?.textContent?.trim() || '',
                            hw_type: tr?.querySelector('[data-col="hw_type"]')?.textContent?.trim() || '',
                            release_date: tr?.querySelector('[data-col="release_date"]')?.textContent?.trim() || '',
                            eosl: tr?.querySelector('[data-col="eosl"]')?.textContent?.trim() || '',
                            qty: tr?.querySelector('[data-col="qty"]')?.textContent?.trim() || '',
                            note: tr?.querySelector('[data-col="note"]')?.textContent?.trim() || ''
                        };
                    }
                    sessionStorage.setItem('os_selected_row', JSON.stringify(payload));
                } catch(_err){}
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> {
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                await createOsRecord(data);
                form.reset();
                closeModal(ADD_MODAL_ID);
                showMessage('운영체제가 등록되었습니다.', '완료');
            } catch(err){
                console.error(err);
                showMessage(err.message || '운영체제 등록 중 오류가 발생했습니다.', '오류');
            }
        });
        // edit modal
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            const existingRow = Number.isInteger(index) && index >= 0 ? state.data[index] : null;
            if(!existingRow){
            ...
            const data = collectForm(form);
            try {
                await updateOsRecord(existingRow.id, data, { existingRow });
                closeModal(EDIT_MODAL_ID);
                showMessage('운영체제가 수정되었습니다.', '완료');
            } catch(err){
                console.error(err);
                showMessage(err.message || '운영체제 수정 중 오류가 발생했습니다.', '오류');
            }
        });
        // csv
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        wsTemplate['!cols'] = UPLOAD_HEADERS_KO.map((h)=>{
            ...
            const mid = ['제조사','유형'];
                ...
                XLSX.writeFile(wb, 'os_upload_template.xlsx');
                ...
                try{
                    ...
                    (async ()=>{
                        ...
                        for(const item of imported){
                            try {
                                await createOsRecord(item);
                                success += 1;
                            } catch(err){
                                failures.push(err.message || `${item.model || '항목'} 등록 실패`);
                            }
                        }
                        if(failures.length){
                            const preview = failures.slice(0, 3).join('\n');
                            const overflow = failures.length > 3 ? `\n...외 ${failures.length - 3}건` : '';
                            showMessage(`총 ${success}건 성공, ${failures.length}건 실패했습니다.\n${preview}${overflow}`, '업로드 결과');
                        } else {
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', async ()=>{
            ...
            try {
                for(const clone of clones){
                    await createOsRecord({
                        model: clone.model,
                        vendor: clone.vendor,
                        hw_type: clone.hw_type,
                        release_date: clone.release_date,
                        eosl: clone.eosl,
                        qty: clone.qty,
                        note: clone.note
                    }, { existingRow: clone });
                ...
                if(success){ showMessage(`${success}개 행이 복제되었습니다.`, '완료'); }
            } catch(err){
                console.error(err);
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 운영체제를 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 운영체제를 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                const removed = await deleteOsRecords(ids);
                state.selected.clear();
                closeModal(DELETE_MODAL_ID);
                if(removed > 0){ setTimeout(()=> showMessage(`${removed}개 항목이 삭제되었습니다.`, '완료'), 0); }
            } catch(err){
                console.error(err);
                showMessage(err.message || '운영체제 삭제 중 오류가 발생했습니다.', '오류');
            }
        });
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 운영체제에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
...
document.addEventListener('keydown', e=>{ if(e.key==='Escape'){ closeModal(STATS_MODAL_ID); }});
    ...
    const GROUP = { title:'운영체제', cols:['model','vendor','eosl'] };
