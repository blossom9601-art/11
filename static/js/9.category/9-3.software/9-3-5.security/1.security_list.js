...
(function(){
    ...
    const COLUMN_MODAL_GROUPS = [
        { group: '보안 소프트웨어', columns: ['model','vendor','hw_type','release_date','eosl','qty'] }
    ];
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(col === 'model'){
                        ...
                        var baseDetail = window.__SW_SECURITY_DETAIL_URL || '/b/cat_sw_security_detail';
                        var href;
    ...
    function fillEditForm(row){
        ...
        form.innerHTML='';
    const group = { title:'보안 소프트웨어', cols:['model','vendor','hw_type','release_date','eosl'] };
...
document.addEventListener('keydown', e=>{ if(e.key==='Escape'){ closeModal(STATS_MODAL_ID); }});
    ...
    const GROUP = { title:'보안 소프트웨어', cols:['model','vendor','eosl'] };
