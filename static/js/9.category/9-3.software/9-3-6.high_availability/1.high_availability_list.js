...
(function(){
    ...
    const ENUM_SETS = { };
    const HA_TYPE_OPTIONS = ['', 'Active-Active','Active-Passive','Active-Standby','Cluster','Geo-Cluster','기타'];
    ...
    const COLUMN_MODAL_GROUPS = [
        { group: '고가용성 유형', columns: ['model','vendor','hw_type','release_date','eosl','qty','note'] }
    ];

    /** 컬럼 메타 (라벨 + 그룹) */
    const COLUMN_META = {
        model:{label:'모델명',group:'고가용성 유형'},
        vendor:{label:'제조사',group:'고가용성 유형'},
        hw_type:{label:'유형',group:'고가용성 유형'},
        release_date:{label:'릴리즈 일자',group:'고가용성 유형'},
        eosl:{label:'EOSL 일자',group:'고가용성 유형'},
        qty:{label:'수량',group:'고가용성 유형'},
        note:{label:'비고',group:'고가용성 유형'}
    };
    ...
    const API_BASE_URL = '/api/sw-ha-types';
    ...
    function normalizeHaItem(item){
        ...
        return {
            ...
            eosl_date: eosl,
            qty,
            ha_count: typeof item.ha_count === 'number' ? item.ha_count : qty,
    ...
    function normalizeHaList(items){
        if(!Array.isArray(items)) return [];
        return items.map(normalizeHaItem).filter(Boolean);
    }

    function upsertRow(record, options={}){
        const normalized = normalizeHaItem(record);
    ...
    function buildApiPayload(data, fallback){
        ...
        // Canonical API keys for /api/sw-ha-types (see sw_high_availability_type_service.py)
        const name = get('model') || trim(data.ha_name ?? base.ha_name ?? data.name ?? base.name ?? '');
        if(name) payload.ha_name = name;

        const mode = get('hw_type') || trim(data.ha_mode ?? base.ha_mode ?? data.mode ?? base.mode ?? '');
        if(mode) payload.ha_mode = mode;

        const vendorName = get('vendor') || trim(data.manufacturer_name ?? base.manufacturer_name ?? data.manufacturer ?? base.manufacturer ?? '');
        ...
        if('qty' in data || 'ha_count' in data){
            const raw = data.qty ?? data.ha_count;
            if(raw === '' || raw == null){
                payload.ha_count = 0;
            } else {
                const parsed = parseInt(raw, 10);
                payload.ha_count = Number.isFinite(parsed) && parsed >= 0 ? parsed : 0;
            }
        }

        const note = get('note') || trim(data.remark ?? base.remark ?? data.description ?? base.description ?? '');
        if(note !== '') payload.remark = note;

        const haCode = trim(data.ha_code ?? base.ha_code ?? '');
        if(haCode) payload.ha_code = haCode;
    ...
    async function createHaItem(data, options={}){
        const payload = buildApiPayload(data);
        if(!payload.ha_name) throw new Error('모델명을 입력하세요.');
        if(!payload.ha_mode) throw new Error('유형을 선택하세요.');
        if(!payload.manufacturer_name && !payload.manufacturer_code) throw new Error('제조사 정보를 입력하세요.');
        if(typeof payload.ha_count === 'undefined') payload.ha_count = 0;
    ...
    async function deleteHaItems(ids, options={}){
    ...
    async function initData(){
        ...
        try {
            ...
            state.data = normalizeHaList(payload.items);
            refreshVendorSelectInAddForm();
        } catch(err){
            console.error(err);
            showMessage(err.message || '고가용성 데이터를 불러오지 못했습니다.', '오류');
        } finally {
        ...
        if(state.filtered.length === 0){
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '고가용성 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 고가용성 유형을 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
    ...
    function fillEditForm(row){
        ...
        form.innerHTML='';
    const group = { title:'고가용성 유형', cols:['model','vendor','release_date','eosl','note'] };
    ...
    function generateFieldInput(col,value=''){
        ...
        if(col==='hw_type'){
            ...
            return `<select name="hw_type" class="form-input search-select" data-searchable="true" data-placeholder="선택" required>${HA_TYPE_OPTIONS.map(o=>`<option value="${o}" ${o===v?'selected':''}>${o===''?'선택':o}</option>`).join('')}</select>`;
        }
        if(col==='qty'){
    ...
    const filename = `ha_type_list_${yyyy}${mm}${dd}.csv`;
    ...
    function bindEvents(){
        ...
        tbodyEl?.addEventListener('click', function(e){
            ...
            try {
                ...
                if(payload){ sessionStorage.setItem('ha_selected_row', payload); }
            } catch(_){}
        });
        // 컬럼 헤더 정렬 클릭
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> {
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                await createHaItem(data);
                form.reset();
                closeModal(ADD_MODAL_ID);
                showMessage('고가용성 유형이 등록되었습니다.', '완료');
            }catch(err){
                console.error(err);
                showMessage(err.message || '고가용성 등록 중 오류가 발생했습니다.', '오류');
            }finally{
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                showMessage('고가용성 유형이 수정되었습니다.', '완료');
            }catch(err){
                console.error(err);
                showMessage(err.message || '고가용성 수정 중 오류가 발생했습니다.', '오류');
            }finally{
                if(btn) btn.disabled = false;
            }
        });
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        // template download — provide an XLSX with Korean headers (no 추가 점수 컬럼) matching expected upload
        ...
        wsTemplate['!cols'] = UPLOAD_HEADERS_KO.map((h)=>{
            ...
            const mid = ['제조사','유형'];
                ...
                XLSX.writeFile(wb, 'ha_type_upload_template.xlsx');
                ...
                try{
                    ...
                    try{
                        for(const item of imported){
                            try{
                                await createHaItem(item, { silent: true });
                                successCount++;
                            }catch(err){
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', async ()=>{
            ...
            try{
                for(const source of originals){
                    ...
                    try{
                        await createHaItem(clone, { silent: true });
                        success++;
                    }catch(err){
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 고가용성 유형을 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 고가용성 유형을 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 고가용성 유형에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
    ...
    function buildBulkForm(){
        ...
        function inputFor(col){
            ...
            if(col === 'hw_type'){
                return `<select class="form-input search-select" data-searchable="true" data-placeholder="선택" data-bulk-field="hw_type">${HA_TYPE_OPTIONS.map(o=>`<option value="${o}">${o===''?'선택':o}</option>`).join('')}</select>`;
            }
    ...
    const GROUP = { title:'고가용성 유형', cols:['model','vendor','eosl','note'] };
