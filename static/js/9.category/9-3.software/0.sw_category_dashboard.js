...
(function () {
  ...
  var SECTION_LABELS = {
    os: '운영체제', database: '데이터베이스', middleware: '미들웨어',
    virtualization: '가상화', security: '보안S/W', high_availability: '고가용성'
  };
  var SECTION_KEYS = ['os', 'database', 'middleware', 'virtualization', 'security', 'high_availability'];
  ...
  function renderKPI(summary) {
    var grid = document.getElementById('sw-kpi-grid');
    if (!grid) return;
    var eosl = summary.eosl || {};
    var total = summary.total || 0;
    var cards = [
      { status: 'total',    label: '전체 자산',     value: total,              sub: '등록된 소프트웨어 유형', dot: COLORS.primary },
  ...
  function renderDonut(eosl) {
    var canvas = document.getElementById('sw-eosl-donut');
    var wrap = document.getElementById('sw-eosl-donut-wrap');
    var emptyPh = document.getElementById('sw-eosl-donut-empty');
    if (!canvas) return;
    destroyChart('sw-eosl-donut');

    var keys = ['healthy', 'imminent', 'expired', 'unknown'];
    var values = keys.map(function (k) { return eosl[k] || 0; });
    var total = values.reduce(function (s, v) { return s + v; }, 0);

    var totalEl = document.getElementById('sw-eosl-total');
    var leg = document.getElementById('sw-eosl-legend');
  ...
  function renderLegend(eosl) {
    var el = document.getElementById('sw-eosl-legend');
    if (!el) return;
    var keys = ['healthy', 'imminent', 'expired', 'unknown'];
  ...
  function renderRiskList(riskTop) {
    var ul = document.getElementById('sw-risk-list');
    var ph = document.getElementById('sw-risk-empty-sticker');
    if (!ul) return;
    ul.innerHTML = '';
    ...
    riskTop.forEach(function (r) {
      ...
      li.innerHTML =
        ...
        '<span class="cat-risk-type">' + (r.sw_type || r.vendor || '-') + '</span>' +
  ...
  function renderInsights(summary) {
    var el = document.getElementById('sw-insight-list');
    if (!el) return;
    var eosl = summary.eosl || {};
    var total = summary.total || 0;
    var items = [];
    ...
    if (eosl.expired > 0) {
      items.push({ type: 'danger', text: '전체 소프트웨어의 <strong>' + pct(eosl.expired, total) + '%</strong>가 EOSL 초과 상태입니다. 업그레이드를 검토하세요.' });
    }
    if (eosl.imminent > 0) {
      items.push({ type: 'warn', text: '30일 내 EOSL 도래 소프트웨어 <strong>' + fmt(eosl.imminent) + '개</strong>가 있습니다.' });
    }
    if (eosl.unknown > 0) {
      items.push({ type: 'info', text: 'EOSL 미등록 소프트웨어 <strong>' + fmt(eosl.unknown) + '개</strong>의 정보를 보완하세요.' });
    }
  ...
  function renderSectionCards(sections) {
    var el = document.getElementById('sw-section-charts');
    if (!el) return;
    el.innerHTML = '';
  ...
  /* ── 데이터 로드 ── */
  function load() {
    fetch('/api/category/sw-dashboard')
      ...
      .catch(function (err) {
        console.error('[SW Dashboard] load error:', err);
      });
  }

  /* ── 액션 버튼 ── */
  function bindActions() {
    ['sw-action-eosl', 'sw-action-version', 'sw-action-unknown'].forEach(function (id) {
