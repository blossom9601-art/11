...
(function(){
    ...
    const API_BASE_URL = '/api/work-categories';
    ...
    function normalizeRow(row){
        ...
        return {
            ...
            wc_name: (row.wc_name ?? row.category_name ?? '').trim(),
            wc_desc: row.wc_desc ?? row.description ?? '',
            hw_count: normalizeCountValue(row.hw_count ?? row.hardware_count),
            sw_count: normalizeCountValue(row.sw_count ?? row.software_count),
    ...
    async function apiListWorkCategories(){
        ...
        if(!res.ok || !data || data.success === false){
            throw new Error((data && data.message) || '업무 분류 목록 조회에 실패했습니다.');
        }
        const items = Array.isArray(data.items) ? data.items : [];
        return items.map(normalizeRow).filter(Boolean);
    }
    async function apiCreateWorkCategory(payload){
        ...
        try { data = await res.json(); } catch(_e){ data = null; }
        if(!res.ok || !data || data.success === false){
            throw new Error((data && data.message) || '업무 분류 등록 중 오류가 발생했습니다.');
        }
        return normalizeRow(data.item || data);
    }
    async function apiUpdateWorkCategory(id, payload){
        ...
        try { data = await res.json(); } catch(_e){ data = null; }
        if(!res.ok || !data || data.success === false){
            throw new Error((data && data.message) || '업무 분류 수정 중 오류가 발생했습니다.');
        }
        return normalizeRow(data.item || data);
    }
    async function apiDeleteWorkCategories(ids){
        ...
        try { data = await res.json(); } catch(_e){ data = null; }
        if(!res.ok || !data || data.success === false){
            throw new Error((data && data.message) || '업무 분류 삭제 중 오류가 발생했습니다.');
        }
    ...
    const COLUMN_MODAL_GROUPS = [
        { group: '업무 분류', columns: ['wc_name','wc_desc','hw_count','sw_count','note'] }
    ];

    /** 컬럼 메타 (라벨 + 그룹) */
    const COLUMN_META = {
        wc_name:{label:'업무 분류',group:'업무 분류'},
        wc_desc:{label:'설명',group:'업무 분류'},
        hw_count:{label:'하드웨어(수량)',group:'업무 분류'},
        sw_count:{label:'소프트웨어(수량)',group:'업무 분류'},
        note:{label:'비고',group:'업무 분류'}
    };
    ...
    // 업무 분류 페이지: 샘플 데이터 5개 제공
    function mockData(count=5){
        const rows = [
            { id: 1, wc_name: '영업관리', wc_desc: '영업 활동 및 고객 관리 관련 업무', hw_count: 12, sw_count: 8, note: '' },
            { id: 2, wc_name: '구매관리', wc_desc: '자재/서비스 구매 및 발주 관리', hw_count: 4, sw_count: 5, note: '' },
            { id: 3, wc_name: '생산관리', wc_desc: '생산 계획 및 재고 관리', hw_count: 18, sw_count: 10, note: '' },
            { id: 4, wc_name: '인사관리', wc_desc: '인사 및 급여 정산', hw_count: 6, sw_count: 7, note: '' },
            { id: 5, wc_name: '재무회계', wc_desc: '회계 결산 및 세무 신고', hw_count: 5, sw_count: 9, note: '' }
        ];
    ...
    async function initData(){
        ...
        render();
        try {
            const rows = await apiListWorkCategories();
            state.data = rows;
        } catch(err){
            console.error(err);
            showMessage((err && err.message) || '업무 분류 데이터를 불러오지 못했습니다.', '오류');
            state.data = [];
        } finally {
        ...
        if(state.filtered.length === 0){
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = ' 업무 분류 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 업무 분류를 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
    ...
    function fillEditForm(row){
        ...
        form.innerHTML='';
        const groups = [
            { title:'업무 분류', cols:['wc_name','wc_desc','note'] }
        ];
        groups.forEach(g=>{
    ...
    const filename = `work_classification_list_${yyyy}${mm}${dd}.csv`;
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> { openModal(ADD_MODAL_ID); });
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                ...
                showMessage('업무 분류가 등록되었습니다.', '완료');
            } catch(err){
                console.error(err);
                showMessage((err && err.message) || '업무 분류 등록 중 오류가 발생했습니다.', '오류');
            }
        });
        // edit modal
    document.getElementById(EDIT_CLOSE_ID)?.addEventListener('click', ()=> closeModal(EDIT_MODAL_ID));
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            if(index < 0 || !state.data[index]){ showMessage('수정할 업무 분류를 찾을 수 없습니다.', '오류'); return; }
            ...
            try {
                ...
                showMessage('업무 분류가 저장되었습니다.', '완료');
            } catch(err){
                console.error(err);
                showMessage((err && err.message) || '업무 분류 수정 중 오류가 발생했습니다.', '오류');
            }
        });
        // csv
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById(UPLOAD_TEMPLATE_BTN_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                XLSX.writeFile(wb, 'work_classification_upload_template.xlsx');
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 업무 분류를 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 업무 분류를 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                await apiDeleteWorkCategories(ids);
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                ...
                for(const id of ids){
                    ...
                    const updated = await apiUpdateWorkCategory(id, payload);
    ...
    if(swEl) swEl.innerHTML = '';
        ...
        // Top 10 by 업무 분류 (sum of counts per 분류)
        const hwByClass = sumBy(rows, 'wc_name', 'hw_count');
        const swByClass = sumBy(rows, 'wc_name', 'sw_count');
        renderStatBlock('stats-software', '하드웨어 Top 10 (업무 분류)', hwByClass, null, { topN: 10, includeOther: false });
        renderStatBlock('stats-software', '소프트웨어 Top 10 (업무 분류)', swByClass, null, { topN: 10, includeOther: false });
    }
    }
