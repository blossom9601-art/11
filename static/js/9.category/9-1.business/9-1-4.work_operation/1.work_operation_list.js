...
(function(){
    ...
    const API_ENDPOINT = '/api/work-operations';
    const JSON_HEADERS = {
    ...
    // Upload template (Work Operation schema)
    const UPLOAD_HEADERS_KO = [
        '업무 운영','설명','하드웨어(수량)','소프트웨어(수량)','비고'
    ];
    ...
    async function apiFetchOperations(){
        const data = await apiRequest(`${API_ENDPOINT}?t=${Date.now()}`);
        const items = Array.isArray(data?.items) ? data.items : [];
        return items.map(normalizeRecord).filter(Boolean);
    }
    async function apiCreateOperation(payload){
    ...
    async function apiUpdateOperation(id, payload){
    ...
    async function apiDeleteOperations(ids){
    ...
    async function reloadOperations(showError){
        state.isLoading = true;
        render();
        try {
            const rows = await apiFetchOperations();
            state.data = rows;
            ...
            render();
            if(showError !== false){
                showMessage(extractErrorMessage(err, '업무 운영 목록을 불러오지 못했습니다.'), '오류');
            }
        }
    }
    ...
    function generateCloneName(baseName, takenSet){
        ...
        let suffix = 2;
        while(takenSet.has(candidate.trim().toLowerCase())){
            candidate = `${trimmed}_COPY${suffix++}`;
        }
        takenSet.add(candidate.trim().toLowerCase());
        return candidate;
    }
    ...
    const COLUMN_MODAL_GROUPS = [
        { group: '업무 운영', columns: ['wc_name','wc_desc','hw_count','sw_count','note'] }
    ];

    /** 컬럼 메타 (라벨 + 그룹) */
    const COLUMN_META = {
        wc_name:{label:'업무 운영',group:'업무 운영'},
        wc_desc:{label:'설명',group:'업무 운영'},
        hw_count:{label:'하드웨어(수량)',group:'업무 운영'},
        sw_count:{label:'소프트웨어(수량)',group:'업무 운영'},
        note:{label:'비고',group:'업무 운영'}
    };
        ...
        if(state.filtered.length === 0){
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '업무 운영 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 업무 운영을 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
    ...
    const filename = `work_operation_list_${yyyy}${mm}${dd}.csv`;
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> { openModal(ADD_MODAL_ID); });
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            if(String(payload.wc_name).trim().length > BUSINESS_WORK_LABEL_MAX_LEN){
                showMessage(`업무 운영은 ${BUSINESS_WORK_LABEL_MAX_LEN}글자 이내로 입력해 주세요.`, '안내');
                return;
            }
            ...
            try {
                ...
                showMessage('업무 운영이 등록되었습니다.', '완료');
            } catch(err){
                showMessage(extractErrorMessage(err, '업무 운영 등록 중 오류가 발생했습니다.'), '오류');
            } finally {
    ...
    document.getElementById(EDIT_CLOSE_ID)?.addEventListener('click', ()=> closeModal(EDIT_MODAL_ID));
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try {
                ...
                await apiUpdateOperation(recordId, payload);
                closeModal(EDIT_MODAL_ID);
                await reloadOperations(false);
                showMessage('업무 운영이 수정되었습니다.', '완료');
            } catch(err){
                showMessage(extractErrorMessage(err, '업무 운영 수정 중 오류가 발생했습니다.'), '오류');
            } finally {
                setButtonLoading(button, false);
            }
        });
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById(UPLOAD_TEMPLATE_BTN_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                XLSX.writeFile(wb, 'work_operation_upload_template.xlsx');
                ...
                try{
                    ...
                    for(let r=1; r<rows.length; r++){
                        ...
                        if(wcTrim.length > BUSINESS_WORK_LABEL_MAX_LEN){
                            errors.push(`Row ${r+1}: 업무 운영은 ${BUSINESS_WORK_LABEL_MAX_LEN}글자 이내로 입력하세요.`);
                        }
                    ...
                    // Import rows
                    await persistImportedOperations(imported);
    ...
    async function persistImportedOperations(records){
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 업무 운영을 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 업무 운영을 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 업무 운영에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', async ()=>{
            ...
            const ids = [...state.selected].map(id=> parseInt(id, 10)).filter(id=> Number.isInteger(id) && id > 0);
            if(!ids.length){ showMessage('일괄변경할 행을 선택하세요.', '안내'); return; }
            ...
            try {
                ...
                for(const id of ids){
                    try {
                        await apiUpdateOperation(id, payload);
                    } catch(err){
    ...
    if(swEl) swEl.innerHTML = '';
        ...
        // Top 10 by 업무 운영 (sum of counts per 운영)
        const hwByClass = sumBy(rows, 'wc_name', 'hw_count');
        const swByClass = sumBy(rows, 'wc_name', 'sw_count');
        renderStatBlock('stats-software', '하드웨어 Top 10 (업무 운영)', hwByClass, null, { topN: 10, includeOther: false });
        renderStatBlock('stats-software', '소프트웨어 Top 10 (업무 운영)', swByClass, null, { topN: 10, includeOther: false });
    }
    }
