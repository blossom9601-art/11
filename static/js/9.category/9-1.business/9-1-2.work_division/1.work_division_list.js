...
(function(){
    ...
    const API_BASE_URL = '/api/work-divisions';
    const JSON_HEADERS = { 'Content-Type': 'application/json' };
    function normalizeCountValue(val){
    ...
    function normalizeRow(row){
        ...
        return {
            ...
            division_code: row.division_code ?? row.wc_code ?? null,
            wc_name: (row.wc_name ?? row.division_name ?? '').trim(),
            wc_desc: row.wc_desc ?? row.description ?? '',
            hw_count: normalizeCountValue(row.hw_count),
            sw_count: normalizeCountValue(row.sw_count),
    ...
    function preparePayload(data){
        const payload = {
            wc_name: (data.wc_name || data.division_name || '').trim(),
            wc_desc: data.wc_desc || data.description || '',
            hw_count: data.hw_count,
            sw_count: data.sw_count,
            note: data.note || data.remark || ''
        };
        ['hw_count','sw_count'].forEach(key=>{
            ...
            payload[key] = Number.isFinite(num) && num >= 0 ? num : null;
        });
        return payload;
    }
    function pruneEmptyPayload(payload){
        const clone = { ...payload };
        Object.keys(clone).forEach(key=>{
            const value = clone[key];
            const isNumber = typeof value === 'number';
            if(!isNumber && (value === undefined || value === null || value === '')){
                delete clone[key];
            }
        });
        return clone;
    ...
    async function apiListWorkDivisions(){
        ...
        if(!res.ok || !data || data.success === false){
            throw new Error((data && data.message) || '업무 구분 목록 조회에 실패했습니다.');
        }
        const items = Array.isArray(data.items) ? data.items : [];
        return items.map(normalizeRow).filter(Boolean);
    }
    async function apiCreateWorkDivision(payload){
        ...
        try { data = await res.json(); } catch(_e){ data = null; }
        if(!res.ok || !data || data.success === false){
            throw new Error((data && data.message) || '업무 구분 등록 중 오류가 발생했습니다.');
        }
        return normalizeRow(data.item || data);
    }
    async function apiUpdateWorkDivision(id, payload){
        ...
        try { data = await res.json(); } catch(_e){ data = null; }
        if(!res.ok || !data || data.success === false){
            throw new Error((data && data.message) || '업무 구분 수정 중 오류가 발생했습니다.');
        }
        return normalizeRow(data.item || data);
    }
    async function apiDeleteWorkDivisions(ids){
        ...
        try { data = await res.json(); } catch(_e){ data = null; }
        if(!res.ok || !data || data.success === false){
            throw new Error((data && data.message) || '업무 구분 삭제 중 오류가 발생했습니다.');
        }
    ...
    // Upload template (Work Classification schema)
    const UPLOAD_HEADERS_KO = [
    '업무 구분','설명','하드웨어(수량)','소프트웨어(수량)','비고'
    ];
    const HEADER_KO_TO_KEY = {
    // 업로드/매핑 호환: 업무 구분(신규), 업무 분류(레거시) 모두 wc_name으로 매핑
    '업무 구분':'wc_name',
    '업무 분류':'wc_name',
        '설명':'wc_desc',
        '하드웨어(수량)':'hw_count',
    ...
    { group: '업무 구분', columns: ['wc_name','wc_desc','hw_count','sw_count','note'] }
    ];

    /** 컬럼 메타 (라벨 + 그룹) */
    const COLUMN_META = {
    wc_name:{label:'업무 구분',group:'업무 구분'},
    wc_desc:{label:'설명',group:'업무 구분'},
    hw_count:{label:'하드웨어(수량)',group:'업무 구분'},
    sw_count:{label:'소프트웨어(수량)',group:'업무 구분'},
    note:{label:'비고',group:'업무 구분'}
    };
    ...
    async function initData(){
        ...
        render();
        try {
            const rows = await apiListWorkDivisions();
            state.data = rows;
        } catch(err){
            console.error(err);
            showMessage((err && err.message) || '업무 구분 데이터를 불러오지 못했습니다.', '오류');
            state.data = [];
        } finally {
        ...
        if(state.filtered.length === 0){
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '업무 구분 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 업무 구분을 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
    ...
    function loadColumnSelection(){
        ...
        state.visibleCols = new Set(BASE_VISIBLE_COLUMNS);
        try {
            localStorage.removeItem('work_division_visible_cols');
    ...
    function updateRow(index,data){
        ...
        if(data && data.id){
            state.data[index] = normalizeRow(data) || state.data[index];
        } else {
    ...
    const filename = `work_division_list_${yyyy}${mm}${dd}.csv`;
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> { openModal(ADD_MODAL_ID); });
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            if(String(payload.wc_name).trim().length > BUSINESS_WORK_LABEL_MAX_LEN){
                showMessage(`업무 구분은 ${BUSINESS_WORK_LABEL_MAX_LEN}글자 이내로 입력해 주세요.`, '안내');
                return;
            }
            ...
            try {
                ...
                showMessage('업무 구분이 등록되었습니다.', '완료');
            } catch(err){
                console.error(err);
                showMessage((err && err.message) || '업무 구분 등록 중 오류가 발생했습니다.', '오류');
            }
        });
        // edit modal
    document.getElementById(EDIT_CLOSE_ID)?.addEventListener('click', ()=> closeModal(EDIT_MODAL_ID));
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            if(index < 0 || !state.data[index]){ showMessage('수정할 업무 구분을 찾을 수 없습니다.', '오류'); return; }
            ...
            try {
                ...
                showMessage('업무 구분이 저장되었습니다.', '완료');
            } catch(err){
                console.error(err);
                showMessage((err && err.message) || '업무 구분 수정 중 오류가 발생했습니다.', '오류');
            }
        });
        // csv
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById(UPLOAD_TEMPLATE_BTN_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                // Set reasonable column widths for division sheet
                wsTemplate['!cols'] = UPLOAD_HEADERS_KO.map((h)=>{
                        const wide = ['업무 구분','설명','비고'];
                        const mid = ['하드웨어(수량)','소프트웨어(수량)'];
                ...
                XLSX.writeFile(wb, 'work_division_upload_template.xlsx');
                ...
                try{
                    ...
                    for(let r=1; r<rows.length; r++){
                        ...
                        if(wcTrim.length > BUSINESS_WORK_LABEL_MAX_LEN){
                            errors.push(`Row ${r+1}: 업무 구분은 ${BUSINESS_WORK_LABEL_MAX_LEN}글자 이내로 입력하세요.`);
                        }
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 업무 구분을 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 업무 구분을 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            if(ids.length === 0){ closeModal(DELETE_MODAL_ID); showMessage('삭제할 행을 선택하세요.', '안내'); return; }
            try {
                await apiDeleteWorkDivisions(ids);
                state.data = state.data.filter(r => !ids.includes(r.id));
                state.selected.clear();
                applyFilter();
                ...
                console.error(err);
                showMessage((err && err.message) || '삭제 처리 중 오류가 발생했습니다.', '오류');
            }
        });
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 업무 구분에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', async ()=>{
            ...
            entries.forEach(({field,value})=>{ baseFormValues[field] = value; });
            try {
                let success = 0;
                for(const id of ids){
                    const payload = pruneEmptyPayload(preparePayload(baseFormValues));
                    if(!Object.keys(payload).length) continue;
                    const rowIndex = state.data.findIndex(r=> r.id === id);
                    const updated = await apiUpdateWorkDivision(id, payload);
                    if(rowIndex !== -1){ updateRow(rowIndex, updated); }
                    success++;
                }
                closeModal(BULK_MODAL_ID);
                if(success){ showMessage(`${success}개 항목에 일괄 변경이 적용되었습니다.`, '완료'); }
            } catch(err){
                console.error(err);
                showMessage((err && err.message) || '일괄 변경 중 오류가 발생했습니다.', '오류');
            }
        });
        // esc close
    ...
    // Top 10 by 업무 그룹 (if available) or 업무 구분 as fallback
    const labelKey = rows.length && (rows[0].work_group != null) ? 'work_group' : 'wc_name';
    const labelName = (labelKey === 'work_group') ? '업무 그룹' : '업무 구분';
    const hwByLabel = sumBy(rows, labelKey, 'hw_count');
    const swByLabel = sumBy(rows, labelKey, 'sw_count');
    renderStatBlock('stats-software', `하드웨어 Top 10 (${labelName})`, hwByLabel, null, { topN: 10, includeOther: false });
    renderStatBlock('stats-software', `소프트웨어 Top 10 (${labelName})`, swByLabel, null, { topN: 10, includeOther: false });
    }
    }
