...
(function(){
    ...
    function normalizeCodeSeed(seed){
        const base = sanitizeString(seed || 'ASSOC').toUpperCase();
    ...
    function isAutoGeneratedCustomerCode(code, memberName){
        const value = sanitizeString(code).toUpperCase();
        if(!value) return false;
        const base = normalizeCodeSeed(memberName);
        const escapedBase = base.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        return new RegExp(`^${escapedBase}(?:_[0-9]+)?$`).test(value);
    }

    function normalizeMemberRow(item){
        if(!item) return null;
        const memberName = sanitizeString(item.member_name || item.customer_name);
        const rawCode = sanitizeString(item.customer_code || '');
        return {
            id: item.id,
    ...
    function toMemberPayload(data, opts){
        ...
        return {
            ...
            customer_code: sanitizeString(data.customer_code) || undefined,
    ...
    const BASE_VISIBLE_COLUMNS = [
        'member_name','customer_code','phone','address','line_qty'
    ];
    const COLUMN_ORDER = [
        'member_name','customer_code','phone','address','line_qty','note'
    ];

    // 컬럼 선택 모달 전용 사용자 정의 그룹/순서 (테이블 렌더 순서에는 영향 주지 않음)
    const COLUMN_MODAL_GROUPS = [
        { group: '준회원사', columns: ['member_name','customer_code','phone','address','line_qty'] }
    ];

    /** 컬럼 메타 (라벨 + 그룹) */
    const COLUMN_META = {
        member_name:{label:'준회원사',group:'준회원사'},
        customer_code:{label:'고객코드',group:'준회원사'},
        phone:{label:'대표번호',group:'준회원사'},
    ...
    function applyFilter(){
        ...
        const searchCols = ['member_name', 'customer_name', ...Object.keys(COLUMN_META)];
        // 1단계: 기본 검색
        let base = [];
        if(!groups.length){
            base = [...state.data];
    ...
    function render(highlightContext){
        ...
        if(!tbody) return;
        // SPA scope guard: skip if table doesn't belong to this page
        const _tbl = document.getElementById(TABLE_ID);
        if(_tbl && !_tbl.querySelector('thead th[data-col="member_name"]')) return;
        tbody.innerHTML='';
        ...
        if(state.filtered.length === 0){
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '고객 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 고객을 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
            ...
            if(link){
                link.addEventListener('click', (ev)=>{
                    ...
                    fetch('/api/category/detail-context', {
                        ...
                        else { blsSpaNavigate(fallbackHref); }
                    }).catch(function(){
    ...
    function applyColumnVisibility(){
        ...
        // SPA scope guard: skip if table doesn't belong to this page
        if(!table.querySelector('thead th[data-col="member_name"]')) return;
    ...
    function fillEditForm(row){
        ...
        const group = { title:'준회원사', cols:['member_name','customer_code','phone','address'] };
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> {
            ...
            const runner = (async ()=>{
                try{
                    ...
                    if(created){
                        ...
                        showMessage('고객이 등록되었습니다.', '완료');
                    }
                }catch(err){
                    console.error(err);
                    showMessage(err.message || '고객 등록 중 오류가 발생했습니다.', '오류');
                }
            })();
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', ()=>{
            ...
            if(!(targetId >= 0)){
                showMessage('수정할 고객을 찾을 수 없습니다.', '안내');
                return;
            }
            const data = collectForm(form);
            const runner = (async ()=>{
                try{
                    ...
                    showMessage('고객 정보가 저장되었습니다.', '완료');
                }catch(err){
                    console.error(err);
                    showMessage(err.message || '고객 수정 중 오류가 발생했습니다.', '오류');
                }
            })();
            withTableLoading(runner);
        });
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 고객을 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 고객을 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', ()=>{
            ...
            const runner = (async ()=>{
                try{
                    ...
                    console.error(err);
                    showMessage(err.message || '고객 삭제 중 오류가 발생했습니다.', '오류');
                }
            })();
            withTableLoading(runner);
        });
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 고객에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
