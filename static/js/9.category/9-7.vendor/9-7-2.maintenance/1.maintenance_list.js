...
(function(){
    ...
    const VENDOR_API_BASE = '/api/vendor-maintenance';
    ...
    const COLUMN_META = {
        ...
        call_center:{label:'고객센터',group:'유지보수사'},
        hardware_qty:{label:'하드웨어(수량)',group:'유지보수사'},
        software_qty:{label:'소프트웨어(수량)',group:'유지보수사'},
        component_qty:{label:'컴포넌트(수량)',group:'유지보수사'},
        note:{label:'비고',group:'유지보수사'}
    };
    ...
    function toMaintenancePayload(data, opts){
        const options = Object.assign({ requireName: false, omitEmpty: false, fillDefaultQuantities: true }, opts || {});
        const vendorName = sanitizeString(data.vendor || data.maintenance_name);
        if(options.requireName && !vendorName){
            throw new Error('유지보수사명을 입력하세요.');
        }
        const payload = {
        ...
        const assignQty = (key, sourceKey)=>{
            const raw = data[sourceKey || key];
        ...
        assignQty('manager_count', 'manager_count');
        assignQty('hardware_qty', 'hardware_qty');
        assignQty('software_qty', 'software_qty');
        assignQty('component_qty', 'component_qty');
    ...
    function normalizeMaintenanceRow(item){
        if(!item) return null;
        return {
            id: item.id,
            ...
            manager_count: item.manager_count ?? 0,
    ...
    async function loadMaintenanceVendors(options){
        ...
        const promise = (async () => {
            try{
                ...
                const items = Array.isArray(data?.items) ? data.items.map(normalizeMaintenanceRow).filter(Boolean) : [];
                state.data = items;
                ...
                console.error(err);
                if(!opts.silent){
                    showMessage(err.message || '유지보수사 목록을 가져오는 중 문제가 발생했습니다.', '오류');
                }
            }
        })();
    ...
    async function createMaintenanceRecord(formData){
        const payload = toMaintenancePayload(formData, { requireName: true });
        const data = await apiRequest(VENDOR_API_BASE, { method: 'POST', body: payload });
        return data?.item || null;
    }

    async function updateMaintenanceRecord(targetId, formData){
        if(!targetId && targetId !== 0) throw new Error('수정 대상이 없습니다.');
        const payload = toMaintenancePayload(formData, { requireName: false });
        const data = await apiRequest(`${VENDOR_API_BASE}/${targetId}`, { method: 'PUT', body: payload });
        return data?.item || null;
    }

    async function deleteMaintenanceRecords(ids){
    ...
    // 유지보수사는 서버 데이터를 기반으로 렌더링하며, 더 이상 mock 데이터를 사용하지 않습니다.
        ...
        if(state.filtered.length === 0){
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '유지보수사 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 유지보수사를 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
        ...
        function withQueryParam(url, key, value){
            try{
                ...
                try{
                    ...
                    return String(url) + sep + encodeURIComponent(String(key)) + '=' + encodeURIComponent(String(value));
                }catch(_){
                    return url;
                }
            }
    ...
    function saveColumnSelection(){
        try { localStorage.setItem('maintenance_visible_cols', JSON.stringify([...state.visibleCols])); } catch(e){}
    }
    function loadColumnSelection(){
        try {
            const raw = localStorage.getItem('maintenance_visible_cols');
            ...
            if(filtered.length > 0){
                ...
                try { localStorage.setItem('maintenance_visible_cols', JSON.stringify([...state.visibleCols])); } catch(_e){}
            } else {
    ...
    function saveSortPreference(){
        try{
            if(state.sortKey){
                localStorage.setItem('maintenance_sort_key', state.sortKey);
                localStorage.setItem('maintenance_sort_dir', state.sortDir==='desc' ? 'desc' : 'asc');
            } else {
                localStorage.removeItem('maintenance_sort_key');
                localStorage.removeItem('maintenance_sort_dir');
            }
        }catch(e){}
    }
    function loadSortPreference(){
        try{
            const key = localStorage.getItem('maintenance_sort_key');
            const dir = localStorage.getItem('maintenance_sort_dir');
    ...
    function generateFieldInput(col,value=''){
        if(col==='manager_count' || col==='hardware_qty' || col==='software_qty' || col==='component_qty'){
    ...
    // CRUD는 서버 API를 사용하므로, 개별 add/update 헬퍼는 사용하지 않습니다.
    ...
    function exportCSV(onlySelected){
        ...
        // Dynamic filename: maintenance_list_YYYYMMDD.csv (local date)
        const d = new Date();
        const yyyy = d.getFullYear();
        const mm = String(d.getMonth()+1).padStart(2,'0');
        const dd = String(d.getDate()).padStart(2,'0');
    const filename = `maintenance_list_${yyyy}${mm}${dd}.csv`;
    ...
    function bindEvents(){
        ...
        if(pageSizeSel){
            pageSizeSel.addEventListener('change', e=>{
                ...
                try { localStorage.setItem('maintenance_page_size', String(state.pageSize)); } catch(err){}
                state.page=1; render();
            });
        }
    ...
    tbodyEl?.addEventListener('click', e=>{
            ...
            if(e.target.closest('.work-name-link')) return; // 링크 클릭 시 토글 금지 (제조사 페이지와 동일)
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> {
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                console.error(err);
                showMessage(err.message || '유지보수사 등록 중 오류가 발생했습니다.', '오류');
            }finally{
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                showMessage('유지보수사 정보가 수정되었습니다.', '완료');
            }catch(err){
                console.error(err);
                showMessage(err.message || '유지보수사 수정 중 오류가 발생했습니다.', '오류');
            }finally{
                if(btn) btn.disabled = false;
            }
        });
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        wsTemplate['!cols'] = UPLOAD_HEADERS_KO.map((h)=>{
            ...
            return { wch: 12 };
                ...
                XLSX.writeFile(wb, 'maintenance_upload_template.xlsx');
            ...
            reader.onload = async ()=>{
                try{
                    ...
                    for(let r=1; r<rows.length; r++){
                        ...
                        if(rec.manager_count !== '' && !isIntegerLike(rec.manager_count)) errors.push(`Row ${r+1}: 담당자수는 숫자만 입력하세요.`);
                        ...
                        // Normalize numbers
                        rec.manager_count = toIntOrBlank(rec.manager_count);
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', async ()=>{
            ...
            try{
                for(const source of originals){
                    ...
                    await createMaintenanceRecord(copy);
                }
                await loadMaintenanceVendors({ silent: true });
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 유지보수사를 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 유지보수사를 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            if(btn) btn.disabled = true;
            try{
                await deleteMaintenanceRecords(ids);
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 유지보수사에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', async ()=>{
            ...
            const payload = toMaintenancePayload(payloadBase, { requireName: false, omitEmpty: true, fillDefaultQuantities: false });
    ...
    const checkEl = document.getElementById('stats-check');
        ...
        // 유지보수사 통계
        renderStatBlock('stats-software', '지역(주소 1단어)', rows.reduce((acc, r)=>{ const a=(r.address||'').trim(); if(!a) return acc; const g=a.split(/\s+/)[0]||'기타'; acc[g]=(acc[g]||0)+1; return acc; }, {}));
        renderStatBlock('stats-versions', '담당자수 분포', rows.reduce((acc, r)=>{ const n=parseInt(r.manager_count||'0',10)||0; const b=(n===0?'0':(n<=3?'1-3':(n<=5?'4-5':'6+'))); acc[b]=(acc[b]||0)+1; return acc; }, {}));
        renderStatBlock('stats-check', '자산 수량(합산) 분포', rows.reduce((acc, r)=>{ const n=(parseInt(r.hardware_qty||'0',10)||0)+(parseInt(r.software_qty||'0',10)||0)+(parseInt(r.component_qty||'0',10)||0); const b=(n===0?'0':(n<=50?'1-50':(n<=100?'51-100':'100+'))); acc[b]=(acc[b]||0)+1; return acc; }, {}));
    }
    }
    ...
    async function init(){
        ...
        try {
            const psRaw = localStorage.getItem('maintenance_page_size');
            if(psRaw){
