...
(function(){
    ...
    const VENDOR_API_BASE = '/api/vendor-manufacturers';
    ...
    // Upload template (Manufacturer schema)
    const UPLOAD_HEADERS_KO = [
        '로고','제조사','주소','사업자번호','고객센터','하드웨어(수량)','소프트웨어(수량)','컴포넌트(수량)','비고'
    ];
    const HEADER_KO_TO_KEY = {
        ...
        '고객센터':'call_center',
        '하드웨어(수량)':'hardware_qty',
        '소프트웨어(수량)':'software_qty',
        '컴포넌트(수량)':'component_qty',
        '비고':'note'
    };
    const ENUM_SETS = { };
    ...
    const COLUMN_MODAL_GROUPS = [
        { group: '제조사', columns: ['logo','vendor','address','business_number','call_center','hardware_qty','software_qty','component_qty'] }
    ];

    /** 컬럼 메타 (라벨 + 그룹) */
    const COLUMN_META = {
        logo:{label:'로고',group:'제조사'},
        vendor:{label:'제조사',group:'제조사'},
        address:{label:'주소',group:'제조사'},
        business_number:{label:'사업자번호',group:'제조사'},
        call_center:{label:'고객센터',group:'제조사'},
        hardware_qty:{label:'하드웨어(수량)',group:'제조사'},
        software_qty:{label:'소프트웨어(수량)',group:'제조사'},
        component_qty:{label:'컴포넌트(수량)',group:'제조사'},
        note:{label:'비고',group:'제조사'}
    };
    ...
    function withTableLoading(promise){
        globalLoadingDepth++;
        toggleTableLoading();
        const finalize = ()=>{ globalLoadingDepth = Math.max(0, globalLoadingDepth - 1); toggleTableLoading(); };
    ...
    function toVendorPayload(data, opts){
        const options = Object.assign({ requireName: false, omitEmpty: false, fillDefaultQuantities: true }, opts || {});
        const vendorName = sanitizeString(data.vendor || data.manufacturer_name);
        if(options.requireName && !vendorName){
            throw new Error('제조사명을 입력하세요.');
        }
        const payload = {
            vendor: vendorName || undefined,
            manufacturer_name: vendorName || undefined,
            ...
            call_center: sanitizeString(data.call_center),
            note: sanitizeString(data.note),
            remark: sanitizeString(data.remark)
        };

        function assignQuantity(key){
            const raw = data[key];
            const trimmed = raw == null ? '' : String(raw).trim();
            if(trimmed === '' && !options.fillDefaultQuantities){
                return;
            }
            payload[key] = sanitizeQuantity(raw);
        }
        assignQuantity('hardware_qty');
        assignQuantity('software_qty');
        assignQuantity('component_qty');

        if(options.omitEmpty){
            Object.keys(payload).forEach(key => {
                const value = payload[key];
                const isNumber = typeof value === 'number';
                if(value === undefined || value === null) delete payload[key];
                else if(!isNumber && value === '') delete payload[key];
            });
        }
        return payload;
    }
    ...
    function normalizeVendorRow(item){
        ...
        return {
            ...
            hardware_qty: item.hardware_qty ?? item.hw_count ?? 0,
            software_qty: item.software_qty ?? item.sw_count ?? 0,
            component_qty: item.component_qty ?? item.component_count ?? 0,
    ...
    async function loadVendors(options){
        ...
        const promise = (async () => {
            try{
                const data = await apiRequest(VENDOR_API_BASE);
                const items = Array.isArray(data?.items) ? data.items.map(normalizeVendorRow).filter(Boolean) : [];
                state.data = items;
                ...
                console.error(err);
                if(!opts.silent){
                    showMessage(err.message || '제조사 목록을 가져오는 중 문제가 발생했습니다.', '오류');
                }
            }
        })();
        return withTableLoading(promise);
    }

    async function createVendorRecord(formData){
        const payload = toVendorPayload(formData, { requireName: true });
        const data = await apiRequest(VENDOR_API_BASE, { method: 'POST', body: payload });
        return data?.item || null;
    }

    async function updateVendorRecord(targetId, formData){
        if(!targetId && targetId !== 0) throw new Error('수정 대상이 없습니다.');
        const payload = toVendorPayload(formData, { requireName: false });
        const data = await apiRequest(`${VENDOR_API_BASE}/${targetId}`, { method: 'PUT', body: payload });
        return data?.item || null;
    }

    async function deleteVendorRecords(ids){
    ...
    function render(highlightContext){
        ...
        if(state.filtered.length === 0){
            if(emptyEl){
                ...
                if(state.search.trim()){
                    ...
                    if(titleEl) titleEl.textContent = '제조사 내역이 없습니다.';
                    if(descEl) descEl.textContent = "우측 상단 '추가' 버튼을 눌러 첫 제조사를 등록하세요.";
                }
            }
        } else if(emptyEl){
            // 데이터가 존재하면 항상 숨김
        ...
        slice.forEach((row)=>{
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    // linkify vendor to detail page
                    if(col==='vendor' && displayVal !== '-' ){
                        // Clean URL: no id in query string — session stores vendor_id server-side
                        const publicId = String(row.public_id || '').trim();
                        const injected = publicId ? `/b/${encodeURIComponent(publicId)}` : (window.__CAT_VENDOR_MANUFACTURER_DETAIL_URL || '/b/cat_vendor_manufacturer_detail');
                        cellValue = `<a href="${injected}" class="work-name-link" data-id="${row.id??''}" data-public-id="${row.public_id??''}" title="제조사 상세로 이동">${cellValue}</a>`;
                    }
            ...
            // Persist context for detail page (title/subtitle & basic fields) on link click
                ...
                link.addEventListener('click', (ev)=>{
                    ...
                    try{
                        const ctx = {
                            ...
                            hardware_qty: row.hardware_qty,
                            software_qty: row.software_qty,
                            component_qty: row.component_qty,
                            note: row.note
                        };
                        sessionStorage.setItem('manufacturer:context', JSON.stringify(ctx));
                    }catch(_){ /* ignore */ }
                    // Store vendor_id in server session then navigate
        ...
        // Fetch live asset counts for vendors on current page
        fetchLiveQtyCounts(slice);
    }

    /* Fetch real hw/sw/comp counts from asset APIs for each vendor in the visible page,
       then patch both state.data and the rendered table cells so list and detail pages
       always show the same live numbers. */
    let _liveQtyGeneration = 0;
    function fetchLiveQtyCounts(slice){
        const gen = ++_liveQtyGeneration;
        if(!slice || !slice.length) return;
        slice.forEach(row => {
            if(!row.id) return;
            const vid = row.id;
            const base = `${VENDOR_API_BASE}/${vid}`;
            let hwCount = 0, swCount = 0, compCount = 0, done = 0;
            function apply(){
                done++;
                if(done < 3) return;
                if(gen !== _liveQtyGeneration) return; // stale page
                // Patch state.data so sort/export uses fresh values
                const item = state.data.find(r => r.id === vid);
                if(item){ item.hardware_qty = hwCount; item.software_qty = swCount; item.component_qty = compCount; }
                // Patch DOM cells
                const tr = document.querySelector(`tr[data-id="${vid}"]`);
                if(tr){
                    const hwTd = tr.querySelector('td[data-col="hardware_qty"]');
                    const swTd = tr.querySelector('td[data-col="software_qty"]');
                    const cmpTd = tr.querySelector('td[data-col="component_qty"]');
                    if(hwTd) hwTd.textContent = hwCount || '-';
                    if(swTd) swTd.textContent = swCount || '-';
                    if(cmpTd) cmpTd.textContent = compCount || '-';
                }
            }
            fetch(base + '/hw-assets', {credentials:'same-origin'}).then(r=>r.json()).then(d=>{ if(d?.success) hwCount = d.total || (d.items||[]).length; }).catch(()=>{}).then(apply);
            fetch(base + '/sw-assets', {credentials:'same-origin'}).then(r=>r.json()).then(d=>{ if(d?.success) swCount = d.total || (d.items||[]).length; }).catch(()=>{}).then(apply);
            fetch(base + '/comp-assets', {credentials:'same-origin'}).then(r=>r.json()).then(d=>{ if(d?.success) compCount = d.total || (d.items||[]).length; }).catch(()=>{}).then(apply);
        });
    }
    ...
    function renderVendorLogo(url, altText){
        ...
        if(!src) return '-';
        const alt = sanitizeString(altText) || '제조사 로고';
    ...
    function applyColumnVisibility(){
        ...
        table.querySelectorAll('thead th[data-col], tbody td[data-col]').forEach(cell=>{
            ...
            if(ALWAYS_HIDDEN_TABLE_COLS.has(col)){
    ...
    function loadColumnSelection(){
        try {
            ...
            if(filtered.length > 0){
                ...
                // Ensure new default columns are visible (union with defaults)
                BASE_VISIBLE_COLUMNS.forEach(col => merged.add(col));
                // Enforce hidden columns never appear in table
                ALWAYS_HIDDEN_TABLE_COLS.forEach(col => merged.delete(col));
                state.visibleCols = merged;
                // persist sanitized/merged version
    ...
    function fillEditForm(row){
        ...
        form.innerHTML='';
        // 수량 필드는 모달에서는 숨김(화면 테이블에는 유지). 값은 hidden으로 유지해서 저장 시 보존.
        ['hardware_qty','software_qty','component_qty'].forEach(name=>{
            const hid = document.createElement('input');
            hid.type = 'hidden';
            hid.name = name;
            hid.value = String(row?.[name] ?? 0);
            form.appendChild(hid);
        });

    const group = { title:'제조사', cols:['logo','vendor','address','business_number','call_center'] };
    ...
    function exportCSV(onlySelected){
        // CSV 다운로드는 숨김 컬럼(주소/비고) 포함, 전체 컬럼을 내보냄
        const exportCols = COLUMN_ORDER.filter(c=>COLUMN_META[c]);
        const headers = ['No', ...exportCols.map(c=>COLUMN_META[c].label)];
        ...
        const rows = dataForCsv.map((r,i)=> [i+1, ...exportCols.map(c=> r[c]??'')]);
    ...
    const filename = `manufacturer_list_${yyyy}${mm}${dd}.csv`;
    ...
    tbodyEl?.addEventListener('click', e=>{
            ...
            if(e.target.closest('.work-name-link')) return; // 링크 클릭 시 토글 금지 (서버 모델명과 동일 스타일)
    ...
    document.getElementById(ADD_BTN_ID)?.addEventListener('click', ()=> {
            ...
            ['hardware_qty','software_qty','component_qty'].forEach(name=>{
                const el = document.querySelector(`#${ADD_FORM_ID} [name="${name}"]`);
                if(el){
                    el.classList.add('qty-dashed-lock');
                    if(!el.getAttribute('placeholder')) el.setAttribute('placeholder','0');
        ...
        document.getElementById(ADD_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            try{
                ...
                await createVendorRecord(data);
                await loadVendors({ silent: true });
                form.reset();
                resetLogoFieldVisuals(form);
                closeModal(ADD_MODAL_ID);
                showMessage('제조사가 등록되었습니다.', '완료');
            }catch(err){
                console.error(err);
                showMessage(err.message || '제조사 등록 중 오류가 발생했습니다.', '오류');
            }finally{
        ...
        document.getElementById(EDIT_SAVE_ID)?.addEventListener('click', async ()=>{
            ...
            if(!Number.isFinite(id) || id < 0){ showMessage('수정할 제조사를 찾을 수 없습니다.', '안내'); return; }
            if(btn) btn.disabled = true;
            try{
                const data = await collectForm(form);
                await updateVendorRecord(id, data);
                await loadVendors({ silent: true });
                closeModal(EDIT_MODAL_ID);
                showMessage('제조사 정보가 수정되었습니다.', '완료');
            }catch(err){
                console.error(err);
                showMessage(err.message || '제조사 수정 중 오류가 발생했습니다.', '오류');
            }finally{
                if(btn) btn.disabled = false;
            }
        });
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        wsTemplate['!cols'] = UPLOAD_HEADERS_KO.map((h)=>{
            const wide = ['제조사','주소','비고'];
            const mid = ['사업자번호','고객센터'];
            ...
            return { wch: 12 };
                ...
                XLSX.writeFile(wb, 'manufacturer_upload_template.xlsx');
            ...
            reader.onload = async ()=>{
                try{
                    ...
                    for(let r=1; r<rows.length; r++){
                        ...
                        // Validation rules (manufacturer)
                        if(rec.hardware_qty !== '' && !isIntegerLike(rec.hardware_qty)) errors.push(`Row ${r+1}: 하드웨어(수량)은 숫자만 입력하세요.`);
                        if(rec.software_qty !== '' && !isIntegerLike(rec.software_qty)) errors.push(`Row ${r+1}: 소프트웨어(수량)은 숫자만 입력하세요.`);
                        if(rec.component_qty !== '' && !isIntegerLike(rec.component_qty)) errors.push(`Row ${r+1}: 컴포넌트(수량)은 숫자만 입력하세요.`);
                        // Normalize numbers
                        rec.hardware_qty = toIntOrBlank(rec.hardware_qty);
                        rec.software_qty = toIntOrBlank(rec.software_qty);
                        rec.component_qty = toIntOrBlank(rec.component_qty);
                        imported.push(rec);
                    }
                    if(errors.length){
                    ...
                    try{
                        for(const entry of imported){
                            ...
                            const rowNo = payload.__row;
                            delete payload.__row;
                            try{
                                await createVendorRecord(payload);
                            }catch(err){
                                const ctx = rowNo ? ` (엑셀 ${rowNo}행)` : '';
                                throw new Error((err.message || '업로드 중 오류가 발생했습니다.') + ctx);
                            }
                        }
                        await loadVendors({ silent: true });
                        ...
                        console.error(err);
                        showMessage(err.message || '업로드 처리 중 오류가 발생했습니다.', '업로드 오류');
                    }finally{
                        if(confirmBtn) confirmBtn.disabled = false;
                    }
        ...
        document.getElementById('system-duplicate-confirm')?.addEventListener('click', async ()=>{
            ...
            try{
                for(const source of originals){
                    const copy = {...source};
                    delete copy.id;
                    copy.vendor = copy.vendor ? `${copy.vendor}_COPY` : 'COPY';
                    await createVendorRecord(copy);
                }
                await loadVendors({ silent: true });
        ...
        document.getElementById(DISPOSE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 제조사를 정말 불용처리하시겠습니까?`; }
    ...
    document.getElementById(DISPOSE_CLOSE_ID)?.addEventListener('click', ()=> closeModal(DISPOSE_MODAL_ID));
        document.getElementById(DISPOSE_CONFIRM_ID)?.addEventListener('click', ()=>{
            ...
            const fields = ['logo','logo_url','vendor','address','business_number','call_center','hardware_qty','software_qty','component_qty'];
        ...
        document.getElementById(DELETE_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 제조사를 정말 삭제처리하시겠습니까?`; }
        ...
        document.getElementById(DELETE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            ...
            if(!ids.length){ closeModal(DELETE_MODAL_ID); showMessage('삭제할 항목을 먼저 선택하세요.', '안내'); return; }
        ...
        document.getElementById(BULK_BTN_ID)?.addEventListener('click', ()=>{
            ...
            if(subtitle){ subtitle.textContent = `선택된 ${count}개의 제조사에서 지정한 필드를 일괄 변경합니다.`; }
            buildBulkForm();
        ...
        document.getElementById(BULK_APPLY_ID)?.addEventListener('click', async ()=>{
            ...
            const payloadBase = {};
            entries.forEach(({field, value})=>{ payloadBase[field] = value; });
            const payload = toVendorPayload(payloadBase, { requireName: false, omitEmpty: true, fillDefaultQuantities: false });
            if(!Object.keys(payload).length){ showMessage('적용할 변경 내용을 확인하세요.', '안내'); return; }
            const ids = [...state.selected];
            if(!ids.length){ showMessage('대상 항목을 선택하세요.', '안내'); return; }
            const btn = document.getElementById(BULK_APPLY_ID);
            if(btn) btn.disabled = true;
            try{
                for(const id of ids){
                    await updateVendorRecord(id, payload);
                }
                await loadVendors({ silent: true });
    ...
    function buildBulkForm(){
        ...
        function inputFor(col){
            if(col === 'hardware_qty' || col === 'software_qty' || col === 'component_qty') return `<input type="number" min="0" step="1" class="form-input qty-dashed-lock" data-bulk-field="${col}" placeholder="0">`;
    ...
    const GROUP = { title:'제조사', cols:['logo','vendor','address','business_number','call_center','hardware_qty','software_qty','component_qty'] };
    ...
    function buildStats(){
        ...
        // 하드웨어, 소프트웨어, 컴포넌트별 제조사 비율 (Top5 + 기타)
        function vendorDist(qtyKey){
            const dist = {};
            rows.forEach(r => {
                const n = parseInt(r[qtyKey] || '0', 10) || 0;
                if(n <= 0) return;
                const name = (r.vendor || r.manufacturer_name || '-').trim() || '-';
                dist[name] = (dist[name] || 0) + n;
            });
            return dist;
        }
        renderStatBlock('stats-ratio', '하드웨어별 제조사 비율', vendorDist('hardware_qty'));
        renderStatBlock('stats-ratio', '소프트웨어별 제조사 비율', vendorDist('software_qty'));
        renderStatBlock('stats-ratio', '컴포넌트별 제조사 비율', vendorDist('component_qty'));
        // illustration card
        try {
            if (ratioEl) {
