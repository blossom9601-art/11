/*
 * tab43-hardware.js  v2.0
 * Vendor manufacturer – 하드웨어 탭 (read-only asset lookup).
 * server_hw_component(type='시스템') JOIN hardware WHERE vendor = 제조사명.
 * Columns: 구분, 유형, 모델명, 용량, 업무명, 시스템명, 할당수량, 비고
 */
(function () {
	'use strict';
	...
	ready(function () {
		...
		if (ctx !== 'vendor-hw-assets') return;
		...
		(function () {
			try {
				var saved = localStorage.getItem('vendor:hw-assets:pageSize');
				if (pageSizeSel && saved && ['10', '20', '50', '100'].indexOf(saved) > -1) { state.pageSize = parseInt(saved, 10); pageSizeSel.value = saved; }
				if (pageSizeSel) pageSizeSel.addEventListener('change', function () { var v = parseInt(pageSizeSel.value, 10); if (!isNaN(v)) { state.page = 1; state.pageSize = v; localStorage.setItem('vendor:hw-assets:pageSize', String(v)); renderPage(); } });
			} catch (_) { }
		})();
		...
		function loadData() {
			...
			fetch('/api/vendor-manufacturers/' + vendorId + '/hw-assets', { credentials: 'same-origin' })
		...
		if (csvConfirm) csvConfirm.addEventListener('click', function () {
			...
			var header = [escapeCSV('구분'), escapeCSV('유형'), escapeCSV('모델명'), escapeCSV('업무명'), escapeCSV('시스템명'), escapeCSV('할당수량')].join(',');
			var lines = [header];
			list.forEach(function (tr) {
				function cell(col) { var td = tr.querySelector('[data-col="' + col + '"]'); return td ? (td.textContent || '').trim() : ''; }
				lines.push([escapeCSV(cell('category')), escapeCSV(cell('type')), escapeCSV(cell('model')), escapeCSV(cell('work_name')), escapeCSV(cell('system_name')), escapeCSV(cell('qty'))].join(','));
			});
			downloadCSV('manufacturer_hardware_assets.csv', lines);
		...
		function renderCatContent(catData, catName) {
			...
			types.forEach(function (type) {
				...
				models.forEach(function (m, i) {
					if (i < 9) { segs.push({ name: m, count: td.models[m] }); }
					else { etcCount += td.models[m]; }
				});
