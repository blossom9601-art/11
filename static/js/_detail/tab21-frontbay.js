...
(function(){
	...
	function init(){
		...
		var _root21 = document.querySelector('.tab21-bay-root') || document.querySelector('.main-content');
		var _rootData21 = (_root21 && _root21.dataset) ? _root21.dataset : {};

		var typeOptions = (_rootData21.typeOptions && _rootData21.typeOptions.trim())
			? _rootData21.typeOptions.split(',').map(function(s){ return s.trim(); })
			: ['서버','스토리지'];
		var hasSpecCol = false;
		var bayCount = (_rootData21.bayCount && parseInt(_rootData21.bayCount, 10) > 0)
			? parseInt(_rootData21.bayCount, 10) : 16;

		
		var storagePrefix = (_rootData21.storagePrefix && _rootData21.storagePrefix.trim())
			? _rootData21.storagePrefix.trim()
		...
		var bayLogTag = 'tab21-frontbay';
		var _apiBase21 = (_rootData21.apiBase && _rootData21.apiBase.trim())
			? _rootData21.apiBase.trim() : '/api/hw-frame-frontbay';
		function apiBasePath(){ return _apiBase21; }

		function getRowId(tr){
			if(!tr) return null;
			var v = tr.dataset.frontbayId || tr.getAttribute('data-frontbay-id');
			var n = parseInt(v, 10);
		...
		function setRowId(tr, id){
			try{
				...
				tr.dataset.frontbayId = String(n);
				tr.setAttribute('data-frontbay-id', String(n));
			}catch(_){ }
		}
...
pageSize:10 };
		...
		var _typeToAssetType = { '서버': 'ON_PREMISE', 'SAN': 'SWITCH,DIRECTOR' };
	...
	window.BlossomTab21Frontbay = { init: init };
