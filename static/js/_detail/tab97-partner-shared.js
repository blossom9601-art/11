/*
 * tab97-partner-shared.js
 * Vendor partner manager tab behavior.
 */
...
(function(){
	...
	function ready(fn){
		...
		document.addEventListener('blossom:pageLoaded', function(){
			...
			|| document.body.classList.contains('page-vpn-manager')
			|| document.body.classList.contains('page-dedicatedline-manager')){
				fn();
			}
		});
	}
	...
	function isVendorPage(){
		...
		return !!(cls && (cls.contains('page-vendor-manufacturer-manager') || cls.contains('page-vendor-maintenance-manager')));
	}
	...
	function inferConfig(){
		...
		if(isVpn){
			return {
				...
				id: getQueryParamInt(['vpn_line_id','vpnLineId','line_id','lineId','id']) || parseInt(body.getAttribute('data-gov-detail-id'),10) || 0,
				includeActorUserId: false,
		...
		if(isDedicated){
			return {
				...
				id: getQueryParamInt(['id','line_id','lineId']) || parseInt(body.getAttribute('data-gov-detail-id'),10) || 0,
