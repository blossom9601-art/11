(function(){
  ...
  ready(async function(){
    ...
    const addBtn = document.getElementById('ad-fqdn-add');
    const emptyEl = document.getElementById('ad-fqdn-empty');
    const table = document.getElementById('ad-fqdn-table');
    const tbody = table ? table.querySelector('tbody') : null;

    const modalClose = document.getElementById('ad-fqdn-close');
    const modalSave = document.getElementById('ad-fqdn-save');
    const modalTitle = document.getElementById('ad-fqdn-modal-title');
    const form = document.getElementById('ad-fqdn-form');
    ...
    function fillForm(item){
      ...
      set('fqdn', item.fqdn);
      set('ip_address', item.ip_address);
      set('role', item.role);
    ...
    function render(){
      ...
      for(const it of items){
        ...
        tr.innerHTML = `
          <td><input type="checkbox" class="ad-fqdn-row" data-id="${escapeHtml(it.fqdn_id)}" aria-label="선택"></td>
          <td>${escapeHtml(it.status || '-') }</td>
          <td>${escapeHtml(it.domain_name || '-') }</td>
          <td>${escapeHtml(it.fqdn || '-') }</td>
          <td>${escapeHtml(it.ip_address || '-') }</td>
          <td>${escapeHtml(it.role || '-') }</td>
          <td>${escapeHtml((it.purpose || it.remark) ? `${it.purpose || ''}${(it.purpose && it.remark) ? ' / ' : ''}${it.remark || ''}` : '-') }</td>
          <td>
            <button type="button" class="btn-primary" data-action="edit" data-id="${escapeHtml(it.fqdn_id)}">수정</button>
            <button type="button" class="btn-secondary" data-action="delete" data-id="${escapeHtml(it.fqdn_id)}">삭제</button>
          </td>
        `;
    ...
    async function refreshList(){
      ...
      const data = await apiRequest(`/api/network/ad/${encodeURIComponent(adId)}/fqdns`, { method: 'GET' });
    ...
    function openCreate(){
      editingId = null;
      if(modalTitle) modalTitle.textContent = '도메인 항목 추가';
      resetForm();
      openModal('ad-fqdn-modal');
    }

    function openEdit(fqdnId){
      const item = items.find(x => String(x.fqdn_id) === String(fqdnId));
      if(!item){ showError('대상을 찾을 수 없습니다.'); return; }
      editingId = item.fqdn_id;
      if(modalTitle) modalTitle.textContent = '도메인 항목 수정';
      fillForm(item);
      openModal('ad-fqdn-modal');
    }
    ...
    async function save(){
      ...
      try{
        if(editingId){
          await apiRequest(`/api/network/ad/fqdns/${encodeURIComponent(editingId)}`, {
            method: 'PUT',
            body: JSON.stringify(payload),
          });
        } else {
          await apiRequest(`/api/network/ad/${encodeURIComponent(adId)}/fqdns`, {
            method: 'POST',
            body: JSON.stringify(payload),
          });
        }
        closeModal('ad-fqdn-modal');
        await refreshList();
      }catch(err){
    ...
    async function remove(fqdnId){
      const confirmed = await confirmMessage('삭제하시겠습니까?', '삭제 확인');
      if(!confirmed) return;
      try{
        await apiRequest(`/api/network/ad/fqdns/${encodeURIComponent(fqdnId)}`, { method: 'DELETE' });
        await refreshList();
      }catch(err){
    ...
    if(modalClose){
      modalClose.addEventListener('click', function(){ closeModal('ad-fqdn-modal'); });
    }
    if(addBtn){
