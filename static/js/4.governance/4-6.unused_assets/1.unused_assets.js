...
(function(){
    ...
    // ── Tab configuration for client-side switching ──
    var TAB_CONFIG = {
        gov_unused_server:  { label: '서버',    title: 'Server',  colKey: 'unused_server_visible_cols', category: 'server' },
        gov_unused_storage: { label: '스토리지', title: 'Storage', colKey: 'unused_storage_visible_cols', category: 'storage' },
        gov_unused_san:     { label: 'SAN',     title: 'SAN',     colKey: 'unused_san_visible_cols', category: 'san' },
        gov_unused_network: { label: '네트워크', title: 'Network', colKey: 'unused_network_visible_cols', category: 'network' },
        gov_unused_security:{ label: '보안장비', title: 'Security',colKey: 'unused_security_visible_cols', category: 'security' }
    };
    var currentTabKey = window.__unusedCurrentKey || 'gov_unused_server';
    // Normalize backward-compat key
    if(currentTabKey === 'gov_unused_hardware') currentTabKey = 'gov_unused_server';
    if(!TAB_CONFIG[currentTabKey]) currentTabKey = 'gov_unused_server';

    function getCurrentAssetLabel(){ return TAB_CONFIG[currentTabKey].label; }
    function getCurrentTabTitle(){ return TAB_CONFIG[currentTabKey].title; }
    function getCurrentCategory(){ return TAB_CONFIG[currentTabKey].category; }

    /** Map API row fields to display keys used by COLUMN_META */
    function apiRowToDisplay(r){
        return {
            id: r.id,
            work_type: r.work_category_name || r.work_category_code || '',
            work_category: r.work_division_name || r.work_division_code || '',
            work_status: r.work_status_name || r.work_status_code || '',
            work_operation: r.work_operation_name || r.work_operation_code || '',
            work_group: r.work_group_name || r.work_group_code || '',
            work_name: r.work_name || '',
            system_name: r.system_name || r.asset_name || '',
            system_ip: r.system_ip || r.mgmt_ip || '',
            manage_ip: r.mgmt_ip || '',
            vendor: r.manufacturer_name || r.manufacturer_code || '',
            model: r.model_name || r.server_code || '',
            serial: r.serial_number || '',
            virtualization: r.virtualization_type || '',
            location_place: r.center_name || r.center_code || '',
            location_pos: r.rack_name || r.rack_code || '',
            slot: r.system_slot != null ? String(r.system_slot) : '',
            u_size: r.system_size != null ? String(r.system_size) : '',
            sys_dept: r.system_dept_name || r.system_dept_code || '',
            sys_owner: r.system_owner_name || r.system_owner_display || r.system_owner_emp_no || '',
            svc_dept: r.service_dept_name || r.service_dept_code || '',
            svc_owner: r.service_owner_name || r.service_owner_display || r.service_owner_emp_no || '',
            confidentiality: r.cia_confidentiality != null ? String(r.cia_confidentiality) : '',
            integrity: r.cia_integrity != null ? String(r.cia_integrity) : '',
            availability: r.cia_availability != null ? String(r.cia_availability) : '',
            security_score: r.security_score || '',
            system_grade: r.system_grade || '',
            core_flag: r.is_core_system ? '핵심' : '일반',
            dr_built: r.has_dr_site ? 'O' : 'X',
            svc_redundancy: r.has_service_ha ? 'O' : 'X',
            _asset_type: r.asset_type || '',
            _work_status_color: r.work_status_color || '',
        };
    }

    function switchTab(newKey){
        if(!TAB_CONFIG[newKey] || newKey === currentTabKey) return;
        currentTabKey = newKey;
        // Update active button
        var bar = document.getElementById('unused-tab-bar');
        if(bar){
            bar.querySelectorAll('.system-tab-btn').forEach(function(btn){
                btn.classList.toggle('active', btn.getAttribute('data-tab-key') === newKey);
                btn.setAttribute('aria-selected', btn.getAttribute('data-tab-key') === newKey ? 'true' : 'false');
            });
        }
        // Update heading
        var titleEl = document.getElementById('tab-title-label');
        if(titleEl) titleEl.textContent = getCurrentTabTitle();
        ...
        // Reload column preferences for this tab
        loadColumnSelection();
        initData();
        // Update URL without reload
        try { history.pushState({tabKey: newKey}, '', '/p/' + newKey); } catch(_e){}
    }
    ...
    var VISIBLE_COLS_KEY = (TAB_CONFIG[currentTabKey] || TAB_CONFIG.gov_unused_server).colKey;
    ...
    function initData(){
        ...
        fetch('/api/gov-unused/assets?category=' + encodeURIComponent(cat) + '&page=1&page_size=200')
            .then(function(res){ return res.json(); })
            .then(function(data){
                if(data.success && Array.isArray(data.rows)){
                    state.data = data.rows.map(function(r, i){ var d = apiRowToDisplay(r); d.id = r.id || (i+1); return d; });
                } else {
                    state.data = [];
                }
        ...
        slice.forEach((row)=>{
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    let rawVal = row[col];
                    if(col === '_asset_type'){ rawVal = getCurrentAssetLabel(); }
    ...
    function loadColumnSelection(){
        VISIBLE_COLS_KEY = (TAB_CONFIG[currentTabKey] || TAB_CONFIG.gov_unused_server).colKey;
        try {
    ...
    function bindEvents(){
        // 탭 클릭 → 클라이언트 전환 (페이지 리로드 없음)
        document.getElementById('unused-tab-bar')?.addEventListener('click', function(e){
            var btn = e.target.closest('.system-tab-btn');
            if(!btn) return;
        ...
        window.addEventListener('popstate', function(e){
            if(e.state && e.state.tabKey && TAB_CONFIG[e.state.tabKey]){
                switchTab(e.state.tabKey);
            } else {
                // URL에서 키 추출
                var m = window.location.pathname.match(/\/p\/(gov_unused_\w+)/);
                if(m && TAB_CONFIG[m[1]]) switchTab(m[1]);
            }
        });
    ...
    document.getElementById(UPLOAD_CLOSE_ID)?.addEventListener('click', ()=>{ try{ uploadAnim?.stop?.(); }catch(_){} closeModal(UPLOAD_MODAL_ID); });
        ...
        document.getElementById(DISPOSE_CONFIRM_ID)?.addEventListener('click', async ()=>{
            // 자산편입: 선택된 불용자산을 일반 자산으로 복원
            const ids = [...state.selected];
            if(!ids.length){ closeModal(DISPOSE_MODAL_ID); return; }
            const btn = document.getElementById(DISPOSE_CONFIRM_ID);
            if(btn){ btn.disabled = true; }
            try {
                const res = await fetch('/api/gov-unused/assets/restore', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ ids: ids, category: getCurrentCategory() })
                });
                const data = await res.json();
                if(!data.success){ showMessage(data.message || '자산편입 실패', '오류'); return; }
                state.data = state.data.filter(r => !state.selected.has(r.id));
                const removed = data.restored || ids.length;
                state.selected.clear();
                applyFilter();
                closeModal(DISPOSE_MODAL_ID);
                if(removed > 0){ setTimeout(()=> showMessage(`${removed}개 자산이 자산편입되었습니다.`, '완료'), 0); }
            } catch(err){
                showMessage('자산편입 중 오류가 발생했습니다.', '오류');
            } finally {
    ...
    function init(){
        ...
        bindEvents();
        render();
        // Set initial tab title from config
        var titleEl = document.getElementById('tab-title-label');
        if(titleEl) titleEl.textContent = getCurrentTabTitle();
        // Replace initial state so popstate works for the landing tab too
        try { history.replaceState({tabKey: currentTabKey}, '', window.location.pathname); } catch(_e){}
