...
(function(){
    ...
    const REQUIRED_VISIBLE_COLUMNS = ['erp_no','acquisition_date','acquisition_cost','residual_value'];
    ...
    const BASE_VISIBLE_COLUMNS = [ // 기본 노출 요구: 업무 상태, 업무 그룹, 업무 이름, 시스템 이름, 시스템 IP, 관리 IP, 시스템 제조사, 시스템 모델명, 시스템 일련번호, 시스템 담당자
        ...
        'erp_no','acquisition_date','acquisition_cost','residual_value','note'
    ];
    const COLUMN_ORDER = [
    ...
    const COLUMN_MODAL_GROUPS = [
        ...
        { group: '불용처리', columns: ['erp_no','acquisition_date','acquisition_cost','residual_value','note'] }
    ];
    ...
    const COLUMN_META = {
        ...
        // 불용처리 섹션
        erp_no:{label:'ERP 번호',group:'불용처리'},
        acquisition_date:{label:'취득일',group:'불용처리'},
        acquisition_cost:{label:'취득비용',group:'불용처리'},
        residual_value:{label:'잔존가치',group:'불용처리'},
        note:{label:'비고',group:'불용처리'}
    };
    ...
    function mockData(count=37){
            ...
            for(let i=1;i<=count;i++){
                ...
                const rec = {
                    ...
                    // 불용처리 필드 (샘플)
                    erp_no: 'ERP-'+('0000'+i).slice(-4),
                    acquisition_date: `2024-${String((i%12)+1).padStart(2,'0')}-${String((i%28)+1).padStart(2,'0')}`,
                    acquisition_cost: String(1000000 + i*10000),
                    residual_value: String(200000 + (i%10)*5000),
                    note: i%3===0 ? '중고 매각 검토' : ''
                };
        ...
        slice.forEach((row)=>{
            ...
            tr.innerHTML = `<td><input type="checkbox" class="system-row-select" data-id="${row.id??''}" ${checked}></td>`
                + COLUMN_ORDER.map(col=>{
                    ...
                    if(['acquisition_cost','residual_value'].includes(col)){
    ...
    function fillEditForm(row){
        ...
        const groups = [
            ...
            { title:'불용처리', cols:['erp_no','acquisition_date','acquisition_cost','residual_value','note'] }
        ];
        groups.forEach(g=>{
    ...
    function generateFieldInput(col,value=''){
        ...
        if(col==='acquisition_date') return `<input name="${col}" type="date" class="form-input" value="${value??''}">`;
        if(['acquisition_cost','residual_value','slot','u_size'].includes(col)) return `<input name="${col}" type="number" min="0" step="1000" class="form-input" value="${value??''}">`;
        if(col==='note') return `<input name="${col}" class="form-input" placeholder="비고" value="${value??''}">`;
