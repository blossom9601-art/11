/* CostOpexDetailPage — OPEX 계약 상세 (하드웨어/소프트웨어/기타) + 탭 */
...
export default class CostOpexDetailPage {
  constructor({ params, query, router }) {
    this._router = router; this._id = params.id; this._activeTab = query.tab || 'basic';
    this._apiBase = '/api/opex-contracts'; this._el = null; this._tabBar = null;
  ...
  async _load() {
    const res = await fetchQuery(['cost', 'opex', 'detail', this._id],
  ...
  _render() {
    ...
    this._el.innerHTML = `
      <div class="spa-page spa-detail-page">
        <div class="spa-page-header">
          ...
          <div class="spa-detail-title"><h2>${esc(i.contract_name || i.name || '')}</h2></div>
    ...
    this._el.querySelector('#btn-back').addEventListener('click', () => this._router.navigate('/cost/opex'));
  ...
  async _save() {
    ...
    if (c?.instance?.save) { const ok = await c.instance.save(); if (ok) { Toast.success('저장 완료'); invalidate(['cost','opex','detail',this._id]); } }
  }
}
