...
const COLUMNS = [
  { key: 'dept_code',        label: '부서코드',   sortable: true, width: '110px' },
  { key: 'dept_name',        label: '부서명',     sortable: true },
  { key: 'parent_dept_code', label: '상위부서',   sortable: true, width: '110px' },
  { key: 'member_count',     label: '인원수',     sortable: true, width: '80px' },
];

const FORM_FIELDS = [
  { key: 'dept_code', label: '부서코드', type: 'text', required: true },
  { key: 'dept_name', label: '부서명', type: 'text', required: true },
  { key: 'parent_dept_code', label: '상위부서코드', type: 'text' },
  { key: 'remark', label: '비고', type: 'textarea' },
];

export default class AdminDepartmentPage {
  ...
  async mount(container) {
    ...
    this._el.innerHTML = `
      <div class="spa-page">
        <div class="spa-page-header"><h2 class="spa-page__title">부서 관리</h2>
    ...
    this._table = new DataTable({ columns: cols, selectable: true, emptyText: '등록된 부서가 없습니다.' });
    this._table.mount(this._el.querySelector('#dept-table'));
    ...
    this._el.querySelector('#btn-export')?.addEventListener('click', () =>
      this._table.exportCsv('부서목록.csv'));
