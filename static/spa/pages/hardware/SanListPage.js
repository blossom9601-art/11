/* SanListPage — SAN 장비 목록 (Director, Switch) */
...
const TYPE_CONFIG = {
  director: { apiBase: '/api/hardware/san/director/assets', label: 'SAN Director' },
  switch:   { apiBase: '/api/hardware/san/switch/assets',   label: 'SAN Switch' },
};

const COLUMNS = [
  { key: 'work_status',  label: '업무 상태', sortable: true, width: '90px',
    render: (v, r) => `<span class="spa-badge"${r.work_status_color ? ` style="background:${esc(r.work_status_color)};color:#fff"` : ''}>${esc(v)}</span>` },
...
function mapRow(item) {
  return { id: item.id, work_status: item.work_status_name || '-', work_status_color: item.work_status_color || '',
    work_group: item.work_group_name || '-', work_name: item.work_name || '-', system_name: item.system_name || '-',
    system_ip: item.system_ip || '-', vendor: item.manufacturer_name || '-', model: item.model_name || item.server_model_name || '-',
    serial: item.serial_number || '-', sys_owner: item.system_owner_display || item.system_owner_name || '-' };
}

export default class SanListPage {
  constructor({ params, query, router }) {
    this._router = router; this._type = params.type || 'director'; this._query = query || {};
    this._cfg = TYPE_CONFIG[this._type] || TYPE_CONFIG.director;
    this._el = null; this._table = null; this._search = null; this._filters = {};
    this._page = parseInt(this._query.page) || 1; this._pageSize = 50;
  }
  ...
  async mount(c) {
    ...
    this._el.querySelector('.spa-scope-tabs').addEventListener('click', e => {
      const btn = e.target.closest('[data-type]'); if (btn && btn.dataset.type !== this._type) this._router.navigate('/hardware/san/' + btn.dataset.type); });
    this._el.querySelector('#btn-export').addEventListener('click', () => this._table.exportCsv(`SAN_${this._type}_목록.csv`));
    this._el.querySelector('#btn-bulk-delete').addEventListener('click', () => this._bulkDelete());
    this._el.querySelector('#btn-register').addEventListener('click', () => this._router.navigate(`/hardware/san/${this._type}/new`));
    const fkOpts = await loadMultipleOptions(['workStatus', 'workGroup']);
    this._search = new SearchBar({ fields: [
    ...
    this._table = new DataTable({ columns: COLUMNS, selectable: true, pageSize: this._pageSize, emptyText: '등록된 SAN 장비가 없습니다.',
      onRowClick: r => this._router.navigate(`/hardware/san/${this._type}/${r.id}`),
      onPageChange: p => { this._page = p; this._fetch(); },
      onSelectionChange: ids => { const btn = this._el.querySelector('#btn-bulk-delete'); if (btn) btn.hidden = ids.length === 0; } });
  ...
  async _fetch() {
    ...
    const res = await fetchQuery(['san', this._type, 'list', params], () => api.get(`${this._cfg.apiBase}?${qs}`, { showError: false }));
    const items = (res.items || res.rows || []).map(mapRow);
    this._table.setData(items, res.total || items.length, this._page);
  }

  async _bulkDelete() {
    const ids = this._table.getSelectedIds(); if (!ids.length) return;
    if (!(await confirm(`선택한 ${ids.length}건을 삭제하시겠습니까?`))) return;
    const r = await api.bulkDelete(this._cfg.apiBase, ids);
    if (r.success) { Toast.success(`${ids.length}건 삭제`); invalidate(['san']); this._fetch(); }
  }
}
