/* SanDetailPage — SAN 장비 상세 (Director/Switch) + Zone 탭 */
...
const TYPE_API = {
  director: '/api/hardware/san/director/assets',
  switch:   '/api/hardware/san/switch/assets',
};

const DETAIL_TABS = [
  { id: '01', label: '기본정보',   loader: () => import('./tabs/Tab01Hardware.js') },
  { id: '04', label: '인터페이스', loader: () => import('./tabs/Tab05Account.js') },
  { id: '33', label: 'Zone 정보', loader: () => import('./tabs/Tab33Zone.js') },
...
export default class SanDetailPage {
  constructor({ params, query, router }) {
    this._router = router; this._type = params.subtype || params.type || 'director'; this._id = params.id;
    this._activeTab = query.tab || '01'; this._apiBase = TYPE_API[this._type] || TYPE_API.director;
    this._el = null; this._tabBar = null; this._tabCache = {}; this._asset = null; this._tabContent = null;
  }

  async mount(c) { this._el = c; this._el.innerHTML = LoadingSpinner.renderPage(); await this._fetchBasic(); }
  ...
  async _fetchBasic() {
    const res = await fetchQuery(['san', this._type, 'detail', this._id],
      () => api.get(`${this._apiBase}/${this._id}`, { showError: false }));
    if (!res?.item) { ErrorBoundary.mount(this._el, res?.error || 'SAN 장비 정보를 찾을 수 없습니다.', () => this._fetchBasic()); return; }
    this._asset = res.item; this._render(); this._loadTab(this._activeTab);
  }
  ...
  _render() {
      ...
      <div class="spa-page spa-detail-page">
        <div class="spa-page-header">
          ...
          <div class="spa-detail-title">
            <h2>${esc(a.system_name || a.work_name || '')}</h2>
    ...
    this._el.querySelector('#btn-back').addEventListener('click', () => this._router.navigate(`/hardware/san/${this._type}`));
    this._el.querySelector('#btn-save').addEventListener('click', () => this._save());
  }

  async _switchTab(id) {
    const c = this._tabCache[this._activeTab]; if (c?.instance?.unmount) c.instance.unmount();
    this._activeTab = id; this._router.updateQuery({ tab: id }); await this._loadTab(id);
  }

  async _loadTab(tabId) {
    const def = DETAIL_TABS.find(t => t.id === tabId); if (!def) return;
  ...
  async _save() {
    ...
    if (c?.instance?.save) { const ok = await c.instance.save(); if (ok) { Toast.success('저장 완료'); invalidate(['san', this._type, 'detail', this._id]); } }
  }
}
