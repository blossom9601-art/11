...
const COLUMNS = [
  { key: 'system_name', label: 'CCTV명', sortable: true },
...
const FORM_FIELDS = [
  { key: 'system_name', label: 'CCTV명', type: 'text', required: true },
  { key: 'location', label: '위치', type: 'text', required: true },
  { key: 'system_type', label: '유형', type: 'text' },
  { key: 'manufacturer', label: '제조사', type: 'text' },
  { key: 'status', label: '상태', type: 'select', options: [{ value: '녹화중', label: '녹화중' }, { value: '점검중', label: '점검중' }, { value: '고장', label: '고장' }, { value: '비활성', label: '비활성' }] }
];

export default class DcCctvPage {
  ...
  async mount(container) {
    ...
    this._el.innerHTML = `
      <div class="spa-page">
        <div class="spa-page-header"><h2 class="spa-page__title">CCTV 관리</h2>
    ...
    this._table = new DataTable({ columns: cols, selectable: true, emptyText: '등록된 CCTV이(가) 없습니다.' });
    ...
    this._el.querySelector('#btn-export')?.addEventListener('click', () =>
      this._table.exportCsv('CCTV관리.csv'));
