...
const COLUMNS = [
  ...
  { key: 'recorded_at', label: '기록일시', sortable: true, width: '150px' },
...
const FORM_FIELDS = [
  ...
  { key: 'temperature', label: '온도(℃)', type: 'number', required: true },
  { key: 'humidity', label: '습도(%)', type: 'number' },
  { key: 'recorded_at', label: '기록일시', type: 'text' },
  { key: 'status', label: '상태', type: 'select', options: [{ value: '정상', label: '정상' }, { value: '경고', label: '경고' }, { value: '위험', label: '위험' }] }
];

export default class DcThermometerPage {
  ...
  async mount(container) {
    ...
    this._el.innerHTML = `
      <div class="spa-page">
        <div class="spa-page-header"><h2 class="spa-page__title">온습도 기록</h2>
    ...
    this._table = new DataTable({ columns: cols, selectable: true, emptyText: '등록된 온습도 기록이(가) 없습니다.' });
    ...
    this._el.querySelector('#btn-export')?.addEventListener('click', () =>
      this._table.exportCsv('온습도기록.csv'));
