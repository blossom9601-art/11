...
const COLUMNS = [
  ...
  { key: 'access_level', label: '등급', sortable: true, width: '80px' },
  { key: 'valid_from', label: '시작일', sortable: true, width: '110px' },
  { key: 'valid_to', label: '만료일', sortable: true, width: '110px' },
...
const FORM_FIELDS = [
  ...
  { key: 'access_level', label: '등급', type: 'select', options: [{ value: '일반', label: '일반' }, { value: 'VIP', label: 'VIP' }, { value: '긴급', label: '긴급' }] },
  { key: 'valid_from', label: '시작일', type: 'date', required: true },
  { key: 'valid_to', label: '만료일', type: 'date' },
  { key: 'status', label: '상태', type: 'select', options: [{ value: '활성', label: '활성' }, { value: '만료', label: '만료' }, { value: '정지', label: '정지' }] }
];

export default class DcAccessControlPage {
  constructor({ params, query, router }) {
    ...
    this._crud   = new TabCrudMixin({
      apiBase: '/api/datacenter/access/permissions',
  ...
  async mount(container) {
    ...
    this._table = new DataTable({ columns: cols, selectable: true, emptyText: '등록된 출입 권한이(가) 없습니다.' });
    ...
    this._el.querySelector('#btn-export')?.addEventListener('click', () =>
      this._table.exportCsv('출입권한관리.csv'));
  ...
  async _fetch() {
    ...
    try {
      const res = await api.get('/api/datacenter/access/permissions');
