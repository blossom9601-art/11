/* DcDeletionSystemPage — 삭제 시스템 관리 */
...
const COLUMNS = [
  ...
  { key: 'certification', label: '인증', sortable: true, width: '100px' },
...
const FORM_FIELDS = [
  ...
  { key: 'system_type', label: '유형', type: 'select', options: [{ value: '물리파쇄기', label: '물리파쇄기' }, { value: '자기소거기', label: '자기소거기' }, { value: 'SW소거', label: 'SW소거' }, { value: '천공기', label: '천공기' }] },
  { key: 'manufacturer', label: '제조사', type: 'text' },
  { key: 'certification', label: '인증', type: 'text' },
  { key: 'status', label: '상태', type: 'select', options: [{ value: '정상', label: '정상' }, { value: '점검중', label: '점검중' }, { value: '폐기', label: '폐기' }] }
];

export default class DcDeletionSystemPage {
  constructor({ params, query, router }) {
    ...
    this._crud   = new TabCrudMixin({
      apiBase: '/api/datacenter/data-deletion-systems',
  ...
  async mount(container) {
    ...
    this._el.innerHTML = `
      <div class="spa-page">
        <div class="spa-page-header"><h2 class="spa-page__title">삭제 시스템 관리</h2>
    ...
    this._table = new DataTable({ columns: cols, selectable: true, emptyText: '등록된 삭제 시스템이(가) 없습니다.' });
    ...
    this._el.querySelector('#btn-export')?.addEventListener('click', () =>
      this._table.exportCsv('삭제시스템관리.csv'));
  ...
  async _fetch() {
    ...
    try {
      const res = await api.get('/api/datacenter/data-deletion-systems');
