 ...
 *  Tab86Resource — 자원관리
 *  ============================================================
 *  프로젝트에 배정된 HW 자산 목록 (읽기 + 배정 모달)
 *  API: GET /api/prj/projects/<id>/tabs/resource
 * ============================================================ */
...
const COLUMNS = [
  { key: 'resourceType', label: '자원유형',   sortable: true, width: '100px' },
  { key: 'resourceName', label: '자원명',     sortable: true },
  { key: 'spec',         label: '사양/역량',  sortable: false },
  { key: 'quantity',     label: '수량',       sortable: true, width: '70px' },
  { key: 'assignDate',   label: '배정일',     sortable: true, width: '100px' },
  { key: 'releaseDate',  label: '반납일',     sortable: true, width: '100px' },
  { key: 'owner',        label: '담당자',     sortable: true, width: '100px' },
  { key: 'note',         label: '비고',       sortable: false },
];

export default class Tab86Resource {
  constructor({ projectId, project, tabKey, access, router }) {
    this._projectId = projectId;
    this._tabKey    = tabKey || 'resource';
    this._access    = access;
    this._el        = null;
    this._table     = null;
  }
  ...
  _initTable() {
    this._table = new DataTable({
      columns: COLUMNS, selectable: this._access === 'write', emptyText: '배정된 자원이 없습니다.',
      onSelectionChange: (ids) => { const btn = this._el.querySelector('#btn-del-rows'); if (btn) btn.hidden = ids.length === 0; }
    });
  ...
  _bindToolbar() {
    this._el.addEventListener('click', async (e) => {
      ...
      if (btn.id === 'btn-add-row') {
        const payload = {}; COLUMNS.forEach(c => { payload[c.key] = ''; });
        const url = `/api/prj/projects/${this._projectId}/tabs/${this._tabKey}`;
        const result = await api.post(url, { payload_json: JSON.stringify(payload) });
        if (result.success) { Toast.success('행이 추가되었습니다.'); invalidate(['project', this._projectId, 'tab', this._tabKey]); await this._fetchData(); }
        ...
        const url = `/api/prj/projects/${this._projectId}/tabs/${this._tabKey}`;
        let deleted = 0;
        for (const id of ids) { const r = await api.delete(`${url}/${id}`); if (r.success) deleted++; }
        if (deleted) { Toast.success(`${deleted}건이 삭제되었습니다.`); invalidate(['project', this._projectId, 'tab', this._tabKey]); await this._fetchData(); }
      }
    });
  }
