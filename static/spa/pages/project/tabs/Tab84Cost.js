 ...
 *  Tab84Cost — 비용관리 (cost)
 *  ============================================================
 *  API: GET/POST/PUT/DELETE /api/prj/projects/<id>/tabs/cost
...
const COLUMNS = [
  ...
  { key: 'planned',  label: '계획비용', sortable: true, width: '110px' },
  { key: 'actual',   label: '실제비용', sortable: true, width: '110px' },
  { key: 'remaining', label: '잔여비용', sortable: true, width: '110px',
    render: (val, row) => {
      const p = Number(row.planned) || 0, a = Number(row.actual) || 0;
      return (p - a).toLocaleString();
    }
  },
  { key: 'rate', label: '사용률', sortable: true, width: '80px',
    render: (val, row) => {
      const p = Number(row.planned) || 0, a = Number(row.actual) || 0;
      return p > 0 ? Math.round(a / p * 100) + '%' : '-';
    }
  },
  { key: 'note', label: '비고', sortable: false },
];

export default class Tab84Cost {
  constructor({ projectId, project, tabKey, access, router }) {
    this._projectId = projectId;
    this._tabKey    = tabKey || 'cost';
    this._access    = access;
    this._router    = router;
    this._el        = null;
    this._table     = null;
