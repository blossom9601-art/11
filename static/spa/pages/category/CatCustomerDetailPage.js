/* CatCustomerDetailPage — 고객/회원사 상세 + 탭 (담당자/작업/이력/파일) */
...
const API_BASE = '/api/customer-members';
...
export default class CatCustomerDetailPage {
  ...
  async _load() {
    const res = await fetchQuery(['category', 'customer', 'detail', this._id],
      () => api.get(`${API_BASE}/${this._id}`, { showError: false }));
    if (!res || (res.success === false && !res.item)) { ErrorBoundary.mount(this._el, res?.error || '고객 정보를 찾을 수 없습니다.', () => this._load()); return; }
  ...
  _render() {
    ...
    this._el.innerHTML = `
      <div class="spa-page spa-detail-page">
        <div class="spa-page-header">
          ...
          <div class="spa-detail-title"><h2>${esc(i.member_name || i.company_name || i.name || '')}</h2></div>
    ...
    this._el.querySelector('#btn-back').addEventListener('click', () => this._router.navigate('/category/customer'));
  ...
  async _save() {
    ...
    if (c?.instance?.save) { const ok = await c.instance.save(); if (ok) { Toast.success('저장 완료'); invalidate(['category','customer','detail',this._id]); } }
  }
}
