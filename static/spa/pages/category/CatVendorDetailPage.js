/* CatVendorDetailPage — 벤더/제조사 상세 + 탭 (담당자/HW/SW/부품/작업/이력/파일) */
...
const API_BASE = '/api/vendor-manufacturers';

const DETAIL_TABS = [
  { id: 'basic',     label: '기본 정보' },
  { id: 'manager',   label: '담당자',     loader: () => import('./tabs/TabCatManager.js') },
  { id: 'hardware',  label: 'HW 자산',   loader: () => import('./tabs/TabCatHardware.js') },
  { id: 'software',  label: 'SW 자산',   loader: () => import('./tabs/TabCatSoftware.js') },
  { id: 'component', label: '부품',       loader: () => import('./tabs/TabCatComponent.js') },
...
export default class CatVendorDetailPage {
  ...
  async _load() {
    const res = await fetchQuery(['category', 'vendor', 'detail', this._id],
      () => api.get(`${API_BASE}/${this._id}`, { showError: false }));
    if (!res || (res.success === false && !res.item)) { ErrorBoundary.mount(this._el, res?.error || '벤더 정보를 찾을 수 없습니다.', () => this._load()); return; }
  ...
  _render() {
    ...
    this._el.innerHTML = `
      <div class="spa-page spa-detail-page">
        <div class="spa-page-header">
          ...
          <div class="spa-detail-title"><h2>${esc(i.company_name || i.name || '')}</h2></div>
    ...
    this._el.querySelector('#btn-back').addEventListener('click', () => this._router.navigate('/category/vendor'));
  ...
  async _save() {
    ...
    if (c?.instance?.save) { const ok = await c.instance.save(); if (ok) { Toast.success('저장 완료'); invalidate(['category','vendor','detail',this._id]); } }
  }
}
