...
const COLUMNS = [
  { key: 'status_code', label: '상태코드', sortable: true, width: '110px' },
  { key: 'status_name', label: '상태명', sortable: true },
  { key: 'status_level', label: '레벨', sortable: true, width: '70px' },
  { key: 'description', label: '설명', sortable: true },
  { key: 'hw_count', label: 'HW수', sortable: true, width: '70px' }
];

const FORM_FIELDS = [
  { key: 'status_code', label: '상태코드', type: 'text', required: true },
  { key: 'status_name', label: '상태명', type: 'text', required: true },
  { key: 'status_level', label: '레벨', type: 'text' },
  { key: 'description', label: '설명', type: 'textarea' }
];

export default class CatWorkStatusPage {
  ...
  async mount(container) {
    ...
    this._el.innerHTML = `
      <div class="spa-page">
        <div class="spa-page-header">
          ...
          <h2 class="spa-page__title">업무 상태</h2>
    ...
    this._table = new DataTable({ columns: cols, selectable: true, emptyText: '등록된 업무 상태이(가) 없습니다.' });
    ...
    this._el.querySelector('#btn-export')?.addEventListener('click', () =>
      this._table.exportCsv('업무상태.csv'));
  ...
  async _fetch() {
    ...
    try {
      const res = await api.get('/api/work-statuses');
