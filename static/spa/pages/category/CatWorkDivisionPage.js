...
const COLUMNS = [
  { key: 'division_code', label: '구분코드', sortable: true, width: '110px' },
  { key: 'division_name', label: '구분명', sortable: true },
...
const FORM_FIELDS = [
  { key: 'division_code', label: '구분코드', type: 'text', required: true },
  { key: 'division_name', label: '구분명', type: 'text', required: true },
  { key: 'description', label: '설명', type: 'textarea' },
  { key: 'remark', label: '비고', type: 'textarea' }
];

export default class CatWorkDivisionPage {
  ...
  async mount(container) {
    ...
    this._el.innerHTML = `
      <div class="spa-page">
        <div class="spa-page-header">
          ...
          <h2 class="spa-page__title">업무 구분</h2>
    ...
    this._table = new DataTable({ columns: cols, selectable: true, emptyText: '등록된 업무 구분이(가) 없습니다.' });
    ...
    this._el.querySelector('#btn-export')?.addEventListener('click', () =>
      this._table.exportCsv('업무구분.csv'));
  ...
  async _fetch() {
    ...
    try {
      const res = await api.get('/api/work-divisions');
