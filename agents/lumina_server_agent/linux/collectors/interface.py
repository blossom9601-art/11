...
class InterfaceCollector(BaseCollector):
    ...
    def collect(self) -> List[Dict[str, Any]]:
        ...
        try:
            raw = subprocess.check_output(
                ["ip", "-o", "addr", "show"], universal_newlines=True, timeout=30
            )
    ...
    def _parse_ip_line(self, line: str) -> Optional[Dict[str, str]]:
    ...
    def _collect_uuids(self) -> Dict[str, str]:
        ...
        try:
            raw = subprocess.check_output(
                ...
                universal_newlines=True, timeout=10, stderr=subprocess.DEVNULL,
            )
            for line in raw.strip().splitlines():
                parts = line.split(":", 1)
