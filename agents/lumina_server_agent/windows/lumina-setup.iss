; Lumina Server Agent — Inno Setup (Lumina AP 연동, 자산 수집)
; 빌드: .\build_installer.ps1  (아이콘 재생성 + PyInstaller + ISCC)
...
[Messages]
; 첫 화면 보조 문구 — 로고는 wizard_image.bmp / wizard_small.bmp (lumina_server_agent 에셋 기반)
english.WelcomeLabel2=This wizard installs Lumina Server Agent on your computer.#13#10#13#10After setup, edit:#13#10{commonappdata}\Lumina\lumina.conf#13#10and set server_url (Lumina AP / Blossom upload URL).
korean.WelcomeLabel2=이 마법사는 Lumina Server Agent(서버 에이전트)를 설치합니다.#13#10#13#10설치 후 아래 파일에서 server_url(Lumina AP·Blossom 업로드 주소)을 설정하세요:#13#10{commonappdata}\Lumina\lumina.conf
...
[Run]
; 서비스 자동 등록 (설치 시)
Filename: "{app}\{#MyAppExeName}"; Parameters: "--install-service"; Flags: runhidden waituntilterminated; StatusMsg: "Lumina Server Agent 서비스 등록 중..."
; 설치 후 GUI 실행 (선택)
Filename: "{app}\{#MyAppExeName}"; Description: "Lumina Server Agent 실행"; Flags: nowait postinstall skipifsilent unchecked
